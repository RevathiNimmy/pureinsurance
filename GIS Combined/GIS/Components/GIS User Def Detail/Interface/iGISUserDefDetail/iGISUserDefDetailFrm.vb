...
Friend Partial Class frmInterface
	...
	Private m_lGISUserDefHeaderIndsId As Integer
	Private m_sGISUserDefHeaderIndsCode As String = ""
	Private m_vAllowedParents As Object
	
	Private m_vHeaderInds( ,  ) As Object
	
	' {* USER DEFINED CODE (End) *}
	
	' Declare an instance of the general interface object.
	Private m_oGeneral As iGISUserDefDetail.General
	...
	Private m_oLookupDetailRates As iGISUserDefDetailRates.Interface_Renamed
	'Private m_oLookupDetailRates As iGISUserDefDetailRates.Interface
	...
	Public Property GISUserDefHeaderIndsId() As Integer
		Get
			Return m_lGISUserDefHeaderIndsId
		End Get
		Set(ByVal Value As Integer)
			m_lGISUserDefHeaderIndsId = Value
		End Set
	End Property
	
	Public Property GISUserDefHeaderIndsCode() As String
		Get
			Return m_sGISUserDefHeaderIndsCode
		End Get
		Set(ByVal Value As String)
			m_sGISUserDefHeaderIndsCode = Value
		End Set
	End Property
	...
	Public Function BusinessToInterface() As Integer
		...
		Try 
			...
			'    lIndex = 0
			'
			'    If IsArray(m_vAllowedParents) Then
			'        For lTemp = LBound(m_vAllowedParents, 2) To UBound(m_vAllowedParents, 2)
			'            cboParent.AddItem m_vAllowedParents(1, lTemp)
			'            If (m_vAllowedParents(0, lTemp) = m_lParentId) Then
			'                lIndex = cboParent.NewIndex
			'            End If
			'        Next lTemp
			'    End If
			'
			'    cboParent.ListIndex = lIndex
			
			If m_lLookupParentId <> -1 Then
				cboParent.Table = m_lLookupParentId
				cboParent.DefaultItemId = m_lParentId
				cboParent.RefreshList()
			End If
			
			m_lReturn = GetHeaderInds()
			
			If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
				Return gPMConstants.PMEReturnCode.PMFalse
			End If
			
			If Information.IsArray(m_vHeaderInds) Then
			...
			Dim cboIndicator_NewIndex As Integer = -1
			cboIndicator_NewIndex = cboIndicator.Items.Add("None")
			VB6.SetItemData(cboIndicator, cboIndicator_NewIndex, 0)
			lIndex = 0
			
			If Information.IsArray(m_vHeaderInds) Then
				For lTemp As Integer = m_vHeaderInds.GetLowerBound(1) To m_vHeaderInds.GetUpperBound(1)
					If CDbl(m_vHeaderInds(ACIIsDeleted, lTemp)) = 0 Then
						cboIndicator_NewIndex = cboIndicator.Items.Add(CStr(m_vHeaderInds(ACIDescription, lTemp)))
						VB6.SetItemData(cboIndicator, cboIndicator_NewIndex, CInt(m_vHeaderInds(ACIGISUserDefHeaderIndsId, lTemp)))
						If CDbl(m_vHeaderInds(ACIGISUserDefHeaderIndsId, lTemp)) = m_lGISUserDefHeaderIndsId Then
							lIndex = cboIndicator_NewIndex
						End If
					End If
				Next lTemp
			End If
			
			cboIndicator.SelectedIndex = lIndex
	...
	Private Function InterfaceToData() As Integer
		...
		Try 
			...
			'        m_sParentCode = ""
			'    Else
			'        m_lParentId = m_vAllowedParents(0, cboParent.ListIndex - 1)
			'        m_sParentCode = cboParent.Text
			'    End If
			
			If m_lLookupParentId <> -1 Then
				m_lParentId = cboParent.ItemId
				m_sParentCode = cboParent.ItemCaption
			Else
				m_lParentId = -1
				m_sParentCode = ""
			End If
			
			If cboIndicator.SelectedIndex < 1 Then
				m_lGISUserDefHeaderIndsId = 0
				m_sGISUserDefHeaderIndsCode = ""
			Else
				m_lGISUserDefHeaderIndsId = VB6.GetItemData(cboIndicator, cboIndicator.SelectedIndex)
				m_sGISUserDefHeaderIndsCode = cboIndicator.Text
			End If
	...
	Private Function SetInterfaceDefaults() As Integer
		...
		Try 
			...
			If (m_iTask = gPMConstants.PMEComponentAction.PMAdd) Or (m_lLookupDetailId = -1) Then
	...
	Private Function DisplayCaptions() As Integer
		...
		Try 
            ...
            lblCode.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACCaptionCode, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
            ...
            lblIndicator.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACCaptionIndicator, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
    ...
    Private Function ValidateForm() As Integer
        ...
        Try
            ...
            m_lReturn = g_oBusiness.GetByCode(m_lLookupHeaderId, m_lLookupDetailId, txtCode.Text.Trim())
	...
	Private Function CallRatesOrIndicators(ByRef sRorI As String) As Integer
		...
		Try 
			...
			If m_oLookupDetailRates Is Nothing Then
				
				' Get an instance of the contact interface object via
				' the public object manager.
				Dim temp_m_oLookupDetailRates As Object
				m_lReturn = g_oObjectManager.GetInstance(temp_m_oLookupDetailRates, sClassName:="iGISUserDefDetailRates.Interface_Renamed", vInstanceManager:=gPMConstants.PMGetLocalInterface)
				m_oLookupDetailRates = temp_m_oLookupDetailRates
				
				' Check for errors.
				If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
					' Log Error Message
					iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to get Lookup Detail component", vApp:=ACApp, vClass:=ACClass, vMethod:="CallRatesOrIndicators", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
			...
			m_lReturn = CType(m_oLookupDetailRates.SetProcessModes(vTask:=gPMConstants.PMEComponentAction.PMEdit), gPMConstants.PMEReturnCode)
			...
			m_oLookupDetailRates.LookupDetailId = m_lLookupDetailId

			m_oLookupDetailRates.Detail = m_sDescription

			m_oLookupDetailRates.RatesOrIndicators = sRorI
			

			m_lReturn = m_oLookupDetailRates.Start()
	...
	'
	' ***************************************************************** '
	Private Function GetHeaderInds() As Integer
		...
		Try 
			...
			m_lReturn = g_oBusiness.GetHeaderInds(lLookupHeaderId:=m_lLookupHeaderId, vHeaderInds:=m_vHeaderInds)
		...
		Catch excep As System.Exception
			...
			' Log Error Message
			iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetHeaderInds Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetHeaderInds", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
	...
	Private Sub Form_Initialize_Renamed()
		...
		Try 
			...
			m_oGeneral = New iGISUserDefDetail.General()
	...
	Private Sub frmInterface_FormClosing(ByVal eventSender As Object, ByVal eventArgs As FormClosingEventArgs) Handles MyBase.FormClosing
		...
		Try 
			...
			If Not (m_oLookupDetailRates Is Nothing) Then
				' Terminate the LookupDetail object

                m_oLookupDetailRates.Dispose()
                ' Destroy the instance of the LookupDetail object
				' from memory.
				m_oLookupDetailRates = Nothing
			End If
