...
Friend Partial Class frmTasks
	...
	Private Function RefreshList() As Integer
		...
		Try 
			...
			m_oBusiness.TaskCurrentRecord = 0
			...
			Do 
				...
				m_lReturn = m_oBusiness.GetNextTask(vPMTaskGroupId:=m_lTaskGroupID, vTaskGroupCode:=m_sTaskGroupCode, vTaskGroupDescription:=m_sTaskGroupDescription, vIsDeleted:=m_iIsDeleted, vEffectiveDate:=m_dEffectiveDate, vIncluded:=m_iIncluded)
                ...
                oListItem = lvwAll.Items.Add("L" & lRow, m_sTaskGroupCode.Trim(), "task")
				With oListItem
				...
				If m_iIncluded = 1 Then
                    ...
                    'developer guide no.49
                    oListItem = lvwContents.Items.Add("L" & lRow, m_sTaskGroupCode.Trim(), "task")
					With oListItem
						ListViewHelper.GetListViewSubItem(oListItem, 1).Text = m_sTaskGroupDescription
						If (m_iIsDeleted = gPMConstants.PMEReturnCode.PMTrue) Or (m_dEffectiveDate > DateTime.Now) Then
	...
	Private Function AddTask(ByRef oListItem As ListViewItem) As Integer
		...
		Try 
			...
			m_lReturn = m_oBusiness.EditIncludeTask(lID)
			
			If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
				iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to ignore selected Group")
				Return result
			End If
	...
	Private Function DeleteTask(ByRef oListItem As ListViewItem) As Integer
		...
		Try 
			...
			m_lReturn = m_oBusiness.EditIgnoreTask(lID)
    ...
    Private Sub frmTasks_Resize(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Resize
        ...
        Try
            ...
            '        Exit Sub
            '    End If
            Panel4.Width += cmdAddAllTasks.Left + 190
