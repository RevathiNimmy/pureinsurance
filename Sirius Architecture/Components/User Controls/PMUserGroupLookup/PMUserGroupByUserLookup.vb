...
Imports SharedFiles
<System.Runtime.InteropServices.ProgId("cboPMUserGroupByUser_NET.cboPMUserGroupByUser")> _
Partial Public Class cboPMUserGroupByUser
    ...
    Const m_def_PMUserID As Integer = 0
    ...
    Private Declare Function SendMessage Lib "user32" Alias "SendMessageA" (ByVal hwnd As IntPtr, ByVal wMsg As UInteger, ByVal wParam As IntPtr, ByVal lParam As IntPtr) As IntPtr

    Private Const CB_SETDROPPEDWIDTH As Integer = &H160S

    Private ListItems As List(Of ValueDescriptionPair)
    ...
    Public ReadOnly Property ItemUserGroupname(Optional ByVal v_vUserGroupID As Object = Nothing) As String
        Get
            ...
            Else
                ...
                nIndex = IndexOfItem(CInt(v_vUserGroupID), sItemText)
                If nIndex < 0 Then
                    Return ""
                Else
                    Return sItemText
    ...
    Private Sub cboUserGroups_SelectedIndexChanged(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cboUserGroups.SelectedIndexChanged
        ...
        With cboUserGroups
            'SelectedIndex<0 gives error
            If (cboUserGroups.SelectedIndex > -1) Then
                m_lUserGroupID = ToSafeInteger(cboUserGroups.Items(.SelectedIndex).ItemData)
                RaiseEvent UserGroupIDChange()
                m_sUserGroupname = ToSafeString(cboUserGroups.Items(.SelectedIndex).Name)
    ...
    Private Sub cboPMUserGroupByUser_DoubleClick(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.DoubleClick
        RaiseEvent DblClick(Me, Nothing)
    End Sub

    Private Sub cboPMUserGroupByUser_KeyDown(ByVal eventSender As Object, ByVal eventArgs As KeyEventArgs) Handles MyBase.KeyDown
        Dim KeyCode As Integer = eventArgs.KeyCode
        Dim Shift As Integer = eventArgs.KeyData \ &H10000
        RaiseEvent KeyDown(Me, New KeyDownEventArgs(KeyCode, Shift))
    End Sub

    Private Sub cboPMUserGroupByUser_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
    ...
    Private Sub cboPMUserGroupByUser_KeyUp(ByVal eventSender As Object, ByVal eventArgs As KeyEventArgs) Handles MyBase.KeyUp
        Dim KeyCode As Integer = eventArgs.KeyCode
        Dim Shift As Integer = eventArgs.KeyData \ &H10000
        RaiseEvent KeyUp(Me, New KeyUpEventArgs(KeyCode, Shift))
    End Sub

    Private Sub cboPMUserGroupByUser_MouseDown(ByVal eventSender As Object, ByVal eventArgs As MouseEventArgs) Handles MyBase.MouseDown
    ...
    Private Sub cboPMUserGroupByUser_MouseMove(ByVal eventSender As Object, ByVal eventArgs As MouseEventArgs) Handles MyBase.MouseMove
        Dim Button As Integer = CInt(eventArgs.Button)
        Dim Shift As Integer = Control.ModifierKeys \ &H10000
        Dim x As Single = VB6.PixelsToTwipsX(eventArgs.X)
        Dim y As Single = VB6.PixelsToTwipsY(eventArgs.Y)
    ...
    Private Sub cboPMUserGroupByUser_MouseUp(ByVal eventSender As Object, ByVal eventArgs As MouseEventArgs) Handles MyBase.MouseUp
        Dim Button As Integer = CInt(eventArgs.Button)
        Dim Shift As Integer = Control.ModifierKeys \ &H10000
        Dim x As Single = VB6.PixelsToTwipsX(eventArgs.X)
        Dim y As Single = VB6.PixelsToTwipsY(eventArgs.Y)
    ...
    Private Sub UserControl_InitProperties()
        ...
        'Modified as per no solutions
        'Font = Ambient.Font
        Font = MyBase.Font
        m_lPMUserID = m_def_PMUserID
    ...
    Private Sub UserControl_ReadProperties(ByRef PropBag As Object)
        ...
        'developer guide no. Modified as per no solutions
        ...
        m_lPMUserID = CInt(PropBag.ReadProperty("PMUserID", m_def_PMUserID))
        ...
        'developer guide no. 'TODO: Tarun commneted 25 Jan 2010
        'MyBase.ScaleMode = PropBag.ReadProperty("ScaleMode", ScaleMode)

        ' Read the list of type PMUserID entries
        If Not DesignMode Then
            RefreshList()
        End If
    ...
    Private Sub cboPMUserGroupByUser_Resize(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Resize
    ...
    'developer guide no.  modified 25 Jan 2010
    Private Sub UserControl_WriteProperties(ByRef PropBag As Object)
        ...
        PropBag.WriteProperty("PMUserID", m_lPMUserID, m_def_PMUserID)
    ...
    Public Sub RefreshList()
        ...
        Try
            If Not DesignMode Then
                ListItems = New List(Of ValueDescriptionPair)
                ...
                If m_sFirstItem <> "" Then
                    ...
                    Dim oList As ValueDescriptionPair = New ValueDescriptionPair(m_sFirstItem, 0)
                    cboUserGroups_NewIndex = cboUserGroups.Items.Add(oList)
                    oList.Index = cboUserGroups_NewIndex
                    ListItems.Add(oList)

                End If
                m_lReturn = GetUserGroups()
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    Throw New System.Exception((Constants.vbObjectError + gPMConstants.PMEReturnCode.PMFalse).ToString() + ", " + ACApp + ", " + "Failed to get Users for PMUserID : " & PMUserID)
                End If
    ...
    Private Function IndexOfItem(ByVal v_lUserGroupID As Integer) As Integer
        ...
        IDToFind = v_lUserGroupID
        Dim result As ValueDescriptionPair = ListItems.Find(AddressOf FindID)
        ...
        Return -1
    End Function
    Private Function IndexOfItem(ByVal v_lUserGroupID As Integer, ByRef sItemText As String) As Integer
        IDToFind = v_lUserGroupID
        Dim result As ValueDescriptionPair = ListItems.Find(AddressOf FindID)
        If Not result Is Nothing Then
            sItemText = result.Name
            Return result.Index
        End If
    ...
    Private Function GetUserGroupLookupBusiness() As Integer
        ...
        Try
            ...
            If m_oUserGroupLookupBusiness Is Nothing Then
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    result = m_lReturn
                    gPMFunctions.LogMessageToFile(sUsername:="", iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to get an instance of the TypePMUserID business object.", vApp:=ACApp, vClass:=ACClass, vMethod:="GetUserGroupLookupBusiness")
                    Return result
                End If
            End If
            Return result
        ...
        Catch excep As System.Exception
            ...
            ' Log Error.
            gPMFunctions.LogMessageToFile(sUsername:="", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to get the type PMUserID business", vApp:=ACApp, vClass:=ACClass, vMethod:="GetUserGroupLookupBusiness", excep:=excep)
    ...
    Private Function GetUserGroups() As Integer
        ...
        Try
            ...
            ' Get the Business
            m_lReturn = GetUserGroupLookupBusiness()
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return m_lReturn
            End If
            ...
            Else
                ...
                m_lReturn = m_oUserGroupLookupBusiness.GetAllEffectiveGroupsByUser(v_lPMUserID:=PMUserID, v_dtEffectiveDate:=DateTime.Now, r_vUserGroupsArray:=m_vUserGroupsArray)
            End If
                ...
                For lRow As Integer = m_vUserGroupsArray.GetLowerBound(1) To m_vUserGroupsArray.GetUpperBound(1)
                    ...
                    If m_lSingleUserGroupID > 0 Then
                        ...
                        If m_lSingleUserGroupID = CInt(m_vUserGroupsArray(0, lRow)) Then
                            Dim cboUserGroups_NewIndex As Integer = -1
                            Dim oList As ValueDescriptionPair = New ValueDescriptionPair(CStr(m_vUserGroupsArray(2, lRow)), CInt(m_vUserGroupsArray(0, lRow)))
                            cboUserGroups_NewIndex = cboUserGroups.Items.Add(oList)
                            oList.Index = cboUserGroups_NewIndex
                            ListItems.Add(oList)
    ...
    Public Function GetSystemSecurityModel(ByRef vValue As Object) As Integer
        ...
        Try
            ...
            m_lReturn = iPMFunc.getProductOptionValue(v_vOptionNumber:=gPMConstants.SIRHiddenOptions.SIROPTAlternativeLogon, v_vBranch:=1, r_vUnderwriting:=CStr(vValue))
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetSystemSecurityModel Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetSystemSecurityModel", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    'JT       31/04/2005
    '**************************************************************
    Public Function AutoSizeDropDownUserControlWidth(ByRef lmaxwidth As Integer) As Integer
        Dim result As Integer = 0
        Try
            ...
            Try
                ...
                m_lReturn = SendMessage(cboUserGroups.Handle.ToInt32(), CB_SETDROPPEDWIDTH, lmaxwidth + 10, tmpPtr)
            Finally
                handle2.Free()
            End Try
    ...
    Private Function FindID(ByVal val As ValueDescriptionPair) As Boolean
        If val.ItemData = IDToFind Then
            Return True
        Else
            Return False
        End If
