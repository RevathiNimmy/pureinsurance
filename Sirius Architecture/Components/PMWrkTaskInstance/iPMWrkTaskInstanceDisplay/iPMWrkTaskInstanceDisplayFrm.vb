...
Friend Partial Class frmInterface
	...
	Public Property TaskDescription() As String
		Get
			Return m_sTaskDescription.Trim()
		End Get
		Set(ByVal Value As String)
			m_sTaskDescription = Value.Trim()
		End Set
	End Property
	...
	Private Function BusinessToInterface() As Integer
		...
		Try 
			...
			'        If (DisplayIcon < 1 Or _
			''                DisplayIcon > albTaskImages.IconsLarge.Count) Then
			'            DisplayIcon = 1
			'        End If
			'        imgIcon1.Picture = albTaskImages.IconsLarge(DisplayIcon).Picture
			'    End If
	...
	Private Function BusinessToData() As Integer
		...
		Dim lUserGroupID, lPMWrkTaskID, lPMWrkTaskGroupID As Integer
		...
		Try 
			...
			' {* USER DEFINED CODE (Begin) *}
			sCustomer = m_sCustomer
			dtDueDate = m_dtTaskDueDate
			sDescription = m_sDescription
			lUserGroupID = m_lPmuserGroupID
			lPMWrkTaskGroupID = m_lPMWrkTaskGroupId
			lPMWrkTaskID = m_lPMWrkTaskId
			

			m_lReturn = m_oBusiness.GetDetails(v_lPMWrkTaskInstanceCnt:=m_lPMWrkTaskInstanceCnt, r_lPMWrkTaskGroupID:=m_lPMWrkTaskGroupId, r_lpmwrktaskid:=m_lPMWrkTaskId, r_scustomer:=m_sCustomer, r_dttaskduedate:=m_dtTaskDueDate, r_lpmusergroupid:=m_lPmuserGroupID, r_iuserid:=m_iUserID, r_sdescription:=m_sDescription, r_itaskstatus:=m_iTaskStatus, r_iisurgent:=m_iIsUrgent, r_dtdatecreated:=m_dtDateCreated, r_icreatedbyid:=m_iCreatedByID, r_dtlastmodified:=m_dtLastModified, r_imodifiedbyid:=m_iModifiedByID, r_sWorkflowInformation:=m_sWorkflowInformation)
			' AMB 20/01/2003
			
			
			m_sCustomer = sCustomer
			m_dtTaskDueDate = dtDueDate
			m_sDescription = sDescription
			m_lPmuserGroupID = lUserGroupID
			
			If lPMWrkTaskGroupID > 0 Then
				m_lPMWrkTaskId = lPMWrkTaskID
				m_lPMWrkTaskGroupId = lPMWrkTaskGroupID
			End If
			
			' {* USER DEFINED CODE (End) *}
			If m_lPMWrkTaskInstanceCnt < 1 Then
				Return result
			End If
	...
	Private Function SetInterfaceDefaults() As Integer
		...
		Try 
			...
			cboDueDate_NewIndex = cboDueDate.Items.Add("Today")
			cboDueDate_NewIndex = cboDueDate.Items.Add("Tomorrow")
			cboDueDate_NewIndex = cboDueDate.Items.Add("Within a Week")
			cboDueDate_NewIndex = cboDueDate.Items.Add("Within a Month")
			cboDueDate_NewIndex = cboDueDate.Items.Add("Within a Quarter")
			cboDueDate_NewIndex = cboDueDate.Items.Add("Within a Year")
			...
			If Information.IsArray(vDiaryDays) Then
				...
				For lCount As Integer = 0 To vDiaryDays.GetUpperBound(1)
                    ...
                    cboDueDate_NewIndex = cboDueDate.Items.Add(CStr(vDiaryDays(1, lCount)))

                    VB6.SetItemData(cboDueDate, cboDueDate_NewIndex, CInt(vDiaryDays(0, lCount)))
				Next lCount
			End If
	...
	Private Sub RefreshTaskList(Optional ByVal v_iTaskId As Integer = 0)
		...
		Try 
			...
			txtTaskDescription.Text = m_sTaskDescription
	...
	Private Sub RefreshUserGroupList(Optional ByVal v_iTaskGroupId As Integer = 0)
		...
		Try 
			...
			'PN 44278
			If cboTaskUserGroup.ListCount > 0 Then
				If cboTaskUserGroup.ListIndex = -1 Then
					cboTaskUserGroup.ListIndex = 0
				End If
			End If
	...
	Private Sub cboDueDate_SelectedIndexChanged(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cboDueDate.SelectedIndexChanged
		...
		Select Case cboDueDate.SelectedIndex
			'DAK070999
			Case 1
				'Today
	...
	Private Sub frmInterface_Load(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Load
		...
		If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
			...
			Exit Sub
        End If
        'deevloper guide no.(Shifted from Class:Interface_Renamed, function:LoadInterface)
