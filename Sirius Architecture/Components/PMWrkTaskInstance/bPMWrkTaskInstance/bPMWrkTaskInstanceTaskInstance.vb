...
Imports SSP.Shared
Friend NotInheritable Class PMWrkTaskInstance
    ...
    Private m_bIsExternalItem As Boolean
    Private m_sguidPMExternalItem As System.Guid
    Private m_nParentTaskId As Integer
    ...
    Public Property IsExternalWorkItem() As Boolean
        Get
            Return m_bIsExternalItem
        End Get
        Set(ByVal value As Boolean)
            m_bIsExternalItem = value
        End Set
    End Property

    Public Property GuidPMExternalItem() As System.Guid
        Get
            Return m_sguidPMExternalItem
        End Get
        Set(ByVal value As System.Guid)
            m_sguidPMExternalItem = value
        End Set
    End Property

    Public Property ParentTaskId() As Integer
        Get
            Return m_nParentTaskId
        End Get
        Set(ByVal value As Integer)
            m_nParentTaskId = value
        End Set
    End Property
    ...
    Public Function Initialise() As Integer
        Return Initialise(vDatabase:=Nothing)
    End Function

    Public Function Initialise(ByRef vDatabase As Object) As Integer
        ...
        Try
            ...
            m_lReturn = CType(gPMComponentServices.CheckDatabase(v_sUsername:="", v_iSourceID:=1, v_iLanguageID:=1, v_lPMProductFamily:=PMProductFamily, r_bNewInstanceCreated:=m_bCloseDatabase, r_oCheckedDatabase:=m_oDatabase, v_vDatabase:=vDatabase), gPMConstants.PMEReturnCode)
    ...
    Public Sub Default_Renamed()
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="DefaultFailed", vApp:=ACApp, vClass:=ACClass, vMethod:="Default", vErrNo:=Informations.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function Delete() As Integer
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Delete Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="Delete", vErrNo:=Informations.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function SelectSingle() As Integer
        Return SelectSingle(vLockMode:=0)
    End Function

    Public Function SelectSingle(ByRef vLockMode As gPMConstants.PMELockMode) As Integer
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="SelectSingle Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="SelectSingle", vErrNo:=Informations.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Private Function SetPropertiesFromDB(ByRef oFields As DataRow) As Integer
        ...
        With oFields
            ...
            PMWrkTaskInstanceCnt = oFields("pmwrk_task_instance_cnt")
            PmwrkTaskGroupID = oFields("pmwrk_task_group_id")
            PmwrkTaskID = oFields("pmwrk_task_id")

            If Convert.IsDBNull(oFields("customer")) Or Informations.IsNothing(oFields("customer")) Then
                Customer = ""
            Else
                Customer = oFields("customer")
            End If
            TaskDueDate = oFields("task_due_date")
            PmuserGroupID = oFields("pmuser_group_id")
            ...
            End If
            TaskStatus = oFields("task_status")
            IsUrgent = oFields("is_urgent")
            ...
            End If
            'DAK080200
            IsVisible = oFields("is_visible")

            ' AMB 21/01/2003

            If Convert.IsDBNull(oFields("workflow_information")) Or Informations.IsNothing(oFields("workflow_information")) Then
                WorkflowInformation = ""
            Else
                WorkflowInformation = oFields("workflow_information")
            End If
            IsTaskReview = gPMFunctions.NullToInteger(oFields("is_task_review"))

            IsExternalWorkItem = gPMFunctions.NullToInteger(oFields("Is_External_WorkItem"))

            If Convert.IsDBNull(oFields("external_workflow_id")) OrElse Informations.IsNothing(oFields("external_workflow_id")) OrElse oFields("external_workflow_id") <> Guid.Empty Then
                GuidPMExternalItem = Guid.Empty
            Else
                GuidPMExternalItem = oFields("external_workflow_id")
            End If

            ParentTaskId = gPMFunctions.NullToInteger(oFields("PMWrk_task_parent_instance_cnt"))
    ...
    '
    ' AMB 20/01/2003 - Workflow_information column added to PMWrk_Task_Instance table
    ' ***************************************************************** '
    Private Function AddInputParam(Optional ByVal bLimitBySource As Boolean = False) As Integer
        ...
        With m_oDatabase
            ...
            If Customer = "" Then
                ...
                m_lReturn = .Parameters.Add(sName:="customer", vValue:=DBNull.Value.ToString(), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMInteger)
            Else
            ...
            If (UserID < 1) Or (Convert.IsDBNull(UserID) Or Informations.IsNothing(UserID)) Then
                ...
                'developers guide no. 85
                m_lReturn = .Parameters.Add(sName:="user_id", vValue:=DBNull.Value.ToString(), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMInteger)
            Else
            ...
            If Description = "" Then
                ...
                'developers guide no. 85
                m_lReturn = .Parameters.Add(sName:="description", vValue:=DBNull.Value.ToString(), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMInteger)
            Else
            ...
            If (ModifiedByID < 1) Or (Convert.IsDBNull(ModifiedByID) Or Informations.IsNothing(ModifiedByID)) Then
                ...
                'developers guide no. 85
                m_lReturn = .Parameters.Add(sName:="last_modified", vValue:=DBNull.Value.ToString(), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMInteger)
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    Return gPMConstants.PMEReturnCode.PMFalse
                End If
                ...
                'developers guide no. 85
                m_lReturn = .Parameters.Add(sName:="modified_by_id", vValue:=DBNull.Value.ToString(), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMInteger)
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    Return gPMConstants.PMEReturnCode.PMFalse
                End If
            ...
            If WorkflowInformation = "" Then
                ...
                'developers guide no. 85
                m_lReturn = .Parameters.Add(sName:="workflow_information", vValue:=DBNull.Value.ToString(), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMInteger)
            Else
            ...
            If SourceID > 0 And bLimitBySource Then
                ...
                End If
            End If
            m_lReturn = .Parameters.Add(sName:="Is_task_review", vValue:=CStr(IsTaskReview), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMInteger)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            If IsExternalWorkItem Then

                m_lReturn = .Parameters.Add(sName:="is_external_workItem", vValue:=CStr(1), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMInteger)
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    Return gPMConstants.PMEReturnCode.PMFalse
                End If

                If ParentTaskId <> 0 Then
                    m_lReturn = .Parameters.Add(sName:="pmwrk_task_parent_instance_cnt", vValue:=CStr(ParentTaskId), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMInteger)
                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                        Return gPMConstants.PMEReturnCode.PMFalse
                    End If
                End If

                If GuidPMExternalItem <> Guid.Empty Then
                    m_lReturn = .Parameters.Add(sName:="external_workflow_id", vValue:=GuidPMExternalItem.ToString(), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMString)
                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                        Return gPMConstants.PMEReturnCode.PMFalse
                    End If
                End If
    ...
    Private Function AddKeyInputParam() As Integer
        ...
        With m_oDatabase
            ...
            m_lReturn = .Parameters.Add(sName:="pmwrk_task_instance_cnt", vValue:=CStr(PMWrkTaskInstanceCnt), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)
    ...
    Private Shared _DefaultInstance As PMWrkTaskInstance = Nothing
    Public Shared ReadOnly Property DefaultInstance() As PMWrkTaskInstance
        Get
            If _DefaultInstance Is Nothing Then
                _DefaultInstance = New PMWrkTaskInstance
            End If
