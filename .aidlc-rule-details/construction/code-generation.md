...
### Repository Routing Rules (Pure Insurance — MANDATORY)

Pure Insurance uses **two separate repositories**, cloned side-by-side in the same parent folder. Before writing a single line of code, identify which repo the change belongs to.

> **Path resolution**: Both repos share the same parent directory. If the current workspace is `{parent}\PureInsurance`, the REST repo is at `{parent}\PureInsurance.REST`. Resolve the sibling path at runtime — never hardcode a username or machine-specific path.

| Change type | Repo |
|-------------|------|
| REST API endpoint, Controller, Handler, Service, Repository, Domain model, API test | `PureInsurance.REST` |
| Portal .aspx page / code-behind | `PureInsurance` |
| Back-office WinForms / `b*` / `i*` / `g*Library` VB.NET component | `PureInsurance` |
| Stored procedure, migration script | `PureInsurance` |
| Navigator XM roadmap XML | `PureInsurance` |
| Spec files (`.aidlc/`, `.ai/memory/`, `.amazonq/`) | `PureInsurance` |

**Most Portal features require changes in BOTH repos.** Always list which files change in which repo in the code generation plan before starting.

**Wrong-repo rule**: If a plan step targets a file in the wrong repo, STOP and correct the plan. Do not write code to the wrong repository.
