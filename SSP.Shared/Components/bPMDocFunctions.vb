...
Public Module bPMDocFunctions
    ...
    '            bPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to start Word", vApp:=ACApp, vClass:=ACClass, vMethod:="GetWordVersion", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
    ...
    '        If lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
    '            bPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to close Word", vApp:=ACApp, vClass:=ACClass, vMethod:="GetWordVersion", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
    ...
    '        ' Log Error Message
    '        bPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetWordVersion Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetWordVersion", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function StartWord(ByRef r_oWord As Word.Application, Optional ByRef r_lWordHandle As Integer = 0, Optional ByRef r_sWordVersion As String = "") As Integer
        ...
        Try
            ...
            If r_lWordHandle = 0 Then
                ...
                bPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=sTemp, vApp:=ACApp, vClass:=ACClass, vMethod:="StartWord")
                Return result
            End If
            ...
            If sWordVersion <> "" Then
                ...
                If Conversion.Val(sWordVersion.Substring(0, iTemp)) < 8 Then
                    ...
                    sTemp = "Incorrect Word version for Sirius (" & sWordVersion & ")." & Strings.ChrW(13) & Strings.ChrW(10) & "Contact Policy Master Support."
                    bPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=sTemp, vApp:=ACApp, vClass:=ACClass, vMethod:="StartWord")
                    'sj 08/11/2001 - end
                    result = gPMConstants.PMEReturnCode.PMFalse
                    r_oWord = Nothing
            ...
            Else
                'sj 08/11/2001 - start
                sTemp = "An error has occurred with Microsoft Word." & Strings.ChrW(13) & Strings.ChrW(10) & "Try starting with Word already open or Contact Policy Master Support."
                bPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=sTemp, vApp:=ACApp, vClass:=ACClass, vMethod:="StartWord")
                'sj 08/11/2001 - end
                result = gPMConstants.PMEReturnCode.PMFalse
                r_oWord = Nothing
        ...
        Catch excep As System.Exception
            ...
            ' Log Error.
            bPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to start Word", vApp:=ACApp, vClass:=ACClass, vMethod:="StartWord", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function CloseWord(ByRef r_oWord As Object, ByVal lHandle As Integer, Optional ByRef bSaveChanges As Boolean = True) As Integer
        ...
        Try
            ...
            For iCnt As Integer = 1 To r_oWord.Templates.Count
                ...
                r_oWord.Templates.Item(ToSafeInteger(iCnt)).saved = True

                Debug.WriteLine(r_oWord.Templates.Item(ToSafeInteger(iCnt)).FullName)
            Next
        ...
        Catch excep As System.Exception
            ...
            Else
                bPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to close Word", vApp:=ACApp, vClass:=ACClass, vMethod:="CloseWord", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
            Return result
        End Try
    End Function
    ...
    Public Function ClearDirectory(ByVal sDirectory As String) As Integer
        ...
        Catch excep As System.Exception
            ...
            Else
                ...
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="ClearDirectory Failed. Directory Name [" & sDirectory & "]",
                                   vApp:=ACApp, vClass:=ACClass, vMethod:="ClearDirectory", vErrNo:=Information.Err().Number,
    ...
    Public Function MoveFolderContents(ByVal sSourceDir As String, ByVal sDestDir As String) As Integer
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                ...
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                   sMsg:="MoveFolderContents Failed." & Strings.ChrW(13) & Strings.ChrW(10) &
                                "Source Directory [" & sSourceDir & "]" & Strings.ChrW(13) & Strings.ChrW(10) &
            ...
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                ...
                                "Target Directory [" & sDestDir & "]", vApp:=ACApp,
                                                   vClass:=ACClass, vMethod:="MoveFolderContents", vErrNo:=Information.Err().Number,
                                                  vErrDesc:=Information.Err().Description, excep:=Nothing)
                'bPMFunc.LogMessage((m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="MoveFolderContents Failed." & Strings.ChrW(13) & Strings.ChrW(10) &
                ' "Source Directory [" & sSourceDir & "]" & Strings.ChrW(13) & Strings.ChrW(10) &
                '  "Target Directory [" & sDestDir & "]", vApp:=ACApp, vClass:=ACClass, vMethod:="MoveFolderContents", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description, excep:=excep)
            End If
            Return result
        End Try
    End Function
            ...
            If Not Directory.Exists(sFolderName) Then
                ...
                If m_bIsCalledFromBatchProcess = True Then
                    ...
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                      ...
                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="CreateFolderTree Failed", vApp:=ACApp,
                                       vClass:=ACClass, vMethod:="CreateFolderTree", vErrNo:=0,
                                       vErrDesc:="Unable to create the directory (" & sFolderName & ")")
                End If
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                ...
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                  ...
                                                  vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="CreateFolderTree Failed attempting to create " & sFolderName, vApp:=ACApp, vClass:=ACClass, vMethod:="CreateFolderTree", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                If m_bIsCalledFromBatchProcess = True Then
                    ...
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                      ...
                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to get Document directory from Registry.", vApp:=ACApp, vClass:=ACClass, vMethod:="GetDocumentDirectory", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                End If
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                ...
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                  ...
                                                  vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetDocumentDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetDocumentDirectory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
    ...
    Public Function GetClientDirectory(ByRef r_sClientDir As String, Optional ByVal bUnique As Boolean = False) As Integer
        ...
        Try
            Dim sDir As String, sTemp As String = ""
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                If m_bIsCalledFromBatchProcess = True Then
                    ...
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                      ...
                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to get Client directory from Registry.", vApp:=ACApp, vClass:=ACClass, vMethod:="GetClientDirectory", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                End If
                result = gPMConstants.PMEReturnCode.PMFalse
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    If m_bIsCalledFromBatchProcess = True Then
                        ...
                        bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                          ...
                                                          vErrDesc:=Information.Err().Description, excep:=Nothing)
                    Else
                        bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to get a temp directory.", vApp:=ACApp, vClass:=ACClass, vMethod:="GetClient", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                    End If
            ...
            If Not Directory.Exists(sDir) Then
                ...
                ' did we succeed...?
                If Not Directory.Exists(sDir) Then
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetClientDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetClientDirectory", vErrNo:=0, vErrDesc:="Unable to create the directory (" & sDir &
                                       ")")
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                ...
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                  ...
                                                  vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetClientDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetClientDirectory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                ...
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                  ...
                                                  vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetUniqueName Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetUniqueName", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
            Return nResult
        End Try
    End Function
    ...
    Public Function GetExportDirectory(ByRef r_sExportDirectory As String, Optional ByVal bUnique As Boolean = False) As Integer
        ...
        Try
            Dim sTemp As String = "", sDir As String = ""
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Or sDir = "" Then
                ...
                If m_bIsCalledFromBatchProcess = True Then
                    ...
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                      ...
                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetExportDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetExportDirectory", vErrNo:=Information.Err().Number, vErrDesc:="Unable to find the registry entry for the PrintFile directory location")
                End If
                result = gPMConstants.PMEReturnCode.PMError
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    If m_bIsCalledFromBatchProcess = True Then
                        ...
                        bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                          ...
                                                          vErrDesc:=Information.Err().Description, excep:=Nothing)
                    Else
                        bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to get a temp directory.", vApp:=ACApp, vClass:=ACClass, vMethod:="GetExportDirectory", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                    End If
                ...
                If Not Directory.Exists(sDir) Then
                    If m_bIsCalledFromBatchProcess = True Then
                        ...
                        bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                          ...
                                                          vErrDesc:=Information.Err().Description, excep:=Nothing)
                    Else
                        bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetExportDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetExportDirectory", vErrNo:=0, vErrDesc:="Unable to create the directory (" & sDir &
                                       ")")
                    End If
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                ...
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                  ...
                                                  vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetExportDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetExportDirectory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
    ...
    Public Function GetZipDirectory(ByRef r_sZipDirectory As String) As Integer
        ...
        Try
            Dim sTemp As String = "", sZipDir As String = ""
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Or sTemp = "" Then
                ...
                If m_bIsCalledFromBatchProcess = True Then
                    ...
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                      ...
                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetZipDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetZipDirectory", vErrNo:=Information.Err().Number, vErrDesc:="Unable to find the registry entry for the doczip directory location")
                End If
            ...
            If Not Directory.Exists(sZipDir) Then
                If m_bIsCalledFromBatchProcess = True Then
                    ...
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                      ...
                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetZipDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetZipDirectory", vErrNo:=0, vErrDesc:="Unable to create the directory (" & sZipDir &
                                   ")")
                End If
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                ...
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                  ...
                                                  vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetZipDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetZipDirectory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
    ...
    Public Function UnZip(ByVal sZipFile As String, ByVal sOutputDirectory As String, Optional ByVal v_bDeleteZipFile As Boolean = False) As Integer
        ...
        Try
            Dim sFileIn As String, sFileOut As String = " ", sTemp As String = ""
            ...
            If Not File.Exists(sZipFile) Then
                ...
                If m_bIsCalledFromBatchProcess = True Then
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                      ...
                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="File (" & sZipFile & ") does not exist", vApp:=ACApp, vClass:=ACClass, vMethod:="UnZip", vErrNo:=Information.Err().Number, vErrDesc:="Unzip failed")
                End If
            ...
            If Not Directory.Exists(sOutputDirectory) Then
                ...
                If m_bIsCalledFromBatchProcess = True Then
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                          ...
                                                                          vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Directory (" & sOutputDirectory & ") does not exist", vApp:=ACApp, vClass:=ACClass, vMethod:="UnZip", vErrNo:=Information.Err().Number, vErrDesc:="Unzip failed")
                End If
            ...
            If sOutputDirectory.ToLower() <> sZipFileFolderName Then
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    ...
                    If m_bIsCalledFromBatchProcess = True Then
                        bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                               sMsg:="Failed to copy file." & Strings.ChrW(13) & Strings.ChrW(10) &
                                       "Source File      : " & sZipFile & Strings.ChrW(13) & Strings.ChrW(10) &
                                       "Destination File : " & sFileIn & Strings.ChrW(13) & Strings.ChrW(10) &
                                       "Error Details    : ", vApp:=ACApp, vClass:=ACClass, vMethod:="UnZip", vErrNo:=Information.Err().Number,
                                                                              vErrDesc:=Information.Err().Description, excep:=Nothing)
                    Else
                        bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to copy file." & Strings.ChrW(13) & Strings.ChrW(10) &
                                       "Source File      : " & sZipFile & Strings.ChrW(13) & Strings.ChrW(10) &
                                       "Destination File : " & sFileIn & Strings.ChrW(13) & Strings.ChrW(10) &
            ...
            oZipper = bPMFunc.CreateLateBoundObject("bPMZipper.Business")
            If oZipper Is Nothing Then
                ...
                If m_bIsCalledFromBatchProcess = True Then
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                          ...
                                                                          vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to load zipper object", vApp:=ACApp, vClass:=ACClass, vMethod:="UnZip", vErrNo:=Information.Err().Number, vErrDesc:="Unzip failed")
                End If
            ...
            bSuccess = oZipper.UnZipFile(ToSafeString(sFileIn), ToSafeString(sOutputDirectory))
                    ...
                    If Not Directory.Exists(sDependencyDir) Then
                        ...
                        If m_bIsCalledFromBatchProcess = True Then
                            bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                                  ...
                                                                                  vErrDesc:=Information.Err().Description, excep:=Nothing)
                        Else
                            bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to create directory (" & sDependencyDir & ")", vApp:=ACApp, vClass:=ACClass, vMethod:="UnZip", vErrNo:=Information.Err().Number, vErrDesc:="Unzip failed")
                        End If
                        Return result
                    End If
                End If
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                       sMsg:="Failed to unzip the document. " & Strings.ChrW(13) & Strings.ChrW(10) &
                               "Zip File [" & sZipFile & "]" & Strings.ChrW(13) & Strings.ChrW(10) &
                               "sOutputDirectory [" & sOutputDirectory & "]", vApp:=ACApp,
                                                                       vClass:=ACClass, vMethod:="UnZip", vErrNo:=Information.Err().Number,
                                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to unzip the document. " & Strings.ChrW(13) & Strings.ChrW(10) &
                               "Zip File [" & sZipFile & "]" & Strings.ChrW(13) & Strings.ChrW(10) &
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                      ...
                                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetFileNameAndType Failed. File Name : " & v_sFolder, vApp:=ACApp, vClass:=ACClass, vMethod:="GetFileNameAndType", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                      ...
                                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="DelDirectory Failed. Directory Name : " & r_sDocDirectory, vApp:=ACApp, vClass:=ACClass, vMethod:="DelDirectory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                      ...
                                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to Check FolderExists for '" & v_sFolderName & "'", vApp:=ACApp, vClass:=ACClass, vMethod:="IsFolderExists", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                      ...
                                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to Check FileExists for '" & v_sFileName & "'", vApp:=ACApp, vClass:=ACClass, vMethod:="IsFileExists", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
                    ...
                    Else
                        ...
                        If m_bIsCalledFromBatchProcess = True Then
                            bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                                  ...
                                                                                  vErrDesc:=Information.Err().Description, excep:=Nothing)
                        Else
                            bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="CreateFolder Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="CreateFolder", vErrNo:=0, vErrDesc:="Unable to create the directory (" & v_sFolderName & ")")
                        End If
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                      ...
                                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="CreateFolder Unable to create the directory (" & v_sFolderName & ")", vApp:=ACApp, vClass:=ACClass, vMethod:="CreateFolder", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
            Return result
    ...
    '                                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
    '            Else
    '                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Unable to Delete file '" & v_sFileName & "'", vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteFile", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
    '            End If
    ...
    '                                                                  vErrDesc:=Information.Err().Description, excep:=Nothing)
    '        Else
    '            bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to Delete file '" & v_sFileName & "'", vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteFile", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    '        End If
    ...
    Public Function CopyFile(ByVal v_sSourceFile As String, ByVal v_sDestinationFile As String, Optional ByVal v_bOverWriteFiles As Boolean = True, Optional ByVal v_bDeleteSoureFile As Boolean = False, Optional ByRef r_vErrorMessage As String = "", Optional ByVal v_bCalledFromBatchProcessing As Boolean = False) As Integer
        ...
        Try
            ...
            If v_sSourceFile.Length = 0 Then
                If m_bIsCalledFromBatchProcess Then
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                  ...
                                  vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Source file name is null", vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFile", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                End If
            ...
            If v_sDestinationFile.Length = 0 Then
                If m_bIsCalledFromBatchProcess Then
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                  ...
                                  vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Destination file name is null", vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFile", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                End If
            ...
            If File.Exists(v_sSourceFile) Then
                    ...
                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                        ...
                        If m_bIsCalledFromBatchProcess Then
                            bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                          ...
                                          vErrDesc:=Information.Err().Description, excep:=Nothing)
                        Else
                            bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Unable to create the Destination Folder (" & sDestinationFolderName & ")", vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFile", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                        End If
                ...
                sErrorMessage = "The folder [" & sDestinationFolderName & "] is having Folling Attributes" & Strings.ChrW(13) & Strings.ChrW(10)

                bCanCopy = True

                If CInt(oFolder.Attributes) = 1 Then
                    sErrorMessage = sErrorMessage & "Read-Only" & Strings.ChrW(13) & Strings.ChrW(10)
                    bCanCopy = False
                End If

                If CInt(oFolder.Attributes) = 4 Then
                    sErrorMessage = sErrorMessage & "System File" & Strings.ChrW(13) & Strings.ChrW(10)
                    bCanCopy = False
                End If

                If CInt(oFolder.Attributes) = 64 Then
                    sErrorMessage = sErrorMessage & "Link or shortcut" & Strings.ChrW(13) & Strings.ChrW(10)
                    bCanCopy = False
                End If
                ...
                Else
                    ...
                    If m_bIsCalledFromBatchProcess Then
                        bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                       sMsg:="Unable to Copy the File (" & v_sSourceFile & ") because" & Strings.ChrW(13) & Strings.ChrW(10) & sErrorMessage, vApp:=ACApp,
                                       vClass:=ACClass, vMethod:="CopyFile", vErrNo:=Information.Err().Number,
                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
                    Else
                        bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Unable to Copy the File (" & v_sSourceFile & ") because" & Strings.ChrW(13) & Strings.ChrW(10) &
            ...
            Else
                ...
                If m_bIsCalledFromBatchProcess Then
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                  ...
                                  vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Source File Missing (" & v_sSourceFile & ")", vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFile", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                End If
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess Then
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                              ...
                              vErrDesc:=Information.Err().Description, excep:=excep)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to copy " & v_sSourceFile & " to " & v_sDestinationFile, vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFile", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
            Return result
    ...
    Public Function CopyFolder(ByVal v_sSourceFolder As String, ByVal v_sDestinationFolder As String, Optional ByVal v_bOverWriteFiles As Boolean = True, Optional ByRef r_vErrorMessage As String = "") As Integer
        ...
        Try
            ...
            If v_sSourceFolder.Length = 0 Then
                ...
                sErrorMessage = sErrorMessage & "Source Folder name is null." & Strings.ChrW(13) & Strings.ChrW(10)
                ' Log Error Message
                If m_bIsCalledFromBatchProcess = True Then
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                          ...
                                                                          vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Source Folder name is null", vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFolder", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                End If
            ...
            If v_sDestinationFolder.Length = 0 Then
                sErrorMessage = sErrorMessage & "Destination Folder name is null." & Strings.ChrW(13) & Strings.ChrW(10)
                ' Log Error Message
                If m_bIsCalledFromBatchProcess = True Then
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                          ...
                                                                          vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Destination Folder name is null", vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFolder", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                End If
            ...
            Else
                ...
                sErrorMessage = sErrorMessage & "Missing Source Folder [" & v_sSourceFolder & "]" & Strings.ChrW(13) & Strings.ChrW(10)
                ...
                If m_bIsCalledFromBatchProcess = True Then
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                          ...
                                                                          vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Missing Source Folder [" & v_sSourceFolder & "]", vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFolder", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                End If
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                       sMsg:="Failed to Copy Directory" & Strings.ChrW(13) & Strings.ChrW(10) &
                               "From : [" & v_sSourceFolder & "]" & Strings.ChrW(13) & Strings.ChrW(10) &
                               "To   : [" & v_sDestinationFolder & "]", vApp:=ACApp,
                                                                       vClass:=ACClass, vMethod:="CopyFolder", vErrNo:=Information.Err().Number,
                                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to Copy Directory" & Strings.ChrW(13) & Strings.ChrW(10) &
                               "From : [" & v_sSourceFolder & "]" & Strings.ChrW(13) & Strings.ChrW(10) &
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                      ...
                                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to Get No Of Files In Directory (" & v_sDirectoryName & ")", vApp:=ACApp, vClass:=ACClass, vMethod:="NoOfFilesInDirectory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
    ...
    Public Function Zip(ByVal sPath As String, ByVal sInputDocumentFileExtension As String) As Integer
        ...
        Dim sFileIn As String = String.Empty
        Dim sFileOut, sParentFile, sDependencyDir, sDependency As String
        Dim oZipper As Object = Nothing
        Dim oFolder As DirectoryInfo = Nothing
        ...
        Try
            ...
            If oZipper Is Nothing Then
                ...
                If m_bIsCalledFromBatchProcess = True Then
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                          ...
                                                                          vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to load zipper object", vApp:=ACApp, vClass:=ACClass, vMethod:="Zip", vErrNo:=Information.Err().Number, vErrDesc:="Zip failed")
                End If
                Return result
            End If

            iTemp = oZipper.ZipFile(sFileIn:=ToSafeString(sFileIn), sFileOut:=ToSafeString(sFileOut))
            If Not iTemp Then
                oZipper = Nothing
                Return gPMConstants.PMEReturnCode.PMFalse
            End If
            ...
            If Directory.Exists(sDependencyDir) Then
                ...
                For Each oFile2 As FileInfo In oFilesCollection
                    ...
                    iTemp = oZipper.addFileToZIP(ToSafeString(sFileOut), ToSafeString(sFileNameWithPath))
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    ...
                    If m_bIsCalledFromBatchProcess = True Then
                        bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                              ...
                                                                              vErrDesc:=Information.Err().Description, excep:=Nothing)
                    Else
                        bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to delete the dependency folder. (" & sDependencyDir & "')", vApp:=ACApp, vClass:=ACClass, vMethod:="Zip", vErrNo:=Information.Err().Number, vErrDesc:="Zip failed")
                    End If
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                ...
                If m_bIsCalledFromBatchProcess = True Then
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                          ...
                                                                          vErrDesc:=Information.Err().Description, excep:=Nothing)
                Else
                    bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to delete the file. (" & sFileIn & "')", vApp:=ACApp, vClass:=ACClass, vMethod:="Zip", vErrNo:=Information.Err().Number, vErrDesc:="Zip failed")
                End If
        ...
        Catch excep As System.Exception
            ...
            If m_bIsCalledFromBatchProcess = True Then
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                                                                      ...
                                                                      vErrDesc:=Information.Err().Description, excep:=Nothing)
            Else
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to zip the document. (" & sFileIn & ")", vApp:=ACApp, vClass:=ACClass, vMethod:="Zip", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            End If
            Return result
    ...
    Public Function ConvertDocumentUsingSiriusDocumentUtility(ByVal v_sSourceDocument As Object, ByVal v_sDestDocument As Object) As Integer
        Dim result As Integer = 0
        Const kMethodName As String = "ConvertDocumentUsingSiriusDocumentUtility"

        Dim oConvert As Object = Nothing
        Try

            result = gPMConstants.PMEReturnCode.PMTrue

            oConvert = bPMFunc.CreateLateBoundObject("SiriusDocumentUtility.Document")
