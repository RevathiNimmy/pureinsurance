Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("PolicyNumberGen_NET.PolicyNumberGen")>
Public Module PolicyNumberGen
    ...
    Public Function GenPolicyNum(ByVal v_lInsurerNo As Integer, ByVal v_lSchemeNo As Integer, ByVal v_dtEffectiveDate As Date, ByVal v_sQMMSuffix As String, ByRef r_sPolicyNum As String) As Integer
        ...
        Try
            ...
            lReturn = oAutoNum.Initialise(ToSafeString(g_sUsername), CStr(g_sPassword), CInt(g_iUserID), CInt(g_iSourceID), CInt(g_iLanguageID), CInt(g_iCurrencyID), CInt(g_iLogLevel), ToSafeString(ACApp))
            ...
            lReturn = oAutoNum.GenerateNewNumber(v_sPMProductCode:=gPMConstants.PMProductCode(gPMConstants.PMEProductFamily.pmePFSiriusSolutions), v_sGroupCode:=CStr(I4MPolicyNums), v_sRangeCode:=CStr(sRangeCode), v_iUserId:=1, r_lNumber:=CInt(lPolNum), r_sPrefix:=CStr(sPrefix), r_sSuffix:=CStr(sSuffix))
