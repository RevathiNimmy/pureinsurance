...
Namespace Nexus
    ...
    Partial Class Controls_ReportControls_RenewalPreList : Inherits BaseReport
        ...
        Protected Sub Page_Load1(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            If IsPostBack Then
                ...
                ' Querying (Lookup) the database to fetch the Products table's information
                Dim oProductLookup As New NexusProvider.LookupListCollection
                oProductLookup = oWebService.GetList(NexusProvider.ListType.PMLookup, "Product", False, False, "Product_ID")

                ' Check if product list is not empty
                If oProductLookup IsNot Nothing AndAlso oProductLookup.Count > 0 Then
                    RP__product_code.Items.Insert(0, New ListItem("ALL", "ALL"))
                    For Each oProduct As NexusProvider.LookupListItem In oProductLookup
                        RP__product_code.Items.Add(New ListItem(oProduct.Description, oProduct.Description))
                    Next
                End If
