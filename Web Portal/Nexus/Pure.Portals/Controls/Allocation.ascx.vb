...
Namespace Nexus
    Partial Class Controls_Allocationtab
        ...
        Dim hfTransdetailKey As HiddenField
        ...
        Protected Shadows Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Session("ModeValue") = "Allocation" Then
                ...
                Dim oustandingamount As Double
                Dim traskeyCount As Integer
                If (Session(CNMultipleAllocation) = True OrElse hdnTextChange.Value <> "txtWriteOffChangeEvent") Then

                    If (Session("ModeValue") = "Allocation" And Request("__EVENTARGUMENT") <> "RefreshAllocate") Then
                    ...
                    CalculateTotals()
                    If (Session("ModeValue") = "Allocation" And Request("__EVENTARGUMENT") <> "") Then
                        Session.Add(CNTransctionforUpdate, oTrasactionDetails)
                    End If
                    ...
                    If Session("ModeValue") = "Allocation" Then
                        ...
                        Else
                            btnAdd.OnClientClick = "tb_show(null , '../SearchTransactions.aspx?AllocationAccountkey=" & iAllocationAccountKey & "&Mode=CashListAllocation&modal=true&KeepThis=true&TB_iframe=true&height=500&width=800' , null);return false;"
                        End If
                    End If
                End If

                If Request("__EVENTARGUMENT") = "ViewAllocationClick" Then
                    hfTransdetailKey = CType(CType(Nexus.Utils.GetMasterPlaceHolder(Page, oNexusFrameWork.MainContainerName), ContentPlaceHolder).FindControl("hfTransdetailKey"), HiddenField)

                    PopulateGrid()
                End If
            End If

            If Request("__EVENTARGUMENT") = "RefreshAllocate" OrElse Session(CNMultipleAllocation) = True Then
                ViewState.Add(CNOTempKEY, Nothing)
                UpdateTransactions()
                Session.Remove(CNMultipleAllocation)
            End If
        End Sub
        ...
        Protected Sub UpdateTransactions()
                ...
                If ViewState(CNOTempKEY) Is Nothing Then
                    If iVarTranscount > 0 Then
                        For i As Integer = 0 To iVarTranscount - 1
                            oAllationDetails = New NexusProvider.AllocationDetails
                            oAllationDetails.TransdetailKey = Convert.ToInt32(DirectCast(Session(CNTransDeatilsKeys), ArrayList)(i))
                            If oTempTranskey.Contains(Convert.ToInt32(DirectCast(Session(CNTransDeatilsKeys), ArrayList)(i))) = False Then
                                oTempTranskey.Add(Convert.ToInt32(DirectCast(Session(CNTransDeatilsKeys), ArrayList)(i)))
                            End If
                            ...
                            If hvKeyToRemove.Value <> "" AndAlso bRemove Then
                                ...
                                If gvAllocate.Rows.Count > 0 Then
                                    ...
                                    If gvAllocate.Rows.Count <> oAllocationDetailsCollections.Count Then
                                        oAllocationDetailsCollections = DirectCast(Session(CNTransctionforUpdate), NexusProvider.AllocationDetailsCollections)
                                    End If
            ...
            Else
                hidden_WriteOffCurrency.Value = String.Empty
            End If
            hdnTextChange.Value = "txtWriteOffChangeEvent"

            txtwriteOffReason.Visible = False  
            writeOffReason.Visible = True  
        End Sub
        ...
        Protected Sub gvAllocate_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvAllocate.RowDataBound
            If e.Row.RowType = DataControlRowType.Header Or e.Row.RowType = DataControlRowType.DataRow Or e.Row.RowType = DataControlRowType.Footer Then
                e.Row.Cells(11).Visible = If(Session("ModeValue") = "Allocation", True, False)
            End If
            ...
            If e.Row.RowType = DataControlRowType.DataRow Then
                ...
                e.Row.Cells(5).Text = oItem.TransactionCurrencyAmount.ToString() 
                ...
                If Session("ModeValue") = "Allocation" Then
                    ...
                    txtAllocated.Text = Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "AllocatedAmount")).ToString("N2")
                     ...
                     Dim txtWriteOff As TextBox = DirectCast(e.Row.FindControl("txtWriteOff"), TextBox)
                    txtWriteOff.Text = Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "WriteOffAmount")).ToString("N2")
...
#Region "Validations"
        ...
        Protected Sub custvldWriteoffAuthority_ServerValidate(source As Object, args As ServerValidateEventArgs) Handles custvldWriteoffAuthority.ServerValidate
            If String.IsNullOrEmpty(hidden_WriteOffUserAuthority.Value.Trim()) Then
                args.IsValid = True
                Exit Sub
            End If
        ...
        Protected Sub custvldWriteoffLimit_ServerValidate(source As Object, args As ServerValidateEventArgs) Handles custvldWriteoffLimit.ServerValidate
            If Page.IsValid Then
                If String.IsNullOrEmpty(hidden_limitwriteOffamount.Value.Trim()) Then
                    args.IsValid = True
                    Exit Sub
                End If
...
#End Region
        ...
        Public Sub ClickOK()
            ...
            Dim mainTrasDetailskey As Integer
            Dim oUpdateAllocation As New NexusProvider.AllocationDetails
            Dim dCreditAllocatedAmount As Double
            'If all condition are satisfied
            'If Page.IsValid Then
            oTrasactionDetails = Session(CNTransctionforUpdate)
            ...
            If oUserAuthority.UserAuthorityValue = "0" Or (oUserAuthority.UserAuthorityValue = "1" And WriteoffAmount = 0) Then
                If oUserAuthority.UserAuthorityValue = "0" Then
                    ...
                    If gvAllocate.Rows.Count > 0 Then
                        lblWriteOffTotal = DirectCast(gvAllocate.FooterRow.FindControl("lblWriteOffTotal"), Label)
                        writeOffAmt = Convert.ToDecimal(lblWriteOffTotal.Text.Trim())
                    End If
                    Dim oCurrencies As Nexus.Library.Config.Currencies
                    oCurrencies = CType(GetSection("NexusFrameWork"), Nexus.Library.Config.NexusFrameWork).Currencies
                    writeOffAuthority = hidden_WriteOffUserAuthority.Value.Trim()
                    ...
                    If (writeOffAuthority <> "1" AndAlso Math.Abs(writeOffAmt) > 0) Then
                        custvldWriteoffAuthority.IsValid = False
                        custvldWriteoffAuthority.Enabled = True
                        Dim errorMsg As String = Convert.ToString(GetLocalResourceObject("lbl_WriteOffAuthority"))
                        custvldWriteoffAuthority.ErrorMessage = errorMsg
                        Exit Sub
                    Else
                        custvldWriteoffAuthority.IsValid = True
                    End If
                End If
                ' When the Write off Option is not enabled or WriteOff Amount Not Specified than this will be executed.
                ' Need to Find the First -Ve value(First Credit and it will be Main Transaction)
            ...
            Else
                ...
                If WriteoffAmount <> 0 Then
                    ...
                    For Each row In gvAllocate.Rows
                        If row.RowType = DataControlRowType.DataRow Then
                            Dim txtWrite As TextBox = DirectCast(row.FindControl("txtWriteOff"), TextBox)
                            If Not String.IsNullOrEmpty(txtWrite.Text) AndAlso Convert.ToDecimal(txtWrite.Text) <> 0 Then
                                writeOffCount = writeOffCount + 1
                            End If
                        End If
                    Next
                    If writeOffCount > 1 Then
                        custvldWriteOffOnlyOne.Enabled = True
                        custvldWriteOffOnlyOne.IsValid = False
                        Exit Sub
                    Else
                        custvldWriteOffOnlyOne.Enabled = False
                        custvldWriteOffOnlyOne.IsValid = True
                    End If
                    If (writeOffReason.Value = "") Then
                        Dim writeOffAmt As Decimal = 0
                        Dim lblWriteOff As Label
                        If gvAllocate.Rows.Count > 0 Then
                            lblWriteOff = DirectCast(gvAllocate.FooterRow.FindControl("lblWriteOffTotal"), Label)
                            ...
                            End If
                            If Math.Abs(Convert.ToDecimal(lblWriteOff.Text.Trim())) > 0 AndAlso (String.IsNullOrEmpty(writeOffReason.Value) OrElse writeOffReason.Value = 0) Then
                                custvldWriteoffReason.Enabled = True
                                custvldWriteoffReason.IsValid = False
                                Exit Sub
                            Else
                                custvldWriteoffReason.IsValid = True
                            End If
                        Else
                            custvldWriteoffReason.IsValid = True
                        End If

                    ElseIf (Convert.ToDecimal(hidden_limitwriteOffamount.Value.Trim()) < WriteoffAmount) Then
                        Dim oCurrencies As Nexus.Library.Config.Currencies
                        custvldWriteoffAuthority.IsValid = False
                        custvldWriteoffAuthority.Enabled = True
                        oCurrencies = CType(GetSection("NexusFrameWork"), Nexus.Library.Config.NexusFrameWork).Currencies
                    ...
                    If hdnTextChange.Value = "txtWriteOffChangeEvent" OrElse Page.IsValid Then
                        If Session(CNTransBranchCode) IsNot Nothing Then
                            oWebService.UpdateAllocation(oUpdateAllocation, Session(CNTransBranchCode).ToString())
                        Else
                            oWebService.UpdateAllocation(oUpdateAllocation)
            ...
            If Session("sSelectedAllocationIndex") IsNot Nothing Then
                Dim oUpdateCashListItem As NexusProvider.PaymentItems
                Dim selectedIndex As List(Of Integer) = New List(Of Integer)
                selectedIndex = Session("sSelectedAllocationIndex")
                For i As Integer = 0 To selectedIndex.Count - 1
                    If Session("ModeType") = "Payment" Then
                        oUpdateCashListItem = CType(Session(CNCashListItem), NexusProvider.PaymentCashListItemType).PaymentItems.Item(selectedIndex(i))
                        ...
                        oUpdateCashListItem.AllocationStatusCode = "Allocated"
                        Session(CNCashListItem) = oPaymentCashListItems
                    Else
                        oUpdateCashListItem = CType(Session(CNCashListItem), NexusProvider.ReceiptCashListItemType).ReceiptItems.Item(selectedIndex(i))
                ...
                Next
                Session.Remove("sSelectedAllocationIndex")
            End If
            ViewState.Add(CNOTempKEY, Nothing)
            ...
            If Session("ModeValue") = "Allocation" Then
                ...
                Session(CNCashListItemAllocationStatus) = "completed"
                Dim sAgentStartPage As String = CType(System.Web.Configuration.WebConfigurationManager.GetSection("NexusFrameWork").Portals.Portal(CMS.Library.Portal.GetPortalID()), Nexus.Library.Config.Portal).AgentStartPage
                'Response.Redirect("~/secure/payment/CashList_tab.aspx?Mode=Receipt&tab=tabCashListItems")
            ...
            SaveGridInObject()
        End Sub
        Public Function btnsDisable() As Boolean
            Dim isBtnDisabled As Boolean = False
            If cstvalidAccount.IsValid AndAlso custvldWriteOffOnlyOne.IsValid AndAlso custvldWriteoffReason.IsValid AndAlso custvldWriteoffAuthority.IsValid Then
                ScriptManager.RegisterStartupScript(Me.Page, GetType(String), "AllocateBtnsDisable", "AllocateBtnsDisable()", True)

                For Each row In gvAllocate.Rows
                    If row.RowType = DataControlRowType.DataRow Then
                        If row.FindControl("lnkbtnSelect") IsNot Nothing Then
                            Dim btnRemove As LinkButton = DirectCast(row.FindControl("lnkbtnSelect"), LinkButton)
        ...
        Protected Sub PopulateGrid()
            Dim DiscTrasKeyIndex As Integer = hfTransdetailKey.Value
            Dim oDictTransdetailKey As New Dictionary(Of Object, Object)
            Dim TransVal As Object
            if Session("TransdetailKeyDisc") IsNot Nothing
               ...
               Dim iCount As Integer = 0
                For Each Key As ArrayList  In oDictTransdetailKey.Keys
                    If iCount  = DiscTrasKeyIndex
                        TransVal = oDictTransdetailKey(Key)
                        Exit For
                    End If
                   iCount = iCount + 1
                Next
            ...
            End If
            Dim oTrasactionDetails As New NexusProvider.AllocationDetailsCollections 
            oTrasactionDetails = DirectCast(TransVal, NexusProvider.AllocationDetailsCollections)
            For i = 0 To oTrasactionDetails.Count - 1
                Dim TransDetailItem As NexusProvider.AllocationDetails = DirectCast (oTrasactionDetails.Item(i),NexusProvider.AllocationDetails)
                If TransDetailItem.WriteOffAmount <> 0
                    txtwriteOffReason.Text = TransDetailItem.AltRef
                    txtwriteOffReason.Visible = True 
                    writeOffReason.Visible = False 
                End If
            Next
            gvAllocate.DataSource = oTrasactionDetails 
            gvAllocate.DataBind()
            CalculateTotals()
                ...
                If row.RowType = DataControlRowType.DataRow Then
                    If row.FindControl("lnkbtnSelect") IsNot Nothing Then
                        ...
                        'make text box read only 
                         Dim txtAllocated As TextBox = CType(row.FindControl("txtAllocated"), TextBox)
                         Dim txtWriteOff As TextBox = CType(row.FindControl("txtWriteOff"), TextBox)
                        Dim txtCurrencyDiff As TextBox = CType(row.FindControl("txtCurrencyDiff"), TextBox)
                        txtAllocated.ReadOnly = True 
                        txtWriteOff.ReadOnly = True 
                        txtCurrencyDiff.ReadOnly = True 
                    End If
                End If
            Next
            'write off reason
            ...
            btnAllocateCancel.Visible = False
            btnAllocateOK.Visible = False
            btnClose.Visible = True

        End Sub

        Protected Sub SaveGridInObject()
        Dim oAllocationDetailsCollection As New NexusProvider.AllocationDetailsCollections
        Dim oNewAllocationDetails As NexusProvider.AllocationDetails

            Dim TransDetailkey As String = String.Empty 
            For i As Integer = 0 To gvAllocate.Rows.Count - 1
                Dim roww As GridViewRow = gvAllocate.Rows(i)
                TransDetailkey = gvAllocate.Rows(0).Cells(0).Text
                Dim lblOSAmount As Label  = CType(roww.FindControl("lblOSAmount"), Label)
                Dim txtAllocated As TextBox = CType(roww.FindControl("txtAllocated"), TextBox)
                Dim txtWriteOff As TextBox = CType(roww.FindControl("txtWriteOff"), TextBox)
                Dim txtCurrencyDiff As TextBox = CType(roww.FindControl("txtCurrencyDiff"), TextBox)
                
                    oNewAllocationDetails  = New NexusProvider.AllocationDetails

                    With oNewAllocationDetails
                        .DocRef = roww.Cells(1).Text
                        .OutStandingamount = Convert.ToDouble(lblOSAmount.Text) 
                        .AllocatedAmount = Convert.ToDouble(txtAllocated.Text)
                        .TransdetailKey = roww.Cells(0).Text
                        .Currency = roww.Cells(6).Text
                        .MediaType = roww.Cells(2).Text
                        .TransactionCurrency = roww.Cells(4).Text
                        .Amount = Convert.ToDouble(roww.Cells(5).Text)
                        .TransactionCurrencyAmount = Convert.ToDouble(roww.Cells(5).Text)                    
                    End With
                    If roww.Cells(2).Text = "&nbsp;"
                        oNewAllocationDetails.MediaType = String.Empty
                    End If
                   If roww.Cells(3).Text = "&nbsp;"
                        oNewAllocationDetails.TaxBand = String.Empty
                    End If
                    If txtWriteOff.Text <> 0 
                        oNewAllocationDetails.WriteOffAmount = Convert.ToDouble(txtWriteOff.Text)
                        oNewAllocationDetails.AltRef = writeOffReason.Text   
                    End If
                    If txtCurrencyDiff.Text <> "" 
                        oNewAllocationDetails.CurrencyDiff = Convert.ToDouble(txtCurrencyDiff.Text)  
                    End If
          ...
          oDictTransdetailKey.Add(Session(CNTransdetailKeyfromCashList),oAllocationDetailsCollection)
          Session("TransdetailKeyDisc") = oDictTransdetailKey
