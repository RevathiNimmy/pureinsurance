<%@ Control Language="VB" AutoEventWireup="false" CodeFile="Allocation.ascx.vb" Inherits="Nexus.Controls_Allocationtab" %>

<%@ Register TagPrefix="NexusProvider" Namespace="NexusProvider" Assembly="NexusProvider" %>
<div id="Controls_Allocate">
       <script language="javascript" type="text/javascript">
           ...
           function CheckWriteOffLimit(oSrc, args) {
               //if any 'write off entry' in the grid than make 'write off reason' mandatory
               var cntMainBody = "ctl00_cntMainBody_";
               var grid = document.getElementById(cntMainBody + "gvAllocate"); //selecting allocate grid
               var inputs = grid.getElementsByTagName("input"); //Retrieve all the input elements from the grid
               var iCountVar = 0;
           ...
           $(document).ready(function () {
               ...
               $('#txtAllocated').keyup(function () {

                   $('#hdnTextChange').val('txtAllocatedChangeEvent');  
               });
               $('#txtWriteOff').keyup(function () {                  
                   $('#hdnTextChange').val('txtWriteOffChangeEvent');
               });
               $('#txtCurrencyDiff').keyup(function () {

                   $('#hdnTextChange').val('txtCurrencyDiffChangeEvent');
               });

           });
           function writeOffReasonWarning(sWarning){
               alert(sWarning);
           }
           function AllocateBtnsEnable() {
               document.getElementById('<%= btnAdd.ClientId %>').removeAttribute("disabled");              
           }
           function AllocateBtnsDisable() {
                document.getElementById('<%= btnAdd.ClientId %>').setAttribute("disabled", true);  
            }
       </script>
    ...
    <div id="Modal_Allocate">
        <div class="card">
                ...
                <asp:UpdatePanel ID="updAllocate" runat="server" UpdateMode="Conditional" ChildrenAsTriggers="True">
                    <ContentTemplate>
                        <div class="grid-card table-responsive">
                            <asp:GridView ID="gvAllocate" runat="server" AutoGenerateColumns="false" GridLines="None" EmptyDataRowStyle-CssClass="noData" ShowFooter="true" EmptyDataText="<%$ Resources:ErrorMessage %>" FooterStyle-HorizontalAlign="Right">
                                <Columns>
                                    ...
                                    <asp:TemplateField HeaderText="<%$ Resources:lbl_WriteOff %>" ItemStyle-CssClass="GridAmtCol" FooterStyle-CssClass="GridAmtCol"
                                        HeaderStyle-CssClass="GridAmtCol" ControlStyle-CssClass="GridAmtCol">
                                        <ItemTemplate>
                                            <asp:TextBox ID="txtWriteOff" runat="server" Enabled="true" Visible="true" CssClass="FormatDecimal Doub form-control form-control-textbox AutoCompleteTextBox"
                            ...
                            <asp:HiddenField ID="hdnTextChange" runat="server" ClientIDMode="Static" Value="N"></asp:HiddenField>
                        </div>
                    </ContentTemplate>
                    <Triggers>
                                ...
                                <div class="row">
                                   <div class="col-md-1"></div>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <asp:TextBox ID="txtwriteOffReason" runat="server" CssClass="form-control" readonly="true" Visible="false" ></asp:TextBox>
            ...
            <div class="card-footer">
                ...
                <asp:LinkButton ID="btnOk" runat="server" Visible ="false" Text="<%$ Resources:btnOk %>" SkinID="btnPrimary" OnClientClick="ShowOverAllocationMsg();" ValidationGroup="grpvalAllocate"></asp:LinkButton>
        ...
        <asp:CustomValidator ID="custvldWriteoffAuthority" runat="server" Display="None" ErrorMessage="<%$ Resources:lbl_WriteOff_Limit %>" Enabled="true" ValidationGroup="grpvalAllocate"></asp:CustomValidator>
        <asp:CustomValidator ID="custvldWriteOffOnlyOne" runat="server" Display="None" ErrorMessage="<%$ Resources:lbl_WriteOff_OnlyOne %>" Enabled="true" ValidationGroup="grpvalAllocate"></asp:CustomValidator>
        <asp:CustomValidator ID="custvldWriteoffLimit" runat="server" Display="None" ErrorMessage="<%$ Resources:lbl_WriteOff_Limit %>" Enabled="true" ValidationGroup="grpvalAllocate"></asp:CustomValidator>
        <asp:CustomValidator ID="custvldWriteoffReason" runat="server" Display="None" ErrorMessage="<%$ Resources:lbl_WriteOff_Reasonselect %>" Enabled="true" ValidationGroup="grpvalAllocate"></asp:CustomValidator>
        <asp:CustomValidator ID="custvldValidateBalance" runat="server" Display="None" ErrorMessage="<%$ Resources:lbl_ValidateBalance %>" Enabled="true" ValidationGroup="grpvalAllocate"></asp:CustomValidator>
        <asp:CustomValidator ID="custVldValidateInputNumbers" runat="server" Display="None" ClientValidationFunction="ValidateTextBox" ErrorMessage="<%$ Resources:lbl_validNumber %>" Enabled="false" ValidationGroup="grpvalAllocate"></asp:CustomValidator>
        <asp:CustomValidator ID="cstvalidAccount" runat="server" Display="None" ErrorMessage="<%$ Resources:lbl_validAccount %>" Enabled="false" ValidationGroup="grpvalAllocate"></asp:CustomValidator>
        <asp:CustomValidator ID="cvSingleSRPnSPY" runat="server" SetFocusOnError="true" Display="None" ErrorMessage="<%$ Resources:lbl_SingleSRPSPYError %>" ClientValidationFunction="ValidateSelectedEntries" Enabled="false" ValidationGroup="grpvalAllocate"></asp:CustomValidator>
        <asp:ValidationSummary ID="ValidationSummary1" DisplayMode="BulletList" HeaderText="<%$ Resources:lbl_ValidationSummary %>" runat="server" CssClass="validation-summary" ValidationGroup="grpvalAllocate"></asp:ValidationSummary>
    </div>
