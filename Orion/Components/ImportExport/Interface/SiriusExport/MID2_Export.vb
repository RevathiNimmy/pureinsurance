...
Public NotInheritable Class MID2_Export : Inherits ExportBase
...
#Region "Public Methods"
    ...
    ''' <summary>
    ''' Display commandline help for MID2 Export
    ''' </summary>
    ''' <remarks></remarks>
    Public Overrides Sub DisplayHelp()
        ' Write syntax help for this command
        OutputLine("Example call : - SIRIUSEXPORT MID2_Export")
        OutputLine()
        ...
        For nItem = 0 To nNoOfCommandLineArgs
            ...
            Try
                ...
                Select Case sArgValues(0).ToUpper
                    ...
                    Case "EXPORT_TYPE"
                        m_sExportType = CStr(sArgValues(1))
    ...
    Public Overrides Sub ProcessExport()
        ...
        Else
            ...
            If nResult <> PMEReturnCode.PMTrue OrElse aoRules Is Nothing OrElse aoRules Is String.Empty Then
                OutputLine(String.Format("No active MID2 rule exists for any of the branches", m_nBatchID))
                Exit Sub
            End If

            For iParam As Integer = aoRules.GetLowerBound(1) To aoRules.GetUpperBound(1)
                Output(String.Format("Process MID2 Export for branch '{0}' ...", aoRules(1, iParam).ToString()))
    ...
    ''' <summary>
    ''' Method used to create an XML file in a format defined by MID2_Export.XSD
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub ExportBatch()
        ...
        If nReturn <> PMEReturnCode.PMTrue Then
            OutputLine(String.Format("Invalid Batch ID for MID2 Export.", m_nBatchID))
            Exit Sub
        End If
        ...
        nReturn = m_oDatabase.SQLSelectForXML("spu_MID2_Export_XML_Select", True, oXML)
        ...
        Else
            ...
            Try
                ...
                oXMLReaderSettings.Schemas.Add("http://www.siriusfs.com/SFI/Export/MID2_Export/20060420", System.AppDomain.CurrentDomain.BaseDirectory() & "\" & "MID2_Export.xsd")
    ...
    ''' <summary>
    ''' Processes transform from XML to FlatFile using XSLT
    ''' </summary>
    ''' <remarks></remarks>
    Public Overloads Sub ProcessTransform()
        ...
        Try
            ...
            Dim oXslTransform As XslCompiledTransform = New XslCompiledTransform()
            Dim oXslSettings As New XsltSettings(False, True)
            oXslTransform.Load(m_sXsltFilename, oXslSettings, Nothing)
            Dim oXmlwriter As New XmlTextWriter(sResultFilename, System.Text.Encoding.Default)
            oXslTransform.Transform(FullPath, oXmlwriter)
            oXmlwriter.Close()
...
#Region "Private Methods"
    ...
    Private Sub CreateBatch()
        ...
        Try
            ...
            'Execure Command
            nReturn = m_oDatabase.SQLAction("spu_MID2Export_CreateBatch", "Create Batch", True)
            If nReturn <> PMEReturnCode.PMTrue Then
                Throw New Exception("Unable to execute 'spu_MID2Export_CreateBatch'")
            End If
    ...
    ''' <summary>
    ''' Validate if a batch id belongs to MID 2
    ''' </summary>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function ValidateBatchForMIDExport()
        ...
        AddParameterLite(m_oDatabase, "mid_type", "MID2", PMEParameterDirection.PMParamInput, PMEDataType.PMString, False)
    ...
    ''' <summary>
    ''' Get all active rules data for Export
    ''' </summary>
    ''' <param name="aoResultArray"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function GetMIDActiveRulesForExport(ByRef aoResultArray(,) As Object)
        Dim nResult As PMEReturnCode = PMEReturnCode.PMTrue

        ' Add parameters
        AddParameterLite(m_oDatabase, "mid_type", "MID2", PMEParameterDirection.PMParamInput, PMEDataType.PMString, True)
        ' Execute command
        nResult = m_oDatabase.SQLSelect("spu_MID_GetActiveRulesForExport", "Confirm Batch", True, vResultArray:=aoResultArray)
        If nResult <> PMEReturnCode.PMTrue Then
            Throw New ApplicationException("Unable to execute 'spu_MID_GetActiveRulesForExport'")
        End If
