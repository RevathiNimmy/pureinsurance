...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:user="http://www.ssp-worldwide.com" xmlns="http://www.siriusfs.com/SFI/Import/MID_Import/20070716">
	<xsl:output omit-xml-declaration="no" media-type="xml"/>
	 ...
	 Function ProcessFieldsFailure(ByVal param , ByVal iStart) 
		...
		If param.AsList.Count > 0 Then
			...
			While Mid(strMIDDoc, iPos, 1) = "X" And Not(InStr(1, Trim(Mid(strMIDDoc, iPos + 49, 35)), "THIS FILE HAS BEEN LOADED") > 0) AND Not(InStr(1, Trim(Mid(strMIDDoc, iPos + 49, 35)), "THIS FILE HAS BEEN RECEIVED") > 0)
				...
				iPos = iPos + 1 
				ProcessFieldsFailure = ProcessFieldsFailure & vbCrLf &"<MID_FAILURES insurance_ref=""" & Trim(Mid(strMIDDoc, iPos, 20)) & """" 
				iPos = iPos + 20 
	...
	<xsl:template match="/">
		<IMPORT_HEADER>
			<xsl:attribute name="interface_name">MID_IMPORT</xsl:attribute>
