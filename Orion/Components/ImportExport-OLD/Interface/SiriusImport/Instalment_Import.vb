...
#Region "Methods"
    ...
    Private Sub RunCreditControlForAllBranches()
        ...
        Dim creditControlManager As bACTFinanceSpoke.Business = DirectCast(CreateBusiness("bACTFinanceSpoke.Business", m_oDatabase), bACTFinanceSpoke.Business)
        ...
        Try
            ...
            If creditControlManager IsNot Nothing AndAlso sourceDetails IsNot Nothing Then
                ...
                For source As Integer = lBound To UBound
                    ...
                    returnCode = creditControlManager.Export(v_sInterfaceCode:=ACSpokeInterfaceCode, _
                            r_sBatchRef:=ACSpokeBatch, _
                            r_sStatusCode:=ACSpokeStatusCode, _
                            r_sMessage:=ACSpokeMessage, _
                            r_sHeaderXML:=ACSpokeHeaderXML, _
                            r_vHeaderData:=CObj(vHeaderData), _
        ...
        Finally
            DestroyCOMInterop(CObj(creditControlManager), False)
        End Try
        ...
        If instalmentId <> 0 Then
            ...
            If returnCode = PMEReturnCode.PMTrue Then
                ...
                If ValidateCreditControlSetup(instalmentId, ProcessMode.Cancellation) Then
                    ...
                    business = DirectCast(CreateBusiness("bACTCreditControlItem.Business", m_oDatabase), bACTCreditControlItem.Business)
                    ...
                    Catch ex As Exception
                        Throw
                    Finally
                        DestroyCOMInterop(CObj(business), False)
                    End Try
    ...
    Private Function ValidateCreditControlSetup(ByVal instalmentId As Integer, ByVal processMode As ProcessMode) As Boolean
        ...
        Else
            ...
            If processMode = processMode.Cancellation Then
                ...
                Dim msg As String = PlanReference + " missing CCStep on CCRule" & _
                            " with config - business_type:=" + businessType & _
                            ", source:= " & sourceDescription & _
                            ", frequency:= " & frequencyDescription & _
                            ", policy is paid:= " & policyIsPaid.ToString & _
                            ", pfinstalment result:= " & instalmentResultDescription & _
            ...
            ElseIf processMode = processMode.Rejection Then
                ...
                '" which has the following configuration -  business_type:=" + businessType & _
                Dim msg As String = PlanReference + " missing CCStep on CCRule" & _
                " with cofig - business_type:=" + businessType & _
                            ", source:= " & sourceDescription & _
                            ", frequency:= " & frequencyDescription & _
                            ", pfinstalment result:= " & instalmentResultDescription & _
    ...
    Private Function UpdateInstalmentPlanForAmendment() As Integer
        ...
        Dim bRaiseAmendmentError As Boolean = _
        ...
        If bRaiseAmendmentError Then
            If Me.CoverStartDate > Date.Now Or _
                Me.CoverExpiryDate < Date.Now Or _
        ...
        Dim oSirMediaTypeValidation As bSIRMediaTypeValidation.Business = DirectCast(CreateBusiness("bSIRMediaTypeValidation.Business", m_oDatabase), bSIRMediaTypeValidation.Business)
        ...
        oSirMediaTypeValidation.ValidateNumber(0, 1, sStrippedString, bValid, "", "", "", "", "", "", CStr(oValidationMessage), bOverridable, "Bank")
    ...
    Private Sub GetPFInstalmentTransactionCodes()
        ...
        dbreturnCode = m_oDatabase.SQLSelect( _
        sSQL:="spu_ACT_Import_Get_PFInstalments_Transaction", _
        sSQLName:="spu_ACT_Import_Get_PFInstalments_Transaction", _
    ...
    Private Sub CreateGroupInstalmentHistoryItems( _
    ...
    Private Sub GetInstalmentsForGroup( _
    ...
    Private Sub RecallInstalments(ByVal groupId As Integer, Optional ByVal instalmentAmount As Decimal = Nothing)
        ...
        Dim recallManager As bSIRPFInstalments.Business = DirectCast(CreateBusiness("bSIRPFInstalments.Business", m_oDatabase), bSIRPFInstalments.Business)
        ...
        Try
            ...
            If recallManager IsNot Nothing AndAlso instalmentDetails IsNot Nothing Then
                ...
                For instalment = lBound To UBound
                    ...
                    returnCode = recallManager.RecallInstalment( _
                    instalmentsTransactionCode, _
                    planTransactionID, _
                    instalmentTransactionId, _
                    instalmentId, _
                    instalmentsStatusId, _
                    defaultCreditControlItemReason, _
                    Me.CreditControlEnabled, _
                    retryLimitReached, _
        ...
        Finally
            DestroyCOMInterop(CObj(recallManager), False)
        End Try
    ...
    Private Sub UpdateLivePlanStatus()
        ...
        Dim planOnHold As Boolean = Util.ToSafeBoolean(GetAttribute("plan_on_hold"), False)
        If planOnHold AndAlso Not (Me.PlanStatus = InstalmentPlanStatus.Superseded OrElse Me.PlanStatus = InstalmentPlanStatus.Cancelled) Then
    ...
    Private Sub GetInstalmentPlan( _
    ByVal planReference As String, _
    ByVal planCCNumber As String, _
    ByVal planBankSortCode As String, _
    ...
    Private Function LocateLivePlan() As PMEReturnCode
        ...
        If Me.PlanId = PlanNotFound Then
            If Me.ActionTypeCode = ActionType.Reinstatement Then
                taskDescription = "Re-instatement of DD rejected. Bank Details do not match"
                returnCode = PMEReturnCode.PMFalse
            Else
                taskDescription = "Update rejected. Client Instalment Plan details not recognised."
                returnCode = PMEReturnCode.PMFalse
            End If
        ElseIf Me.PlanStatus = InstalmentPlanStatus.OnHold Then
            If Me.ActionTypeCode = ActionType.Amendment Then
                Dim bRaiseAmendmentErrorForInstalmentPlanOnHold As Boolean = _
                ...
                If bRaiseAmendmentErrorForInstalmentPlanOnHold Then
                    taskDescription = "Bank details amendment required to instalment plan on hold."
                    returnCode = PMEReturnCode.PMFalse
                End If
            End If

        ElseIf Me.PlanStatus <> InstalmentPlanStatus.Live AndAlso _
            ...
            taskDescription = "Update rejected. Client Instalment Plan is not live or completed."
            returnCode = PMEReturnCode.PMFalse
    ...
    Private Sub GetDefaultTaskDetails( _
    ByRef taskId As Integer, _
    ByRef taskGroupId As Integer, _
    ByRef userGroupId As Integer, _
    ...
    Private Sub CreateTaskInstance( _
        ByVal taskGroupId As Integer, _
    ...
    Private Sub CreateTaskInstance( _
    ByRef taskInstanceCnt As Integer, _
    ByVal taskGroupId As Integer, _
    ByVal taskId As Integer, _
    ...
    Private Sub GetInstalmentPlanDetailsForGroupId( _
    ByRef groupId As Integer, _
    ByRef countOfInvalidInstalmentStatusesWithinGroup As Integer)
        ...
        countOfInvalidInstalmentStatusesWithinGroup = Util.ToSafeInt(m_oDatabase.Parameters.Item("count").Value, 0)
    ...
    Private Function GetNextInstalmentToBeProcessedOnCancelledPlan( _
    ByRef instalmentId As Integer, _
    ...
    Private Function GetInstalmentsResultFromCode( _
    ...
    Private Function ValidateInstalmentsGroup(ByVal groupId As Integer) As PMEReturnCode
        ...
        Else
            ...
            Dim countOfInvalidInstalmentStatusesWithinGroup As Integer = 0

            ' confirm group id exists
            GetInstalmentPlanDetailsForGroupId(groupId, countOfInvalidInstalmentStatusesWithinGroup)

            If groupId = 0 Then
                ' if the group id hasnt been found
                CreateTaskInstance(Me.PlanReference + " - Instalment failed. Instalment Group not recognised.")
            ElseIf countOfInvalidInstalmentStatusesWithinGroup <> 0 Then
    ...
    Private Function GetMediatypeHistoryId(ByRef iMediaHistoryPrev As Integer, _
