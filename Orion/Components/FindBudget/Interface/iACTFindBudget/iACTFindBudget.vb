...
Partial Friend Class frmInterface
    ...
    Private m_iBudgetStatus As Integer
    ...
    Private m_oGeneral As iACTFindBudget.General

    ' Declare an instance of the Business object.
    Private m_oBusiness As Object


    Private m_oBudget As Object 'iACTBudget.Interface_Renamed

    Private m_oBudgetDetail As Object ' iACTBudgetDetail.Interface_Renamed
    ...
    Public Property BudgetStatus() As Integer
        Get
            Return m_iBudgetStatus
        End Get
        Set(ByVal Value As Integer)
            m_iBudgetStatus = Value
        End Set
    End Property
    ...
    Public Function GetBusiness() As Integer
        ...
        Dim lDocStatus As Integer
        Dim sReference, sYear As String
        ...
        Try
            ...
            If cboStatus.SelectedIndex = -1 Then
                lDocStatus = -1
            Else
                lDocStatus = VB6.GetItemData(cboStatus, cboStatus.SelectedIndex)
            End If
            ...
            m_lReturn = g_oBusiness.SearchByQuery(lNumberOfRecords:=ACMaxSearchDetails, vResultArray:=m_vSearchData, v_vReference:=sReference, v_vYear:=sYear, v_vStatus:=lDocStatus)
    ...
    Public Function DataToInterface() As Integer
        ...
        Dim sBudgetStatus, sReference As String
        Dim lRevisesBudgetID As Integer
        Dim iCRLFPosition As Integer
        ...
        Try
            ...
            lvwSearchDetails.Sorting = SortOrder.None
            ...
            For lRow As Integer = m_vSearchData.GetLowerBound(1) To m_vSearchData.GetUpperBound(1)
                ...
                oListItem = lvwSearchDetails.Items.Add(CStr(m_vSearchData(ACBudgetRef, lRow)).Trim(), 1)

                ' Assign details to other the columns
                'ListViewHelper.GetListViewSubItem(oListItem, 1).Text = CStr(m_vSearchData(ACPeriodYearName, lRow)).Trim()
                oListItem.SubItems.Add(CStr(m_vSearchData(ACPeriodYearName, lRow)).Trim())
                ' Assign details to other the columns
                ' Get the postingstatus description for the lookup values.
                m_lReturn = GetLookupDesc(sLookupTable:=gACTLibrary.ACTLookupPostingStatus, lLookupID:=CInt(m_vSearchData(ACBudgetStatusID, lRow)), sLookupDesc:=sBudgetStatus)

                ListViewHelper.GetListViewSubItem(oListItem, 3).Text = sBudgetStatus


                ' Description column - if CRLFs then truncate data in listview to
                ' prevent control chars being seen  PN20423
                iCRLFPosition = (CStr(m_vSearchData(ACBudgetDescription, lRow)).Trim().IndexOf(Strings.Chr(13) & Strings.Chr(10)) + 1)
                If iCRLFPosition = 0 Then
                    ListViewHelper.GetListViewSubItem(oListItem, 4).Text = CStr(m_vSearchData(ACBudgetDescription, lRow)).Trim()
                Else
                    ListViewHelper.GetListViewSubItem(oListItem, 4).Text = CStr(m_vSearchData(ACBudgetDescription, lRow)).Trim().Substring(0, iCRLFPosition - 1)
                End If

                ' Assign details to other the columns
                lRevisesBudgetID = CInt(Conversion.Val(CStr(m_vSearchData(ACRevisesBudgetID, lRow))))
                If lRevisesBudgetID <> 0 Then

                    m_lReturn = g_oBusiness.GetBudgetRef(v_lBudgetID:=lRevisesBudgetID, r_sReference:=sReference)
                    ListViewHelper.GetListViewSubItem(oListItem, 2).Text = sReference
                End If
                ' {* USER DEFINED CODE (End) *}
            ...
            Next lRow
            sortColumn = 0
            lvwSearchDetails.Sorting = SortOrder.Ascending
            lvwSearchDetails.Sort()
            lvwSearchDetails.ListViewItemSorter = New ListViewItemComparer(0, lvwSearchDetails.Sorting)

            ' Select the first item.
            lvwSearchDetails.Items.Item(0).Selected = True
            lvwSearchDetails.Select()
    ...
    Public Function DataToProperties() As Integer
        ...
        Try
            ...
            m_lBudgetID = CInt(m_vSearchData(ACBudgetID, lSelectedItem))
            m_sReference = CStr(m_vSearchData(ACBudgetRef, lSelectedItem))
            m_sYear = CStr(m_vSearchData(ACPeriodYearName, lSelectedItem))
            m_iBudgetStatus = CInt(m_vSearchData(ACBudgetStatusID, lSelectedItem))
            m_sDescription = CStr(m_vSearchData(ACBudgetDescription, lSelectedItem))

            If CStr(m_vSearchData(ACRevisesBudgetID, lSelectedItem)) = "" Then
                m_lRevisesBudgetID = 0
            Else
                m_lRevisesBudgetID = CInt(m_vSearchData(ACRevisesBudgetID, lSelectedItem))
            End If

            ' Set the navigator title.
            m_sNavigatorTitle = CStr(m_vSearchData(ACBudgetRef, lSelectedItem)).Trim()
    ...
    Public Function DisplayLookupDetails() As Integer
        ...
        Try
            ...
            cboStatus_NewIndex = cboStatus.Items.Add(m_sComboAny)
            VB6.SetItemData(cboStatus, cboStatus_NewIndex, -1)

            m_lReturn = GetLookupDetails(sLookupTable:=gACTLibrary.ACTLookupPostingStatus, ctlLookup:=cboStatus)
    ...
    Private Function SetInterfaceDefaults() As Integer
        ...
        Try
            ...
            'PN10277
            If Revising Then
                For lCntr As Integer = 1 To cboStatus.Items.Count
                    If VB6.GetItemString(cboStatus, lCntr) = "Posted" Then
                        cboStatus.SelectedIndex = lCntr
                        Exit For
                    End If
                Next lCntr
            ...
            lvwSearchDetails.Columns.Item(0).Width = CInt(VB6.TwipsToPixelsX(1000))
    ...
    Private Function SetFirstLastControls() As Integer
        ...
        Try
            ...
            m_ctlTabFirstLast(ACControlEnd, 0) = cboStatus
    ...
    Private Function DisplayCaptions() As Integer
        ...
        Try
            ...
            lblReference.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACReferenceTitle, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))


            lblStatus.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACStatusTitle, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))


            lblYear.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACYearTitle, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))



            lvwSearchDetails.Columns.Item(0).Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACReferenceListTitle, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))



            lvwSearchDetails.Columns.Item(1).Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACYearListTitle, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))



            lvwSearchDetails.Columns.Item(2).Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACRevisesListTitle, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))



            lvwSearchDetails.Columns.Item(3).Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACStatusListTitle, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))



            lvwSearchDetails.Columns.Item(4).Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACDescriptionListTitle, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
    ...
    Private Function GetLookupDetails(ByRef sLookupTable As String, ByRef ctlLookup As Control) As Integer
        ...
        Try
            ...
            For lCntr = CInt(m_vLookupValues(ACValueStartPos, lRow)) To CInt((CDbl(m_vLookupValues(ACValueStartPos, lRow)) + CDbl(m_vLookupValues(ACValueNumber, lRow))) - 1)
                ' Add the details to the control.

                Dim newIndex As Integer = -1
                newIndex = ReflectionHelper.Invoke(ReflectionHelper.GetMember(ctlLookup, "Items"), "Add", New Object() {New VB6.ListBoxItem(m_vLookupDetails(ACDetailDesc, lCntr), m_vLookupDetails(ACDetailKey, lCntr))})

                ' Check if this is the selected index.
                If m_vLookupValues(ACValueID, lRow).Equals(m_vLookupDetails(ACDetailKey, lCntr)) Then
                    ReflectionHelper.SetMember(ctlLookup, "SelectedIndex", newIndex)
                End If
            Next lCntr
    ...
    Private Function CheckMandatory() As Integer
        ...
        Try
            ...
            If cboStatus.SelectedIndex <> -1 Then
                If VB6.GetItemData(cboStatus, cboStatus.SelectedIndex) <> -1 Then
                    Return gPMConstants.PMEReturnCode.PMTrue
                End If
            End If
    ...
    Private Sub Form_Initialize_Renamed()
        ...
        Try
            ...
            m_oGeneral = New iACTFindBudget.General()
            ...
            Dim temp_m_oBudget As Object
            m_lReturn = g_oObjectManager.GetInstance(temp_m_oBudget, sClassName:="iACTBudget.Interface_Renamed", vInstanceManager:=gPMConstants.PMGetLocalInterface)
            m_oBudget = temp_m_oBudget
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                m_lErrorNumber = gPMConstants.PMEReturnCode.PMFalse
                Exit Sub
            End If

            ' Create an instance of budget detail
            Dim temp_m_oBudgetDetail As Object
            m_lReturn = g_oObjectManager.GetInstance(temp_m_oBudgetDetail, sClassName:="iACTBudgetDetail.Interface_Renamed", vInstanceManager:=gPMConstants.PMGetLocalInterface)
            m_oBudgetDetail = temp_m_oBudgetDetail
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                m_lErrorNumber = gPMConstants.PMEReturnCode.PMFalse
                Exit Sub
            End If
    ...
    Private Sub frmInterface_Load(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Load
        ...
        Try
            Me.stbStatus.Width = Me.Width
            Me._stbStatus_Panel1.Width = Me.Width
            stbStatus.Refresh()
    ...
    Private Sub frmInterface_FormClosing(ByVal eventSender As Object, ByVal eventArgs As FormClosingEventArgs) Handles MyBase.FormClosing
        ...
        Try
            ...
            m_oBudgetDetail.Dispose()
            m_oBudgetDetail = Nothing
    ...
    Private Sub cmdFindNow_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cmdFindNow.Click
        ...
        Try
            ...
            _stbStatus_Panel1.Text = "Searching. please wait ..."
            stbStatus.Refresh()
            ' Set the mouse pointer to busy.
            iPMFunc.SetMousePointer(gPMConstants.PMEMousePointerStatus.PMMouseBusy)
    ...
    ' Name: CallBudgetForm
    '
    ' Description: Creates an instance of iACTBudget in the given mode
    '
    ' ***************************************************************** '
    Private Function CallBudgetForm(ByVal v_iTask As Integer) As Integer

        Dim result As Integer = 0
        Dim lBudgetID As Integer
        Dim sReference, sYear As String
        Dim iBudgetStatus As Integer
        Dim lRevisesBudgetID, lBasedOnBudgetID As Integer
        ...
        Try
            ...
            ' Initialise the object so we get all the look ups etc... set

            m_lReturn = CType(m_oBudget, SSP.S4I.Interfaces.ILocalInterface).Initialise()
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            ' Set the task mode
            m_lReturn = m_oBudget.SetProcessModes(vTask:=v_iTask)
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            ' Set the task mode
            m_lReturn = m_oBudgetDetail.SetProcessModes(vTask:=v_iTask)
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If
            ...
            If v_iTask = gPMConstants.PMEComponentAction.PMEdit Then
                ...
                With m_oBudget
                    ...
                    .BudgetId = m_lBudgetID

                    .BudgetRef = m_sReference

                    .PeriodYearName = m_sYear

                    .BudgetStatusID = m_iBudgetStatus

                    .RevisesBudgetID = m_lRevisesBudgetID

                    .BasedOnBudgetID = m_lBasedOnBudgetID
            ...
            m_lReturn = m_oBudget.Start()
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If


            If m_oBudget.Status = gPMConstants.PMEReturnCode.PMOK Then
                ...
                With m_oBudget
                    ...
                    sReference = .BudgetRef

                    sYear = .PeriodYearName

                    iBudgetStatus = .BudgetStatusID

                    lRevisesBudgetID = .RevisesBudgetID

                    lBasedOnBudgetID = .BasedOnBudgetID

                    sDescription = .Description
                End With

                ' Pass them to budget detail
                With m_oBudgetDetail

                    .BudgetId = lBudgetID

                    .RevisesBudgetID = lRevisesBudgetID

                    .BasedOnBudgetID = lBasedOnBudgetID

                    .PeriodYearName = sYear
                End With

                ' Start budget detail

                m_lReturn = m_oBudgetDetail.Start()
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    result = gPMConstants.PMEReturnCode.PMFalse
                End If
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="CallBudgetFormFailed", vApp:=ACApp, vClass:=ACClass, vMethod:="CallBudgetForm", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Private Sub cmdNew_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cmdNew.Click
        ...
        Try
            ...
            m_lReturn = CallBudgetForm(v_iTask:=gPMConstants.PMEComponentAction.PMAdd)
            ...
            If g_oBusiness Is Nothing Then
                ...
                m_lReturn = g_oObjectManager.GetInstance(temp_g_oBusiness, "bACTFindBudget.Form", vInstanceManager:=gPMConstants.PMGetViaClientManager)
    ...
    Private Sub cmdEdit_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cmdEdit.Click
        ...
        Try
            ...
            m_lReturn = CallBudgetForm(v_iTask:=gPMConstants.PMEComponentAction.PMEdit)
