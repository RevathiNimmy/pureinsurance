...
Partial Friend Class frmInterface
    ...
    'Private m_oBusiness As bSirPerilAllocationDataFix.Business
        ...
        Try
            ...
            If m_iTask <> gPMConstants.PMEComponentAction.PMDelete Then
                ...
                If m_sTransactionType <> "MTC" Or bIsMandatoryRisks Or bIsMTCRatingRulesEnabled Then
                    ...
                    m_lReturn = oObject.SetProcessModes(vTask:=m_iTask, _
                                        vNavigate:=m_lNavigate, _
                                        vProcessMode:=m_lProcessMode, _
                                        vTransactionType:=m_sTransactionType, _
    ...
    Private Function RecalculatePremium() As Integer
        ...
        Try
            ...
            For Each oListItem As ListViewItem In lvwRatingSection.Items
                ...
                If Not IsRoundingSection(oListItem) Then
            ...
            For Each oListItem As ListViewItem In lvwRatingSection.Items
                ' Check for rounding section
                If IsRoundingSection(oListItem) Then
                    ' We have a rounding section, so round
                    cThisPremium = GetRoundAmount(cTotalThisPremium + m_cReturnPremium)
                    ' 05/08/2003 Peter Finney - We MUST calc the annual rounding premium or
                    ' we will not be able to properly calculate MTA's...
                    ' ...and we must do it according to the pro-rata rules otherwise it knocks
                    ' out later calculation in RI.

                    'We do not need to apply the pro-rata on the rounding section. - Amit PN44022
                    cPremium = 0
                    '            If m_oBusiness.ProRataRate = 0 Then
                    '                cPremium = 0
                    '            Else
                    '                cPremium = cThisPremium / m_oBusiness.ProRataRate
                    '            End If

                    ' Set premiums for section
                    ...
                    '  oListItem.SubItems(ACPremiumCol) = FormatField(PMFormatMoney, cPremium)
    ...
    Private Sub Form_Initialize_Renamed()
        ...
        Try
            ...
            m_lReturn = g_oObjectManager.GetInstance(temp_m_oBusiness, "bSirPerilAllocationDataFix.Business", vInstanceManager:=gPMConstants.PMGetViaClientManager)
