...
Partial Friend Class frmInterface
    ...
    'policy version list
    Private Const ACFieldInsurnceRef As Integer = 0
    Private Const ACFieldPVDocumentRef As Integer = 1
    Private Const ACFieldPVShortName As Integer = 2
    Private Const ACFieldPVShortCode As Integer = 3
    Private Const ACFieldPVDocDate As Integer = 4
    Private Const ACFieldPVTransType As Integer = 5
    Private Const ACFieldPVAmount As Integer = 6
...
Finally_Renamed:
    ...
    Private Sub frmInterface_Load(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Load
        ...
        tabPolicyVersion.TabPages.Remove(TabPage1)
    ...
    Private Sub cmdGetPolicyVersion_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cmdGetPolicyVersion.Click
        ...
        Try
            ...
            stbMain.Items.Item("MESSAGE").Text = "Getting all versions of this policy"
              ...
              txtSqlQuery.Text="spu_Select_Tax_PM059028"
           ...
           ' End If
            ...
            For lCount As Integer = 0 To aoResultArray.GetUpperBound(1)
                ...
                sListItem(ACFieldInsurnceRef) = aoResultArray(ACFieldInsurnceRef, lCount).ToString.Trim()
                sListItem(ACFieldPVDocumentRef) = aoResultArray(ACFieldPVDocumentRef, lCount).ToString.Trim()
                sListItem(ACFieldPVShortName) = aoResultArray(ACFieldPVShortName, lCount).ToString.Trim()
                sListItem(ACFieldPVShortCode) = aoResultArray(ACFieldPVShortCode, lCount).ToString.Trim()
                sListItem(ACFieldPVDocDate) = aoResultArray(ACFieldPVDocDate, lCount).ToString.Trim()
                sListItem(ACFieldPVTransType) =  aoResultArray(ACFieldPVTransType, lCount).ToString.Trim()  
                sListItem(ACFieldPVAmount) =  aoResultArray(ACFieldPVAmount, lCount).ToString.Trim()  

               ' sListItem(ACFieldPVAmount) = DateTime.Parse(CStr(aoResultArray(ACFieldPVAmount, lCount)).Trim()).ToString("d")
...
End_CreateTransactionExport:
    ...
    End Sub
      ...
      Private Sub ProceedTaxFix( )

        Try

            For Each oListItem As ListViewItem In Me.lvwPolicyVersion.Items
                If oListItem.Checked Then
                    Dim sDocumentRef As String = oListItem.SubItems(1).Text
                    If sDocumentRef <> "" Then
                         
                        m_lReturn = ProceedTaxProcess( v_sDocumentRef:=sDocumentRef)
                        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed ProceedTaxFix()", vApp:=ACApp, vClass:=ACClass, vMethod:="ReversePolicyVersionTransaction", vErrNo:=CStr(Information.Err().Number), vErrDesc:=Information.Err().Description)
        ...
        Catch ex As Exception
            ...
            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to ProceedTaxFix ", vApp:=ACApp, vClass:=ACClass, vMethod:="ReversePolicyTransaction", vErrNo:=CStr(Information.Err().Number), vErrDesc:=Information.Err().Description)
        End Try
    ...
    Private Sub RepostPolicyVersion()
        ...
        For Each oListItem As ListViewItem In Me.lvwPolicyVersion.Items
            If oListItem.Checked Then
                ...
                Select Case Convert.ToString(oListItem.SubItems.Item(ACFieldPVShortCode).Tag)
                    Case 2 'new business
                        sTransactionType = "NB"
                    Case 3 'renewal
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to for account details of this policy version - " & oListItem.SubItems.Item(ACFieldPVDocumentRef - 1).Text, vApp:=ACApp, vClass:=ACClass, vMethod:="RepostPolicyVersion", vErrNo:=CStr(Information.Err().Number), vErrDesc:=Information.Err().Description)
                ...
                If MessageBox.Show("Do you want to recalculate reinsurance?", ACApp, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                    ...
                    m_lReturn = oReinsurance.SetProcessModes(vTransactionType:=sTransactionType, vEffectiveDate:=oListItem.SubItems.Item(ACFieldPVTransType - 1).Text)
            ...
            For Each oListItem As ListViewItem In Me.lvwPolicyVersion.Items
                If oListItem.Checked Then
                    ...
                    If Not (sDocumentRef.StartsWith("SDD") Or sDocumentRef.StartsWith("SDR")) Then
                        If (chkUpdateFileType.Checked) Then
                            m_lReturn = m_oBusiness.UpdateFileType(nInsuranceFileCnt:=CInt(oListItem.Text), nFileTypeId:=oListItem.SubItems.Item(ACFieldPVShortCode).Tag)
                            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                                m_oBusiness.RollbackTrans()
                                Continue For
                            End If
    ...
    Private Sub AddRIToPolicy()
        ...
        If sMessage <> "Successful" Then
            If sInsuranceRef.ToUpper() = "ALL" Then
                MessageBox.Show("Failed to add RI model (" & sRIModelCode & ") to policies with no reinsurance" & Strings.Chr(13) & Strings.Chr(10) &
                                sMessage, Application.ProductName)
            Else
                MessageBox.Show("Failed to add RI model (" & sRIModelCode & ") to policy " & sInsuranceRef & Strings.Chr(13) & Strings.Chr(10) &
                                sMessage, Application.ProductName)
            End If
        Else
...
Finally_Renamed:
    ...
    Private Sub btnGetAllocationDetails_Click(sender As Object, e As EventArgs) Handles btnGetAllocationDetails.Click
        ...
        Try
            ...
            For lCount As Integer = 0 To aoResultArray.GetUpperBound(1)
                ...
                'oListItem.SubItems(ACFieldPVShortCode).Tag = aoResultArray(ACFieldPVShortName, lCount).Trim()
    ...
    Private Sub btnGetDuplicatePolicyVersions_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles btnGetDuplicatePolicyVersions.Click
        ...
        Try
            ...
            For lCount As Integer = 0 To aoResultArray.GetUpperBound(1)
                ...
                sListItem(ACFieldPVShortCode) = aoResultArray(ACFieldPVShortCode, lCount).ToString.Trim()
                sListItem(ACFieldPVDocDate) = aoResultArray(ACFieldPVDocDate, lCount).ToString.Trim()
                sListItem(ACFieldPVTransType) = DateTime.Parse(CStr(aoResultArray(ACFieldPVTransType, lCount)).Trim()).ToString("d")
                sListItem(ACFieldPVAmount) = DateTime.Parse(CStr(aoResultArray(ACFieldPVAmount, lCount)).Trim()).ToString("d")
    ...
    Private Sub btnGetClonePolicyVersion_Click(sender As Object, e As EventArgs) Handles btnGetClonePolicyVersion.Click
        ...
        Try
            ...
            For lCount As Integer = 0 To aoResultArray.GetUpperBound(1)
                ...
                'insurance file cnt
                oListItem = Me.lvwClonePolicyVersion.Items.Add(CStr(aoResultArray(ACFieldInsurnceRef, lCount)))
                ...
                'policy type id                
                oListItem.SubItems.Insert(ACFieldPVShortName,
                                          New ListViewItem.ListViewSubItem(oListItem, aoResultArray(ACFieldPVShortName, lCount).Trim()))

                'policy type                
                oListItem.SubItems.Insert(ACFieldPVShortCode,
                                          New ListViewItem.ListViewSubItem(oListItem, aoResultArray(ACFieldPVShortCode, lCount).Trim()))

                oListItem.SubItems(ACFieldPVShortCode).Tag = aoResultArray(ACFieldPVShortName, lCount).Trim()

                'Document Ref               
                oListItem.SubItems.Insert(ACFieldPVDocDate,
                                          New ListViewItem.ListViewSubItem(oListItem, aoResultArray(ACFieldPVDocDate, lCount).Trim()))

                'policy start                
                oListItem.SubItems.Insert(ACFieldPVTransType,
                                          New ListViewItem.ListViewSubItem(oListItem, aoResultArray(ACFieldPVTransType, lCount).Trim()))

                'document Date               
                oListItem.SubItems.Insert(ACFieldPVAmount,
                                          New ListViewItem.ListViewSubItem(oListItem, aoResultArray(ACFieldPVAmount, lCount).Trim()))
    ...
    Private Function ProceedTaxProcess( ByVal v_sDocumentRef As String) As Integer
        Try
            ...
            m_oBusiness.BeginTrans()
                ...
                m_lReturn = m_oBusiness.ProceedTaxFix(v_sDocumentRef)    
                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                        m_oBusiness.RollbackTrans()
                        Return gPMConstants.PMEReturnCode.PMFalse
                    End If
