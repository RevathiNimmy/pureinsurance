...
Partial Friend Class frmInterface
    ...
    Private Const ACFieldPVMax As Integer = 6
    ...
    Private Const ACFieldRDTransType As Integer = 6
    Private Const ACFieldRDpolicyType As Integer = 7
    'transaction export
    ...
    Private Const ACFieldRevClaimIsDefferdPolicy As Integer = 4
    Private Const ACFieldRevClaimIsDefferdCount As Integer = 5
    Private Const ACFieldRev As Integer = 5
    ...
    Private bNonRegenDocFound As Boolean
    ...
    Public Property NonRegenDocFound() As Boolean
        Get

            Return bNonRegenDocFound

        End Get
        Set(ByVal Value As Boolean)

            bNonRegenDocFound = Value
...
Finally_Renamed:
    ...
    Private Sub frmInterface_Load(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Load
       ...
       ' tabPolicyVersion.TabPages.Remove(_tabPolicyVersion_TabPage0)
        '''''''''''''''
        _optSinglePolicy_5.Visible = False
        chkUpdateFileType.Visible = False
    End Sub
    ...
    Private Sub cmdGetPolicyVersion_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cmdGetPolicyVersion.Click
        ...
        Try
            ...
            lvwPolicyVersion.Items.Clear()
            ReDim sListItem(ACFieldPVMax)
            ReDim oListItem(aoResultArray.GetUpperBound(1))
            For lCount As Integer = 0 To aoResultArray.GetUpperBound(1)

                lstItem = Nothing

                sListItem(ACFieldPVInsuranceFileCnt) = aoResultArray(ACFieldPVInsuranceFileCnt, lCount).ToString.Trim()
                sListItem(ACFieldPVInsuranceRef) = aoResultArray(ACFieldPVInsuranceRef, lCount).ToString.Trim()
                sListItem(ACFieldPVPolicyTypeID) = aoResultArray(ACFieldPVPolicyTypeID, lCount).ToString.Trim()
                sListItem(ACFieldPVPolicyType) = aoResultArray(ACFieldPVPolicyType, lCount).ToString.Trim()
                sListItem(ACFieldPVDocumentRef) = aoResultArray(ACFieldPVDocumentRef, lCount).ToString.Trim()
                sListItem(ACFieldPVPolicyStart) = DateTime.Parse(CStr(aoResultArray(ACFieldPVPolicyStart, lCount)).Trim()).ToString("d")
                sListItem(ACFieldPVDocumentDate) = DateTime.Parse(CStr(aoResultArray(ACFieldPVDocumentDate, lCount)).Trim()).ToString("d")

                lstItem = New ListViewItem(sListItem)
                oListItem(lCount) = lstItem

            Next

            lvwPolicyVersion.Items.AddRange(oListItem)
...
Err_SearchListView:
    ...
    Private Sub ProcessSinglePolicy()
        ...
        If chkUpdateFileType.Checked Then
            MsgBox(chkUpdateFileType.Text & " " & "This Process is only for non REGEN transaction documents. Like SND, SED, SNC etc", , "Data Fix Utility")
        End If

        If _optSinglePolicy_5.Checked Then
            MsgBox(_optSinglePolicy_5.Text & " " & "This Process is only for non REGEN transaction documents. Like SND, SED, SNC etc", , "Data Fix Utility")
        End If
        ...
        Select Case lCount
            ...
            Case 2 'Update Tax Calculation for Marine products where Vat is not applicable but still it is applied and calculated the incorrect tax
...
Finally_Renamed:
        ...
        Try
            ...
            For Each oListItem As ListViewItem In Me.lvwPolicyVersion.Items
                If oListItem.Checked Then
                    ...
                    If sDocumentRef <> "" Then
                        If sDocumentRef.StartsWith("C") Then
                            Continue For
                        End If
        ...
        Try
            ...
            If nStatus = gPMConstants.PMEReturnCode.PMNotFound Then
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    ...
                    'iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to Reversse Stats ", vApp:=ACApp, vClass:=ACClass, vMethod:="ReversePolicyVersionTransaction", vErrNo:=CStr(Information.Err().Number), vErrDesc:=Information.Err().Description)m_lReturn = m_oBusiness.AddReversalDocument(v_lOldDocumentRef:=v_sDocumentRef, v_nStatsFolderCnt:=nStatsFolderCnt, r_vDocumentID:=nDocumentId)
                ...
                If Not _optSinglePolicy_4.Checked Or v_bIsCalledFromDuplicateVersionTab Or optClonePolicyReverse.Checked Or optClonePolicyReverseRegenerate.Checked Then
                    m_lReturn = m_oBusiness.AddDataFixUtilityLog(v_sPMNumber:=txtPMNumber.Text, v_sCreatedBy:="DataFixUtility", v_lInsuranceFileCnt:=v_nInsuranceFileCnt,
        ...
        Try
            ...
            For Each oListItem As ListViewItem In Me.lvwPolicyVersion.Items
                If oListItem.Checked Then
                    ...
                    m_lReturn = m_oBusiness.GetTransactionTypeCode(v_sDocumentRef:=sDocumentRef, r_Transaction_Type_Code:=sTransactionType)
                    If sDocumentRef = "" Then
                        MessageBox.Show("Selected version is not valid for reposting", ACApp, MessageBoxButtons.OK, MessageBoxIcon.Information)
                        'Throw New System.Exception
                    End If

                    If sDocumentRef.StartsWith("SRD") Then
                        sTransactionType = "REN"
                    End If
                    ...
                    If chkRIRefresh.Checked Then
                        m_lReturn = m_oBusiness.RIRefresh(nInsuranceFileCnt:=CInt(oListItem.Text), sTransactionType:=sTransactionType,bIs2007:=1 )
                        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                            m_oBusiness.RollbackTrans()
                            If MsgBox("RI model does not exist for the selected policy version - " & CInt(oListItem.Text) & vbCrLf & "Are you sure you want to proceed for the next record?", vbYesNo + vbQuestion, ACApp) <> vbYes Then
                                Exit Sub
                            End If
                            Continue For
                        End If
                    ...
                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                        ...
                        Throw New System.Exception
                    End If
                    'add code for Update file type 
                    If Not (sDocumentRef.StartsWith("SDD") Or sDocumentRef.StartsWith("SDR")) Then
                        If (chkUpdateFileType.Checked) Then
                            m_lReturn = m_oBusiness.UpdateFileType(nInsuranceFileCnt:=CInt(oListItem.Text), nFileTypeId:=oListItem.SubItems.Item(ACFieldPVPolicyType).Tag)
                            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                                m_oBusiness.RollbackTrans()
                                Continue For
                            End If
                    ...
                    oControlTrans.InsuranceFileCnt = CInt(oListItem.Text)
                    If (sDocumentRef.StartsWith("SDD") Or sDocumentRef.StartsWith("SDR")) Then
                        m_lReturn = RegenerateClonePolicyVersionTransaction(CInt(oListItem.Text), sDocumentRef)
                    Else
                        m_lReturn = oControlTrans.Start()
                    End If
    ...
    Private Sub UpdateTaxCalculation()
        Try
            Dim oControlTrans As bControlTrans.Automated
            ...
            For Each oListItem As ListViewItem In Me.lvwPolicyVersion.Items
                If oListItem.Checked Then
                    ...
                    m_lReturn = m_oBusiness.UpdateTaxCalc(nInsuranceFileCnt:=CInt(oListItem.Text), sTransactionType:=sTransactionType)
                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                        'iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed UpdateTaxCalc()", vApp:=ACApp, vClass:=ACClass, vMethod:="UpdateTaxCalc()", vErrNo:=CStr(Information.Err().Number), vErrDesc:=Information.Err().Description)
                        Throw New System.Exception
                    End If
        ...
        Catch ex As Exception
            bIsSucesfullyCompleted = False
            m_oBusiness.RollbackTrans()
            'iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed UpdateTaxCalculation", vApp:=ACApp, vClass:=ACClass, vMethod:="UpdateTaxCalculation", vErrNo:=CStr(Information.Err().Number), vErrDesc:=Information.Err().Description)
        End Try
        ...
        Try
            ...
            If Information.IsArray(aoResultArray) Then
                For lCount As Integer = 0 To aoResultArray.GetUpperBound(1)
                    ...
                    oListItem.SubItems.Insert(ACFieldRDTransType, New ListViewItem.ListViewSubItem(oListItem, aoResultArray(ACFieldRDTransType, lCount).Trim()))
                    oListItem.SubItems.Insert(ACFieldRDpolicyType, New ListViewItem.ListViewSubItem(oListItem, aoResultArray(ACFieldRDpolicyType, lCount).Trim()))
...
Finally_Renamed:
    ...
    Private Sub ProcessMultiClaim()
        ...
        Try
            ...
            If IsTick(oListView:=LvwClaimVersion) > 0 Then
                ...
                End If
            End If
            Dim bIsFailure As Boolean = False
            Dim iFailureCnt As Integer = 0
            Dim iNonRegenDocCnt As Integer = 0
            Dim sLastProcessedClaim As String = ""
            Select Case lCount
                Case 0 'Reverse
                    For Each oListItem As ListViewItem In Me.LvwClaimVersion.Items
                        ...
                        If oListItem.Checked = True Then
                            ...
                            If m_oBusiness.ProcessClaimTransactions(v_lClaimId:=CLng(oListItem.Text), _
                                                                v_sDocumentRef:=(oListItem.SubItems(3).Text), v_sRefNumber:=txtPMNumber.Text) <> gPMConstants.PMEReturnCode.PMTrue Then
                ...
                Case 1 'Reverse and Reraise
                    For Each oListItem As ListViewItem In Me.LvwClaimVersion.Items
                        If oListItem.Checked = True Then
                            ...
                            stbMain.Items.Item("MESSAGE").Text = "Processing Claim " & CStr(oListItem.SubItems(1).Text) & "; Document Ref " & CStr(oListItem.SubItems(2).Text)
                            stbMain.Refresh()
                            If oListItem.SubItems(3).Text.Substring(0, 3) = "SDD" OrElse oListItem.SubItems(3).Text.Substring(0, 3) = "SDR" OrElse String.IsNullOrEmpty(oListItem.SubItems(3).Text) Then
                                If sLastProcessedClaim <> oListItem.SubItems(1).Text Then
                                    bIsFailure = False
                                End If
                                If Not (bIsFailure AndAlso sLastProcessedClaim = oListItem.SubItems(1).Text) Then
                                    If m_oBusiness.ProcessClaimTransactions(v_lClaimId:=CLng(oListItem.Text), _
                                                                        v_sDocumentRef:=oListItem.SubItems(3).Text, v_sRefNumber:=txtPMNumber.Text, v_bRePost:=True, v_bRIRefresh:=ChkRIRefreshClonedTrans.Checked) <> gPMConstants.PMEReturnCode.PMTrue Then
                                        bIsFailure = True
                                        iFailureCnt = iFailureCnt + 1
                                    End If
                                End If
                            Else
                                iFailureCnt = iFailureCnt + 1
                                iNonRegenDocCnt = iNonRegenDocCnt + 1
                                bIsFailure = True
                            End If
                        End If
                        sLastProcessedClaim = oListItem.SubItems(1).Text
                    Next
            End Select

            stbMain.Items.Item("MESSAGE").Text = "Process Completed"
            If (iFailureCnt = 0 AndAlso iNonRegenDocCnt = 0) Then
                MsgBox("Item Processed successfully", , "Data Fix Utility")
            ElseIf iFailureCnt > 0 And iNonRegenDocCnt = 0 Then
                MsgBox("Process Completed with some failures", , "Data Fix Utility")
            ElseIf iFailureCnt = 0 And iNonRegenDocCnt > 0 Then
                MsgBox("Some non-REGEN document(s) found. Please process these type of document(s) in 'Claim Transaction Repost' tab", , "Data Fix Utility")
            Else
                MsgBox("Process Completed with some failures. Some non-REGEN document(s) also found. Please process non-REGEN document(s) in 'Claim Transaction Repost' tab", , "Data Fix Utility")
            End If
    ...
    Private Sub CmdGetClaimVersionsTrans_Click(sender As Object, e As EventArgs) Handles CmdGetClaimVersionsTrans.Click
        ...
        Try
            ...
            If txtClaimNo.Text <> "" Then
                sSql = "SELECT sf.loss_id , sf.loss_code ,sf.insurance_ref,sf.document_ref,'',0 FROM stats_folder sf "
            ...
            ReDim sListItem(ACFieldRev)
            ReDim oListItem(aoResultArray.GetUpperBound(1))
            For lCount As Integer = 0 To aoResultArray.GetUpperBound(1)

                lstItem = Nothing

                sListItem(ACFieldRevClaimTransid) = aoResultArray(ACFieldRevClaimTransid, lCount).ToString.Trim()
                sListItem(ACFieldRevClaimTransNumber) = aoResultArray(ACFieldRevClaimTransNumber, lCount).ToString.Trim()
                sListItem(ACFieldClaimTransInsuranceref) = aoResultArray(ACFieldClaimTransInsuranceref, lCount).ToString.Trim()
                sListItem(ACFieldRevClaimTransDocumentRef) = aoResultArray(ACFieldRevClaimTransDocumentRef, lCount).ToString.Trim()
                sListItem(ACFieldRevClaimIsDefferdPolicy) = aoResultArray(ACFieldRevClaimIsDefferdPolicy, lCount).ToString.Trim()
                sListItem(ACFieldRevClaimIsDefferdCount) = aoResultArray(ACFieldRevClaimIsDefferdCount, lCount).ToString.Trim()
                
                lstItem = New ListViewItem(sListItem)
                oListItem(lCount) = lstItem

            Next
            LvwClaimVersionTrans.Items.AddRange(oListItem)
        ...
        Catch ex As Exception
            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed CmdGetClaimVersionsTrans", vApp:=ACApp,
                                       vClass:=ACClass, vMethod:="CmdGetClaimVersionsTrans", vErrNo:=CStr(Information.Err().Number),
                                       vErrDesc:=Information.Err().Description, excep:=ex)
        Finally
    ...
    Private Sub CmdClaimTrans_Click(sender As Object, e As EventArgs) Handles CmdClaimTrans.Click
        ...
        Dim lRecordNotProcessed As Integer = 0
        ...
        ElseIf OptReverseRegClmTrans.Checked Then
            lCount = 1
        ElseIf OptGenerateClaimTrans.Checked Then
            lCount = 2
        End If
            ...
            Case 0 'Reverse
                For Each oListItem As ListViewItem In Me.LvwClaimVersionTrans.Items
                    ...
                    If oListItem.Checked = True Then
                        ...
                        If String.IsNullOrEmpty(oListItem.SubItems(3).Text) Then
                            Continue For
                        Else
                            sDocumentType = oListItem.SubItems(3).Text.Substring(0, 3)
                        End If

                        If sDocumentType.StartsWith("S") Then
                            Continue For
                        End If

                        If sDocumentType = "CLC" OrElse sDocumentType = "CLD" Then
                            If m_oBusiness.ProcessClaimTransactions(v_lClaimId:=CLng(oListItem.Text),
                                                                v_sDocumentRef:=(oListItem.SubItems(3).Text), v_sRefNumber:=txtPMNumber.Text) <> gPMConstants.PMEReturnCode.PMTrue Then
                                iFailureCnt = iFailureCnt + 1
                            End If
                        Else
                            If m_oBusiness.ProcessTransactions(CLng(oListItem.Text), oListItem.SubItems(3).Text, v_bRePost:=False, v_bRIRefresh:=chkRIRefreshClmTrans.Checked, v_sRefNumber:=txtPMNumber.Text ,bIs2007:=1) <> gPMConstants.PMEReturnCode.PMTrue Then
                                iFailureCnt = iFailureCnt + 1
                            End If
                        End If
                    End If
                ...
                For Each oListItem As ListViewItem In Me.LvwClaimVersionTrans.Items
                    If oListItem.Checked = True Then
                        ...
                        End If
                          ...
                          If val(oListItem.SubItems(5).Text)>0 Then
                            iFailureCnt = iFailureCnt + 1
                            Continue For
                        End If
                        ...
                        If Not (bIsFailure AndAlso sLastProcessedClaim = oListItem.SubItems(1).Text) Then
                            If sDocumentType = "CLC" OrElse sDocumentType = "CLD" Then
                                If m_oBusiness.ProcessClaimTransactions(v_lClaimId:=CLng(oListItem.Text),
                                                                            v_sDocumentRef:=oListItem.SubItems(3).Text, v_sRefNumber:=txtPMNumber.Text, v_bRePost:=True, v_bRIRefresh:=chkRIRefreshClmTrans.Checked,bIs2007:=1) <> gPMConstants.PMEReturnCode.PMTrue Then
                                    bIsFailure = True
                                    iFailureCnt = iFailureCnt + 1
                                End If
                            Else
                                if sDocumentType="CLP" Then
                                    chkRIRefreshClmTrans.Checked=False
                                End if
                                    
                                If m_oBusiness.ProcessTransactions(CLng(oListItem.Text), oListItem.SubItems(3).Text, v_bRePost:=True, v_bRIRefresh:=chkRIRefreshClmTrans.Checked, v_sRefNumber:=txtPMNumber.Text,bIs2007:=1) <> gPMConstants.PMEReturnCode.PMTrue Then
                                    bIsFailure = True
                                    iFailureCnt = iFailureCnt + 1
                                End If
                            End If
                    ...
                    sLastProcessedClaim = oListItem.SubItems(1).Text
                Next
            Case 2 'Generate Claim transactions
                For Each oListItem As ListViewItem In Me.LvwClaimVersionTrans.Items
                    If oListItem.Checked = True Then
                        lRecordCount = lRecordCount + 1
                        stbMain.Items.Item("COUNT").Text = lRecordCount
                        ...
                        ' Generate Transaction code comes here

                        If m_oBusiness.GenerateClaimTransactions(CLng(oListItem.Text), oListItem.SubItems(3).Text, v_bRIRefresh:=chkRIRefreshClmTrans.Checked, v_sRefNumber:=txtPMNumber.Text ,bIs2007:=1) <> gPMConstants.PMEReturnCode.PMTrue Then
                            lRecordNotProcessed = lRecordNotProcessed + 1
                            Continue For
                        End If
                    End If
                Next
    ...
    Private Sub _optSinglePolicy_4_CheckedChanged(sender As Object, e As EventArgs) Handles _optSinglePolicy_4.CheckedChanged
        ...
        'chkUpdateFileType.Enabled = True
    ...
    Private Sub _optSinglePolicy_3_CheckedChanged(sender As Object, e As EventArgs) Handles _optSinglePolicy_3.CheckedChanged
        ...
        'chkUpdateFileType.Enabled = False
        'chkUpdateFileType.Checked = False
    End Sub

    Private Sub _optSinglePolicy_5_CheckedChanged(sender As Object, e As EventArgs) Handles _optSinglePolicy_5.CheckedChanged
        chkRIRefresh.Enabled = False
        chkRIRefresh.Checked = False
        'chkUpdateFileType.Enabled = False
        'chkUpdateFileType.Checked = False
    ...
    Private Sub OptGenerateClaimTrans_CheckedChanged(sender As Object, e As EventArgs) Handles OptGenerateClaimTrans.CheckedChanged
        chkRIRefreshClmTrans.Enabled = True
    End Sub
    ...
    Private Sub btnGetAllocationDetails_Click(sender As Object, e As EventArgs) Handles btnGetAllocationDetails.Click
        ...
        Try
            ...
            For lCount As Integer = 0 To aoResultArray.GetUpperBound(1)
                ...
                'Document Ref               
                oListItem.SubItems.Insert(ACFieldPVFACAccountId,
                                          New ListViewItem.ListViewSubItem(oListItem, aoResultArray(ACFieldPVFACAccountId, lCount).Trim()))

                'policy start                
                oListItem.SubItems.Insert(ACFieldPVFACAccount,
        ...
        Catch ex As Exception
            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed btnGetAllocationDetails_Click", vApp:=ACApp,
                                         vClass:=ACClass, vMethod:="btnGetAllocationDetails_Click", vErrNo:=CStr(Information.Err().Number),
                                         vErrDesc:=Information.Err().Description, excep:=ex)
        Finally
    ...
    Private Sub ReverseAndAllocateSingleDocument(ByVal AllocationId As Integer, ByVal AllocatedDocRef As String, ByVal AssociatedDocRef As String)
        Try
            ...
            m_lReturn = m_oBusiness.AddDataFixUtilityLog(v_sPMNumber:=txtPMNumber.Text, v_sCreatedBy:="DataFixUtility", v_lInsuranceFileCnt:=0,
                                            v_sOldDocumentRef:=AllocatedDocRef, v_sNewDocumentid:=0, v_bIsReversal:=True, v_AllocationID:=AllocationId, v_AssociatedDocRef:=AssociatedDocRef)
    ...
    Private Sub btnGetDuplicatePolicyVersions_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles btnGetDuplicatePolicyVersions.Click
        ...
        Try
            ...
            For lCount As Integer = 0 To aoResultArray.GetUpperBound(1)
                ...
                oListItem(lCount) = lstItem
            Next
            lvwDuplicatePolicyVersion.Items.AddRange(oListItem)
        ...
        Catch ex As Exception
            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed btnGetDuplicatePolicyVersions_Click", vApp:=ACApp,
                                         vClass:=ACClass, vMethod:="btnGetDuplicatePolicyVersions_Click", vErrNo:=CStr(Information.Err().Number),
                                         vErrDesc:=Information.Err().Description, excep:=ex)
        Finally
    ...
    Private Sub btnOkClonePolicyVersionsOk_Click(sender As Object, e As EventArgs) Handles btnOkClonePolicyVersionsOk.Click
        ...
        If bIsSucesfullyCompleted Then
            MsgBox("Item Processed successfully", vbInformation, "Data Fix Utility")
        Else
            If NonRegenDocFound Then
                MsgBox("Some non-REGEN document(s) found which cannot be processed in 'REGEN - Policy Version' tab", vbInformation, "Data Fix Utility")
            Else
                MsgBox("Item Processed with some failures", vbInformation, "Data Fix Utility")
            End If
        End If
    ...
    Private Sub btnGetClonePolicyVersion_Click(sender As Object, e As EventArgs) Handles btnGetClonePolicyVersion.Click
        ...
        Try
            ...
            For lCount As Integer = 0 To aoResultArray.GetUpperBound(1)
                ...
                'policy type id                
                oListItem.SubItems.Insert(ACFieldPVPolicyTypeID,
                                          New ListViewItem.ListViewSubItem(oListItem, aoResultArray(ACFieldPVPolicyTypeID, lCount).Trim()))

                'policy type                
                oListItem.SubItems.Insert(ACFieldPVPolicyType,
                ...
                'policy start                
                oListItem.SubItems.Insert(ACFieldPVPolicyStart,
                                          New ListViewItem.ListViewSubItem(oListItem, aoResultArray(ACFieldPVPolicyStart, lCount).Trim()))

                'document Date               
                oListItem.SubItems.Insert(ACFieldPVDocumentDate,
        ...
        Catch ex As Exception
            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed btnGetClonePolicyVersion_Click", vApp:=ACApp,
                                         vClass:=ACClass, vMethod:="btnGetClonePolicyVersion_Click", vErrNo:=CStr(Information.Err().Number),
                                         vErrDesc:=Information.Err().Description, excep:=ex)
        Finally
        ...
        Try
            ...
            For Each oListItem As ListViewItem In Me.lvwClonePolicyVersion.Items
                If oListItem.Checked Then
                    ...
                    If sDocumentRef.Substring(0, 3) = "SDD" Or sDocumentRef.Substring(0, 3) = "SDR" Then
    ...
    Private Function RegenerateClonePolicyVersionTransaction(ByVal nInsuranceFileCnt As Integer, ByVal sDocumentRef As String) As Integer
        Try
            ...
            If nInsuranceFileCnt > 0 Then
                m_lReturn = CreateAndPostStats(nInsuranceFileCnt:=nInsuranceFileCnt, IsSDDTransaction:=IsSDDTransaction, nClonedInsuranceFileCnt:=nInsuranceFileCnt, bReverseCloned:=sInsuranceFileType <> "POLICY", sTransactionType:=sTransactionType, sDocumentRef:=sDocumentRef)
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
    ...
    Public Function CreateAndPostStats(ByVal nInsuranceFileCnt As Integer, ByVal IsSDDTransaction As Boolean, Optional ByVal nClonedInsuranceFileCnt As Integer = 0, Optional ByVal bReverseCloned As Boolean = False, Optional ByVal sTransactionType As String = "", Optional ByVal sDocumentRef As String = "", Optional ByRef r_sMessage As String = "") As Integer
        Dim result As Integer = 0
        ...
        Try
            ...
            m_lReturn = m_oBusiness.CreateStats(nStatsFolderCnt:=nStatsFolderCnt, sDocumentRef:=sDocumentRef)
    ...
    '    Dim lReturn As Integer
    '    Dim oPerilAllocation As iPMUPerilAllocationDataFix.Interface_Renamed
    ...
    '        lReturn = g_oObjectManager.GetInstance(temp_oPerilAllocation, sClassName:="iPMUPerilAllocationDataFix.Interface_Renamed", vInstanceManager:=gPMConstants.PMGetLocalInterface)
    '        oPerilAllocation = temp_oPerilAllocation
    '        If lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
    '            gPMFunctions.RaiseError(kMethodName, "GetInstance of iPMUPerilAllocationDataFix.Interface Failed", gPMConstants.PMELogLevel.PMLogError)
    '        End If

    '        lReturn = oPerilAllocation.SetProcessModes(vTask:=2, vTransactionType:=sTransactionType)
    '        If lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
    '            gPMFunctions.RaiseError(kMethodName, "iPMUPerilAllocationDataFix.Interface.SetProcessModes Failed", gPMConstants.PMELogLevel.PMLogError)
    '        End If
    ...
    '            If lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
    '                gPMFunctions.RaiseError(kMethodName, "iPMUPerilAllocationDataFix.Interface.Start Failed", gPMConstants.PMELogLevel.PMLogError)
    '            End If
    ...
    Private Sub btnRefreshRating_Click(sender As Object, e As EventArgs) Handles btnRefreshRating.Click

        'If MsgBox("If you will proceed further in refreshing the rating section then it will not be rolled back, Do you want to proceed?", vbYesNo + vbQuestion, ACApp) <> vbYes Then
        '    Exit Sub
        'End If

        'Const kMethodName As String = "lvwRisk_DoubleClick"
        'Dim oAgentCommission As Object
        'Dim sTransactionType As String = ""
        'Dim lReturn As Integer
        'Dim oPerilAllocation As iPMUPerilAllocationDataFix.Interface_Renamed
        'Dim sDocumentRef As String = (lvwRisk.FocusedItem.SubItems(6).Text)
        'Dim icount As Integer = 0
        'Dim nOriginalLegExist As Integer
        ...
        'If icount > 1 Then
        '    MsgBox("You have selected more than one policy version. Please select only one version on Policy Vesrion tab.")
        '    Exit Sub
        'End If
        ...
        'Select Case Convert.ToString((lvwRisk.FocusedItem.SubItems(7).Text))
        '    Case 2 'new business
        '        If sDocumentRef.StartsWith("SND") Then
        '            sTransactionType = "NB"
        '        Else
        ...
        '        sTransactionType = "REN"
        '    Case 4, 10, 11
        '        If String.IsNullOrEmpty(sDocumentRef) Then
        '            sTransactionType = ""
        '        End If
        '    Case 8
        '        sTransactionType = "MTC"
        ...
        '    m_lReturn = m_oBusiness.ValidateRiskForOriginalLine(nRiskCnt:=CInt(Conversion.Val(lvwRisk.FocusedItem.SubItems(1).Text)), nInsuranceFileCnt:=CInt(Conversion.Val(lvwRisk.FocusedItem.SubItems(0).Text)), nOriginalLegExist:=nOriginalLegExist)
        '    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
        '        Throw New System.Exception
        '    End If

        '    If nOriginalLegExist = 0 Then
        '        MessageBox.Show("This is not a valid risk to process.", ACApp, MessageBoxButtons.OK, MessageBoxIcon.Information)
        '        Exit Sub
        '    End If

        '    ' Insert into new datafix tables - peril and rating section datafix
        '    m_lReturn = m_oBusiness.InsertRatingSectionAndPeril(nRiskCnt:=CInt(Conversion.Val(lvwRisk.FocusedItem.SubItems(1).Text)))
        '    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
        '        Throw New System.Exception
        '    End If
        ...
        '    oPerilAllocation.InsuranceFolderCnt = CInt(Conversion.Val(lvwRisk.FocusedItem.SubItems(5).Text))
        '    oPerilAllocation.InsuranceFileCnt = CInt(Conversion.Val(lvwRisk.FocusedItem.SubItems(0).Text))
        '    oPerilAllocation.RiskId = CInt(Conversion.Val(lvwRisk.FocusedItem.SubItems(1).Text))
        '    oPerilAllocation.CallingAppName = "iPMUDataFixUtility"
        ...
        '    lReturn = oPerilAllocation.Start()
        '    If lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
        '        gPMFunctions.RaiseError(kMethodName, "iPMUPerilAllocationDataFixDataFix.Interface.Start Failed", gPMConstants.PMELogLevel.PMLogError)
        '    End If
        ...
        '    m_lReturn = m_oBusiness.UpdateInsuranceFilePremium(nInsuranceFileCnt:=CInt(Conversion.Val(lvwRisk.FocusedItem.SubItems(0).Text)))
        '    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
        ...
        '        Throw New System.Exception
        '    End If
        '    m_lReturn = m_oBusiness.CalculateAgentCommission(nInsuranceFileCnt:=CInt(Conversion.Val(lvwRisk.FocusedItem.SubItems(0).Text)), sTransactionType:=sTransactionType, r_vntResult:=oAgentCommission)
        '    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
        '        iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to Process Risk", vApp:=ACApp, vClass:=ACClass, vMethod:="RepostPolicyVersion", vErrNo:=CStr(Information.Err().Number), vErrDesc:=Information.Err().Description)
        '        Throw New System.Exception
        '    End If

        '    m_lReturn = m_oBusiness.RecalculateRiskTaxes(nInsuranceFileCnt:=CInt(Conversion.Val(lvwRisk.FocusedItem.SubItems(0).Text)), nRiskCnt:=CInt(Conversion.Val(lvwRisk.FocusedItem.SubItems(1).Text)), v_lTask:=2, sTransactionType:=sTransactionType)
        '    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
        '        iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to Process Risk", vApp:=ACApp, vClass:=ACClass, vMethod:="RepostPolicyVersion", vErrNo:=CStr(Information.Err().Number), vErrDesc:=Information.Err().Description)
        '        Throw New System.Exception
        '    End If

        '    m_lReturn = m_oBusiness.RIRefreshForSingleRisk(nInsuranceFileCnt:=CInt(Conversion.Val(lvwRisk.FocusedItem.SubItems(0).Text)), sTransactionType:=sTransactionType, nRiskCnt:=CInt(Conversion.Val(lvwRisk.FocusedItem.SubItems(1).Text)))
        '    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
        '        iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to Process Risk", vApp:=ACApp, vClass:=ACClass, vMethod:="RepostPolicyVersion", vErrNo:=CStr(Information.Err().Number), vErrDesc:=Information.Err().Description)
        '        Throw New System.Exception
        '    End If

        '    m_lReturn = m_oBusiness.UpdateRiskStatus(v_lRiskCnt:=CInt(Conversion.Val(lvwRisk.FocusedItem.SubItems(1).Text)), v_sRiskStatusCode:="QUOTED")
        '    If m_lReturn <> PMEReturnCode.PMTrue Then
        ...
        'Else
        '    MessageBox.Show("Only MTA versions are allowed for refresh rating", ACApp, MessageBoxButtons.OK, MessageBoxIcon.Information)
        'End If
    End Sub
