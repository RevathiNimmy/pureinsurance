...
Partial Friend Class frmInterface
    ...
    Private Const ACFieldRev As Integer = 3
    ...
    Private Sub cmdGetPolicyVersion_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cmdGetPolicyVersion.Click
        ...
        Try
            ...
            If txtPolicyNumber.Text <> "" Then
                sSql = "SELECT i.insurance_file_cnt,i.insurance_ref,i.insurance_file_type_id,ift.description,d.document_ref,i.cover_start_date,ISNULL(document_date,'') FROM insurance_file i "
                sSql = sSql & " INNER JOIN Insurance_File_Type ift ON ift.insurance_file_type_id=i.insurance_file_type_id "
                sSql = sSql & "LEFT JOIN document d ON d.insurance_file_cnt = i.insurance_file_cnt WHERE (document_ref LIKE 'S%' OR document_ref IS NULL) AND  (I.insurance_file_cnt NOT IN (select  insurance_file_cnt from   datafixutility_log) ) AND "
        ...
        Try
            ...
            For Each oListItem As ListViewItem In Me.lvwPolicyVersion.Items
                If oListItem.Checked Then
                    ...
                    If ChkPolicyTaxRefresh.Checked Then
                        m_lReturn = m_oBusiness.RefreshTax(nInsuranceFileCnt:=CInt(oListItem.Text), sTransactionType:=sTransactionType)
                        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                            m_oBusiness.RollbackTrans()
                            Continue For
                        End If
                ...
                For Each oListItem As ListViewItem In Me.LvwClaimVersionTrans.Items
                    If oListItem.Checked = True Then
                        ...
                        If Not (bIsFailure AndAlso sLastProcessedClaim = oListItem.SubItems(1).Text) Then
                            If sDocumentType = "CLC" OrElse sDocumentType = "CLD" Then
                                If m_oBusiness.ProcessClaimTransactions(v_lClaimId:=CLng(oListItem.Text),
                                                                            v_sDocumentRef:=oListItem.SubItems(3).Text, v_sRefNumber:=txtPMNumber.Text, v_bRePost:=True, v_bRIRefresh:=chkRIRefreshClmTrans.Checked) <> gPMConstants.PMEReturnCode.PMTrue Then
                                    bIsFailure = True
                                    iFailureCnt = iFailureCnt + 1
                                End If
                            Else
            ...
            Case 2 'Generate Claim transactions
                For Each oListItem As ListViewItem In Me.LvwClaimVersionTrans.Items
                    If oListItem.Checked = True Then
                        ...
                        If m_oBusiness.GenerateClaimTransactions(CLng(oListItem.Text), oListItem.SubItems(3).Text, v_bRIRefresh:=chkRIRefreshClmTrans.Checked, v_sRefNumber:=txtPMNumber.Text) <> gPMConstants.PMEReturnCode.PMTrue Then
