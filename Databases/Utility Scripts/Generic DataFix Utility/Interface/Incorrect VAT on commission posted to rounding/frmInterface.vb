...
Friend Partial Class frmInterface
    ...
    Private Sub chkReserve_MouseUp(ByVal eventSender As Object, ByVal eventArgs As MouseEventArgs)
    ...
    Private Sub lvwClaim_MouseDown(ByVal eventSender As Object, ByVal eventArgs As MouseEventArgs)
        Dim Button As Integer = CInt(eventArgs.Button)
        Dim Shift As Integer = Control.ModifierKeys \ &H10000
        Dim X As Single = VB6.PixelsToTwipsX(eventArgs.X)
        Dim Y As Single = VB6.PixelsToTwipsY(eventArgs.Y)
    ...
    Private Sub lvwClaimMisc_MouseDown(ByVal eventSender As Object, ByVal eventArgs As MouseEventArgs)
        Dim Button As Integer = CInt(eventArgs.Button)
        Dim Shift As Integer = Control.ModifierKeys \ &H10000
        Dim X As Single = VB6.PixelsToTwipsX(eventArgs.X)
        Dim Y As Single = VB6.PixelsToTwipsY(eventArgs.Y)
    ...
    Private Sub lvwClaimPosting_MouseDown(ByVal eventSender As Object, ByVal eventArgs As MouseEventArgs)
        Dim Button As Integer = CInt(eventArgs.Button)
        Dim Shift As Integer = Control.ModifierKeys \ &H10000
        Dim X As Single = VB6.PixelsToTwipsX(eventArgs.X)
        Dim Y As Single = VB6.PixelsToTwipsY(eventArgs.Y)
    ...
    Private Sub lvwImbalancedClosedClaim_MouseDown(ByVal eventSender As Object, ByVal eventArgs As MouseEventArgs)
        Dim Button As Integer = CInt(eventArgs.Button)
        Dim Shift As Integer = Control.ModifierKeys \ &H10000
        Dim X As Single = VB6.PixelsToTwipsX(eventArgs.X)
        Dim Y As Single = VB6.PixelsToTwipsY(eventArgs.Y)
    ...
    Private Sub optReservePayment_MouseUp(ByVal eventSender As Object, ByVal eventArgs As MouseEventArgs)
        Dim Button As Integer = CInt(eventArgs.Button)
        Dim Shift As Integer = Control.ModifierKeys \ &H10000
        Dim X As Single = VB6.PixelsToTwipsX(eventArgs.X)
        Dim Y As Single = VB6.PixelsToTwipsY(eventArgs.Y)
    ...
    Private Sub cmdOK_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cmdOK.Click
        ...
        Select Case SSTabHelper.GetSelectedIndex(Me.tabMain)
            ...
            Case 0 'Single policy
                ...
                stbMain.Items.Item("COUNT").Text = CStr(Me.lvwPolicyVersion.Items.Count)
            Case 1 'Claim
                Select Case ACClaimTabFailedClaimTransaction 'failed claim's transactions
        ...
        Try
            ...
            For Each oListItem As ListViewItem In Me.lvwPolicyVersion.Items
                If oListItem.Checked Then
                    ...
                    sSQL = "update AC set AC.party_cnt=INF.lead_agent_cnt from Agent_Commission AC " & _
                            "inner join Insurance_File INF ON AC.insurance_file_cnt=INF.insurance_file_cnt " & _
                            "where AC.insurance_file_cnt = " & CInt(oListItem.Text)
                    m_lReturn = m_oBusiness.UpdateSQLAction(sSQL:=sSQL)

                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                        iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to update agent commission", vApp:=ACApp, vClass:=ACClass, vMethod:="ReverseAndRegenerateTransaction", vErrNo:=CStr(Information.Err().Number), vErrDesc:=Information.Err().Description)
                    End If
...
Finally_Renamed:
    ...
    Private Sub txtPolicyNumber_TextChanged(sender As Object, e As EventArgs)
        txtSqlQuery.Text = ""
    End Sub
