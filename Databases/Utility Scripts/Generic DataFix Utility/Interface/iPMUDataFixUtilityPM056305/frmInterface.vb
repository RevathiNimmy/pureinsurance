...
Finally_Renamed:
    ...
    Private Sub frmInterface_Load(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Load
        ...
        _optSinglePolicy_4.Enabled = False
        _optSinglePolicy_3.Enabled = False
...
Finally_Renamed:
    ...
    Private Function ReversePolicyVersionTransaction(ByVal v_nInsuranceFileCnt As Integer, ByVal v_sDocumentRef As String, Optional ByVal v_bIsCalledFromDuplicateVersionTab As Boolean = False, Optional ByRef r_nReverseDocId As Integer = 0) As Integer
        Try
    ...
    Private Sub ReverseAndRegenerateTransaction()
        Try
            ...
            Dim nNewDocumentId As Integer
            Dim nReverseDocID As Integer
            Dim sRevserseDocRef As String = ""
            Dim temp_oControlTrans As Object
            m_lReturn = g_oObjectManager.GetInstance(temp_oControlTrans, "bControlTrans.Automated", vInstanceManager:=gPMConstants.PMGetViaClientManager)
            oControlTrans = temp_oControlTrans
            ...
            For Each oListItem As ListViewItem In Me.lvwPolicyVersion.Items
                If oListItem.Checked Then
                    ...
                    If sDocumentRef <> "" Then
                        'Reverse Transaction
                        m_lReturn = ReversePolicyVersionTransaction(v_nInsuranceFileCnt:=CInt(oListItem.Text), v_sDocumentRef:=sDocumentRef, r_nReverseDocId:=nReverseDocID)
                        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed ReversePolicyVersionTransaction()", vApp:=ACApp, vClass:=ACClass, vMethod:="ReversePolicyVersionTransaction", vErrNo:=CStr(Information.Err().Number), vErrDesc:=Information.Err().Description)
                            Throw New System.Exception
                        End If
                    ...
                    nNewDocumentId = m_oBusiness.GetNewDocumentID(CInt(oListItem.Text))

                    If nNewDocumentId > 0 Then

                        If nReverseDocID > 0 Then
                            m_lReturn = m_oBusiness.GetDocumentRef(v_nDocumentId:=nReverseDocID, V_sDocRef:=sRevserseDocRef)
                        End If
                        If sRevserseDocRef <> "" Then
                            m_lReturn = m_oBusiness.AllocateTransactions(v_sOldDocumentRef:=sRevserseDocRef, v_lNewDocumentid:=nNewDocumentId)
                            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                                iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to post transactions", vApp:=ACApp, vClass:=ACClass, vMethod:="ProcessFailedTransaction", vErrNo:=CStr(Information.Err().Number), vErrDesc:=Information.Err().Description)
                                Throw New System.Exception
                            End If
