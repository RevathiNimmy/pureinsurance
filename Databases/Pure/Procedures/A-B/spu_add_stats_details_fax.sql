...
Execute DDLDropProcedure 'spu_add_stats_details_fax'
GO

CREATE PROCEDURE spu_add_stats_details_fax  
    @stats_folder_cnt int  
AS  
  
    --**************************************************************************************  
    -- sp_add_stats_details_fac creates stats details records for  
    -- FAC reinsurance by product/class/peril/reinsurer.  
    --  
    ...
    -- 1.6          Only get new or changed risks                           13/03/2001  RWH  
    --               i.e. those with status_flag = 'C'  
    -- 1.7          A number of fixes to retrieve fac data correctly.       21/06/2001  RWH  
    -- 1.8          ri commission - divide by 100 omitted                   25/06/2001  JMK  
    -- 1.9          Reverse signs for premiums and commission. (Now -ve)    11/07/01    RWH  
    -- 2.0          Check this_premium instead of annual_premium  
    --              when retrieving peril info eliminating only  
    --              those that are 0.                                       08/08/2001  RWH  
    -- 2.1          Removed check on @peril_share_with_coinsurer flag as this  
    --              should only apply to tax.                               20/08/2001  RWH  
    -- 2.2          Retrieve ri_party_type from associated reinsurance_type  
    --              record.                                                 06/09/2001  RWH  
    -- 2.4          Multi-currency stuff.                                   25/10/2001  TOT  
    -- 2.5          Apply coins to commission.                              05/11/2001  RWH  
    -- 2.6          Also process deleted risks.                             15/02/2002  Tom  
    -- 2.3          include items where is_levy_tax = true                  19/07/2002  Thinh  
    ...
    DECLARE  
        ...
        @peril_is_levy_tax int,            -- Thinh Nguyen (19/07/2002) Accounting for levies  
        @peril_original_flag int,  
        -- FAC variables  
       @fac_ri_share_percent float,  
        @fac_ri_party_cnt int,  
        @fac_ri_shortname varchar(20),  
        @fac_premium_percent float,  
        @fac_commission_percent float,  
        @fac_sum_insured money,  
        @fac_premium_value money,  
        @fac_commission_value money,  
        @fac_ri_party_type char(3),  
        @fac_original_sum_insured money,  
        @fac_is_commission_modified int,  
        -- Output values  
   ...
   @Insurance_file_type varchar(5),  
  @stats_count INT,  
  @original_stats_count INT,  
  @peril_total_premium NUMERIC(19,4),  
  @peril_total_original_premium NUMERIC(19,4),  
  @stats_version INT  
  
	--Start (Renuka) - (Tech Spec - TRAC 4761 Stats Detail to RI Line.docx) - (6.2)  
	-- Adding variable to get RI_Arrangement_Line_ID
	DECLARE @RI_Arrangement_Line_ID INT
	--End (Renuka) - (Tech Spec - TRAC 4761 Stats Detail to RI Line.docx) - (6.2)  
  ...
  EXEC spu_SIR_Get_Premium_Across_Perils  
   @risk_cnt = @peril_risk_id,  
   @peril_total_premium = @peril_total_premium OUTPUT,  
   @peril_total_original_premium = @peril_total_original_premium OUTPUT  
  
        -- Get FAX arrangement records  
        DECLARE FAX_Cursor CURSOR FAST_FORWARD FOR  
            SELECT  R.this_share_percent,  
                   ...
                   (SELECT SUM(R2.sum_insured)  
                    ...
                    WHERE  R2.party_cnt = R.party_cnt  
                    AND    R2.line_limit = R.line_limit  
            ...
            JOIN    Party_Insurer PIN      ON PIN.party_cnt = R.party_cnt  
        JOIN    Reinsurance_Type RT    ON RT.reinsurance_type_id = PIN.reinsurance_type  
            ...
            AND     R.type = 'FX'  
   AND  R.premium_value <> 0  
  
        -- Open the FAX cursor  
        OPEN FAX_Cursor  
        FETCH NEXT FROM FAX_Cursor  
            INTO    @fac_ri_share_percent,  
                    @fac_ri_party_cnt,  
                    @fac_ri_shortname,  
                    @fac_premium_percent,  
                    @fac_commission_percent,  
                    @fac_sum_insured,  
                    @fac_premium_value,  
                    @fac_commission_value,  
                    @fac_ri_party_type,  
                    @fac_original_sum_insured,  
                    @fac_is_commission_modified,
					--Start (Renuka) - (Tech Spec - TRAC 4761 Stats Detail to RI Line.docx) - (6.2)
        ...
        WHILE @@FETCH_STATUS = 0  
        BEGIN  
            --    Would be lovely if we could use the stored premium value  
   ...
   IF ((@peril_total_premium <> 0 AND @peril_original_flag=0) OR (@peril_total_original_premium<>0 AND @peril_original_flag = 1)) BEGIN  
              SELECT  @fac_premium_value = @peril_this_premium_original * @fac_premium_percent / 100  
    SELECT @fac_sum_insured = @peril_this_sum_insured * @fac_ri_share_percent / 100  
    SELECT  @stats_version=13  
   END  
   ELSE IF @peril_is_levy_tax<>1 AND @peril_original_flag=0 BEGIN  
             SELECT  @fac_sum_insured = @peril_this_sum_insured * @fac_ri_share_percent / 100  
    SELECT  @stats_version=0  
   END  
   ELSE IF @peril_is_levy_tax<>1 AND @peril_original_flag=1 BEGIN  
             SELECT  @fac_sum_insured = @peril_this_sum_insured * @fac_ri_share_percent / 100  
    SELECT  @stats_version=1  
   END  
   ELSE  
    SELECT  @stats_version=13  
            ...
            IF (@is_coinsured_policy > 0) --AND (@peril_share_with_coinsurer = 1)  
                SELECT  @fac_premium_value = @fac_premium_value * @retained_coins_percent,  
                        @fac_sum_insured = @fac_sum_insured * @retained_coins_percent  
  
            -- calculate lead_commission from adjusted premium  
            SELECT @fac_commission_value = @fac_premium_value * @fac_commission_percent / 100  
  
        -- Calculate the sum_insured_change figure  
            SELECT @out_sum_insured_change = @fac_sum_insured + ISNULL(@fac_original_sum_insured, 0)  
  
      /*Get amounts in base currency*/  
      EXEC spu_ACT_Do_Currency_Conversion  
                @company_id = @company_id,  
                @currency_id = @out_currency_id,  
                @currency_amount_unrounded = @fac_premium_value,  
                @mode = 'ALL',  
      ...
      EXEC spu_ACT_Do_Currency_Conversion  
                @company_id = @company_id,  
                @currency_id = @out_currency_id,  
                @currency_amount_unrounded = @fac_sum_insured,  
                @mode = 'ALL',  
      ...
      EXEC spu_ACT_Do_Currency_Conversion  
                @company_id = @company_id,  
                @currency_id = @out_currency_id,  
                @currency_amount_unrounded = @fac_commission_value,  
                @mode = 'ALL',  
 @base_amount = @out_lead_commission_value_home OUTPUT,  
                @system_amount = @out_lead_commission_value_system OUTPUT,  
                @currency_base_xrate = @out_currency_rate OUTPUT,  
                @system_base_xrate = @out_system_rate OUTPUT,  
                @return_status = @return_status OUTPUT  
   ...
   EXEC spu_add_stats_details_spread_ri @stats_folder_cnt, 'FAX'  
    ...
    -- ********************************************************************  
    --          TAX RECORDS FOR REINSURER PREMIUM AND COMMISSION  
    -- ********************************************************************  
 ...
 JOIN  ri_arrangement_line RAL     ON RAL.ri_arrangement_line_id=TC.ri_arrangement_line_id  
        WHERE   TC.insurance_file_cnt = @insurance_file_cnt  
        AND     TC.transtype IN ('TTRIFP','TTRIFC')  
  ...
  AND RAL.type='FX'  
    ...
    BEGIN  
      ...
      IF @transtype='TTRIFP'  
          SELECT  @out_stats_detail_type = 'TFP'  
      ELSE  
       SELECT  @out_stats_detail_type = 'TFC', @tax_value=-@tax_value  
