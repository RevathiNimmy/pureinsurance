...
Execute DDLDropProcedure 'spu_add_stats_details_treaty_xol'
GO


CREATE PROCEDURE spu_add_stats_details_treaty_xol
...
AS    
    ...
    BEGIN    
        ...
        DECLARE TTY_Cursor CURSOR FAST_FORWARD FOR    
            SELECT  R.this_share_percent,    
                    T.treaty_id,    
                    T.code,    
                    Case When R.premium_percent = 0 then R.default_share_percent else R.premium_percent end,    
	    ...
	    END  
			ELSE IF @peril_is_levy_tax<>1 AND @peril_original_flag=0
	            SELECT  @treaty_sum_insured = @peril_this_sum_insured * @treaty_premium_percent / 100,
						@stats_version=0
			ELSE IF @peril_is_levy_tax<>1 AND @peril_original_flag=1
	            SELECT  @treaty_sum_insured = @peril_this_sum_insured * @treaty_premium_percent / 100,
						@stats_version=1
			ELSE
				SELECT 	@stats_version=13
    ...
    DECLARE Taxes_Cursor CURSOR FAST_FORWARD FOR    
        ...
        AND     (RAL.Type = 'TX' OR RAL.Type = 'TC')
    OPEN Taxes_Cursor    
