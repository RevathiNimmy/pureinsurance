...
GO
EXECUTE DDLDropProcedure 'spu_ACT_PrepareAgentSummaryAllocation'
GO

CREATE Procedure spu_ACT_PrepareAgentSummaryAllocation  
(  
...
IF YEAR(@allocation_date_to)=1899 SET @allocation_date_to = NULL  
...
SET @SQL = @SQL + ' FROM #Temp T JOIN Transdetail TD ON T.Transdetail_ID=TD.Transdetail_ID  INNER JOIN DOCUMENT D ON D.document_id =TD.document_id  
LEFT JOIN STATS_FOLDER SF ON SF.document_ref =D.document_ref '  
SET @SQL = @SQL + ' LEFT JOIN Insurance_file IFC ON T.Insurance_file_cnt = IFC.Insurance_file_cnt '  
SET @SQL = @SQL + ' LEFT JOIN Account AC on AC.account_id = TD.account_id '  
SET @SQL = @SQL + ' LEFT JOIN Party P on P.party_cnt = AC.account_key '  
SET @SQLWhere= ' TD.outstanding_amount <> 0 AND T.SPARE = ''COMM'' AND  
(T.INSURANCE_FILE_CNT in(select DISTINCT SF.insurance_file_cnt  from TransDetail TD  
inner join AllocationDetail ad on td.transdetail_id =ad.transdetail_id  
INNER JOIN DOCUMENT D ON D.document_id =TD.document_id  
INNER JOIN STATS_FOLDER SF ON SF.document_ref =D.document_ref  
where td.outstanding_amount=0 and ad.fully_matched =1)) AND  
 (T.binder_indicator=0 OR T.Document_id IN (SELECT TRD.Document_id FROM  
  transdetail TRD  
  JOIN account ACC ON ACC.account_id=TRD.account_id  
  JOIN AccountType ACCT ON ACC.accounttype_id=ACCT.accounttype_id  
   WHERE  TRD.document_id =T.Document_id and ACCT.accounttype_id='+  Cast(@kClientAccountTypeId  AS VarChar)  +' AND outstanding_account_amount=0 )) '  
...
BEGIN  
 If @SQLWhere <> ''  
 BEGIN  
  SET @SQLWhere= @SQLWhere + ' AND CAST(TD.accounting_date as varchar) >=' + '''' + CAST(@allocation_date_from AS VARCHAR) + ''''  
 END  
 Else  
 BEGIN  
  SET @SQLWhere= ' AND CAST(TD.accounting_date as varchar) >=' + '''' + CAST(@allocation_date_from AS VARCHAR) + ''''  
 END  
END  
  
IF ISNULL(@allocation_date_to,'') <> ''  
 BEGIN  
  If @SQLWhere <> ''  
  BEGIN  
   SET @SQLWhere= @SQLWhere + ' AND CAST(TD.accounting_date as varchar) <=' + '''' + CAST(@allocation_date_to AS VARCHAR) + ''''  
  END  
  Else  
  BEGIN  
   SET @SQLWhere= ' AND CAST(TD.accounting_date as varchar) <=' + '''' + CAST(@allocation_date_to AS VARCHAR) + ''''  
  END  
END  
...
--INNER JOIN Insurance_File ifl on p.party_cnt= ifl.insured_cnt  
