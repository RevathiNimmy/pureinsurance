...
Execute DDLDropProcedure 'spu_add_stats_details_fac'
GO


CREATE PROCEDURE spu_add_stats_details_fac  
...
AS  
 ...
 Select @Is_RI2007Enabled = isnull(value,0) From Hidden_options  
 WHERE option_number= 88  
    ...
    BEGIN  
        ...
        -- get FAC arrangement records  
        -- RWH(06/09/01) Retrieve ri_party_type from associated reinsurance_type record.  
        -- Also get shortname from Party.  
        -- ISS4752 Peter Finney - Get original sum_insured  
        DECLARE FAC_Cursor CURSOR FAST_FORWARD FOR  
            SELECT  R.this_share_percent,  
                   ...
                   (SELECT R2.sum_insured  
                    ...
                    AND    R2.ri_arrangement_id = @orig_ri_arrangement_id AND  R2.priority = R.priority AND R2.type = 'F'
					--Added by Nitesh for resolving PN-67182
					AND	   R.ri_arrangement_line_id=R2.ri_arrangement_line_id), 
					---End (Nitesh) 
     ...
     --End (Prakash Varghese) - (Tech Spec - TRAC 4761 Stats Detail to RI Line.docx) - (6.2)  
        ...
        BEGIN  
            ...
            IF @fac_premium_percent > 0 AND @peril_this_premium_original<>0  
             SELECT  @fac_premium_value = @peril_this_premium_original * @fac_premium_percent / 100  
   ELSE  
    SELECT  @fac_premium_value = 0  
  ...
  If ((Select sum(Round(premium, 2)) From ri_arrangement Where risk_cnt = @peril_risk_id Group By risk_cnt) = 0 ) AND @Is_RI2007Enabled <> 1  
  Begin  
      SELECT @fac_commission_value = 0  
  End  
     Else  
  Begin  
