...
EXECUTE DDLDropProcedure 'spu_add_trans_details_clm_reins'
GO

CREATE PROCEDURE spu_add_trans_details_clm_reins
...
AS  
	...
	Select  @Claim_id=loss_id,
			@Stats_Transaction_type_code=transaction_type_code 
    ...
    DECLARE RI_Cursor CURSOR FAST_FORWARD FOR  
        ...
        AND     d.stats_detail_type IN('TTY', 'TAT', 'XOL', 'TAL', 'FAC', 'TAF', 'FAX', 'TYX')  
    ...
    BEGIN  
        ...
        BEGIN  
            ...
            ELSE BEGIN  
				...
				IF @out_transaction_ledger_code = 'NO'  
					IF @ri_stats_detail_type IN ('TTY', 'TAT')  
						SELECT  @out_account_type_code = 'CLMOSRITR'  
				...
				ELSE  
 				...
 				IF @transaction_type = 'C_CO' OR @transaction_type = 'C_CR'  
					 SELECT @transdetail_type_code = 'CLMRIRES'
				ELSE IF @transaction_type = 'C_RV' OR @transaction_type = 'C_SA'   
					 SELECT @transdetail_type_code = 'CLMRIREC' 
				ELSE IF @transaction_type = 'C_CP'  
					SELECT @transdetail_type_code = 'CLMRIPAY' 
				
				IF @ri_stats_detail_type NOT IN ('TAG', 'TAN')
                SELECT  @transaction_amount = CASE
                        WHEN @transaction_type  IN ('C_SA', 'C_RV')
                        THEN -@ri_premium_total
                        ELSE @ri_premium_total
                    END	
            -- Insert detail row  
            ...
            BEGIN  
				...
				ELSE IF @transaction_type IN ('C_SA')
                    SELECT  @out_account_type_code = 'CLMRISAL',
                            @out_nominal_ledger_account = 'CLMRISAL'
				ELSE IF @transaction_type IN ('C_RV')
                    SELECT  @out_account_type_code = 'CLMRITP',  
                            @out_nominal_ledger_account = 'CLMRITP'  
                ...
                ELSE IF @cob_ri_stats_detail_type IN ('TYX')  
                   SELECT  @out_account_type_code = @out_account_type_code + 'TTY',  
                           @out_nominal_ledger_account = @out_nominal_ledger_account + 'TTY'
				...
				@transaction_amount = CASE
					WHEN @transaction_type IN ('C_SA', 'C_RV')
                    THEN @cob_premium_total
                    ELSE -@cob_premium_total
					END
                -- Insert output row  
                INSERT INTO Transaction_Export_Detail  
                       (transaction_export_folder_cnt,  
