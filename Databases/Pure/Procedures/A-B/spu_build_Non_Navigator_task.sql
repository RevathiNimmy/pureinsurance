...
EXECUTE DDLDropProcedure 'spu_build_Non_Navigator_task'
GO


CREATE PROCEDURE spu_build_Non_Navigator_task
    @pmwrk_task_code char(10),
    @pmwrk_task_description varchar(255),
    @pmwrk_task_object char(30),
    @pmwrk_task_Class char(30),
    @pmwrk_task_group_code char(10),
    @pmwrk_task_group_desc varchar(255),
    @pmuser_group_code char(10)
AS

/* AK 140601 - procedure to add tasks, which are not navigator-process based */
...
BEGIN
    ...
    SELECT @pmwrk_task_category_id =
        (SELECT pmwrk_task_category_id FROM PMWrk_Task_Category WHERE code = 'SBO')
    ...
    VALUES
    (@caption_id, @pmwrk_task_code, @pmwrk_task_description, 0, getdate(), 0, 1, null, @pmwrk_task_object, @pmwrk_task_class, 0, 1, 0, null, null, null, 1, @pmwrk_task_category_id )
END
