...
EXECUTE DDLDropProcedure 'spu_ACT_Select_Trans_For_Receipt_Allocation'
GO

CREATE PROCEDURE spu_ACT_Select_Trans_For_Receipt_Allocation
    @account_id INT,
	@company_id INT=NULL,
	@instalment_plan_ref VARCHAR(20)= NULL,      
    @receipt_transdetail_id INT= NULL,    
...
BEGIN
    ...
    AND  	insurance_ref=ISNULL(@instalment_plan_ref,insurance_ref)       	
	...
	IF @base_currency_count=1 BEGIN
		...
		AND t.company_id=ISNULL(@company_id,company_id)
		AND t.insurance_ref=ISNULL(@instalment_plan_ref,t.insurance_ref)        
