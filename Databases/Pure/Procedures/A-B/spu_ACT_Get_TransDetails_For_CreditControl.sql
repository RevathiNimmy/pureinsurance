...
Description   : This is used to select details from Account table
Test Code     : Exec spu_ACT_Get_TransDetails_For_SingleInstalementPlan  
 */
...
Execute DDLDropProcedure 'spu_ACT_Get_TransDetails_For_SingleInstalementPlan'
GO

CREATE PROCEDURE spu_ACT_Get_TransDetails_For_SingleInstalementPlan  
 @nAccount_id INT,  
...
INSERT INTO #temptransaction  
  SELECT DISTINCT PFTransaction_id.insurance_file_cnt,PFTransaction_id.pfprem_finance_cnt  
        FROM PFTransaction_id  
  INNER JOIN insurance_file ON PFTransaction_id.insurance_file_cnt=insurance_file.insurance_file_cnt
  WHERE PFTransaction_id.pfprem_finance_cnt=@nPFprem_finance_cnt  
  AND insurance_file.insurance_file_type_id not in (5,8) 


SELECT t.transdetail_id,t.currency_amount,dt.CODE,d.document_id,d.document_ref,d.documenttype_id,i.insurance_file_cnt 
	FROM Account a INNER JOIN TransDetail t ON t.account_id = a.account_id  
	...
	WHERE t.account_id=@nAccount_id
