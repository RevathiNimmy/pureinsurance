...
if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_Report_Daily_Audit1_AH]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_Report_Daily_Audit1_AH]
GO


CREATE PROCEDURE sp_Report_Daily_Audit1_AH
...
-- Select Client Debit Transaction(s) into Table 1
...
AND	(
	(
		(
			...
			AND	
			TEF.cover_start_date <= @end_date
		)
		OR
		(
			D.document_date < @start_date
			AND
			TEF.cover_start_date >= @start_date
			AND	
			TEF.cover_start_date <= @end_date
		)
	)
...
AND	D.documenttype_id IN (4, 15, 17, 31, 35)	
...
BEGIN	
	...
	(
		SELECT	R.transdetail_id + 1
		FROM	report_audit_debit_table2	R,
			orion_for_broking..transdetail	T
		WHERE	T.transdetail_id = R.transdetail_id 
		AND	(
				R.comment = 'COMM ADJ'
				OR
				R.comment = 'AGENT ADJ'
	...
	/* Only if outside range */
	DELETE
	FROM	report_audit_debit_table2 
	WHERE	transdetail_id IN
	(
		SELECT	R.transdetail_id
		FROM	report_audit_debit_table2	R,
			orion_for_broking..transdetail	T
		WHERE	T.transdetail_id = R.transdetail_id 
		AND	(
	...
	SELECT	DISTINCT
		...
		FROM	report_audit_debit_table1 	T1,
			...
			orion_for_broking.dbo.Account	A
		WHERE	T1.transdetail_id = @document_id
		AND 	T2.ledger_id = 2
	...
	AND 	(
			(
			T2.amount < 0
			AND	ISNULL(T2.comment, '') <> 'Reversal'
			)
			OR		
			(
			T2.amount > 0
	...
	AND	A.short_code =
		...
		AND	T2.comment <> 'COMM ADJ'
		AND	T2.comment <> 'AGENT ADJ'
	...
	AND	comment <> 'COMM ADJ'
	AND	comment <> 'AGENT ADJ'
	...
	AND	R2.ledger_id = 4
	AND	R2.comment <> 'COMM ADJ'
	AND	R2.comment <> 'AGENT ADJ'
...
*/
	...
	SET	sub_agent_fees = 
		(
			...
			FROM		report_audit_debit_table2	R2,
					orion_for_broking..Account	A	
			WHERE	A.account_id = R2.account_id
			AND		(
					R2.comment = 'COMM ADJ'
					OR	R2.comment = 'AGENT ADJ'
					)
		),
		sub_agent_short_code = 
		(
			SELECT 	MIN(A.short_code)
			FROM		report_audit_debit_table2	R2,
					orion_for_broking..Account	A	
			WHERE	A.account_id = R2.account_id
			AND		(
	...
	-- Delete Commission Adjustments from table 2
	DELETE FROM report_audit_debit_table2
	WHERE	(
			comment = 'COMM ADJ'
			OR	comment = 'AGENT ADJ'
		)
...
/* Final check on dates for adj items only */
UPDATE	report_audit_debit_table3 
SET		this_premium = 0,
		insurer_premium = 0,
		net_commission = 0,
		...
		commission_amount = 0,
		disc = 0,
		document_date = T.ref_date,
		cover_start_date = T.ref_date
FROM		report_audit_debit_table3	R,
		orion_for_broking..Transdetail	T
WHERE	T.document_id = R.transdetail_id 
AND
(
	T.spare = 'COMM ADJ' 
	OR
