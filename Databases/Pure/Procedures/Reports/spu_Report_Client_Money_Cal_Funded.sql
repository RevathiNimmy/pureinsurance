...
EXECUTE DDLDropProcedure 'spu_Report_Client_Money_Cal_Funded'
GO

CREATE PROCEDURE spu_Report_Client_Money_Cal_Funded
    @branch_id INT,
    @end_date DATETIME,
    @funded MONEY OUTPUT,
    @returns MONEY OUTPUT
AS
            ...
            (
                (
                    ...
                    (
                        ...
                        WHERE ad.allocationdetail_id = tm.allocationdetail_id
                        AND ad.transdetail_id = tm.transdetail_id
                        AND ad.cashlistitem_id IS NOT NULL
                    )
                )         
                OR
                (
                    ...
                    (
                        ...
                        JOIN document d2
                            ON d2.document_id = td2.document_id
                            AND d2.documenttype_id IN (22,23)/*SRP,SPY*/
        ...
        DECLARE c_payment CURSOR FAST_FORWARD FOR
            ...
            /*AND NOT EXISTS
                (
                    ...
                    AND tdx.document_id = @document_id
                    AND tmx.is_reversed IS NULL
                    AND tmx.allocationdetail_id IS NOT NULL
                )*/
...
SELECT 
    @funded = ISNULL(SUM(ISNULL(insurer_payment_amount,0)),0) + ISNULL(SUM(ISNULL(client_payment_amount,0)),0)
FROM #temp_trans
WHERE debit_credit = 1

SELECT 
    @returns = ISNULL(SUM(ISNULL(client_payment_amount,0)),0) + ISNULL(SUM(ISNULL(insurer_payment_amount,0)),0)
FROM #temp_trans
