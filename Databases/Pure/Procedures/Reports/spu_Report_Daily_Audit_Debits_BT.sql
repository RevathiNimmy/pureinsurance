...
EXECUTE DDLDropProcedure 'spu_Report_Daily_Audit_Debits_BT'
GO
-- MJ 07/2002 revisisted the SP from top to bottom
-- MJ 07/2002 This report should show the transaction as raised and as such will not include any adjustments made
-- MJ 07/2002 shared preium transactions are unlikely to work correctly
-- ECK 29/07/2002 Leave client premium as zero when sub agent posting is present
--ECK 28/10/02 Allow selection by specific Business Type

CREATE PROCEDURE spu_Report_Daily_Audit_Debits_BT
    (
    @branch_id int,
    @start_date datetime,
    @end_date datetime,
    @date_type varchar(11),
    @business_type   char(10) = NULL
	)
AS

DECLARE @iBranchID int,
        @document_id int,
	@sbusiness_type char(10),
  	@sBusiness_Type_description as varchar(255)

SELECT @iBranchID = ISNULL(@branch_id, 0)

SELECT @sBusiness_type = ISNULL(@business_type,'ALL')
SELECT @sBusiness_Type_description =  isnull( (select description from business_type 
					    where code = @sBusiness_Type ) ,'') 

set nocount on

-- report_audit_debit_table2 used to hold the basic transaction details for processing within the cursor
...
SELECT DISTINCT
    ...
    ISNULL(TEF.cover_start_date, '') as 'cover_start_date',	--1.6.9
    0 as 'this_premium',
...
AND D.documenttype_id IN (2, 4, 15, 17, 31, 35)
AND ( @iBranchID = 0
        OR
        ( @iBranchID <> 0
            AND
...
SET radt.insurance_ref = isnull(IFI.insurance_ref, TD.insurance_ref),
    ...
    radt.cover_start_date = ISNULL(TEF.cover_start_date, ''),		-- 1.6.9
		...
		AND	NOT 
			...
			AND
				...
				(	SELECT	transdetail_id
					...
					ON	TD1.spare = 'DDREV ' + D1.document_ref
					AND	TD1.company_id = d1.company_id
					WHERE D1.document_id = @document_id
				)
			)
    )
, 0.00)
...
-- Again this may not be correct for shared premuims
    ...
    IF ( SELECT COUNT(DISTINCT account_id)
            ...
            WHERE ledger_id IN (SELECT ledger_id FROM ledger WHERE ledger_short_name = 'IN') --PSL16/06/2003 Remove Hard coded ledger_id4
        ) > 1
    BEGIN
        insert into report_audit_debit_table3
        ( year_name,
...
--eck281002
DELETE  FROM report_audit_debit_table3  
	WHERE account_handler <> @sBusiness_type_description 
	AND @sBusiness_type <> 'ALL'

DELETE  FROM report_audit_debit_table3  

	 
	WHERE lower(comment) like '%revers%'
	AND @date_type = 'Effective'
