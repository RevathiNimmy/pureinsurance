...
EXECUTE DDLDropProcedure 'spu_Report_Trial_Balance_Currency_SFU'
GO

 

CREATE PROCEDURE spu_Report_Trial_Balance_Currency_SFU (
	@branch_id		int,
...
-- TF240701	        
-- DC021001 resturctured following to use document_sequence rather than transaction_detail_id
--		and check for actual COMM ADJ record or AGENT ADJ record that was being missed
--		out before
	    	AND		
		--start of check on COMM ADJ and AGENT ADJ	
		(
			(
			--spare NULL and not part of COMM ADJ transaction
			D.document_date <= @dBFEndDate
			AND	(
				ISNULL(TD.spare, '') NOT IN ('COMM ADJ', 'AGENT ADJ')
			...
			OR
			(
			--spare NULL and part of COMM ADJ transaction
