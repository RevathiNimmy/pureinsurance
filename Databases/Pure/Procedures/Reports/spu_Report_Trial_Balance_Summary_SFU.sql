...
EXECUTE DDLDropProcedure 'spu_Report_Trial_Balance_Summary_SFU'
GO

CREATE PROCEDURE spu_Report_Trial_Balance_Summary_SFU    
    @branch_id int,    
...
DECLARE    
  ...
  SELECT    
   0,    
   isnull(sum(ROUND(TD.amount,2)), 0.0),    
   0.0,    
   0.0,    
   @sBalBF,    
   A.Account_ID,    
...
WHERE   TB.Node1 = 'Nominal Ledger'    
    ...
    AND TB.Node3 = 'Assets'    
        AND ( TB.Node4 = 'Fixed Assets'    
    
                    )    
    
SELECT @Positives = ( SELECT    
    SUM(TB.AmountCF)    
                      FROM  #TBSummary TB    
                      WHERE  ( TB.AmountCF > 0 )    
...
WHERE   TB.Node1 = 'Nominal Ledger'    
    ...
    AND TB.Node4 = 'Current Assets'    
    AND TB.Node5 = 'Sales Ledger'    
    AND (ISNULL(TB.Node6, '') = 'Claims Receivable' or TB.Node6 like 'CLM%')    
SELECT @Positives = ( SELECT    
    SUM(TB.AmountCF)    
                      FROM  #TBSummary TB    
                      WHERE  ( TB.AmountCF > 0 )    
...
UPDATE #TBSummaryOutput    
    SET Positives = ISNULL(@Positives,0),    
        Negatives = ISNULL(@Negatives,0)    
    WHERE Heading = 'Claims Receivable'    
...
SELECT  8,    
        1,    
        'Other Assets(O/s Claims RI Treaty)',    
        SUM(TB.AmountBF),    
        SUM(TB.Amount),    
        NULL,    
        NULL,    
...
WHERE   TB.Node1 = 'Nominal Ledger'    
    ...
    AND ( TB.Node5 = 'Fees Receivable'    
          OR TB.Node5 LIKE 'O/S Claims RI%' )    
SELECT @Positives = ( SELECT    
    SUM(TB.AmountCF)    
                      FROM  #TBSummary TB    
                      WHERE  ( TB.AmountCF > 0 )    
...
UPDATE #TBSummaryOutput    
    SET Positives = ISNULL(@Positives,0),    
        Negatives = ISNULL(@Negatives,0)    
    WHERE Heading = 'Other Assets(O/s Claims RI Treaty)'    
...
UPDATE #TBSummaryOutput    
    SET Positives = ISNULL(@Positives,0),    
        Negatives = ISNULL(@Negatives,0)    
    WHERE Heading = 'Commission Accounts'      
...
WHERE   TB.Node1 = 'Nominal Ledger'    
    AND TB.Node2 = 'Balance Sheet'    
    AND TB.Node3 = 'Liabilities'    
   AND ((TB.Node4 = 'Current Liabilities' AND TB.Node5 = 'RI Payable')OR (TB.Node4 = 'RETAINPROF' AND TB.Node5 = 'RISUSP'))   
SELECT @Positives = ( SELECT    
    SUM(TB.AmountCF)    
                      FROM  #TBSummary TB    
                      WHERE  ( TB.AmountCF > 0 )    
...
UPDATE #TBSummaryOutput    
    SET Positives = ISNULL(@Positives,0),    
        Negatives = ISNULL(@Negatives,0)    
    WHERE Heading = 'Claims Payable'    
    
    
INSERT INTO #TBSummaryOutput    
SELECT  3,    
        2,    
        'O/S Claims Adjustments',    
        SUM(TB.AmountBF),    
        SUM(TB.Amount),    
        NULL,    
        NULL,    
...
UPDATE #TBSummaryOutput    
    SET Positives = ISNULL(@Positives,0),    
        Negatives = ISNULL(@Negatives,0)    
    WHERE Heading = 'O/S Claims Adjustments'    
...
WHERE   TB.Node1 = 'Nominal Ledger'    
    ...
    AND TB.Node3 = 'Liabilities'    
   AND ((TB.Node4 = 'Current Liabilities'    
                          AND (TB.Node5 = 'Tax' OR TB.Node5 = 'VAT'))    
                                OR (TB.Node4 = 'Share Capital & Res' )
                                OR (TB.Node4 = 'Long Term Liabilitie' ))    
