...
EXECUTE DDLDropProcedure 'spu_Report_Comm_Outstanding_NZ'
GO

CREATE PROCEDURE spu_Report_Comm_Outstanding_NZ 
(
    @end_date DATETIME,
    @branch_id INT,
...
SELECT  
        ...
        (
            ...
            (
                (
                    @TypeOfCurrency = 'Base' 
                    AND 
                    td.amount_currency_id = td2.amount_currency_id
                )
                OR
                (
                    @TypeOfCurrency = 'Transaction' 
                    AND 
                    td.currency_id = td2.currency_id
                )
            )
        )
        -
    ...
    (
	SELECT ROUND(SUM(epa1.tax_amount),2)
	FROM document d
	...
	WHERE d.document_id = d1.document_id AND pat.description = 'AGENT'
    ),
    /* Sub Agent Tax Calculation*/
    (
	SELECT ROUND(SUM(epa1.tax_amount),2)
	FROM document d
	JOIN transaction_export_folder tef 
	...
	JOIN party_agent_type pat 
		ON pat.party_agent_type_id = pa.party_agent_type_id
	WHERE d.document_id = d1.document_id AND pat.description = 'SUB AGENT'
    ),
    /* Commssion Tax Calculation*/
	...
	WHERE d.document_id = d1.document_id
    ),
    /*  Fee Tax Calculation*/
    (
	SELECT ROUND(SUM(epf.tax_amount),2)
	FROM document d
	JOIN transaction_export_folder tef 
		ON tef.source_id=D.company_id
		AND tef.document_ref=d.document_ref
	...
	WHERE d.document_id = d1.document_id AND pa.party_type_id = 9
    ),
    (
        CASE @TypeOfCurrency
...
/* Update agent*/
UPDATE #TransLines
SET agent = agent + ISNULL(agent_tax,0)
...
/* Update sub agent*/
UPDATE #TransLines
SET subagent = subagent + ISNULL(subagent_tax,0)

/* Update fees */
UPDATE #TransLines
SET fees = fees + ISNULL(fee_tax,0)
...
/* Update net commission*/
UPDATE #Translines
SET net_commission = net_commission + ISNULL(comm_tax,0)
