...
EXECUTE DDLDropProcedure 'spu_Report_Profit_and_Loss_agency_SFU'
GO

/**********************************************************************************************************************************
** Created by Gaurav Arora
** Created On 20 Feb 2006
** Created For S4I Reports - Profit_&_Loss_ALL_agency.rpt**
**********************************************************************************************************************************
...
CREATE                  PROCEDURE spu_Report_Profit_and_Loss_agency_SFU
    @branch_id int,
...
--                company_id = 1
    ...
    WHILE @@FETCH_STATUS = 0 BEGIN
        ...
        SELECT @sYearNameThisYear = (SELECT(ISNULL(year_name, ''))                         		FROM Period
                         		WHERE company_id = @iCompanyID
                         		AND period_end_date = @dPeriodEndDate
       ...
       SELECT @d1stPeriodEndDate = (SELECT ISNULL(MIN(period_end_date), @dPeriodEndDate)
        ...
        SELECT @dThisPeriodLastYearEndDate = (SELECT ISNULL(MAX(period_end_date),
                              ...
                              FROM Period
                              WHERE SUBSTRING(period_name, 1, 2) =
                                    SUBSTRING(@sPeriodNameThisYear, 1, 2)
                              AND period_end_date < @dPeriodEndDate)
        ...
        SELECT @d2YearsAgoEndDate = (SELECT ISNULL(MAX(period_end_date), @dLastDay)
         ...
         INSERT INTO #Report_Transaction(            account_id,
    ...
    SELECT @nLastYearToDate = (  SELECT Case @TypeOfCurrency
                    ...
                    (  SELECT Case @TypeOfCurrency
    										When 'System' Then ISNULL(SUM(ROUND(ISNULL(TransDetail.system_amount, 0.0),2)), 0.0)    										When 'Base' Then ISNULL(SUM(ROUND(ISNULL(TransDetail.amount, 0.0),2)), 0.0)
    									END
                        FROM TransDetail TransDetail
                        JOIN Document Document
                        ON TransDetail.document_id = Document.document_id
...
FROM #Report_TreePathNames RTPN
	...
	(	
		...
		LOWER(@IncClaim) = 'no' 
		AND
			RTPN.element_name4 NOT IN('Gross Written Premiu','RI Treaty Premium',
			'RI Other Premium','RI Treaty Commission','RI Other Commission',
			'Claims Salvage Rec.','Claims TP Rec.','RI Other Claims Rec.',
			'RI TTY Claims Rec.','CI Claims Rec.','Grs Claims Incurred','Gross Claims Incurre',
			'RI Other Salvage Rec','RI TTY Salvage Rec.','CI Salvage Rec.','CI TP Rec.',
			'RI Other TP Rec.','RI TTY TP Rec.') 
	     )
	     OR
             (
  	        LOWER(@IncClaim) = 'yes'
