...
EXECUTE DDLDropProcedure 'spu_Report_Business_Performance'
GO


CREATE PROCEDURE spu_Report_Business_Performance
            @company_id int,
...
**                                      rename 'NET' = 'RET'
** 01/08/2002   AMJ - branch specific changes
***********************************************************************************************************************************
** VER      DATE        WHO     WHAT
...
-- get the year start period ID and date
...
SELECT @24PeriodsAgoID = period_id
FROM Period
WHERE period_end_date >= dateadd(mm, -24, @dtSelectedPeriodEnd)
AND   sub_branch_id = @sub_branch_id
...
SELECT @YTDRange = DATEDIFF(mm, @dtSelectedPeriodEnd, @YearStartDate) 
...
SELECT  'Premium',
    ...
    (   SELECT period_id
        FROM Period
        WHERE period_end_date >= dateadd(mm, -23, @dtSelectedPeriodEnd) 
        and period_end_date <= dateadd(mm, -12, @dtSelectedPeriodEnd) 
        AND   sub_branch_id = @sub_branch_id
    )
-- AMJ BETWEEN @24PeriodsAgoID + 1 AND @12PeriodsAgoID
        ) LY12,
    ...
    (   SELECT period_id
        FROM Period
        WHERE period_end_date >= dateadd(mm, -11, @dtSelectedPeriodEnd) 
        and period_end_date <= @dtSelectedPeriodEnd
        AND   sub_branch_id = @sub_branch_id
    )
-- AMJ BETWEEN @12PeriodsAgoID + 1 AND @SelectedPeriodID
        ) TY12,
    ...
    (   SELECT period_id
        FROM Period
        WHERE period_end_date >= dateadd(mm, (@YTDRange - 21), @dtSelectedPeriodEnd) 
        and period_end_date <= dateadd(mm, -12, @dtSelectedPeriodEnd) 
        AND   sub_branch_id = @sub_branch_id
    )
-- AMJ BETWEEN @12PeriodsAgoID - @YTDRange AND @12PeriodsAgoID
        ) LYYTD,
...
-- AMJ BETWEEN @YearStartPeriodID AND @SelectedPeriodID
        ...
        AND sf.posting_period_number in
    (   SELECT period_id
        FROM Period
        WHERE period_end_date >= dateadd(mm, -23, @dtSelectedPeriodEnd) 
        and period_end_date <= @dtSelectedPeriodEnd
...
-- ANJ BETWEEN @24PeriodsAgoID + 1 AND @SelectedPeriodID
        AND sd.stats_detail_type IN ('GRS', 'COI','NET','TTY', 'FAC')
/* AMJ  invalid filter the one above will handle this
        AND sf.posting_period_number > 
    (   SELECT period_id
        FROM Period
        WHERE period_end_date >= dateadd(mm, -24, @dtSelectedPeriodEnd) 
        AND   sub_branch_id = @sub_branch_id
...
-- AMJ BETWEEN @12PeriodsAgoID + 1 AND @SelectedPeriodID
    ...
    (   SELECT period_id
        FROM Period
        WHERE period_end_date >= dateadd(mm, (@YTDRange - 12), @dtSelectedPeriodEnd) 
        and period_end_date <= dateadd(mm, -12, @dtSelectedPeriodEnd) 
        AND   sub_branch_id = @sub_branch_id
    )
-- AMJ BETWEEN @12PeriodsAgoID - @YTDRange AND @12PeriodsAgoID
...
-- AMJ BETWEEN @24PeriodsAgoID + 1 AND @SelectedPeriodID
        AND sd.stats_detail_type IN ('GRS', 'COI','NET','TTY', 'FAC')
/* AMJ  invalid filter the one above will handle this
        AND sf.posting_period_number >  
    (   SELECT period_id
