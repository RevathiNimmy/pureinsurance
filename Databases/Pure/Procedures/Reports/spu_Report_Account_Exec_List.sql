...
AccountExecutives.rpt
*/

SET QUOTED_IDENTIFIER OFF SET ANSI_NULLS OFF
GO

EXECUTE DDLDropProcedure 'spu_Report_Account_Exec_List'
GO

CREATE PROCEDURE spu_Report_Account_Exec_List
...
SELECT P.party_cnt,
    ...
    ISNULL(
        (
            ...
            FROM Department
            WHERE department_id = PC.department_id
        )
, '') department,
    '' phone
FROM Party P,
    Party_Type PT,
    Party_Handler PC    --DC290903 -PS256 - new table instead of party_consultant
WHERE P.party_type_id = PT.party_type_id
AND (PT.code = 'CO' OR PT.Code = 'HC')  --DC290903 -PS256 -check for executive handlers
AND PC.party_cnt = P.party_cnt
