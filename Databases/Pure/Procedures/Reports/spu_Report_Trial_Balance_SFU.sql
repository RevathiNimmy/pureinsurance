...
EXECUTE DDLDropProcedure 'spu_Report_Trial_Balance_SFU'
GO


CREATE PROCEDURE spu_Report_Trial_Balance_SFU        
    @branch_id      int,        
    @PeriodDate varchar(30),        
    @Basis varchar(50),        
    @TypeOfCurrency VARCHAR(15),        
    @GroupBy VARCHAR(20),     
    @Exclude_EOY_Journals varchar(3)    
...
-- TB 11/11/02: Report Basis, need the period ID's of those end dates        
 ...
 SELECT @iPeriodLastYearEnd = (        
  ...
  -- First calculate the opening balances for For Assets,liability and suspense accounts only  
  -- Period must be before last year end only  
  ...
  AND (   
   (@iBasis = 0 AND (TD.period_id <= @iPeriodLastYearEnd))          
  OR          
   (@iBasis = 1 AND (D.document_date <= @dtLastYearEnd))          
    )      
         
   And(       
      (@Exclude_EOY_Journals='Yes' And  D.Comment NOT Like 'Year End Retained Profit%' )       
   OR ( @Exclude_EOY_Journals='No')      
       )      
  GROUP BY          
   A.account_id,          
   A.short_code,          
   ...
   --Calculated the Opening Balance between last year and current period for All Accounts 
   -- if last year end date is not equal to previous period end date 

IF @dtLastYearEnd <> @dBFEndDate
BEGIN 
   INSERT INTO #Report_Transaction   
  (          
   TransdetailID,          
  ...
  WHERE D.company_id = @iCompanyid          
  AND (   
   (@iBasis = 0 AND (TD.period_id > @iPeriodLastYearEnd AND TD.period_id < @iPeriodEnd))          
  OR          
   (@iBasis = 1 AND (D.document_date > @dtLastYearEnd AND D.document_date <= @dBFEndDate))          
    )      
  ...
  GROUP BY          
   ...
   CASE @TypeOfCurrency          
    ...
    WHEN 'Transaction' THEN C.description          
   END           
END -- of IF @dtLastYearEnd <> @dBFEndDate 
          
  -- Now calculate the current period balances for all accounts  
  ...
  JOIN Currency C     ON C.currency_id = TD.currency_id          
  WHERE D.company_id = @iCompanyid          
  AND ( --This is the case of current period balances    
     (@iBasis = 0 AND (TD.period_id = @iPeriodEnd))          
    OR          
  ...
  GROUP BY          
   ...
   -- Finally calculate the balance C/F to next period  
   -- There are two case a) balances from previous FY only Assests,Liabiliy and suspense account  
   -- and b) balance of current FY will be C/F  
    -- case (a)  
    INSERT INTO #Report_Transaction            
    (          
     TransdetailID,          
    ...
    --Case (b)      
    INSERT INTO #Report_Transaction  -- for all accounts      
    (          
     TransdetailID,          
     AmountBF,          
     Amount,          
    ...
    AND (   
     (@iBasis = 0 AND (TD.period_id > @iPeriodLastYearEnd AND TD.period_id <= @iPeriodEnd))          
    OR          
     (@iBasis = 1 AND (D.document_date > @dtLastYearEnd AND D.document_date <= @dPeriodEndDate))          
    )       
   And(       
      (@Exclude_EOY_Journals='Yes' And  D.Comment NOT Like 'Year End Retained Profit%' )       
   OR (@Exclude_EOY_Journals='No')      
