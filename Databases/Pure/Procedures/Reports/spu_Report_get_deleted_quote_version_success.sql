...
EXECUTE DDLDropProcedure 'spu_Report_get_deleted_quote_version_success'
GO



CREATE PROCEDURE spu_Report_get_deleted_quote_version_success  
...
AS  
 DECLARE @dtPeriodStartDate DATE,  
   ...
   @Status            INT=1  
     ...
     BEGIN  
     select @dtPeriodStartDate = CONVERT (DATE, @startDTCombo),  
            @dtPeriodEndDate = CONVERT (DATE, @endDTCombo)  
