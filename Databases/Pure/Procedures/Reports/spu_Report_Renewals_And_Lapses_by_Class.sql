...
EXECUTE DDLDropProcedure 'spu_Report_Renewals_And_Lapses_by_Class'
GO


CREATE PROCEDURE spu_Report_Renewals_And_Lapses_by_Class
...
** 1.00
**
** 1.01 - CMG/PB 05082002 Add source_id parameter to give filtering by Sub Branch
**
        ...
        BEGIN
            SELECT  @RenMonthCount = CASE   WHEN isnull(@RenMonthValue, 0) <> 0         -- count per month
                    ...
                    @LapMonthCount = CASE   WHEN isnull(@LapMonthValue, 0) <> 0
                                                ...
                                                ELSE @LapMonthCount
                                            END,
                    @InvMonthCount = CASE   WHEN isnull(@InvMonthValue, 0) <> 0
                                                THEN @InvMonthCount + 1
                                                ELSE @InvMonthCount
                                            END,
                    @CRenMonthCount = CASE   WHEN isnull(@RenMonthValue, 0) <> 0        -- Cumulative count per month
                                                THEN @CRenMonthCount + 1
                                                ELSE @CRenMonthCount
                                            END,
                    @CLapMonthCount = CASE   WHEN isnull(@LapMonthValue, 0) <> 0
...
BEGIN
    INSERT INTO #tmpRnwlsAndLapsesSum
        ...
        GROUP BY ClassOfBusiness, MonthID, MonthName, CurrentPeriodEnd
END
SET NOCOUNT OFF
