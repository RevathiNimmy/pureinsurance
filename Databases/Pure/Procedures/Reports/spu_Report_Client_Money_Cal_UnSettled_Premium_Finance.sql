...
EXECUTE DDLDropProcedure 'spu_Report_Client_Money_Cal_UnSettled_Premium_Finance'
GO

CREATE PROCEDURE spu_Report_Client_Money_Cal_UnSettled_Premium_Finance
    @branch_id INT,    
    @end_date DATETIME,
    @finance_comm_os MONEY OUTPUT
...
SELECT 
    @finance_comm_os = ISNULL(SUM(os_commission),0) * -1
FROM #unsettled
