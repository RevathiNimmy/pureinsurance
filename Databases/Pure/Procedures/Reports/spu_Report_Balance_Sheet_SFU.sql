...
EXECUTE DDLDropProcedure 'spu_Report_Balance_Sheet_SFU'
GO

CREATE PROCEDURE spu_Report_Balance_Sheet_SFU 
	@branch_id          INT,
...
TRUNCATE TABLE Report_TreePathNames      
    ...
    SELECT @dPeriodEndDateIn = ISNULL(convert (datetime,@period_end_date), GETDATE())   
    --SELECT @dPeriodEndDateIn =ISNULL((SELECT CONVERT(VARCHAR(10), @period_end_date, 111) AS [YYYY/MM/DD] ) ,GETDATE())   
