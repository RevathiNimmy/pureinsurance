...
EXECUTE DDLDropProcedure 'spu_Report_Income_Transacted'
GO

CREATE PROCEDURE spu_Report_Income_Transacted
    @branch_id INT,
    @start_date DATETIME,
    @end_date DATETIME,
...
DECLARE
    ...
    @current_gross_premium MONEY,
    @current_ipt_amount MONEY,
    @current_gross_commission MONEY,
    @current_agent_commission MONEY,
    @current_fee_amount MONEY,
    @current_discount_amount MONEY,
    @current_extra_premium MONEY,
    @current_extra_commission MONEY,
    @current_this_premium MONEY,
    @current_commission_amount MONEY,
    @current_annual_premium MONEY,
    @current_net_premium MONEY,
...
WHERE T.spare = 'BROK ADJ'
	...
	AND EXISTS (SELECT document_id FROM transdetail T1 
    	WHERE T1.document_id = D.document_id
        AND T1.ref_date BETWEEN @start_date AND @end_date
	AND T1.document_sequence = 1)
	)
    	OR
   	(@date_type = 'Trans/Effective Date' 
    	AND (
    	    (T.ref_date BETWEEN @start_date AND @end_date) 
    	    OR EXISTS (SELECT document_id FROM transdetail T1 
    	    WHERE T1.document_id = D.document_id
            AND T1.ref_date BETWEEN @start_date AND @end_date
	    AND T1.document_sequence = 1)
            )
...
SET IT.document_ref = D.document_ref,
    ...
    IT.no_of_third_party =
        (
            ...
            JOIN ledger L
                ON L.ledger_id = A.ledger_id
                AND L.ledger_short_name IN ('AG', 'TR', 'UB')
            WHERE TD.document_id = D.document_id
            AND TD.spare IN ('AGENT', 'Reversed AGENT', 'Reversal AGENT')
        ),
...
/*     IT.comm_tax =
    ...
    IT.agent_tax =
    (
        SELECT ROUND(epa1.tax_amount,2)
        FROM document d1
    ...
    IT.subagent_tax = 
    (
        SELECT isnull(ROUND(epa1.tax_amount,2),0)
        FROM document d1
        JOIN transaction_export_folder tef 
        ON tef.source_id=d1.company_id 
        AND tef.document_ref=d1.document_ref
...
BEGIN
    ...
    UPDATE #IncomeTrans
    SET third_party_code = 'MULTI',
        third_party_name = 'Multi Agent'
    ...
    BEGIN
        ...
        SELECT
        	...
        	(
                case
                when L.ledger_short_name IN ('AG', 'TR') then
                    (
                        SELECT ISNULL(SUM(ROUND(TD1.amount,2)),0)
                        FROM Transdetail    TD1
                        JOIN Account A2
                            ON A2.account_id = TD1.account_id
                        WHERE TD1.document_id = IT.Document_id
                        AND A2.ledger_id IN (SELECT ledger_id FROM ledger WHERE ledger_short_name IN ('AG', 'TR') )
                        AND ISNULL(TD1.spare, '') <> 'AGENT ADJ'
                        AND A.account_id = TD1.account_id
                    )
                when L.ledger_short_name = 'UB' then
                    /*Sub Agent Amount (DD) : 0 if no sub agent or isn't a direct debit transaction*/
                    (
                        SELECT ISNULL(SUM(ROUND(TD1.amount,2)),0)
                        FROM Transdetail TD1
                        ...
                        (
                            ...
                            AND EXISTS
                                (
                                    ...
                                    JOIN Account A2
                                        ON A2.account_id = TD2.account_id
                                    WHERE TD2.document_id = IT.Document_id
                                    AND A2.ledger_id IN (SELECT ledger_id FROM ledger WHERE ledger_short_name = 'UB')
                                    AND ISNULL(TD2.spare, '') <> 'AGENT ADJ'
                                    --AND A.account_id = TD1.account_id
                                )
...
--            (
--                SELECT 
--                    ISNULL(SUM(TDI.amount), 0.0) * -1
--                FROM transdetail TDI
--                WHERE TDI.document_id = IT.document_id
--                AND TDI.account_id = A.account_id
--                AND TDI.spare IN ('AGENT', 'Reversed AGENT', 'Reversal AGENT')
--            ),
--             0,
            IT.document_date,
            IT.branch_id,
    ...
    BEGIN
        ...
        SELECT
            ...
            @current_ipt_amount = 0,
            @current_gross_commission = 0,
            @current_agent_commission = 0,
            @current_fee_amount = 0,
            @current_discount_amount = 0,
            @current_extra_premium = 0,
            @current_extra_commission = 0,
            @current_this_premium = 0,
            @current_commission_amount = 0,
            @current_annual_premium = 0,
        ...
        BEGIN
            ...
            IF @current_share = @no_of_shares
            BEGIN
                UPDATE #IncomeTrans
                SET gross_premium = @gross_premium - @current_gross_premium,
                    ipt_amount = @ipt_amount - @current_ipt_amount,
                    gross_commission = @gross_commission - @current_gross_commission,
                    agent_commission = @agent_commission - @current_agent_commission,
                    fee_amount = @fee_amount - @current_fee_amount,
                    discount_amount = @discount_amount - @current_discount_amount,
                    extra_premium = @extra_premium - @current_extra_premium,
                    extra_commission = @extra_commission - @current_extra_commission,
                    this_premium = @this_premium - @current_this_premium,
                    commission_amount = @commission_amount - @current_commission_amount,
                    annual_premium = @annual_premium - @current_annual_premium,
                    net_premium = @net_premium - @current_net_premium
            ...
            SELECT
                @current_gross_premium = @current_gross_premium + @share_gross_premium,
                @current_ipt_amount = @current_ipt_amount + @share_ipt_amount,
                @current_gross_commission = @current_gross_commission + @share_gross_commission,
                @current_agent_commission = @current_agent_commission + @share_agent_commission,
                @current_fee_amount = @current_fee_amount + @share_fee_amount,
                @current_discount_amount = @current_discount_amount + @share_discount_amount,
                @current_extra_premium = @current_extra_premium + @share_extra_premium,
                @current_extra_commission = @current_extra_commission + @share_extra_commission,
                @current_this_premium = @current_this_premium + @share_this_premium,
                @current_commission_amount = @current_commission_amount + @share_commission_amount,
                @current_annual_premium = @current_annual_premium + @share_annual_premium,
                @current_net_premium = @current_net_premium + @share_net_premium,
