...
EXECUTE DDLDropProcedure 'spu_Report_Client_Money_Cal_Comm_Outstanding'
GO

CREATE PROCEDURE spu_Report_Client_Money_Cal_Comm_Outstanding
    @branch_id INT, 
    @end_date DATETIME,
    @client_comm_os MONEY OUTPUT,
    @subagent_comm_os MONEY OUTPUT 
...
SELECT  
    ...
    (
        (
            ...
            WHERE td.account_id = a1.account_id
    ...
    CASE
        ...
        ELSE 'Y'
    END,
    ISNULL(SUM(CASE l2.ledger_short_name WHEN 'AG' THEN td2.amount ELSE 0 END),0),
    ISNULL(SUM(CASE l2.ledger_short_name WHEN 'UB' THEN td2.amount ELSE 0 END),0), /*needs client amount subtracted from it*/
    ISNULL(SUM(CASE l2.ledger_short_name WHEN 'CO' THEN td2.amount ELSE 0 END),0),
    ISNULL(SUM(CASE l2.ledger_short_name WHEN 'FE' THEN td2.amount ELSE 0 END),0),
    ISNULL(SUM(CASE l2.ledger_short_name WHEN 'SA' THEN td2.amount ELSE 0 END),0), /*needs subtrating from subagent amount*/
    0,
    (
                SELECT ISNULL(SUM(ROUND(tdy.amount,2)),0)
    ...
    ,
    (
        SELECT ISNULL(SUM(ROUND(tmx.base_match_amount,2)),0)
    ...
    (
        ...
        AND NOT EXISTS
            (
                ...
                JOIN document d4
                    ON d4.document_id = td4.document_id
                WHERE td3.document_id = d1.document_id
                AND td3.document_sequence = 1
                AND td3.amount = 0 --Exclude DDs where balance is zero (but not where there is outstanding legal expenses for example  PN24764
                AND mg3.match_date <= @end_date
...
UPDATE TL
SET comm_adj = (SELECT ISNULL(SUM(td2.amount), 0) * -1
...
BEGIN
    ...
    SELECT
        @total_client_amount = SUM(ROUND(td.amount,2))
    FROM transdetail td
    JOIN account a
        ON a.account_id = td.account_id
    JOIN ledger l
    ...
    BEGIN
       ...
       SELECT
            @share_client_amount = SUM(ROUND(td.amount,2))
        FROM transdetail td
        WHERE td.document_id = @document_id
...
SELECT
    @client_comm_os = ISNULL(SUM(os_commission),0) * -1
FROM #TransLines
WHERE ledger_short_name = 'SA'

SELECT
    @subagent_comm_os = ISNULL(SUM(os_commission),0) * -1
FROM #TransLines
