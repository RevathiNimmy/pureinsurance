...
EXECUTE DDLDropProcedure 'spu_Report_Portfolio_Accumulation_SFU'
GO
...
** 1.3      07/08/2003  PSL     Fixed Issue 4652 removed the group clause in the party subqueries
**                              These would cause more than one row to be returned, which is not allowed,
**                              and would mean more than one row for each client in the report, with the figures duplicated
** 1.4		17/08/2004	JT		Multicurrency changes
***********************************************************************************************************************************/
CREATE PROCEDURE spu_Report_Portfolio_Accumulation_SFU
        @AccumCode 	varchar (10),
        @Class_Of_Bus	varchar	(255),
        @TypeOfCurrency	Varchar(30),
        @GroupByCode	Varchar(30)
AS

DECLARE @sCOB 	varchar (255),
	@iCOB	int

IF @Class_Of_Bus <> 'ALL' 
BEGIN
	SELECT @iCOB = class_of_business_id FROM Class_Of_Business
		WHERE description = @Class_Of_Bus
END
ELSE SELECT @iCOB = 0
...
INSERT INTO #tempStats
    SELECT  sd.class_of_business_id,
        ...
        (SELECT 
        Case @TypeOfCurrency WHEN 'Base' THEN sd.sum_insured_home
    ...
    AND ( @iCOB = 0 OR
    		(@iCOB <> 0 AND @iCOB = sd.class_of_business_id))
    

CREATE TABLE #tempParty
    (
        InsHolderCnt int NULL,
        InsuredFullName varchar(255) NULL,
        ABI varchar(70) NULL,
...
INSERT INTO #tempParty
    ...
    UNION
        ...
        (SELECT p.resolved_name
            ...
            IN (SELECT min(pr.relation_cnt)
                FROM party_relationship pr
                WHERE pr.party_cnt = pp.party_cnt ))            -- JMK 09/11/2001
        FROM party pp
...
SELECT ts.COB,
    ...
    ta.level9_description,ts.CompanyCode,ts.CompanyDesc,
    ts.CurrencyCode,ts.CurrencyDesc, 
    Case @GroupByCode 
    	WHEN 'Branch' THEN ts.CompanyCode
    	WHEN 'Branch And Currency' THEN ts.CompanyCode
    ELSE ''
    END 'GroupByCode'
