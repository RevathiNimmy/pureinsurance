...
SET ANSI_NULLS ON
GO
Execute DDLDropProcedure 'spu_event_SIR_Calculate_Section_Tax_Amounts_SFB'
GO


CREATE PROCEDURE spu_event_SIR_Calculate_Section_Tax_Amounts_SFB
...
--If not multi
IF Not Exists(Select * from event_policy_coinsurers where insurance_Section_id = @insurance_section_id)
BEGIN 
	SELECT    
	...
	FROM event_insurance_COB_section iCOBs
	JOIN event_insurance_file i on i.insurance_file_cnt  = iCOBs.insurance_file_cnt   
...
BEGIN 
	...
	PO.party_cnt    
	FROM event_insurance_COB_section iCOBs
        JOIN event_policy_coinsurers PO on PO.insurance_section_id = iCOBS.insurance_section_id 
	JOIN event_insurance_file i on i.insurance_file_cnt  = iCOBs.insurance_file_cnt   
