...
SET ANSI_NULLS ON
GO
Execute DDLDropProcedure 'spu_claims_recovery_reins_select'
GO

CREATE PROCEDURE spu_claims_recovery_reins_select  
    @peril_id int,  
    @is_salvage tinyint  
AS  
    ...
    Select  r.recovery_id,  
            rt.description,  
            ral.party_cnt,  
            Case When ral.type = 'F' OR ral.type = 'FX' Then p.name Else t.description End name,  
      ...
      Join    claim_payment_item wcpi  
            ...
            ral.type,  
            ral.payment,
           (ral.salvage-ral.this_salvage) salvage,  
		    ral.this_salvage ,  
		    (ral.recovery-ral.this_recovery) [recovery],  
		    ral.this_recovery			
