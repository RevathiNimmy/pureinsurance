...
SET ANSI_NULLS ON
GO
Execute DDLDropProcedure 'spu_CLM_Get_Claim_Payment_Accounts_And_Trans_Details'
GO

CREATE PROCEDURE spu_CLM_Get_Claim_Payment_Accounts_And_Trans_Details  
 @claim_id int = 0,  
  ...
  LEFT JOIN account a ON  
   cp.party_cnt = a.account_key  
	...
	INNER JOIN TransDetail td ON
		td.account_id = CASE WHEN cp.party_cnt IS NULL THEN @CLMPAYABLE_account_id  
						ELSE a.account_id  
  						END 
		AND td.document_id = cp.document_id
