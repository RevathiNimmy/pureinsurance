...
EXECUTE DDLDropProcedure 'spu_RI_Arrangement_calc'
GO


CREATE PROCEDURE spu_RI_Arrangement_calc  
...
AS
    ...
    Declare Line_Cursor Cursor Fast_Forward For
        ...
        Into @line_id, @default_percent, @priority, @number_of_lines, @line_limit,@is_obligatory, @type, @sum_insured  
    ...
    Begin
        ...
        Begin
            ...
            Select  @last_priority = @priority,
					...
					If ISNULL(@line_limit,0) = 0
						SELECT @priority_limit = @sum_insured
	
		    IF @is_obligatory_checked=1  
		    	BEGIN  
	    ...
	    IF @is_obligatory = 1  
	       BEGIN  
	         SET @priority_share = 1  
	       	 SET @is_obligatory_checked=1  
	       END  
	    ELSE  
	       BEGIN  
            	-- Get priority share
	    ...
	    Select  @running_si = @running_si - @this_si,  
	            @running_premium = @running_premium - @priority_premium  
        if @is_obligatory = 1  
        BEGIN  
            SET @band_premium = @running_premium  
            SET @band_si  = @running_si   
        END 
        -- Next line
        Fetch Next From Line_Cursor
    
     	--Note:- @is_obligatory is added newly  
        Into @line_id, @default_percent, @priority, @number_of_lines, @line_limit,@is_obligatory  ,@type, @sum_insured  	  
