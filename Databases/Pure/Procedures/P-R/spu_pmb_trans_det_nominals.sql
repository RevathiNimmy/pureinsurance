...
EXECUTE DDLDropProcedure 'spu_pmb_trans_det_nominals'
GO


CREATE PROCEDURE spu_pmb_trans_det_nominals
    @transaction_export_folder_cnt int,
    @transaction_type char(1),
    @agent_amount_calc numeric(19,4),
    --  @total_fees_calc            numeric(19,4),

    @total_extras_calc numeric(19,4),
    @total_extras_comm_calc numeric(19,4),
    @total_extras_ipt_calc numeric(19,4),
    @total_insurers_calc numeric(19,4),
    @total_insurers_comm_calc numeric(19,4),
    @total_insurers_ipt_calc numeric(19,4),
    @total_coinsurers_calc numeric(19,4),
    @total_coinsurers_comm_calc numeric(19,4),
    @total_coinsurers_ipt_calc numeric(19,4)
AS
...
SELECT  @mapping_code = 'PREM ACCOUNT'
SELECT  @transaction_account_key = NULL
/* Set transaction amount */
SELECT  @transaction_amount = @total_insurers_calc + @total_insurers_ipt_calc
                  + @total_coinsurers_calc + @total_coinsurers_ipt_calc
                  + @total_extras_calc + @total_extras_ipt_calc
                  + @agent_amount_calc
...
SELECT  @commission_excluded = 0
SELECT  @withholding_tax_excluded = 0
SELECT  @mapping_code = 'INSURER NET'
SELECT  @transaction_account_key = NULL
/* Set transaction amount */
SELECT  @transaction_amount = @total_insurers_calc + @total_insurers_ipt_calc
                  + @total_coinsurers_calc  + @total_coinsurers_ipt_calc
