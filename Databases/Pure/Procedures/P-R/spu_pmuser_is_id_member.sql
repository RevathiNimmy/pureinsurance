...
EXECUTE DDLDropProcedure 'spu_pmuser_is_id_member'
GO


CREATE PROCEDURE spu_pmuser_is_id_member
    @user_id integer,
    @group_code char(10)
AS

/********************************************************************************************************/
/* sp_pmuser_is_id_member selects the supplied group if the user is a member of it. */
/********************************************************************************************************/
/********************************************************************************************************/
...
CREATE TABLE #Temp_Check_Groups (
    ...
    BEGIN
        ...
        WHERE ugg.pmuser_member_group_id = @check_group_id
          ...
          AND ugg.pmuser_group_id NOT IN (SELECT pmuser_group_id
             FROM #Temp_Groups)
    ...
    SELECT ug.code
    FROM #Temp_Groups tg,
        pmuser_group ug WHERE ug.pmuser_group_id = tg.pmuser_group_id
