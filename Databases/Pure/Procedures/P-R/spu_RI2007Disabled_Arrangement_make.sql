...
EXECUTE DDLDropProcedure 'spu_RI2007Disabled_Arrangement_make'
GO


CREATE PROCEDURE spu_RI2007Disabled_Arrangement_make
...
AS
    ...
    If @is_auto_reinsured = 1 BEGIN
        ...
        Where   ri_arrangement_id = @ri_arrangement_id
                -- Don't delete new fac lines!!!
        And     type <> 'F'
	END

    -- If we still have data in this arrangement don't update the model
    If Exists (Select * From ri_arrangement_line Where ri_arrangement_id = @ri_arrangement_id And type <> 'F')
        Return
    ...
    Select  @ri_arrangement_id,
            ...
            rml.priority,
            rml.number_of_lines,
            IsNull(@line_limit, rml.line_limit * @model_currency_rate),
            0,
            0,
            0,
            0
    ...
    If @is_auto_reinsured = 1
        Execute spu_RI2007Disabled_Arrangement_calc 
