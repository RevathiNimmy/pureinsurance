...
Execute DDLDropProcedure 'spu_RI2007Disabled_Arrangement_copy'
GO
CREATE  PROCEDURE spu_RI2007Disabled_Arrangement_copy      
    @insurance_file_cnt int,      
    @risk_cnt int,      
    @effective_date datetime,      
    @Trans_type varchar(5) = '',      
    @IsDeletedRiskInPT tinyint = 0      
AS      
 ...
 Delete From RI_Arrangement_line_Broker_Participants      
     ...
     Where ril.ri_arrangement_id IN (Select ri_arrangement_id From ri_arrangement Where risk_cnt = @risk_cnt and original_flag =1)      
      
 Delete From ri_arrangement_Line      
     Where ri_arrangement_id IN (Select ri_arrangement_id From ri_arrangement Where risk_cnt = @risk_cnt and original_flag =1)      
   Delete From ri_arrangement Where risk_cnt = @risk_cnt and original_flag =1      
...
-- Check if we have already copied      
 ...
 SELECT  @insurance_file_type_id = ISNULL(i.insurance_file_type_id,0)      
  FROM   Insurance_File i  WHERE i.insurance_file_cnt=@Insurance_file_cnt      
      
 IF @Trans_type ='PT' AND @insurance_file_type_id <> 2      
  BEGIN      
      
    DECLARE  @insurance_folder_cnt INT, @risk_folder_cnt INT, @processed_insurance_file_cnt INT, @insurance_file_cnt_prev INT      
  ...
  END      
    ...
    Where   insurance_file_cnt = (      
            ...
            And     IsNull(ifi.insurance_file_status_id, 3) In (3, 4, 5, 6, 1)      
...
Set @IsTempMTA = 0      
if exists ( select * from insurance_file where insurance_file_cnt = @insurance_file_cnt and insurance_file_type_id in (6,7))      
Set @IsTempMTA = 1      
...
Select @pro_rata_rate=1      
    ...
    Declare Arrangement_Cursor Cursor Fast_Forward For      
        ...
        Where   risk_cnt = @original_risk_cnt      
         And     ((original_flag = 0 AND ISNULL(@IsDeletedRiskInPT, 0) = 0)      
                    OR ( original_flag = 1 AND @IsDeletedRiskInPT = 1))      
    ...
    While (@@Fetch_Status = 0) Or (@original_flag = 0) Begin      
       ...
       IF @IsDeletedRiskInPT = 1      
            Set  @combined_rate = @combined_rate * (-1)      
 ...
 Delete from ri_arrangement_line where ri_arrangement_id=@new_ri_arrangement_id      
   ...
   From RI_Band_Version where ri_band_id=(Select ri_band_id From ri_arrangement Where ri_arrangement_id = @old_ri_arrangement_id)    
...
SET @new_grouping_id = NULL      
      ...
      If Exists(Select * from ri_arrangement_line      
        ...
        Select @Replaced_by_treaty_id = ISNULL(Replaced_by_treaty_id,0),      
         ...
         if (@Replaced_by_treaty_id<>0 And @Replaced_by_effective_date <= @effective_date)      
                   ...
                   Select  @new_ri_arrangement_id,      
                           ...
                           Case When @original_flag = 0 Then 0 Else Round(commission_tax * @combined_rate * @pro_rata_rate, 2) End,      
   ...
   priority,      
                      ...
                      Round(lower_limit * Abs(@combined_rate), 2),      
                      Round(sum_insured * @combined_rate, 2),      
                      Case When (@original_flag = 0 and Rtrim(@trans_type) ='MTC') Then 0 Else Round(premium_value * @combined_rate * @pro_rata_rate, 4) End,      
                      Case When (@original_flag = 0 and Rtrim(@trans_type) ='MTC') Then 0 Else Round(commission_value * @combined_rate * @pro_rata_rate, 4) End,      
                      Case When @original_flag = 0 Then 0 Else Round(premium_tax * @combined_rate * @pro_rata_rate, 4) End,      
              ...
              From    ri_arrangement_line      
              Where   ri_arrangement_line_id = @old_ri_arrangement_line_id      
              And    (@original_flag = 1 Or type IN ('F','FX') )      
      
             If @@ROWCOUNT > 0      
    -- Get new id      
    Select  @new_ri_arrangement_line_id = @@Identity      
   Else      
    Set @new_ri_arrangement_line_id = 0      
   ...
   BEGIN      
         DECLARE Brokers_FAC CURSOR FOR      
           Select ri_party_cnt,participation_percent From                Ri_Arrangement_line_Broker_Participants      
            Where ri_arrangement_line_id=@old_ri_arrangement_line_id      
