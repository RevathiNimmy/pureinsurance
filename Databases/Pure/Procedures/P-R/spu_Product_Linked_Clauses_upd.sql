...
SET ANSI_NULLS ON
GO
EXECUTE DDLDropProcedure 'spu_Product_Linked_Clauses_upd'
GO
CREATE PROCEDURE spu_Product_Linked_Clauses_upd   
    @Product_id int,    
...
IF ISNULL(@code, ' ') = ' '    
    IF ISNULL(@gis_property_id,' ')=' ' OR ISNULL(@gis_object_id,' ')=' '    
        ...
        WHERE   wpl.product_id = @Product_id and dt.is_deleted=0 and dt.effective_date <getdate()  
        ORDER BY dt.code    
    ELSE    
    BEGIN    
