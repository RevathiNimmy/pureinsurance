...
EXECUTE DDLDropProcedure 'spu_pmb_trans_det_tests'
GO


CREATE PROCEDURE spu_pmb_trans_det_tests
...
SELECT  @transaction_amount = @premium_amount + @total_extras_calc + @total_fees_calc
...
SELECT  @transaction_account_key = NULL
/* Set transaction amount */
SELECT  @transaction_amount = @agent_amount_calc + @total_insurers_comm_calc
            + @total_coinsurers_comm_calc + @total_extras_comm_calc
IF @transaction_amount > 0
    SELECT @transaction_amount = @transaction_amount * -1
/* Set new detail_id */
SELECT  @transaction_export_detail_id = MAX(transaction_export_detail_id) + 1
...
SELECT  @transaction_account_key = NULL
/* Set transaction amount */
SELECT  @transaction_amount = @total_insurers_calc + @total_coinsurers_calc
                + @total_extras_calc + @tax_amount
IF @transaction_amount < 0
    SELECT @transaction_amount = @transaction_amount * -1
/* Set new detail_id */
SELECT  @transaction_export_detail_id = MAX(transaction_export_detail_id) + 1
