...
EXECUTE DDLDropProcedure 'spu_pmb_trans_det_add_NBD'
GO


CREATE PROCEDURE spu_pmb_trans_det_add_NBD
...
DECLARE @transaction_export_detail_id   int,
    ...
    @source_id          smallint,
    @lead_insurer_account_key   int,

        @lead_insurer_shortname     varchar(255),
...
SELECT  @premium_amount = this_premium,
    ...
    @lead_insurer_account_key = P.party_id,
    @lead_insurer_shortname = P.shortname,
...
/* Debit Gross Premium to the Client's Account */
...
SELECT  @transaction_account_key = @insurance_holder_account_key
...
VALUES (
    ...
    @mapping_code,
    @transaction_account_key)
IF @lead_insurer_shortname = "MULTI"
BEGIN
    /*post to Co-Insurers */

    EXECUTE
    @return_status = spu_pmb_coinsurer_trans
            @transaction_export_folder_cnt,
                @insurance_file_cnt
    IF @return_status <> 0
...
ELSE
BEGIN
    /* Credit Net Premium to the Lead Insurer Account */
    SELECT  @transaction_ledger_code = 'GL'
    ...
    SELECT  @commission_excluded = 0
    SELECT  @withholding_tax_excluded = 0
    SELECT  @mapping_code = @lead_insurer_shortname
    SELECT  @transaction_account_key = @lead_insurer_account_key
    /* Set transaction amount */
    ...
    VALUES (
        ...
        @mapping_code,
        @transaction_account_key)
    /* Credit Tax to the Lead Insurer Account */
    SELECT  @transaction_ledger_code = 'GL'
    SELECT  @account_type_code = 'PURCHLEDGR'
    SELECT  @ceded_ref = NULL
    SELECT  @cover_share_percent = 0
...
VALUES (
    ...
    @mapping_code,
    @transaction_account_key)
/* Credit Commission to the Brokers' Commission Nominal Account */
SELECT  @transaction_ledger_code = 'GL'
...
VALUES (
    ...
    @mapping_code,
    @transaction_account_key)
/* Debit Commission to the Nominal Income Account */
SELECT  @transaction_ledger_code = 'GL'
SELECT  @account_type_code = 'INCOME'
SELECT  @ceded_ref = NULL
SELECT  @cover_share_percent = 0
