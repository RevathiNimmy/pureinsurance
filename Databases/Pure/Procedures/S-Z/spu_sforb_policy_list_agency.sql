...
EXECUTE DDLDropProcedure 'spu_sforb_policy_list_agency'
GO
CREATE PROCEDURE spu_sforb_policy_list_agency
...
BEGIN
    ...
    -- Do the business
        SELECT Insurance_File.insurance_file_id,
            ...
            AND (Insurance_File.source_id=@source_id OR @multi_company<>2)	-- Multi-company filter
    ...
    END ELSE BEGIN
        ...
        SELECT Insurance_File.insurance_file_id,
            ...
            WHERE Insurance_file.policy_ignore IS NULL
            AND Insurance_File_Type.code = ISNULL(@ins_file_type, '%')
            AND (Insurance_File.source_id=@source_id OR @multi_company<>2)	-- Multi-company filter
