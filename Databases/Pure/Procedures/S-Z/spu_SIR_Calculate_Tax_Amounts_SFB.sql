...
SET ANSI_NULLS ON
GO
Execute DDLDropProcedure 'spu_SIR_Calculate_Tax_Amounts_SFB'
GO

CREATE PROCEDURE spu_SIR_Calculate_Tax_Amounts_SFB      
 @company_id int,      
 @tax_group_id int,      
  ...
  BEGIN      
   ...
   IF @individual_tax_amount <> 0 
	...
	CASE WHEN ISNULL(@insurer_cnt, 0) <> 0 THEN @insurer_cnt 
        ELSE NULL
	END,
        CASE  	WHEN @transtype = 'TTRITC' Then 1
				WHEN @transtype = 'TTRITC' Then 1
				WHEN @transtype = 'TTIC' Then 1
				WHEN @transtype = 'TTFC' Then 1
				WHEN @transtype = 'TTAC' Then 1
		ELSE 0
	END
     )  
     END  
