...
DECLARE @offset INT,
	...
	IF @bIsSuppressDecimal=1 
	  SELECT  @insurance_file_no_of_dp=0

IF @insurance_file_no_of_dp IS NULL
SELECT @insurance_file_no_of_dp=4
...
BEGIN
    ...
    (
        ...
        @sum_insured,
        @annual_premium,
       ROUND(@this_premium,@insurance_file_no_of_dp), 
        @original_flag,
        @currency_id,
        @country_id,
        @state_id,
...
IF @original_flag = 1 and @is_pt=0
BEGIN

    IF ISNULL(@previous_risk_cnt, 0) = 0 
	...
	ELSE
		SELECT @original_risk_cnt = original_risk_cnt
		FROM   insurance_file_risk_link
		WHERE  risk_cnt = @previous_risk_cnt
   ...
   -rating_sum_insured,    -- rating_sum_insured
        ...
        annual_rate,
        -ROUND(annual_premium,@insurance_file_no_of_dp),    -- annual_premium,
        ROUND(CASE										--Rounding on Rating Section - Populate old rounding values (@this premium)
			WHEN @factor=1 THEN @this_premium		--Do not prorate the rounding amount - Amit
			ELSE ISNULL(ROUND(this_premium,@insurance_file_no_of_dp) * @factor,0)
		END,@insurance_file_no_of_dp),
	...
	BEGIN
	    ...
	    SELECT    @risk_id,
	        ...
	        p.annual_rate,
	        -ROUND(p.annual_premium,@insurance_file_no_of_dp),
	        ROUND(CASE
			WHEN @factor=1 THEN @this_premium
			ELSE ISNULL(ROUND(p.this_premium,@insurance_file_no_of_dp)* @factor,0)
			END,@insurance_file_no_of_dp),
	        lead_commission_band,
	        sub_commission_band,
	        tax_group,
...
ELSE IF (@original_flag = 0 AND @transaction_type='MTCR')
		BEGIN
			IF Isnull(@previous_risk_cnt, 0) = 0 
            SELECT @original_risk_cnt = original_risk_cnt 
            FROM   insurance_file_risk_link 
          ...
          ELSE 
			...
			INSERT INTO peril 
                ...
                SELECT @risk_id, 
                       ...
                       Isnull(p.rate_type_id, 1), 
                       p.annual_rate, 
                       ROUND(@annual_premium,@insurance_file_no_of_dp), 
                       ROUND(@this_premium,@insurance_file_no_of_dp), 
                       lead_commission_band, 
                       sub_commission_band, 
                       tax_group, 
