...
EXEC DDLDROPPROCEDURE [spu_SAM_find_policy_latestversions_By_RiskIndex]
GO
CREATE PROCEDURE spu_SAM_find_policy_latestversions_By_RiskIndex  
...
CREATE TABLE #tmpResults(insurancefolderCnts VARCHAR(MAX),insurancefilecnts VARCHAR(MAX),riskcnts VARCHAR(MAX))
INSERT INTO #tmpResults EXECUTE spu_SAM_FindPolicies_By_RiskIndex @search_value=@search_value

DECLARE @sInsuranceFileCnts VARCHAR(MAX)
SELECT top 1 @sInsuranceFileCnts = insurancefilecnts from #tmpResults
...
BEGIN  
 ...
 SET @Where = @Where + ' ( P.resolved_name like ''' + @sInsuredName + ''' OR P1.party_cnt IN (select ifa1.party_cnt from Insurance_file_associates ifa1 Join Party P2 ON ifa1.party_cnt = P2.party_cnt where P2.resolved_name like ''' + @sInsuredName + ''' an
d ISNULL(ifa1.Is_Deleted,0) <> 1 ))'  
END  
...
SET @WHERE =  @WHERE + ' ift.code IN (''QUOTE'', ''POLICY'', ''MTA PERM'', ''MTACAN'', ''MTAREINS'', ''WRITTEN'')'  
END  
IF @sQuoteType = 'QUOTE'  
BEGIN  
IF @WHERE <> ''  
...
SET @Where = @Where + ' ifi.insurance_file_cnt IN (' + @sInsuranceFileCnts + ')'
...
(  
    ...
    WHERE ifi.product_id = ISNULL(@nProductId, ifi.product_id)  
            ...
            AND ((ift.code IN ('QUOTE', 'POLICY', 'MTA PERM', 'MTACAN', 'MTAREINS', 'WRITTEN') AND @sQuoteType = 'ALL')  
                                    OR  
  ...
  isnull(ifs.last_modified,IFS.date_created) AS IssueDate,  
  ISNULL((SELECT TOP 1 isnull(d.document_date, created_date) FROM Document D WHERE  
  ifi.insurance_file_cnt = D.insurance_file_cnt AND d.document_ref LIKE 'S%' ORDER BY D.document_id ASC),  
  IFS.last_modified ) AS TransactionDate,  
  ifi.renewal_date RenewalDate,  
   ...
   INNER JOIN (Select InsuranceFolderCnt, CoverStartDate, NoOfVersions, Max(iFileCnt.insurance_file_cnt) InsuranceFileCnt  
          ...
          INNER JOIN Insurance_file_type IFT ON iFileCnt.Insurance_file_type_id=IFT.Insurance_file_type_id  
                                        Where ISNull(iFileCnt.out_of_sequence_replaced, 0) = 0 AND IFT.Code in('QUOTE', 'POLICY', 'MTA PERM', 'MTACAN', 'MTAREINS','WRITTEN')  
