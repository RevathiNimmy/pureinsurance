...
--***********************************
EXECUTE DDLDropProcedure 'spu_SIRRen_Get_Renewal_Invitation_Policy_Totals'
GO
...
CREATE PROCEDURE spu_SIRRen_Get_Renewal_Invitation_Policy_Totals
...
BEGIN  
	...
	IF @AllAgents = 1 AND @Include_Direct_Policies = 1  -- (11) ALL AGENTS AND ALL DIRECT BUSINESS
        BEGIN 
		SELECT 
		  COUNT(InsFile.insurance_file_cnt)
		...
		AND BRJP.Batch_Renewal_Job_ID = @Batch_Renewal_Job_ID
                AND (InsFile.Source_Id IN (SELECT s.source_id 
                                            FROM  source s 
