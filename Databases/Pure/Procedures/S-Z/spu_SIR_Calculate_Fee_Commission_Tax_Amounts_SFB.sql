...
SET ANSI_NULLS ON
GO
Execute DDLDropProcedure 'spu_SIR_Calculate_Fee_Commission_Tax_Amounts_SFB'
GO


CREATE PROCEDURE spu_SIR_Calculate_Fee_Commission_Tax_Amounts_SFB    
 @policy_fee_id int,
...
SELECT    
    @tax_group_id = tc.tax_group_id,    
    @fee_currency_id = pf.base_currency_id,    
    @company_id = i.source_id,    
    @fee_amount = pf.commission_amount,
    @insurance_file_cnt = pf.insurance_file_cnt, 
    @risk_cnt = NULL,
    @effective_date = i.cover_start_date   
FROM policy_fee pf
JOIN insurance_file i on i.insurance_file_cnt  = pf.insurance_file_cnt   
JOIN risk_code rc on rc.risk_code_id = i.risk_code_id
JOIN tax_calculation tc on tc.insurance_file_cnt = pf.insurance_file_cnt
