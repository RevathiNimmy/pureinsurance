...
EXECUTE DDLDropProcedure 'spu_SirRen_Renewal_Sel'
GO


CREATE PROCEDURE spu_SirRen_Renewal_Sel
...
BEGIN
    ...
    INNER JOIN insurance_file inf ON rc.old_insurance_file_cnt = inf.insurance_file_cnt
    INNER JOIN insurance_file inf2 ON rc.old_insurance_file_cnt = inf2.insurance_file_cnt 
    LEFT OUTER JOIN Gis_Scheme gs ON rc.renewal_gis_scheme_id = gs.gis_scheme_id
    LEFT JOIN   Policy_Agents PA on RC.renewal_insurance_file_cnt = PA.insurance_file_cnt
    JOIN gis_business_type gbt ON gs.gis_business_type_id = gbt.gis_business_type_id
    JOIN policy_type pt
