...
SET ANSI_NULLS ON
GO
EXECUTE DDLDropProcedure 'spu_SAM_ACT_Do_FindReceipt'
GO
CREATE PROCEDURE spu_SAM_ACT_Do_FindReceipt  
       @payee_name VARCHAR(60) = NULL,  
    @account_id INT = NULL,  
    @Receipt_type_id INT = NULL,  
    @Receipt_media_type_id INT = NULL,  
    @media_reference VARCHAR(60) = NULL,  
    @Receipt_status_id INT = NULL,  
    ...
    @DocumentReference VARCHAR(60)=NULL,
	@AgentKey INT=0
AS  
...
DECLARE @Receipttype_id INT,  
        @CODE VARCHAR(15),  
        @NumericSearch BIT,  
        @Receipt_status_id_UnissuedCheques INT,  
        @ResetMedia INT,  
 @sSql VARCHAR(8000),  
 @where VARCHAR(1000)  
...
SET ROWCOUNT @MaxRowsToFetch  
 ...
 WHERE mediatype.mediatype_id=Isnull(@Receipt_media_type_id,0)  
...
END  
 ...
 (  
  ...
  account_id BIGINT,  
  Receipt_description VARCHAR(100),  
  Receipt_code VARCHAR(100),  
  Media_description VARCHAR(100),  
  mediatype_id INT,  
  media_ref VARCHAR(100),  
  amount FLOAT,  
  Receipt_Status_Description VARCHAR(100),  
  Receipt_Status_code VARCHAR(100),  
  ...
  Receipt_type_description VARCHAR(100),  
  ...
  bank_reconciliation_date DATETIME,  
                Allocated BIT,  
                Receipt_account_code VARCHAR(60),  
                Receipt_branch_code VARCHAR(30),  
  ID INT IDENTITY  
 )  
 ...
 SELECT @sSql = 'INSERT INTO #CashListDetails  
  ...
  Else  
    ...
    INNER JOIN CashList C  
        ON    I.cashlist_id = C.cashlist_id AND C.cashlisttype_id =2  
    LEFT JOIN TransDetail TD  
        ON I.TransDetail_ID = TD.Transdetail_id  
    INNER JOIN cashlistitem_Receipt_type PT  
        ON  I.cashlistitem_Receipt_type_id = PT.cashlistitem_Receipt_type_id  
    INNER JOIN Document D  
        ON D.Document_id = TD.Document_id  
    INNER JOIN Account A  
        ON I.account_id = A.account_id  
    INNER JOIN CashListItem_Receipt_Status PS  
        ON I.cashlistitem_Receipt_status_id = PS.cashlistitem_Receipt_status_id  
    INNER JOIN MediaType M  
        ON I.mediaType_id = M.mediatype_id  
    INNER JOIN CashListItem_Receipt_Type CLIPT  
        ON I.CashListItem_Receipt_Type_id = CLIPT.CashListItem_Receipt_Type_id  
  ...
  BEGIN  
   ...
   SELECT @where = ' AND '  
  END  
  IF @Receipt_type_id IS NOT NULL --Filter by @Receipt_type_id if passed  
  BEGIN  
   SELECT @sSQL = @sSQL + @where + 'I.cashlistitem_Receipt_type_id = ' + cast(@Receipt_type_id AS VARCHAR)  
   SELECT @where = ' AND '  
  END  
  IF @Receipt_media_type_id IS NOT NULL ----Filter by @Receipt_media_type_id if passed  
  BEGIN  
   SELECT @sSQL = @sSQL + @where + 'I.mediatype_id = ' + cast(@Receipt_media_type_id AS VARCHAR)  
   SELECT @where = ' AND '  
  END  
  ...
  BEGIN  
   ...
   SELECT @where = ' AND '  
  END  
  IF @Receipt_status_id IS NOT NULL --Filter by @Receipt_status_id if passed  
  BEGIN  
   SELECT @sSQL = @sSQL + @where + 'I.cashlistitem_Receipt_status_id = ' + cast(@Receipt_status_id AS VARCHAR)  
   SELECT @where = ' AND '  
  END  
  ...
  BEGIN  
   IF (SELECT CHARINDEX ('%',@batch_ref)) > 0  
    ...
    SELECT @sSQL = @sSQL + @where + 'B.batch_ref = ''' + @batch_ref  + ''''  
     SELECT @where = ' AND '  
  END  
   IF @DocumentReference IS NOT NULL --Filter by @DocumentReference if passed  
  BEGIN  
   IF (SELECT CHARINDEX ('%',@DocumentReference)) > 0  
    SELECT @sSQL = @sSQL + @where + 'D.document_ref LIKE ''' + @DocumentReference  + ''''  
     ELSE  
    SELECT @sSQL = @sSQL + @where + 'D.document_ref = ''' + @DocumentReference  + ''''  
     SELECT @where = ' AND '  
  END  
  -- End Change PN 61824  
...
BEGIN  
  ...
  BEGIN  
   ...
   WHILE  @RowCount < 500 AND @MediaRef <= @MAX AND @MediaRef <= @media_To  
   BEGIN  
...
SELECT  
 ...
 Receipt_Status_Description 'ReceiptStatus' ,  
 ...
 Receipt_type_description 'ReceiptType',  
 media_ref 'MediaType',  
 username 'User',  
 Receipt_name 'PayeeName' ,  
 ...
 Allocated,  
    ...
    Receipt_branch_code 'ReceiptBranchCode'  
