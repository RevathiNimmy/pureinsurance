...
EXECUTE DDLDropProcedure 'spu_SAM_Clone_Quote_From_Live_Policy'
GO
...
CREATE PROCEDURE [dbo].[spu_SAM_Clone_Quote_From_Live_Policy]  		
	@insurance_file_cnt			INT, 
	@new_insurance_ref			VARCHAR(255) OUTPUT,
	@new_insurance_file_cnt		INT	OUTPUT,
	@new_insurance_folder_cnt	INT	OUTPUT,
	@userID INT  ,
	@CloneQuoteFromLivePolicy TINYINT  
AS  
...
DECLARE @old_insurance_ref VARCHAR(255)
DECLARE @iIndex int
Declare @subStr as VARCHAR(10)
Declare @policy_ref varchar(255)

  --- Logic to retain the policy number
 IF @CloneQuoteFromLivePolicy <> 0
 BEGIN
	 SELECT @old_insurance_ref = RTRIM(LTRIM(insurance_ref)),
			@insurance_folder_cnt = insurance_folder_cnt,
			@insurance_file_type_id = insurance_file_type_id
	 FROM Insurance_File
	 WHERE insurance_file_cnt = @insurance_file_cnt

	 SET @subStr = SUBSTRING(@old_insurance_ref,LEN(@old_insurance_ref)-2,3)

	 SET @iIndex = CHARINDEX('_',@subStr)
		 
     IF @iIndex = 1
		 BEGIN
			SET @old_insurance_ref = SUBSTRING(@old_insurance_ref,0,LEN(@old_insurance_ref)-2)			
		 END
     ELSE IF  @iIndex = 2
		 BEGIN
			SET @old_insurance_ref = SUBSTRING(@old_insurance_ref,0,LEN(@old_insurance_ref)-1)			
		 END

		 Set @policy_ref = RTRIM(LTRIM(@old_insurance_ref))

	 SET @old_insurance_ref = CONCAT(RTRIM(LTRIM(@old_insurance_ref)),'%')

     SELECT TOP 1 @new_insurance_ref = RTRIM(LTRIM(insurance_ref)) from insurance_file where insurance_ref like  @old_insurance_ref    order by LEN(insurance_ref) desc, insurance_ref desc

	 SET @iIndex = 0

	 If Len(@policy_ref) <>  LEN(@new_insurance_ref)
	 BEGIN
		SET @iIndex = CONVERT(INT,SUBSTRING(@new_insurance_ref,LEN(@policy_ref)+2,3))
	 END 
 END

  SET @new_insurance_ref =CONCAT(@policy_ref,'_',@iIndex+1)
	...
	SET @sSQL = 'INSERT INTO insurance_folder(insurance_folder_id, source_id, insurance_holder_cnt, code, description, inception_date, arc_archive_folder_id, quote_insurance_ref, next_insurance_ref, last_insurance_ref, renewal_count, renewal_NCD_Year, renewal_NCD_contents, last_edi_message_count_received, last_edi_message_count_sent) ' + 
				' SELECT ifd.insurance_folder_id, ifd.source_id, ifd.insurance_holder_cnt, ''' + CAST(@new_insurance_ref AS VARCHAR(255)) + ''', ifd.description, ifd.inception_date, ifd.arc_archive_folder_id,  ''' + CAST(@new_insurance_ref AS VARCHAR(255)) + ''', ifd.next_insurance_ref, ifd.last_insurance_ref, ifd.renewal_count, ifd.renewal_NCD_Year, ifd.renewal_NCD_contents, ' + 
