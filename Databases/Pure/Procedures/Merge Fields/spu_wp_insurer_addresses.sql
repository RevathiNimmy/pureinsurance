...
EXECUTE DDLDropProcedure 'spu_wp_insurer_addresses'
go

CREATE PROCEDURE spu_wp_insurer_addresses
    @PartyCnt INT,
...
WHILE @@FETCH_STATUS = 0 BEGIN
    ...
    /* DC091104 PN11127 if passed insurancefilecnt, set insurer to the insurer on the policy */
	IF ISNULL(@InsuranceFileCnt, '') <> ''
	BEGIN 
		select @partycnt = ( select lead_insurer_cnt from insurance_file where insurance_file_cnt = @InsuranceFileCnt )
	END

    exec spu_wp_get_Insurer_address @PartyCnt, @InsuranceFileCnt, @ClaimCnt,@AddressCode,@Address1 OUTPUT,@Address2 OUTPUT,@Address3 OUTPUT,@Address4 OUTPUT,@Postcode OUTPUT,@Country Output
    ...
    BEGIN
    	exec spu_wp_get_Insurer_address @PartyCnt,@InsuranceFileCnt,@ClaimCnt,@CorrespondenceCode,@Address1 OUTPUT,@Address2 OUTPUT,@Address3 OUTPUT,@Address4 OUTPUT,@Postcode OUTPUT,@Country Output
    END
