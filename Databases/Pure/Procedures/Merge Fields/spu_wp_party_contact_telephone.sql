...
EXECUTE DDLDropProcedure 'spu_wp_party_contact_telephone'
GO
 
CREATE PROCEDURE spu_wp_party_contact_telephone
...
WHERE P.party_cnt=@PartyCnt AND CT.Code='TELEPHONE'
...
BEGIN
	SELECT @complete_number=rtrim(@area_code) + ' ' + rtrim(@number) + ' '
	if @extension <> ''
		SELECT @complete_number=@complete_number + 'ext. ' + rtrim(@extension)
	SELECT @complete_number=rtrim(ltrim(@complete_number))
END
