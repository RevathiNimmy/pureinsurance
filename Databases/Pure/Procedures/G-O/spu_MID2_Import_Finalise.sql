...
EXECUTE DDLDropProcedure spu_MID2_Import_Finalise
GO

CREATE PROCEDURE spu_MID2_Import_Finalise
...
Begin
	...
	SELECT @Mid_Batch_Type_Id = batch_type_id from batch_type where code =  'MID2'
		...
		WHERE batch_ref = @Batch_Ref
			...
			AND company_id = (SELECT  TOP 1 source_id FROM MID_Rule
							WHERE Site_Number = @site_number AND Supplier_id = @supplier_id
								AND Is_Deleted = 0 AND MID_Type = 'MID2') 
			 AND interface_code LIKE'%EXPORT%'	
	END
