...
GO
EXECUTE DDLDropTrigger 'tr_PMUser_Source_audit_log'
GO
CREATE TRIGGER tr_PMUser_Source_audit_log ON dbo.PMUser_Source 
...
SELECT  @ModuleId=5,@ModuleName='User Maintenance', @PKColName = 'source_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=user_id,@UniqueID=UniqueID,@source_id=source_id FROM #del
ELSE
	SELECT @UserId=user_id,@UniqueID=UniqueID,@source_id=source_id FROM #ins
...
SELECT @FieldDesc='Allow Branch Access'
