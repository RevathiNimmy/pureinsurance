...
GO
EXECUTE DDLDropTrigger 'tr_Party_Certificate_Year_audit_log'
GO
CREATE TRIGGER tr_Party_Certificate_Year_audit_log ON dbo.Party_Certificate_Year
...
SELECT @ModuleId = 7, @ModuleName = 'Reinsurer Maintenance' ,@PKColName = 'Party_Certificate_Year_ID',@ValueColName='ScreenHierarchy';
...
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@IsDeleted = IsDeleted ,@ScreenHierarchy = ScreenHierarchy FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy') AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND (FieldName IN ('New Re-Insurer Added')))
RETURN

--IF @Type = 'U' AND EXISTS(SELECT 1 FROM #del d INNER JOIN #ins i ON i.UniqueId = d.UniqueId AND i.is_deleted <> d.is_deleted
--							INNER JOIN configuration_audit_master cam ON cam.UniqueId = d.UniqueId
...
--			@Document_Template_Id,
--			@ScreenHierarchy,
--			CASE WHEN @IsDeleted = 1 THEN 'Document Template Deleted' ELSE 'Document Template Undeleted' END,
--			CASE WHEN @IsDeleted = 1 THEN 'Document Template Deleted' ELSE 'Document Template Undeleted' END
--			)
--	RETURN
--END
...
IF @Type='I'
    SELECT @FieldValue='New Certificate Year Added'
ELSE IF @Type='D'
	SELECT @FieldValue='Certificate Year Deleted'
