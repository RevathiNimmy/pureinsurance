...
GO
EXECUTE DDLDropTrigger 'tr_PMUser_audit_log'
GO
CREATE TRIGGER tr_PMUser_audit_log ON dbo.PMUser 
...
SELECT @PKColName = 'user_id',@ValueColName='ScreenHierarchy';
...
--Return
IF @Type='D'
	SELECT @UserId=ModifiedBy,@UniqueID=UniqueID,@user_id=user_id,@ScreenHierarchy = ScreenHierarchy FROM #del
ELSE
	SELECT @UserId=ModifiedBy,@UniqueID=UniqueID,@user_id=user_id,@ScreenHierarchy = ScreenHierarchy  FROM #ins

IF UPPER(LEFT(@ScreenHierarchy,4))='USER' 
	SELECT @ModuleId=5,@ModuleName='User Maintenance'
Else IF UPPER(LEFT(@ScreenHierarchy,7)) = 'LICENCE'
	SELECT @ModuleId=25,@ModuleName='Licence Administrator'
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'timestamp', 'pie_last_updated','date_deleted','user_config_xml_dataset', 'lastlogin', 'ModifiedBy','secure_password','is_temp_password','password') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I'
    SELECT @FieldValue='New User Added'
ELSE IF @Type='D'
	SELECT @FieldValue='User Deleted'
...
IF @fieldname = 'alternative_identifier'
BEGIN
...
END
IF @fieldname = 'alternative_identifier' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@user_id AND FieldName=@fieldname AND OldValue=@NewValue)     
BEGIN
...
BEGIN
   ...
   IF @fieldname = 'party_handler_id'
	BEGIN
		UPDATE i SET FieldDisplayName=('User Type ' + pt.description + '?') FROM configuration_audit_details i 
			INNER JOIN Party p ON p.resolved_name = CASE WHEN ISNULL(i.NewValue,'') = '' THEN i.OldValue ELSE i.NewValue END
			INNER JOIN Party_Type pt ON pt.party_type_id = p.party_type_id
			WHERE i.FieldName = 'party_handler_id' AND pt.code IN ('AH','CO')
			AND configuration_audit_master_id = @configuration_audit_master_id

        UPDATE configuration_audit_details SET FieldDisplayName='User Type Account Handler And Executive' WHERE FieldDisplayName = 'party_handler_id' AND configuration_audit_master_id = @configuration_audit_master_id
	END
   IF @fieldname = 'party_cnt'
	BEGIN
		UPDATE i SET FieldDisplayName=('User Type ' + pt.description + '?') FROM configuration_audit_details i 
			INNER JOIN Party p ON p.resolved_name = CASE WHEN ISNULL(i.NewValue,'') = '' THEN i.OldValue ELSE i.NewValue END
			INNER JOIN Party_Type pt ON pt.party_type_id = p.party_type_id
			WHERE i.FieldName = 'party_cnt' AND pt.code IN ('AG','IN')
			AND configuration_audit_master_id = @configuration_audit_master_id
	END
	
	If @fieldname IN ('server_printer')
	BEGIN
		UPDATE configuration_audit_details SET OldValue = CASE WHEN OldValue = '-1' Then 'Default' ELSE OldValue END WHERE configuration_audit_master_id = @configuration_audit_master_id
	END

	If @fieldname IN ('signature_file')
	BEGIN
		UPDATE configuration_audit_details SET NewValue = 'Signature Updated' WHERE configuration_audit_master_id = @configuration_audit_master_id AND FieldName='signature_file'
	END

	DELETE configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND ISNULL(OldValue,'')=ISNULL(NewValue,'') AND FieldDisplayName <> 'New User Added'
	DELETE configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND @fieldname IN ('server_printer') AND NewValue = '-1'
	DELETE configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND @fieldname IN ('logged_on_at_client') AND @ModuleId = 5
