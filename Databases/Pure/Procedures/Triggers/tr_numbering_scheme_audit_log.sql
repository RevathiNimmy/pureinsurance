...
GO
EXECUTE DDLDropTrigger 'tr_numbering_scheme_audit_log'
GO
CREATE TRIGGER tr_numbering_scheme_audit_log ON dbo.numbering_scheme
...
DECLARE @Numbering_Sscheme_id INT;
DECLARE @ScreenHierarchy VARCHAR(500);

SELECT @ModuleId = 24, @ModuleName = 'Maintain Numbering Scheme' ,@PKColName = 'numbering_scheme_id',@ValueColName='ScreenHierarchy';
...
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@IsDeleted = is_deleted ,@Numbering_Sscheme_id = numbering_scheme_id,@ScreenHierarchy = ScreenHierarchy FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','caption_id','pie_guid','date_last_generated','pie_last_updated','effective_date') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type = 'U' AND EXISTS(SELECT 1 FROM #del d INNER JOIN #ins i ON i.UniqueId = d.UniqueId AND d.is_deleted <> @IsDeleted
...
BEGIN
	...
	VALUES (
			...
			'Numbering Scheme',
			'numbering_scheme_id',
			@Numbering_Sscheme_id,
			@ScreenHierarchy,
			CASE WHEN @IsDeleted = 1 THEN 'Numbering Scheme Deleted' ELSE 'Numbering Scheme Undeleted' END,
			CASE WHEN @IsDeleted = 1 THEN 'Numbering Scheme Deleted' ELSE 'Numbering Scheme Undeleted' END
			)
	RETURN
END
...
IF @Type='I'
    SELECT @FieldValue='New Numbering Scheme Added'
