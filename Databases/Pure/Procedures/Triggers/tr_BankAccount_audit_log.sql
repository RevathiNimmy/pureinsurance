...
GO
EXECUTE DDLDropTrigger 'tr_BankAccount_audit_log'
GO
CREATE TRIGGER tr_BankAccount_audit_log ON dbo.BankAccount
...
SELECT @ModuleId = 19, @ModuleName = 'Bank Maintenance' ,@PKColName = 'bankaccount_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@BankId = bank_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@BankId = bank_id,@ScreenHierarchy = ScreenHierarchy  FROM #ins
...
If @Type = 'U' AND @BankId IS NOT NULL AND @ScreenHierarchy LIKE 'Bank%'
BEGIN
	UPDATE cad SET key_field_desc = @ScreenHierarchy FROM configuration_audit_details cad
	INNER JOIN configuration_audit_master cam ON cam.configuration_audit_master_id = cad.configuration_audit_master_id
	INNER JOIN #ins i ON i.UniqueId  = cam.UniqueId AND i.bankaccount_id = cad.key_field_value
	WHERE TableName  = 'BankAccount' AND cam.UniqueId = @UniqueID
	
	UPDATE cad
	SET cad.key_field_desc = @ScreenHierarchy + '\Receipt Delay(' + mt.description + ')'
	FROM configuration_audit_details cad
	INNER JOIN configuration_audit_master cam ON cam.configuration_audit_master_id = cad.configuration_audit_master_id
	INNER JOIN #ins i ON i.UniqueId = cam.UniqueId
	INNER JOIN BankAccount_Delay bad ON bad.bankaccount_delay_id = cad.key_field_value AND i.bankaccount_id = bad.bankaccount_id
	INNER JOIN MediaType mt ON mt.mediatype_id = bad.mediatype_id
	WHERE cad.TableName = 'BankAccount_Delay' AND cam.UniqueId = @UniqueID
	
	UPDATE cad SET key_field_desc = @ScreenHierarchy + '\Branches' FROM configuration_audit_details cad
	INNER JOIN configuration_audit_master cam ON cam.configuration_audit_master_id = cad.configuration_audit_master_id
	INNER JOIN #ins i ON i.UniqueId  = cam.UniqueId
	INNER JOIN BankAccount_Source bas ON bas.source_id = cad.key_field_value
	WHERE cad.TableName = 'BankAccount_Source' AND cam.UniqueId = @UniqueID 
END

IF @fieldname NOT IN ('UniqueId','User_Id','ScreenHierarchy','bank_id','caption_id','reconciled_date') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I'
    SELECT @FieldValue='New Bank Account Added'
ELSE If @Type='D'
	SELECT @FieldValue='Bank Account Deleted'
