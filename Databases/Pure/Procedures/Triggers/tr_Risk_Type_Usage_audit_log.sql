...
GO
EXECUTE DDLDropTrigger 'tr_Risk_Type_Usage_audit_log'
GO
CREATE TRIGGER tr_Risk_Type_Usage_audit_log ON dbo.Risk_Type_Usage 
...
SELECT  @ModuleId=27,@ModuleName='Risk Type Maintenance', @PKColName = 'risk_type_group_id',@ValueColName='ScreenHierarchy';
...
SELECT @FieldDesc='Risk Group Selected'
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@risk_type_group_id AND FieldName=@fieldname AND Type = 'D')     
BEGIN
