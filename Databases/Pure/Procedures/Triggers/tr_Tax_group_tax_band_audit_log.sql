...
GO
EXECUTE DDLDropTrigger 'tr_Tax_group_tax_band_audit_log'
GO
CREATE TRIGGER tr_Tax_group_tax_band_audit_log ON dbo.Tax_group_tax_band
    FOR INSERT,DELETE 
AS
SET NOCOUNT ON;
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@tax_band_id=tax_band_id,@key_field_desc=ScreenHierarchy FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@tax_band_id=tax_band_id,@key_field_desc=ScreenHierarchy FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'pie_last_updated', 'tax_group_id', 'pie_guid') AND @UniqueID IS NOT NULL
BEGIN	
...
BEGIN
	...
	IF EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
				AND key_field_value=@tax_band_id AND FieldName=@fieldname AND (OldValue=@NewValue OR FieldName IN ('tax_group_id')))     
	BEGIN
		DELETE configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
				AND key_field_value=@tax_band_id AND FieldName=@fieldname AND (OldValue=@NewValue OR FieldName IN ('tax_group_id'))
	END
	ELSE IF EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
				AND key_field_value=@tax_band_id AND FieldName=@fieldname AND OldValue<>@NewValue )  
	BEGIN
	...
	UPDATE configuration_audit_details SET Type='U', NewValue=@NewValue WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@tax_band_id AND FieldName=@fieldname 
    ELSE
	BEGIN
	DECLARE @SQL1 NVARCHAR(MAX)
		SET @SQL1 = '
			...
			WHERE cad.configuration_audit_master_id = @p_configuration_audit_master_id
			  ...
			  AND cad.key_field_value = @p_tax_band_id
