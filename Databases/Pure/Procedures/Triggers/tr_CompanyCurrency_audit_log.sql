...
GO
EXECUTE DDLDropTrigger 'tr_CompanyCurrency_audit_log'
GO
CREATE TRIGGER tr_CompanyCurrency_audit_log ON dbo.CompanyCurrency
...
SELECT  @ModuleId=18,@ModuleName='Branch Maintenance', @PKColName = 'currency_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueId,@CurrencyId=currency_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueId,@CurrencyId=currency_id FROM #ins
...
SELECT @FieldDesc='Currency Chosen'

IF @fieldname ='currency_id' AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName='New Branch Added')
RETURN
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@CurrencyId AND FieldName=@fieldname)     
BEGIN
