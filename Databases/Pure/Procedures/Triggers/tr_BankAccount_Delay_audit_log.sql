...
GO
EXECUTE DDLDropTrigger 'tr_BanKAccount_Delay_audit_log'
GO
CREATE TRIGGER tr_BanKAccount_Delay_audit_log ON dbo.BankAccount_Delay
...
SELECT @ModuleId = 19, @ModuleName = 'Bank Maintenance' ,@PKColName = 'bankaccount_delay_id',@ValueColName='ScreenHierarchy';
...
IF @fieldname NOT IN ('UniqueId','User_Id','ScreenHierarchy') AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName In('New Bank Account Added'))
RETURN


DECLARE @FieldValue VARCHAR(50)
IF @Type='I'
    SELECT @FieldValue='New BankAccount Delay Added'
ELSE If @Type='D'
	SELECT @FieldValue='BankAccount Delay Deleted'
