...
GO
EXECUTE DDLDropTrigger 'tr_Product_Claim_Events_audit_log'
GO
CREATE TRIGGER tr_Product_Claim_Events_audit_log ON dbo.Product_Claim_Events 
...
DECLARE @claim_event_description_id INT

SELECT  @ModuleId=28,@ModuleName='Product Risk Maintenance', @PKColName = 'claim_event_description_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@claim_event_description_id=claim_event_description_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@claim_event_description_id=claim_event_description_id FROM #ins
...
SELECT @FieldDesc='Events Chosen'

IF @fieldname ='claim_event_description_id' AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@claim_event_description_id AND FieldName=@fieldname AND Type = 'D')     
BEGIN
