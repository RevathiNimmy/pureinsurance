...
GO
EXECUTE DDLDropTrigger 'tr_TreatyParty_audit_log'
GO
CREATE TRIGGER tr_TreatyParty_audit_log ON dbo.Treaty_Party 
...
SELECT @ModuleId=8,@ModuleName='Treaty Maintenance', @PKColName = 'party_cnt',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@party_cnt=party_cnt,@ScreenHierarchy = ScreenHierarchy FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@party_cnt=party_cnt,@ScreenHierarchy = ScreenHierarchy FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'pie_last_updated', 'treaty_party_id', 'treaty_id','pie_guid') AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName='New Treaty Added')
RETURN
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@party_cnt AND FieldName=@fieldname AND (OldValue=@NewValue OR FieldName='party_cnt'))     
BEGIN
...
ELSE IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@party_cnt AND FieldName=@fieldname AND OldValue<>@NewValue )  
BEGIN
	UPDATE configuration_audit_details SET Type='U', NewValue=@NewValue WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@party_cnt AND FieldName=@fieldname 
END
ELSE 
BEGIN
SELECT 
