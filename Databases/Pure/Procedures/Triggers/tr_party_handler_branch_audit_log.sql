...
GO
EXECUTE DDLDropTrigger 'tr_party_handler_branch_audit_log'
GO
CREATE TRIGGER tr_party_handler_branch_audit_log ON dbo.party_handler_branch
...
SELECT  @ModuleId=15,@ModuleName='Account Handler', @PKColName = 'source_id',@ValueColName='ScreenHierarchy';
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName='New Account Handler Added')
RETURN
