...
GO
EXECUTE DDLDropTrigger 'tr_previous_accidents_audit_log'
GO
CREATE TRIGGER tr_previous_accidents_audit_log ON dbo.previous_accidents
    FOR INSERT, DELETE 
AS
SET NOCOUNT ON;
...
SELECT @ModuleId = 21, @ModuleName = 'Other Party Maintenance' ,@PKColName = 'party_cnt',@ValueColName='ScreenHierarchy';
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','previous_accidents_id','party_cnt') AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND (FieldName IN ('New Other Party Added')))
RETURN
...
BEGIN
    ...
    SET @delSql = N'
        ...
        INNER JOIN #ins i
            ON CONVERT(VARCHAR(100),  i.' + QUOTENAME(@PKColName) + N') = cad.key_field_value
           AND CONVERT(VARCHAR(1000), i.' + QUOTENAME(@ValueColName) + N') = cad.key_field_desc
        WHERE cad.configuration_audit_master_id = @MasterId
          AND cad.FieldName = @FieldName
          AND cad.Type = ''D''
          AND ISNULL(cad.OldValue, '''') = ISNULL(CONVERT(VARCHAR(1000), i.' + QUOTENAME(@fieldname) + N'), '''');

        SELECT @DeletedOut = @@ROWCOUNT;
    ';

    EXEC sp_executesql
        @delSql,
        N'@MasterId INT, @FieldName SYSNAME, @DeletedOut INT OUTPUT',
        @MasterId = @configuration_audit_master_id,
        @FieldName = @fieldname,
        @DeletedOut = @Deleted OUTPUT;
