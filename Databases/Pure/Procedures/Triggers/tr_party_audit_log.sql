...
GO
EXECUTE DDLDropTrigger 'tr_party_audit_log'
GO
CREATE TRIGGER tr_party_audit_log ON dbo.party
...
SELECT @PKColName = 'party_cnt',@ValueColName='ScreenHierarchy'
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'resolved_name', 'last_modified', 'correspondence_type_id', 'tob_letter', 'modified_by_id','loyalty_number','alternative_identifier','marketing_segment_ind','trading_name') AND @UniqueID IS NOT NULL 
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND (FieldName IN ('New Agent Added', 'New Fee Added', 'New Re-Insurer Added', 'New Discount Account Added', 'New Extra Account Added','New Agent Group Added', 'New Other Party Added')))
RETURN

DECLARE @FieldValue VARCHAR(50)
IF @Type='I'
    IF UPPER(LEFT(@ScreenHierarchy,3))='FEE' 
		SELECT @FieldValue='New Fee Added'
	Else IF UPPER(LEFT(@ScreenHierarchy,11)) = 'AGENT GROUP'
		SELECT @FieldValue='New Agent Group Added'
	ELSE IF UPPER(LEFT(@ScreenHierarchy,5))='AGENT' 
		SELECT @FieldValue='New Agent Added' 
	ELSE IF UPPER(LEFT(@ScreenHierarchy,9))='REINSURER' 
		SELECT @FieldValue='New Re-Insurer Added'
	Else IF UPPER(LEFT(@ScreenHierarchy,17)) = 'ACCOUNT EXECUTIVE'
		SELECT @FieldValue='New Account Executive Added'
	Else IF UPPER(LEFT(@ScreenHierarchy,15)) = 'ACCOUNT HANDLER'
		SELECT @FieldValue='New Account Handler Added'
	Else IF UPPER(LEFT(@ScreenHierarchy,8)) = 'DISCOUNT'
		SELECT @FieldValue='New Discount Account Added'
	Else IF UPPER(LEFT(@ScreenHierarchy,5)) = 'EXTRA'
		SELECT @FieldValue='New Extra Account Added'
	Else IF UPPER(LEFT(@ScreenHierarchy,5)) = 'OTHER'
		SELECT @FieldValue='New Other Party Added'
	Else IF UPPER(LEFT(@ScreenHierarchy,11)) = 'AGENT GROUP'
		SELECT @FieldValue='New Agent Group Added'
ELSE IF @Type='D'
	IF UPPER(LEFT(@ScreenHierarchy,3))='FEE' 
		SELECT @FieldValue='Fee Deleted'
	Else IF UPPER(LEFT(@ScreenHierarchy,11)) = 'AGENT GROUP'
		SELECT @FieldValue='Agent Group Deleted'
	ELSE IF UPPER(LEFT(@ScreenHierarchy,5))='AGENT' 
		SELECT @FieldValue='Agent Deleted' 
	ELSE IF UPPER(LEFT(@ScreenHierarchy,9))='REINSURER' 
		SELECT @FieldValue='Reinsurer Deleted'
	Else IF UPPER(LEFT(@ScreenHierarchy,17)) = 'ACCOUNT EXECUTIVE'
		SELECT @FieldValue='Account Executive Deleted'
	Else IF UPPER(LEFT(@ScreenHierarchy,15)) = 'ACCOUNT HANDLER'
		SELECT @FieldValue='Account Handler Deleted'
	Else IF UPPER(LEFT(@ScreenHierarchy,7)) = 'DISCOUNT'
		SELECT @FieldValue='Discount Account Deleted'
	Else IF UPPER(LEFT(@ScreenHierarchy,5)) = 'EXTRA'
		SELECT @FieldValue='Extra Account Deleted'
	Else IF UPPER(LEFT(@ScreenHierarchy,5)) = 'OTHER'
		SELECT @FieldValue='Other Party Deleted'
...
SELECT 
   ...
   If @ModuleName='Fee Maintenance' 
   BEGIN
	 UPDATE i SET FieldDisplayName=a.FieldDescription  FROM configuration_audit_details i 
			INNER JOIN Audit_trail_custom_fields a ON a.TableName= i.TableName AND a.FieldName=i.FieldName AND a.fieldValue='Fee' WHERE i.FieldName=@fieldname AND i.configuration_audit_master_id=@configuration_audit_master_id 		
   END
   Else If @ModuleName='Reinsurer Maintenance' 
   BEGIN
	 UPDATE i SET FieldDisplayName=a.FieldDescription  FROM configuration_audit_details i 
			INNER JOIN Audit_trail_custom_fields a ON a.TableName= i.TableName AND a.FieldName=i.FieldName AND a.fieldValue='Reinsurer' WHERE i.FieldName=@fieldname AND i.configuration_audit_master_id=@configuration_audit_master_id 		
   END
   Else If @ModuleName='Agent Maintenance' 
   BEGIN
	 UPDATE i SET FieldDisplayName=a.FieldDescription  FROM configuration_audit_details i 
			INNER JOIN Audit_trail_custom_fields a ON a.TableName= i.TableName AND a.FieldName=i.FieldName AND a.fieldValue='Agent' WHERE i.FieldName=@fieldname AND i.configuration_audit_master_id=@configuration_audit_master_id 		
   END
   Else If @ModuleName='Account Executive' 
   BEGIN
	 UPDATE i SET FieldDisplayName=a.FieldDescription  FROM configuration_audit_details i 
			INNER JOIN Audit_trail_custom_fields a ON a.TableName= i.TableName AND a.FieldName=i.FieldName AND a.fieldValue='Account Executive' WHERE i.FieldName=@fieldname AND i.configuration_audit_master_id=@configuration_audit_master_id 		
   END
   Else If @ModuleName='Account Handler' 
   BEGIN
	 UPDATE i SET FieldDisplayName=a.FieldDescription  FROM configuration_audit_details i 
			INNER JOIN Audit_trail_custom_fields a ON a.TableName= i.TableName AND a.FieldName=i.FieldName AND a.fieldValue='Account Handler' WHERE i.FieldName=@fieldname AND i.configuration_audit_master_id=@configuration_audit_master_id 		
   END
   Else If @ModuleName='Discount Maintenance' 
   BEGIN
	 UPDATE i SET FieldDisplayName=a.FieldDescription  FROM configuration_audit_details i 
			INNER JOIN Audit_trail_custom_fields a ON a.TableName= i.TableName AND a.FieldName=i.FieldName AND a.fieldValue='Discount Account' WHERE i.FieldName=@fieldname AND i.configuration_audit_master_id=@configuration_audit_master_id 		
   END
   Else If @ModuleName='Extra Maintenance' 
   BEGIN
	 UPDATE i SET FieldDisplayName=a.FieldDescription  FROM configuration_audit_details i 
			INNER JOIN Audit_trail_custom_fields a ON a.TableName= i.TableName AND a.FieldName=i.FieldName AND a.fieldValue='Extra Account' WHERE i.FieldName=@fieldname AND i.configuration_audit_master_id=@configuration_audit_master_id 		
   END
    Else If @ModuleName='Agent Group Maintenance' 
   BEGIN
	 UPDATE i SET FieldDisplayName=a.FieldDescription  FROM configuration_audit_details i 
			INNER JOIN Audit_trail_custom_fields a ON a.TableName= i.TableName AND a.FieldName=i.FieldName AND a.fieldValue='Agent Group' WHERE i.FieldName=@fieldname AND i.configuration_audit_master_id=@configuration_audit_master_id 		
     DELETE configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id and FieldName='currency_id'
   END
   Else If @ModuleName='Other Party Maintenance' 
   BEGIN
	 UPDATE i SET FieldDisplayName=a.FieldDescription  FROM configuration_audit_details i 
			INNER JOIN Audit_trail_custom_fields a ON a.TableName= i.TableName AND a.FieldName=i.FieldName AND a.fieldValue='Other Party' WHERE i.FieldName=@fieldname AND i.configuration_audit_master_id=@configuration_audit_master_id 		
   END
    END
	END
END
