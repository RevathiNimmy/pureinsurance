...
GO
EXECUTE DDLDropTrigger 'tr_Party_Agent_Group_audit_log'
GO
CREATE TRIGGER tr_Party_Agent_Group_audit_log ON dbo.Party_Agent_Group
...
SELECT @ModuleId = 22, @ModuleName = 'Agent Group Maintenance' ,@PKColName = 'party_cnt',@ValueColName='ScreenHierarchy';
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND (FieldName IN ('New Agent Group Added')))
RETURN
