...
GO
EXECUTE DDLDropTrigger 'tr_Credit_Control_Rule_Insurance_File_Status_audit_log'
GO
CREATE TRIGGER tr_Credit_Control_Rule_Insurance_File_Status_audit_log ON Credit_Control_Rule_Insurance_File_Status 
...
SELECT @ModuleId=2,@ModuleName='Credit Control Maintenance', @PKColName = 'insurance_file_status_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@insurance_file_status_id=insurance_file_status_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@insurance_file_status_id=insurance_file_status_id FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'credit_control_rule_id') AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName='New Rule Added')
RETURN
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@insurance_file_status_id AND FieldName=@fieldname AND (OldValue=@NewValue OR FieldName='insurance_file_status_id'))     
BEGIN
...
ELSE IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@insurance_file_status_id AND FieldName=@fieldname AND OldValue<>@NewValue )  
BEGIN
	UPDATE configuration_audit_details SET Type='U', NewValue=@NewValue WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@insurance_file_status_id AND FieldName=@fieldname 
END
ELSE 
BEGIN
SELECT 
