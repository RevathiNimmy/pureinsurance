...
GO
EXECUTE DDLDropTrigger 'tr_Risk_Type_audit_log'
GO
CREATE TRIGGER tr_Risk_Type_audit_log ON dbo.Risk_Type
...
SELECT @ModuleId = 27, @ModuleName = 'Risk Type Maintenance' ,@PKColName = 'risk_type_id',@ValueColName='ScreenHierarchy';
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','caption_id', 'pie_last_updated', 'pie_guid') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I'
    SELECT @FieldValue='New Risk Added'
ELSE If @Type='D'
	SELECT @FieldValue='Risk Deleted'
