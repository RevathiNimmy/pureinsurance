...
GO
EXECUTE DDLDropTrigger 'tr_Report_Group_User_Groups_audit_log'
GO
CREATE TRIGGER tr_Report_Group_User_Groups_audit_log ON dbo.Report_Group_User_Groups 
    FOR INSERT, DELETE 
AS
SET NOCOUNT ON;
...
SELECT  @ModuleId=13,@ModuleName='LookUp Maintenance', @PKColName = 'pmuser_group_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@pmuser_group_id=pmuser_group_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@pmuser_group_id=pmuser_group_id FROM #ins
...
SELECT @FieldDesc='Report Group User Groups'

IF @fieldname ='pmuser_group_id' AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName='New Record Added')
RETURN
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@pmuser_group_id AND FieldName=@fieldname)     
BEGIN
