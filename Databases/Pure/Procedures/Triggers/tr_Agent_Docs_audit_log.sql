...
GO
EXECUTE DDLDropTrigger 'tr_Agent_Docs_audit_log'
GO
CREATE TRIGGER tr_Agent_Docs_audit_log ON dbo.Agent_Docs 
...
DECLARE @AllowFiltering INT

SELECT  @ModuleId=6,@ModuleName='Agent Maintenance', @PKColName = 'process_type',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@process_type = process_type FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@process_type = process_type FROM #ins
...
SELECT @FieldDesc='Agent Available Documents'

IF @fieldname ='process_type' AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@process_type AND FieldName=@fieldname)     
BEGIN
...
SELECT 
        ...
        EXEC (@sql);
	END
		DELETE cad FROM configuration_audit_details cad INNER JOIN Process_Types_Docs ptd
		ON ptd.process_types_docs_id = cad.key_field_value WHERE cad.configuration_audit_master_id = @configuration_audit_master_id
		AND allow_filtering = 0
    END
	END
END
SET NOCOUNT OFF;
