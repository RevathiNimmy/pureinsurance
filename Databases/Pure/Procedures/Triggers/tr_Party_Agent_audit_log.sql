...
GO
EXECUTE DDLDropTrigger 'tr_Party_Agent_audit_log'
GO
CREATE TRIGGER tr_Party_Agent_audit_log ON dbo.Party_Agent 
...
SELECT @ModuleId=6,@ModuleName='Agent Maintenance', @PKColName = 'party_cnt',@ValueColName='ScreenHierarchy';
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'trading_name','domiciled_for_tax','agency_agreement_date') AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName='New Agent Added')
RETURN
...
SELECT 
   ...
   UPDATE i SET NewValue=a.FieldDescription  FROM configuration_audit_details i 
     ...
     IF @fieldname IN ('date_cancelled', 'agency_next_review_date')
    BEGIN
    UPDATE configuration_audit_details
    SET 
        OldValue = CASE 
            WHEN ISDATE(OldValue) = 1 AND CAST(OldValue AS DATETIME) = '1899-12-29 00:00:00.000' THEN '' 
            ELSE OldValue 
        END,
        NewValue = CASE 
            WHEN ISDATE(NewValue) = 1 AND CAST(NewValue AS DATETIME) = '1899-12-29 00:00:00.000' THEN '' 
            ELSE NewValue 
        END
    WHERE FieldName IN ('date_cancelled', 'agency_next_review_date')
      AND configuration_audit_master_id = @configuration_audit_master_id
    END
    
	IF @fieldname IN ('is_in_transfer_mode')	
	BEGIN
		UPDATE configuration_audit_details SET OldValue = 0 WHERE ISNULL(OldValue,'') = '' AND configuration_audit_master_id = @configuration_audit_master_id AND FieldName = 'is_in_transfer_mode'
		UPDATE configuration_audit_details SET NewValue = 0 WHERE ISNULL(NewValue,'') = '' AND configuration_audit_master_id = @configuration_audit_master_id AND FieldName = 'is_in_transfer_mode'
	END

	IF @fieldname IN ('can_make_live_cashdeposit','use_override_commission_rate','days_allowed','can_make_live_BankGuarantee','produce_agent_renewal_list','is_single_instalment_plan')
	BEGIN
		Delete from configuration_audit_details Where ISNULL(OldValue,'') = '' AND configuration_audit_master_id = @configuration_audit_master_id AND FieldName = @fieldname
	END

	IF @fieldname IN ('payment_method','payment_frequency')
	BEGIN
		Delete from configuration_audit_details Where ISNULL(OldValue,'') = '' AND ISNULL(NewValue,'') = '' AND configuration_audit_master_id = @configuration_audit_master_id AND FieldName = @fieldname
	END
    END
	END
END
