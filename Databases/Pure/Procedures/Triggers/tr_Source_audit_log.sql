...
GO
EXECUTE DDLDropTrigger 'tr_Source_audit_log'
GO
CREATE TRIGGER tr_Source_audit_log ON dbo.Source
...
SELECT @ModuleId = 18, @ModuleName = 'Branch Maintenance' ,@PKColName = 'source_id',@ValueColName='ScreenHierarchy';
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','effective_date','pie_last_updated','caption_id') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I'
    SELECT @FieldValue='New Branch Added'
ELSE If @Type='D'
	SELECT @FieldValue='Branch Deleted'
...
SELECT 
   ...
   UPDATE i SET NewValue=a.FieldDescription  FROM configuration_audit_details i 
    ...
    UPDATE configuration_audit_details SET  OldValue ='' where FieldName = 'is_deleted' AND  configuration_audit_master_id =@configuration_audit_master_id and OldValue ='Re-Open'
    END
	END
END
SET NOCOUNT OFF;
