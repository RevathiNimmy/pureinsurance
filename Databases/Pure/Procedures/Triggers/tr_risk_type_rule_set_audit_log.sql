...
GO
EXECUTE DDLDropTrigger 'tr_risk_type_rule_set_audit_log'
GO
CREATE TRIGGER tr_risk_type_rule_set_audit_log ON dbo.risk_type_rule_set
    FOR INSERT, UPDATE
AS
SET NOCOUNT ON;
...
DECLARE @risk_type_rule_set_id INT;
DECLARE @ScreenHierarchy VARCHAR(500);

SELECT @ModuleId = 27, @ModuleName = 'Risk Type Maintenance' ,@PKColName = 'risk_type_rule_set_id',@ValueColName='ScreenHierarchy';
...
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@IsDeleted = is_deleted ,@risk_type_rule_set_id = risk_type_rule_set_id,@ScreenHierarchy = ScreenHierarchy FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','pie_last_updated','pie_guid','caption_id') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type = 'U' AND EXISTS(SELECT 1 FROM #ins i INNER JOIN #del d ON i.UniqueId = d.UniqueId AND d.is_deleted <> @IsDeleted
...
BEGIN
	...
	VALUES (
			...
			@risk_type_rule_set_id,
			@ScreenHierarchy,
			CASE WHEN @IsDeleted = 1 THEN 'Rule Deleted' ELSE 'Rule Undeleted' END,
			CASE WHEN @IsDeleted = 1 THEN 'Rule Deleted' ELSE 'Rule Undeleted' END
			)
	RETURN
END
