...
GO
EXECUTE DDLDropTrigger 'tr_RI_Model_Line_audit_log'
GO
CREATE TRIGGER tr_RI_Model_Line_audit_log ON dbo.Ri_Model_Line 
...
SELECT @ModuleId=9,@ModuleName='RI Model Maintenance', @PKColName = 'treaty_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@treaty_id=treaty_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@treaty_id=treaty_id FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'pie_last_updated', 'ri_model_line_id', 'ri_model_id', 'pie_guid') AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName='New RIModel Added')
RETURN
...
-- Resolve foreign key description for NewValue
SELECT @isForeignKey = COUNT(*),
...
WHERE fk.TABLE_NAME = @TableName AND fk.COLUMN_NAME = @fieldname
GROUP BY fk.foreign_key_table_name, fk.foreign_key_column_name, fk.foreign_key_value_column_name;

IF ISNULL(@isForeignKey, 0) > 0
BEGIN
    SET @newSql = 'SELECT @NewValue = CONVERT(VARCHAR(100), p.' + @parentValueColumn + ') FROM #ins i INNER JOIN ' + @parentTable + ' p ON i.' + @fieldname + ' = p.' + @parentKeyColumn
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@treaty_id AND FieldName=@fieldname AND (OldValue=@NewValue OR FieldName IN ('treaty_type_id','treaty_id')))     
BEGIN
...
ELSE IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@treaty_id AND FieldName=@fieldname AND OldValue<>@NewValue )  
BEGIN
	UPDATE configuration_audit_details SET Type='U', NewValue=@NewValue WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@treaty_id AND FieldName=@fieldname 
END
ELSE
BEGIN
SELECT 
