...
GO
EXECUTE DDLDropTrigger 'tr_Risk_Type_RI_Values_audit_log'
GO
CREATE TRIGGER tr_Risk_Type_RI_Values_audit_log ON dbo.Risk_Type_RI_Values
...
DECLARE @gis_user_def_header_inds_id1 INT;
DECLARE @Value Numeric(19,4)

SELECT @ModuleId = 27, @ModuleName = 'Risk Type Maintenance' ,@PKColName = 'gis_user_def_header_inds_id1',@ValueColName='ScreenHierarchy';
...
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@gis_user_def_header_inds_id1 = gis_user_def_header_inds_id1, @Value = value FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','caption_id','gis_user_def_header_inds_id1','gis_user_def_header_inds_id2','gis_user_def_header_inds_id3','risk_type_ri_limit_version_id','risk_type_id','risk_type_ri_values_id') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type = 'I' AND EXISTS(SELECT 1 FROM configuration_audit_details cad INNER JOIN configuration_audit_master cam
							ON cad.configuration_audit_master_id = cam.configuration_audit_master_id  WHERE key_field_name = 'gis_user_def_header_inds_id1'
							AND key_field_value = @gis_user_def_header_inds_id1 AND NewValue IS NUll)
BEGIN

	DELETE cad FROM configuration_audit_details cad INNER JOIN configuration_audit_master cam
	ON cad.configuration_audit_master_id = cam.configuration_audit_master_id  WHERE key_field_name = 'gis_user_def_header_inds_id1'
	AND key_field_value = @gis_user_def_header_inds_id1 AND OldValue = @Value AND NewValue IS NUll

	UPDATE cad SET NewValue = @Value FROM configuration_audit_details cad INNER JOIN configuration_audit_master cam
	ON cad.configuration_audit_master_id = cam.configuration_audit_master_id  WHERE key_field_name = 'gis_user_def_header_inds_id1'
	AND key_field_value = @gis_user_def_header_inds_id1 AND OldValue <> @Value AND NewValue IS NUll
