...
GO
EXECUTE DDLDropTrigger 'tr_Earning_Pattern_Usage_audit_log'
GO
CREATE TRIGGER tr_Earning_Pattern_Usage_audit_log ON Earning_Pattern_Usage
    FOR INSERT,DELETE 
AS
SET NOCOUNT ON;
...
SELECT @ModuleId=13,@ModuleName='Lookup Maintenance', @PKColName = 'Rating_Section_type_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@Rating_Section_type_id=Rating_Section_type_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@Rating_Section_type_id=Rating_Section_type_id FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'pie_last_updated', 'Rating_Section_type_id','Earning_Pattern_Usage_id','is_deleted', 'pie_guid') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@Rating_Section_type_id AND FieldName=@fieldname AND (OldValue=@NewValue OR FieldName IN ('peril_group_id')))     
BEGIN
...
ELSE IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@Rating_Section_type_id AND FieldName=@fieldname AND OldValue<>@NewValue )  
BEGIN
	...
	UPDATE configuration_audit_details SET Type='U', NewValue=@NewValue WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@Rating_Section_type_id AND FieldName=@fieldname 
    ELSE
	BEGIN
	DECLARE @SQL1 NVARCHAR(MAX)
		...
		EXEC sp_executesql
			...
			@p_tax_band_id = @Rating_Section_type_id,
