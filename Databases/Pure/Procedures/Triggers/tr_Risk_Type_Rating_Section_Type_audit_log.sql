...
GO
EXECUTE DDLDropTrigger 'tr_Risk_Type_Rating_Section_Type_audit_log'
GO
CREATE TRIGGER tr_Risk_Type_Rating_Section_Type_audit_log ON dbo.Risk_Type_Rating_Section_Type 
...
SELECT  @ModuleId=27,@ModuleName='Risk Type Maintenance', @PKColName = 'rating_section_type_id',@ValueColName='ScreenHierarchy';
...
SELECT @FieldDesc='Rating Section Chosen'

IF @fieldname ='rating_section_type_id' AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@rating_section_type_id AND FieldName=@fieldname AND Type = 'D')     
BEGIN
