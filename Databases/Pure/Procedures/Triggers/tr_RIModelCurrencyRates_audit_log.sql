...
GO
EXECUTE DDLDropTrigger 'tr_RIModelCurrencyRates_audit_log'
GO
CREATE TRIGGER tr_RIModelCurrencyRates_audit_log ON dbo.RIModelCurrencyRates 
...
SELECT @ModuleId=9,@ModuleName='RI Model Maintenance', @PKColName = 'ri_model_id',@ValueColName='ScreenHierarchy';
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','currency_id') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I'
    SELECT @FieldValue='New Conversion Rate Added'
ELSE IF @Type='D'
	SELECT @FieldValue='Conversion Rate Deleted'
