...
GO
EXECUTE DDLDropTrigger 'tr_Primary_Cause_Risk_Type_Group_audit_log'
GO
CREATE TRIGGER tr_Primary_Cause_Risk_Type_Group_audit_log ON Primary_Cause_Risk_Type_Group
    FOR INSERT,DELETE 
AS
SET NOCOUNT ON;
...
SELECT @ModuleId=13,@ModuleName='Lookup Maintenance', @PKColName = 'risk_type_group_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@risk_type_group_id=risk_type_group_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@risk_type_group_id=risk_type_group_id FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'pie_last_updated', 'primary_cause_risk_type_group_id','primary_cause_id', 'pie_guid') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@risk_type_group_id AND FieldName=@fieldname AND (OldValue=@NewValue OR FieldName IN ('risk_type_group_id')))     
BEGIN
...
ELSE IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@risk_type_group_id AND FieldName=@fieldname AND OldValue<>@NewValue )  
BEGIN
	...
	UPDATE configuration_audit_details SET Type='U', NewValue=@NewValue WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@risk_type_group_id AND FieldName=@fieldname 
    ELSE
	BEGIN
	DECLARE @SQL1 NVARCHAR(MAX)
		...
		EXEC sp_executesql
			...
			@p_tax_band_id = @risk_type_group_id,
