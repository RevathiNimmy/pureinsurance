...
GO
EXECUTE DDLDropTrigger 'tr_Account_Executive_Handler_audit_log'
GO
CREATE TRIGGER tr_Account_Executive_Handler_audit_log ON dbo.Party_Handler 
    FOR INSERT, UPDATE 
AS
SET NOCOUNT ON;
...
DECLARE @HandlerType VARCHAR(50);
DECLARE @Party_Cnt VARCHAR(50);
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@Party_Cnt=party_cnt FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@Party_Cnt=party_cnt FROM #ins

SELECT @HandlerType = code FROM Party_Type pt INNER JOIN
Party p  ON pt.party_type_id = p.party_type_id 
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','commisiion_cnt') AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName= (CASE WHEN @HandlerType = 'CO' THEN 'New Account Executive Added' ELSE 'New Account Handler Added' END))
RETURN

DECLARE @FieldValue VARCHAR(50)
IF @Type='I'
    SELECT @FieldValue=CASE WHEN @HandlerType = 'CO' THEN 'New Account Executive Added' ELSE 'New Account Handler Added' END
ELSE IF @Type='D'
	SELECT @FieldValue=CASE WHEN @HandlerType = 'CO' THEN 'Account Executive Deleted' ELSE 'Account Handler Deleted' END
