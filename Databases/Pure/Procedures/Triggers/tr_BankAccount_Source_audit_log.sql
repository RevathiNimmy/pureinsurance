...
GO
EXECUTE DDLDropTrigger 'tr_BankAccount_Source_audit_log'
GO
CREATE TRIGGER tr_BankAccount_Source_audit_log ON dbo.BankAccount_Source
...
SELECT  @ModuleId=19,@ModuleName='Bank Maintenance', @PKColName = 'source_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueId,@SourceId=source_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueId,@SourceId=source_id FROM #ins
...
IF @fieldname ='source_id' AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@SourceId AND FieldName=@fieldname)     
BEGIN
	DELETE configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@SourceId AND FieldName=@fieldname AND Type = 'D'
END
ELSE
BEGIN
SELECT 
