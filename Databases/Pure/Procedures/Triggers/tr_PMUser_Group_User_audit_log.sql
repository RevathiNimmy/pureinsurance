...
GO
EXECUTE DDLDropTrigger 'tr_PMUser_Group_User_audit_log'
GO
CREATE TRIGGER tr_PMUser_Group_User_audit_log ON dbo.PMUser_Group_User 
...
SELECT  @ModuleId=5,@ModuleName='User Maintenance', @PKColName = 'pmuser_group_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=user_id,@UniqueID=UniqueID, @pmuser_group_id = pmuser_group_id FROM #del
ELSE
	SELECT @UserId=user_id,@UniqueID=UniqueID, @pmuser_group_id = pmuser_group_id FROM #ins
...
SELECT @FieldDesc='Selected Groups'
