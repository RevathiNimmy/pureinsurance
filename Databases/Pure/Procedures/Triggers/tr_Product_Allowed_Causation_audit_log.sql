...
GO
EXECUTE DDLDropTrigger 'tr_Product_Allowed_Causation_audit_log'
GO
CREATE TRIGGER tr_Product_Allowed_Causation_audit_log ON dbo.Product_Allowed_Causation 
...
DECLARE @primary_cause_id INT

SELECT  @ModuleId=28,@ModuleName='Product Risk Maintenance', @PKColName = 'primary_cause_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@primary_cause_id=primary_cause_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@primary_cause_id=primary_cause_id FROM #ins
...
SELECT @FieldDesc='Allowed Causation'

IF @fieldname ='primary_cause_id' AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@primary_cause_id AND FieldName=@fieldname AND Type = 'D')     
BEGIN
