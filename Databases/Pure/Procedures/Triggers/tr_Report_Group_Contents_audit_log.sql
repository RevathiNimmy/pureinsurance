...
GO
EXECUTE DDLDropTrigger 'tr_Report_Group_Contents_audit_log'
GO
CREATE TRIGGER tr_Report_Group_Contents_audit_log ON dbo.Report_Group_Contents 
    FOR INSERT, DELETE 
AS
SET NOCOUNT ON;
...
SELECT  @ModuleId=13,@ModuleName='LookUp Maintenance', @PKColName = 'report_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@report_id=report_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@report_id=report_id FROM #ins
...
SELECT @FieldDesc='Report Group Contents'

IF @fieldname ='report_id' AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@report_id AND FieldName=@fieldname)     
BEGIN
