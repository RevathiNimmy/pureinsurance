...
GO
EXECUTE DDLDropTrigger 'tr_InstalmentScheme_audit_log'
GO
CREATE TRIGGER tr_InstalmentScheme_audit_log ON dbo.PFScheme 
...
SELECT @ModuleId=1,@ModuleName='Instalment Scheme Maintenance', @PKColName = 'PFScheme_Id',@ValueColName='ScreenHierarchy';
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','PaymentMethod_cnt') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I'
    SELECT @FieldValue='New Scheme Added'
ELSE IF @Type='D'
	SELECT @FieldValue='Scheme Deleted'
