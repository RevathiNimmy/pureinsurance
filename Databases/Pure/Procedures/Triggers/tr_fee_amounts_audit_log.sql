...
GO
EXECUTE DDLDropTrigger 'tr_fee_amounts_audit_log'
GO
CREATE TRIGGER tr_fee_amounts_audit_log ON dbo.fee_amounts
...
DECLARE @PartyTypeCode VARCHAR(50);

SELECT @PKColName = 'fee_amount_Id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=User_Id,@UniqueID=UniqueID,@PartyCnt=party_cnt FROM #del
ELSE
	SELECT @UserId=User_Id,@UniqueID=UniqueID,@PartyCnt=party_cnt FROM #ins

SELECT @PartyTypeCode = code FROM Party_Type pt INNER JOIN
Party p ON p.party_type_id = pt.party_type_id WHERE party_cnt = @PartyCnt

IF @PartyTypeCode = 'FE'
	SELECT @ModuleId = 4,@ModuleName = 'Fee Maintenance'
ELSE IF @PartyTypeCode = 'EX'
...
IF @fieldname NOT IN ('UniqueId','User_Id','ScreenHierarchy','Timestamp','transaction_type_id','Date') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I'
    SELECT @FieldValue='New Fee Definition Added'
ELSE IF @Type='D'
	SELECT @FieldValue='Fee Definition Deleted'
...
SELECT 
   ...
   UPDATE i SET NewValue=a.FieldDescription  FROM configuration_audit_details i 
	...
	IF @fieldname IN ('transaction_sub_type', 'MakeLiveOptions_id')
	BEGIN
		UPDATE configuration_audit_details
		SET 
			OldValue = CASE WHEN ISNULL(OldValue,'') = '' THEN 'ALL' ELSE OldValue END,
			NewValue = CASE WHEN ISNULL(NewValue,'') = '' THEN 'ALL' ELSE NewValue END
		WHERE 
			FieldName = @fieldname
