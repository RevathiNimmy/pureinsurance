...
GO
EXECUTE DDLDropTrigger 'tr_GIS_user_def_header_inds_audit_log'
GO
CREATE TRIGGER tr_GIS_user_def_header_inds_audit_log ON dbo.GIS_user_def_header_inds
...
DECLARE @gis_user_def_header_inds_id INT
DECLARE @ScreenHierarchy VARCHAR(500)

SELECT @ModuleId=12,@ModuleName='User Maintainable Rate Tables', @PKColName = 'gis_user_def_header_inds_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@gis_user_def_header_inds_id = gis_user_def_header_inds_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@gis_user_def_header_inds_id = gis_user_def_header_inds_id,@ScreenHierarchy = ScreenHierarchy FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'pie_last_updated', 'caption_id', 'effective_date', 'pie_guid', 'gis_user_def_header_inds_id', 'gis_user_def_header_id') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@gis_user_def_header_inds_id AND FieldName=@fieldname AND OldValue=@NewValue )     
BEGIN
...
BEGIN
	UPDATE configuration_audit_details SET Type='U', NewValue=@NewValue WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@gis_user_def_header_inds_id AND FieldName=@fieldname 
END
ELSE IF @Type = 'I'
BEGIN
