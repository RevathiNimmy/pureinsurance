...
GO
EXECUTE DDLDropTrigger 'tr_Credit_Control_Rule_audit_log'
GO
CREATE TRIGGER tr_Credit_Control_Rule_audit_log ON dbo.Credit_Control_Rule 
...
SELECT @ModuleId=2,@ModuleName='Credit Control Maintenance', @PKColName = 'credit_control_rule_id',@ValueColName='ScreenHierarchy';
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'pie_last_updated') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I'
    SELECT @FieldValue='New Rule Added'
ELSE IF @Type='D'
	SELECT @FieldValue='Rule Deleted'
...
SELECT 
   ...
   EXEC (@sql);
     ...
     -- Insert Logic
IF @fieldname = 'use_effective_date'
BEGIN
    IF EXISTS (
        SELECT 1
        FROM configuration_audit_details cd
		join configuration_audit_master cm on cm.configuration_audit_master_id = cd.configuration_audit_master_id
		join Credit_Control_Rule cc on cc.UniqueId = cm.UniqueId
		 
        WHERE cd.configuration_audit_master_id = @configuration_audit_master_id
            AND FieldName = 'use_effective_date'
            AND (
                (
                    TRY_CAST(OldValue AS INT) = 1 AND 
                    TRY_CAST(NewValue AS INT) = 0 AND
                    OldValue IS NOT NULL AND NewValue IS NOT NULL
                )
                OR 
                (
                    TRY_CAST(OldValue AS INT) = 0 AND 
                    TRY_CAST(NewValue AS INT) = 1 AND
                    OldValue IS NOT NULL AND NewValue IS NOT NULL
                )
            )
			AND cc.business_type = 'REN WTG UPDATE'
    )
    AND NOT EXISTS (
        SELECT 1
        FROM configuration_audit_details
        WHERE configuration_audit_master_id = @configuration_audit_master_id
            AND FieldDisplayName = 'Use Renewal Print Date'
    )
    BEGIN
        INSERT INTO configuration_audit_details (
            configuration_audit_master_id, Type, TableName, key_field_name,
            key_field_value, key_field_desc, FieldName, FieldDisplayName,
            OldValue, NewValue
        )
        SELECT TOP 1
            configuration_audit_master_id, Type, TableName, key_field_name,
            key_field_value, key_field_desc, FieldName, 'Use Renewal Print Date',
            TRY_CAST(NewValue AS INT), TRY_CAST(OldValue AS INT) -- Reversing the values
        FROM configuration_audit_details
        WHERE configuration_audit_master_id = @configuration_audit_master_id
            AND FieldName = 'use_effective_date'
            AND (
                (
                    TRY_CAST(OldValue AS INT) = 1 AND TRY_CAST(NewValue AS INT) = 0
                )
                OR 
                (
                    TRY_CAST(OldValue AS INT) = 0 AND TRY_CAST(NewValue AS INT) = 1
                )
            )
    END
END
