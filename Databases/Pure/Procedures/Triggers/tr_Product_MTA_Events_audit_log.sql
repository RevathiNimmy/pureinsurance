...
GO
EXECUTE DDLDropTrigger 'tr_Product_MTA_Events_audit_log'
GO
CREATE TRIGGER tr_Product_MTA_Events_audit_log ON dbo.Product_MTA_Events 
...
DECLARE @mta_event_description_id INT

SELECT  @ModuleId=28,@ModuleName='Product Risk Maintenance', @PKColName = 'mta_event_description_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@mta_event_description_id=mta_event_description_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@mta_event_description_id=mta_event_description_id FROM #ins
...
IF @fieldname ='mta_event_description_id' AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@mta_event_description_id AND FieldName=@fieldname AND Type = 'D')     
BEGIN
