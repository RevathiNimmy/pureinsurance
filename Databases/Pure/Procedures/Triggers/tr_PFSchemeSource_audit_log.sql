...
GO
EXECUTE DDLDropTrigger 'tr_PFSchemeSource_audit_log'
GO
CREATE TRIGGER tr_PFSchemeSource_audit_log ON dbo.PFSchemeSource 
...
SELECT  @ModuleId=1,@ModuleName='Instalment Scheme Maintenance', @PKColName = 'source_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@source_id=source_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@source_id=source_id FROM #ins
...
SELECT @FieldDesc='Branch Added/Removed'
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@source_id AND FieldName=@fieldname)     
BEGIN
