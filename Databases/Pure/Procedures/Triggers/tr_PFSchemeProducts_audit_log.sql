...
GO
EXECUTE DDLDropTrigger 'tr_PFSchemeProducts_audit_log'
GO
CREATE TRIGGER tr_PFSchemeProducts_audit_log ON dbo.PFSchemeProducts 
...
SELECT @ModuleId=1,@ModuleName='Instalment Scheme Maintenance', @PKColName = 'product_id',@ValueColName='ScreenHierarchy';
...
SELECT @FieldDesc='Product Added/Removed'

IF @fieldname ='product_id' AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName='New Scheme Added')
RETURN
