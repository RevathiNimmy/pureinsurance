...
GO
EXECUTE DDLDropTrigger 'tr_Product_Claims_Workflow_audit_log'
GO
CREATE TRIGGER tr_Product_Claims_Workflow_audit_log ON dbo.Product_Claims_Workflow
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','pie_guid','pie_last_updated','generate_claim_notification_doc') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I'
    SELECT @FieldValue='New Claims WorkFlow  Added'
        ...
        BEGIN
            SET @sql = ('
				...
				' LEFT JOIN ' + @parentTable + ' AS pd ON d.' + @fieldname + ' = pd.' + @parentKeyColumn
                        ...
                        + @fieldname
                        + ' IS NULL)'
						+ ' AND ISNULL(i.' + @fieldname + ',0) <> ISNULL(d.' + @fieldname + ',0)'
            );
        END
   EXEC (@sql);
