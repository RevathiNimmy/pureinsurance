...
GO
EXECUTE DDLDropTrigger 'tr_Party_Insurer_audit_log'
GO
CREATE TRIGGER tr_Party_Insurer_audit_log ON dbo.Party_Insurer 
...
SELECT @ModuleId=7,@ModuleName='Reinsurer Maintenance', @PKColName = 'party_cnt',@ValueColName='ScreenHierarchy';
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'insurer_type_id', 'risk_transfer_editable','domiciled_for_tax','terms_of_payment_id') AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND (FieldName='New Agent Added' OR FieldName='New Fee Added' OR FieldName = 'New Re-Insurer Added'))
RETURN
...
SELECT 
   ...
   EXEC (@sql);
    ...
    IF @fieldname = 'agency_number'
	BEGIN
    IF EXISTS (
        SELECT 1
        FROM System_Options so
        WHERE so.option_number = 5005
          AND so.branch_id = 1
          AND so.value = 0
