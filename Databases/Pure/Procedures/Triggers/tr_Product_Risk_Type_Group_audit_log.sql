...
GO
EXECUTE DDLDropTrigger 'tr_Product_Risk_Type_Group_audit_log'
GO
CREATE TRIGGER tr_Product_Risk_Type_Group_audit_log ON dbo.Product_Risk_Type_Group 
...
DECLARE @risk_type_name VARCHAR(200);
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@risk_type_group_id=risk_type_group_id,@product_id = product_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@risk_type_group_id=risk_type_group_id,@product_id = product_id FROM #ins
...
SELECT @FieldDesc='Risk Type Group'
SELECT @risk_type_name = LTRIM(RTRIM(description)) FROM risk_type_group WHERE risk_type_group_id = @risk_type_group_id

IF @fieldname ='risk_type_group_id' AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@product_id AND FieldName=@fieldname)     
BEGIN
	UPDATE configuration_audit_details SET NewValue = @risk_type_name,Type = 'U' WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@product_id AND FieldName=@fieldname 
END
ELSE
BEGIN
SELECT 
		...
		END
			...
			AND key_field_value=@product_id AND FieldName=@fieldname AND OldValue = NewValue
    END
	END
END
SET NOCOUNT OFF;
