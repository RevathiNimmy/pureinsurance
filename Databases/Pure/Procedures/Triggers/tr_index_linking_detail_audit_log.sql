...
GO
EXECUTE DDLDropTrigger 'tr_index_linking_detail_audit_log'
GO
CREATE TRIGGER tr_index_linking_detail_audit_log ON index_linking_detail
    FOR INSERT,DELETE 
AS
SET NOCOUNT ON;
...
DECLARE @index_linking_detail_id INT;
DECLARE @IsDeleted INT;
DECLARE @key_field_desc VARCHAR(255)
SELECT @ModuleId=13,@ModuleName='Lookup Maintenance', @PKColName = 'index_linking_detail_id',@ValueColName='ScreenHierarchy';
...
DELETE FROM #ins WHERE is_deleted = 1;
DELETE FROM #del WHERE is_deleted = 1;

IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@index_linking_detail_id=index_linking_detail_id,@key_field_desc=ScreenHierarchy FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@index_linking_detail_id=index_linking_detail_id,@key_field_desc=ScreenHierarchy FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','index_linking_detail_id','index_linking_id','is_deleted') AND @UniqueID IS NOT NULL
BEGIN	
...
BEGIN
	...
	DELETE configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName
				AND key_field_value=@index_linking_detail_id AND FieldName='Detail Deleted'

	DELETE configuration_audit_details WHERE configuration_audit_master_id<@configuration_audit_master_id AND TableName=@TableName AND Type='D' AND FieldName<>'Detail Deleted'

	IF EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
				AND key_field_value=@index_linking_detail_id AND FieldName=@fieldname AND (OldValue=@NewValue OR FieldName IN ('@index_linking_detail_id')))     
	BEGIN
		DELETE configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
				AND key_field_value=@index_linking_detail_id AND FieldName=@fieldname AND (OldValue=@NewValue OR FieldName IN ('index_linking_detail_id'))
	END
	ELSE IF EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
				AND key_field_value=@index_linking_detail_id AND FieldName=@fieldname AND OldValue<>@NewValue )  
	BEGIN
	...
	UPDATE configuration_audit_details SET Type='U', NewValue=@NewValue WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@index_linking_detail_id AND FieldName=@fieldname 
    ELSE
	BEGIN
	DECLARE @SQL1 NVARCHAR(MAX)
		SET @SQL1 = '
			UPDATE cad
			SET cad.NewValue = t.description, Type= ''U''
			FROM configuration_audit_details cad
			INNER JOIN ' + QUOTENAME(@parentTable) + ' t
				ON cad.key_field_value = t.' + QUOTENAME(@parentKeyColumn) + '
			WHERE cad.configuration_audit_master_id = @p_configuration_audit_master_id
			  AND cad.TableName = @p_TableName
			  AND cad.key_field_value = @p_index_linking_detail_id
			  AND cad.FieldName = @p_fieldname;
		';

		-- Execute with parameters
		EXEC sp_executesql
			@SQL1,
			N'@p_configuration_audit_master_id NVARCHAR(50),
			  @p_TableName NVARCHAR(128),
			  @p_index_linking_detail_idNVARCHAR(128),
			  @p_fieldname NVARCHAR(128)',
			@p_configuration_audit_master_id = @configuration_audit_master_id,
			@p_TableName = @TableName,
			@p_index_linking_detail_id = @index_linking_detail_id,
			@p_fieldname = @fieldname;
		END
	END
	ELSE 
		INSERT INTO configuration_audit_details (configuration_audit_master_id,
                    Type,
                    TableName,
                    key_field_name,
                    key_field_value,
           ...
           SELECT  
				...
				WHERE NOT EXISTS (
					...
					WHERE cad.configuration_audit_master_id = @configuration_audit_master_id
					  AND cad.TableName = @TableName
					  AND cad.key_field_value = @index_linking_detail_id		  
					  AND cad.FieldName = 'New Detail Added' ) AND @NewValue IS NOT NULL;
...
BEGIN
		...
		BEGIN
			...
			SELECT ' + CONVERT(VARCHAR(30), @configuration_audit_master_id) + ',
				   ...
				   ''Detail Deleted''
			FROM #ins AS i
			FULL OUTER JOIN #del AS d ' + @PKCols + '
			WHERE (i.' + @fieldname + ' <> d.' + @fieldname + '
			  ...
			  AND NOT EXISTS (
					SELECT 1
					FROM configuration_audit_details cad
					WHERE cad.configuration_audit_master_id = ' + CONVERT(VARCHAR(30), @configuration_audit_master_id) + '
					  AND cad.TableName = ''' + @TableName + '''
					  AND cad.key_field_name = ''' + @PKColName + '''
					  AND cad.key_field_value = ISNULL(i.' + @PKColName + ', d.' + @PKColName + ')
					  AND cad.FieldName = ''Detail Deleted''
					  AND cad.Type = ''' + @Type + '''
			  );';
...
END		
END
   DELETE configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND OldValue=NewValue
