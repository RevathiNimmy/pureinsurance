...
GO
EXECUTE DDLDropTrigger 'tr_Document_Templates_audit_log'
GO
CREATE TRIGGER tr_Document_Templates_audit_log ON dbo.Document_Template
...
SELECT @ModuleId = 26, @ModuleName = 'Maintain Document Templates' ,@PKColName = 'document_template_id',@ValueColName='ScreenHierarchy';
...
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@IsDeleted = is_deleted ,@Document_Template_Id = document_template_id,@ScreenHierarchy = ScreenHierarchy FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','pie_guid','pie_last_updated','modified_by_id','created_by_id','date_created','last_modified') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I'
    SELECT @FieldValue='New Document Template Added'
