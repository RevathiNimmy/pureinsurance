...
GO
EXECUTE DDLDropTrigger 'tr_User_Authorities_audit_log'
GO
CREATE TRIGGER tr_User_Authorities_audit_log ON dbo.User_Authorities 
...
SELECT @ModuleId=5,@ModuleName='User Maintenance', @PKColName = 'user_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=ModifiedBy,@UniqueID=UniqueID FROM #del
ELSE
	SELECT @UserId=ModifiedBy,@UniqueID=UniqueID FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'pie_last_updated','fee_discount','has_transaction_write_off_authority', 'transaction_write_off_amount',
'has_refund_authority', 'has_transfer_authority', 'is_view_client', 'is_edit_client', 'is_edit_policy', 'is_edit_claim',
'is_edit_finance_plan', 'is_raise_debit', 'is_raise_credit', 'is_raise_fee', 'is_raise_cash', 'is_reverse_transactions',
'is_reverse_allocations', 'is_raise_manual_DID', 'is_edit_scheme_policy', 'is_delete_policy', 'can_reverse_and_replace_transactions',
'is_delete_client', 'transaction_write_off_currency_id','ModifiedBy') AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
        ...
        WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName='New User Added')
RETURN

IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
        INNER JOIN configuration_audit_details cad ON cam.configuration_audit_master_id=cad.configuration_audit_master_id 
        WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName='is_deleted')
RETURN

DECLARE @FieldValue VARCHAR(50)
IF @Type='I'
    SELECT @FieldValue='New User Authority Added'
ELSE IF @Type='D'
	SELECT @FieldValue='User Authority Deleted'
...
SELECT 
   ...
   UPDATE i SET NewValue=a.FieldDescription  FROM configuration_audit_details i 
	...
	UPDATE configuration_audit_details SET OldValue = Case WHEN ISNULL(OldValue,'') = '' Then 0 ELSE OldValue END WHERE FieldName IN ('allow_reverse_allocations', 'allow_receipt_reversal', 'can_override_cheque_Numbers') AND configuration_audit_master_id=@configuration_audit_master_id
	UPDATE configuration_audit_details SET NewValue = Case WHEN ISNULL(NewValue,'') = '' Then 0 ELSE NewValue END WHERE FieldName IN ('allow_reverse_allocations', 'allow_receipt_reversal', 'can_override_cheque_Numbers') AND configuration_audit_master_id=@configuration_audit_master_id
