...
GO
EXECUTE DDLDropTrigger 'tr_Product_audit_log'
GO
CREATE TRIGGER tr_Product_audit_log ON dbo.Product
...
SELECT @ModuleId = 28, @ModuleName = 'Product Risk Maintenance' ,@PKColName = 'product_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@Product_Id = product_id FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','caption_id','pie_guid','pie_last_updated') AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
        ...
        WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName='New Product Added')
RETURN

DECLARE @FieldValue VARCHAR(50)
IF @Type='I'
    SELECT @FieldValue='New Product Added'
...
SELECT 
		...
		DECLARE @FieldDisplayName NVARCHAR(200)

		SET @FieldDisplayName = 
			CASE 
				WHEN @fieldname = 'allow_backdated_mtas' THEN 'Backdated MTA''s Allowed'
				ELSE @FieldDesc 
			END
        SET @FieldDisplayName = REPLACE(@FieldDisplayName, '''', '''''')
        ...
        BEGIN
            SET @sql = ('
                ...
                SELECT ' + convert(varchar(30),@configuration_audit_master_id) + ',''' 
					...
					+ @fieldname + ''','''
                    + @FieldDisplayName + ''''
					+ ',CASE d.' + @fieldname + ' WHEN 0 Then ' + '''ALL''' + ' ELSE CONVERT(VARCHAR(1000),pd.' + @parentValueColumn + ') END'
                    + ',CASE i.' + @fieldname + ' WHEN 0 Then ' + '''ALL''' + ' ELSE CONVERT(VARCHAR(1000),pi.' + @parentValueColumn + ') END' + 
                ' FROM #ins AS i FULL OUTER JOIN #del AS d'
                        + @PKCols + 
				...
				' LEFT JOIN ' + @parentTable + ' AS pd ON d.' + @fieldname + ' = pd.' + @parentKeyColumn
						  --+	' WHERE ISNULL(i.' + @fieldname + ',0) <> ISNULL(d.' + @fieldname + ',0)'
                        + ' WHERE i.' + @fieldname + ' <> d.' + @fieldname 
                        + ' OR (I.' + @fieldname + ' is null and  d.'
                        + @fieldname
                        + ' IS NOT NULL)' 
                        ...
                        + @fieldname
                        + ' IS NULL)' 
						+ ' OR ISNULL(i.' + @fieldname + ',0) <> ISNULL(d.' + @fieldname + ',0)'
            );
        END
   EXEC (@sql);
   ...
   UPDATE i SET NewValue=a.FieldDescription  FROM configuration_audit_details i 
	...
	IF @Type = 'U' AND EXISTS(SELECT 1 FROM configuration_audit_details cad INNER JOIN configuration_audit_master cam ON cam.configuration_audit_master_id = cad.configuration_audit_master_id
								WHERE FieldName = 'is_deleted' AND OldValue <> NewValue AND key_field_value = @Product_Id AND cad.Type = 'U')
	BEGIN
		UPDATE cad 
		SET FieldName = CASE WHEN NewValue = 1 THEN 'Product Deleted' ELSE 'Product Undeleted' END,
		FieldDisplayName = CASE WHEN NewValue = 1 THEN 'Product Deleted' ELSE 'Product Undeleted' END,
		Type = 'D',
		OldValue = NULL,
		NewValue = NULL
		FROM configuration_audit_details cad INNER JOIN configuration_audit_master cam ON cam.configuration_audit_master_id = cad.configuration_audit_master_id
		WHERE FieldName = 'is_deleted' AND OldValue <> NewValue AND key_field_value = @Product_Id
	END
	
			IF EXISTS(SELECT 1 FROM configuration_audit_details WHERE FieldName = 'suppress_reserves' AND (OldValue IS NULL OR NewValue IS NULL) AND configuration_audit_master_id = @configuration_audit_master_id)
			BEGIN
				UPDATE cad SET OldValue = 0  From configuration_audit_details cad WHERE FieldName = 'suppress_reserves' AND  OldValue IS NULL AND configuration_audit_master_id = @configuration_audit_master_id
				UPDATE cad SET NewValue = 0  From configuration_audit_details cad WHERE FieldName = 'suppress_reserves' AND  NewValue IS NULL AND configuration_audit_master_id = @configuration_audit_master_id
			END	
			ELSE IF EXISTS(SELECT 1 FROM configuration_audit_details WHERE FieldName = 'suppress_payments' AND (OldValue IS NULL OR NewValue IS NULL) AND configuration_audit_master_id = @configuration_audit_master_id)
			BEGIN
				UPDATE cad SET OldValue = 0  From configuration_audit_details cad WHERE FieldName = 'suppress_payments' AND  OldValue IS NULL AND configuration_audit_master_id = @configuration_audit_master_id
				UPDATE cad SET	NewValue = 0  From configuration_audit_details cad WHERE FieldName = 'suppress_payments' AND  NewValue IS NULL AND configuration_audit_master_id = @configuration_audit_master_id
			END
			ELSE IF EXISTS(SELECT 1 FROM configuration_audit_details WHERE FieldName = 'suppress_recoveries' AND (OldValue IS NULL OR NewValue IS NULL) AND configuration_audit_master_id = @configuration_audit_master_id)
			BEGIN
				UPDATE cad SET OldValue = 0  From configuration_audit_details cad WHERE FieldName = 'suppress_recoveries' AND  OldValue IS NULL AND configuration_audit_master_id = @configuration_audit_master_id
				UPDATE cad SET	NewValue = 0  From configuration_audit_details cad WHERE FieldName = 'suppress_recoveries' AND  NewValue IS NULL AND configuration_audit_master_id = @configuration_audit_master_id
			END
    END
	END
END
