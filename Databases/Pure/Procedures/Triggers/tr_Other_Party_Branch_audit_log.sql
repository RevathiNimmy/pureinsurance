...
GO
EXECUTE DDLDropTrigger 'tr_Other_Party_Branch_audit_log'
GO
CREATE TRIGGER tr_Other_Party_Branch_audit_log ON dbo.Other_Party_Branch 
...
SELECT  @ModuleId=21,@ModuleName='Other Party Maintenance', @PKColName = 'source_id',@ValueColName='ScreenHierarchy';
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName='New Other Party Added')
RETURN
