...
GO
EXECUTE DDLDropTrigger 'tr_PMUser_Authority_Level_audit_log'
GO
CREATE TRIGGER tr_PMUser_Authority_Level_audit_log ON dbo.PMUser_Authority_Level
...
DECLARE @Authority_Level_Type_Name VARCHAR(100);
DECLARE @Authority_Level_Type_Id INT;
DECLARE @Product_Id INT;
DECLARE @ProductName VARCHAR(200);

SELECT @ModuleId = 23, @ModuleName = 'Maintain User Authority Level' ,@PKColName = 'user_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@Product_Id=product_id FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@Product_Id=product_id,@Authority_Level_Type_Id = authority_level_type_id FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'pie_guid','pie_last_updated') AND @UniqueID IS NOT NULL
BEGIN	
...
SELECT @Authority_Level_Type_Name = description FROM Authority_Level_Type WHERE authority_level_type_id = @Authority_Level_Type_Id
SELECT @ProductName = description FROM Product WHERE product_id = @Product_Id

DECLARE @FieldValue VARCHAR(50)
IF @Type='I'
    SELECT @FieldValue='New User Authority Level Added'
ELSE If @Type='D'
	SELECT @FieldValue='User Authority Level Deleted'
...
SELECT 
   ...
   UPDATE i SET NewValue=a.FieldDescription  FROM configuration_audit_details i 
	...
	IF @Type = 'U' AND EXISTS(SELECT 1 FROM configuration_audit_details cad INNER JOIN configuration_audit_master cam ON cam.configuration_audit_master_id = cad.configuration_audit_master_id
							INNER JOIN #ins i ON i.user_id = cad.key_field_value WHERE FieldName = 'authority_level_type_id'
							AND OldValue <> @Authority_Level_Type_Name AND NewValue IS NULL AND key_field_desc LIKE '%' + @ProductName + '%')
	BEGIN
		DELETE cad FROM configuration_audit_details cad INNER JOIN configuration_audit_master cam ON cam.configuration_audit_master_id = cad.configuration_audit_master_id
							INNER JOIN #ins i ON i.user_id = cad.key_field_value WHERE FieldName = 'authority_level_type_id'
							AND NewValue = @Authority_Level_Type_Name AND OldValue IS NULL AND key_field_desc LIKE '%' + @ProductName + '%'

		UPDATE cad SET NewValue = @Authority_Level_Type_Name FROM configuration_audit_details cad INNER JOIN configuration_audit_master cam ON cam.configuration_audit_master_id = cad.configuration_audit_master_id
		INNER JOIN #ins i ON i.user_id = cad.key_field_value WHERE FieldName = 'authority_level_type_id'
		AND OldValue <> @Authority_Level_Type_Name AND NewValue IS NULL AND key_field_desc LIKE '%' + @ProductName + '%'
	END
END
