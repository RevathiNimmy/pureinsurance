...
GO
EXECUTE DDLDropTrigger 'tr_GIS_user_def_detail_audit_log'
GO
CREATE TRIGGER tr_GIS_user_def_detail_audit_log ON dbo.GIS_user_def_detail
...
SELECT @ModuleId=12,@ModuleName='User Maintainable Rate Tables', @PKColName = 'gis_user_def_detail_id',@ValueColName='ScreenHierarchy';
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'pie_last_updated', 'caption_id') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I'
    SELECT @FieldValue='New LookUp Detail Added'
ELSE IF @Type='D'
	SELECT @FieldValue='LookUp Detail Deleted'
...
SELECT 
        ...
        BEGIN
            SET @sql = ('
				...
				' LEFT JOIN ' + @parentTable + ' AS pd ON d.' + @fieldname + ' = pd.' + @parentKeyColumn
                        + ' WHERE ISNULL(i.' + @fieldname + ',0) <> ISNULL(d.' + @fieldname 
                        + ',0) OR (I.' + @fieldname + ' is null and  d.'
                        + @fieldname
                        + ' IS NOT NULL)' 
                        + ' OR (I.' + @fieldname + ' IS NOT NULL AND  D.' 
                        + @fieldname
   ...
   UPDATE i SET OldValue=a.description  FROM configuration_audit_details i 
            INNER JOIN GIS_user_def_detail g ON g.gis_user_def_detail_id=i.key_field_value
			INNER JOIN GIS_user_def_header_inds a ON a.gis_user_def_header_id= g.gis_user_def_header_id AND CAST(a.gis_user_def_header_inds_id AS VARCHAR)=i.OldValue WHERE i.FieldName='GIS_user_def_header_inds_id' AND i.configuration_audit_master_id=@configuration_audit_master_id
   UPDATE i SET NewValue=a.description  FROM configuration_audit_details i 
            INNER JOIN GIS_user_def_detail g ON g.gis_user_def_detail_id=i.key_field_value
			INNER JOIN GIS_user_def_header_inds a ON a.gis_user_def_header_id= g.gis_user_def_header_id AND CAST(a.gis_user_def_header_inds_id AS VARCHAR)=i.NewValue WHERE i.FieldName='GIS_user_def_header_inds_id' AND i.configuration_audit_master_id=@configuration_audit_master_id
