...
GO
EXECUTE DDLDropTrigger 'tr_Risk_Type_RI_Model_Usage_audit_log'
GO
CREATE TRIGGER tr_Risk_Type_RI_Model_Usage_audit_log ON dbo.Risk_Type_RI_Model_Usage
...
SELECT @ModuleId = 27, @ModuleName = 'Risk Type Maintenance' ,@PKColName = 'risk_type_ri_model_usage_cnt',@ValueColName='ScreenHierarchy';
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','pie_last_updated','pie_guid') AND @UniqueID IS NOT NULL
BEGIN	
...
IF EXISTS(SELECT NULL FROM configuration_audit_master cam 
		...
		WHERE cam.configuration_audit_master_id=@configuration_audit_master_id AND FieldName='New Risk Added')
RETURN

DECLARE @FieldValue VARCHAR(50)
IF @Type='I'
    SELECT @FieldValue='New Model Added'
ELSE If @Type='D'
	SELECT @FieldValue='Model Deleted'
