...
GO
EXECUTE DDLDropTrigger 'tr_ComissionMaintenance_audit_log'
GO
CREATE TRIGGER tr_ComissionMaintenance_audit_log ON commission_arrangement 
...
SELECT @ModuleId=3,@ModuleName='Commission Maintenance', @PKColName = 'commission_arrangement_id',@ValueColName='ScreenHierarchy';
...
IF @PKCols IS NULL
BEGIN
    RAISERROR('No primary key on table %s', 16, -1, @TableName);
    RETURN;
END
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy')
BEGIN	
...
IF @Type='I'
    SELECT @FieldValue='New Rates Added'
ELSE IF @Type='D'
	SELECT @FieldValue='Rate Deleted'
If @Type IN ('I','D')
BEGIN
            SET @sql = ('
