...
GO
EXECUTE DDLDropTrigger 'tr_Peril_Type_Usage_audit_log'
GO
CREATE TRIGGER tr_Peril_Type_Usage_audit_log ON Peril_Type_Usage
    FOR INSERT,DELETE 
AS
SET NOCOUNT ON;
...
SELECT @ModuleId=13,@ModuleName='Lookup Maintenance', @PKColName = 'peril_type_id',@ValueColName='ScreenHierarchy';
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueID,@peril_type_id=peril_type_id,@key_field_desc=ScreenHierarchy FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueID,@peril_type_id=peril_type_id,@key_field_desc=ScreenHierarchy FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'pie_last_updated', 'peril_group_id', 'pie_guid') AND @UniqueID IS NOT NULL
BEGIN	
...
EXEC sp_executesql @newSql, N'@NewValue VARCHAR(100) OUTPUT', @NewValue OUTPUT
	...
	DELETE configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName
				AND key_field_value=@peril_type_id  AND FieldName='Record Deleted'
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@peril_type_id AND FieldName=@fieldname AND (OldValue=@NewValue OR FieldName IN ('peril_group_id')))     
BEGIN
	DELETE configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@peril_type_id AND FieldName=@fieldname AND (OldValue=@NewValue OR FieldName IN ('peril_group_id'))
END
ELSE IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@peril_type_id AND FieldName=@fieldname AND OldValue<>@NewValue )  
BEGIN
	...
	UPDATE configuration_audit_details SET Type='U', NewValue=@NewValue WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@peril_type_id AND FieldName=@fieldname 
    ELSE
	BEGIN
	DECLARE @SQL1 NVARCHAR(MAX)
		SET @SQL1 = '
			UPDATE cad
			SET cad.NewValue = t.description
           ...
           SELECT  
				...
				WHERE NOT EXISTS (
					...
					WHERE cad.configuration_audit_master_id = @configuration_audit_master_id
					  ...
					  AND cad.key_field_value = @peril_type_id			  
