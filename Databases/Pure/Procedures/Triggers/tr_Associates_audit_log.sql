...
GO
EXECUTE DDLDropTrigger 'tr_Associates_audit_log'
GO
CREATE TRIGGER tr_Associates_audit_log ON dbo.Party_Relationship 
    FOR INSERT, DELETE 
AS
SET NOCOUNT ON;
...
DECLARE @AssociatedShortName varchar(100)
...
IF @Type='D'
	SELECT @UserId=UserId,@UniqueID=UniqueId,@party_cnt=party_cnt,@relation_cnt=relation_cnt FROM #del
ELSE
	SELECT @UserId=UserId,@UniqueID=UniqueId,@party_cnt=party_cnt,@relation_cnt=relation_cnt FROM #ins
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy','party_cnt','description','commission_transaction') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @fieldname IN ('relationship_type_id')
BEGIN
	SELECT @NewValue = description FROM Relationship_Type WHERE relationship_type_id = @NewValue
END
ELSE IF @fieldname IN ('relation_cnt')
BEGIN
	SELECT @NewValue = name from Party where party_cnt= @NewValue
END
END

SELECT @AssociatedShortName = shortname from Party where party_cnt= @relation_cnt
...
IF @Type='I' AND EXISTS(SELECT 1 FROM configuration_audit_details WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@party_cnt AND FieldName=@fieldname AND key_field_desc LIKE '%' + LTRIM(RTRIM(@AssociatedShortName)) + '%' AND OldValue=@NewValue)     
BEGIN
...
BEGIN
	UPDATE configuration_audit_details SET Type='U', NewValue=@NewValue WHERE configuration_audit_master_id=@configuration_audit_master_id AND TableName=@TableName 
			AND key_field_value=@party_cnt AND key_field_desc LIKE '%' + LTRIM(RTRIM(@AssociatedShortName)) + '%' AND FieldName=@fieldname 
END
ELSE
BEGIN
SELECT 
