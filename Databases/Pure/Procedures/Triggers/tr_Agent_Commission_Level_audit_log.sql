...
GO
EXECUTE DDLDropTrigger 'tr_Agent_Commission_Level_audit_log'
GO
CREATE TRIGGER tr_Agent_Commission_Level_audit_log ON dbo.Agent_Commission_Level 
...
SELECT @ModuleId=6,@ModuleName='Agent Maintenance', @PKColName = 'agent_commission_level_id',@ValueColName='ScreenHierarchy';
...
IF @fieldname NOT IN ('UniqueId','UserId','ScreenHierarchy', 'Agent_Commission_Level_Id', 'Party_agent_cnt') AND @UniqueID IS NOT NULL
BEGIN	
...
IF @Type='I'
    SELECT @FieldValue='New Agent Commission Level Added'
ELSE IF @Type='D'
	SELECT @FieldValue='Agent Commission Level Deleted'
