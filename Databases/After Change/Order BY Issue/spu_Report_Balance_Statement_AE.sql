...
EXECUTE DDLDropProcedure 'spu_Report_Balance_Statement_AE'
GO 
...
-- eck160702 Inlude Account Handler Option      
CREATE procedure spu_Report_Balance_Statement_AE      
    @end_date datetime,      
    @branch_id integer,      
...
DECLARE      
    ...
    @iACcountExecCnt integer, --eck160702      
    @sAccount_exec varchar(20) --eck160702      
...
-- eck160702 - Include account executive parameter      
    IF @account_exec = 'ALL' BEGIN      
        SELECT @account_exec = NULL      
    END      
      
    SELECT @saccount_exec = ISNULL(@account_exec, '')      
...
--eck160702      
    extra_int3, /* Account_exec.party_cnt */      
    extra_char3, /* Account_exec.shortname */      
--eck160702end      
...
--MKW130603      
    (CASE @InsuranceFileAccountExec WHEN '1' THEN MAX(ISNULL(Account_exec2.Party_cnt, ISNULL(Account_exec.Party_cnt, 0))) ELSE MAX(ISNULL(Account_exec.Party_cnt, 0)) end),      
    (CASE @InsuranceFileAccountExec WHEN '1' THEN MAX(ISNULL(Account_exec2.shortname, ISNULL(Account_exec.shortname, 'No Account Exec'))) ELSE MAX(ISNULL(Account_exec.shortname, 'No Account Exec')) end),      
    --MAX(ISNULL(Account_exec.Party_cnt, 0)),      
    --MAX(ISNULL(Account_Exec.shortname, 'No Account Exec')),      
--MKW130603      
...
--eck140602      
    ...
    LEFT OUTER JOIN Party Account_exec      
        ON Party.consultant_cnt = Account_exec.Party_cnt      
      
    AND (@sAccount_exec = '' OR (@sAccount_exec <> '' AND Account_exec.shortname = @sAccount_exec))      
--eck160702      
--MKW130603      
    LEFT OUTER JOIN Party Account_exec2      
        ON Insurance_File.account_executive_cnt = Account_exec2.Party_cnt      
    AND (@sAccount_exec = '' OR (@sAccount_exec <> '' AND Account_exec2.shortname = @sAccount_exec))      
--MKW130603      
   ...
   AND (      
    ...
    BEGIN      
        ...
        END      
    END      
--eck160702 remove "No Account EXECUTE " lines where type selected      
DELETE Report_Transaction      
    WHERE @sAccount_exec <> ''      
    AND extra_int3 = 0      
--      
...
--eck160702 include account EXECUTE in criteria      
    FETCH NEXT FROM cAccount INTO @iAccountID, @sAccountName, @iACcountExecCnt      
...
--eck160702 include account EXECUTE in criteria      
        SELECT @nAccountTotal = SUM(round(RT.Amount,2))      
            ...
            AND RT.extra_int3 = @iACcountExecCnt      
        ...
        AND extra_int3 = @iAccountExecCnt      
...
--eck160702      
        RT.extra_int3 Account_exec_cnt,      
        RT.extra_char3 Account_exec_shortname      
--eck160702End      
    FROM Report_Transaction RT      
        JOIN Account Account      
