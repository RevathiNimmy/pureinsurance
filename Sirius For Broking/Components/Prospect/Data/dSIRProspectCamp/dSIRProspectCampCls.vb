...
Imports SharedFiles
<System.Runtime.InteropServices.ProgId("SIRProspectCamp_NET.SIRProspectCamp")> _
Public NotInheritable Class SIRProspectCamp
    Implements IDisposable
    ' ***************************************************************** '
    ' Class Name: SIRProspectCamp
    '
    ' Date: 29/04/1999
    '
    ' Description: Describes the SIRProspectCamp attributes.
    '
    ' Edit History:
    ' ***************************************************************** '
    ...
    Private Const ACClass As String = "SIRProspectCamp"
    ...
    Public Property CampaignID() As Byte
        Get
    ...
    Public Function Add() As Integer
        ...
        Try
            ...
            With m_oDatabase
                ...
                ' Add PrimaryKey as INPUT parameter
    ...
    '
    ' Description: Selects the required SIRProspectCamp
    '
    ' ***************************************************************** '
    ...
    '
    ' Description: Sets the supplied SIRProspectCamp properties from a database
    '              record.
    ' ***************************************************************** '
    ...
    Public Function SetPropertiesFromDB(ByRef oFields As DataRow) As Integer
        ...
        Try
            ...
            With oFields
                ...
                RecordNo = gPMFunctions.NullToLong(oFields("record_no"))

                If Convert.IsDBNull(oFields("campaign_id")) Or IsNothing(oFields("campaign_id")) Then
    ...
    '
    ' Description: Gets all records for the passed party_cnt
    '
    ' ***************************************************************** '
    Public Function GetRecordsForParty(ByVal v_vPartyCnt As Object, ByRef r_vRecords As Object) As Integer
        ...
        Try
            ...
            ' Construct the SQL
            sSQL = "SELECT record_no FROM prospect_campaign WHERE " & _
                   "party_cnt = {party_cnt}"
            ...
            ' Perform the SQL
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=sSQL, sSQLName:="GetRecordsForParty", bStoredProcedure:=False, vResultArray:=vResultArray)
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If
    ...
    Public Function GetCampaignDetails(ByVal v_vPartyCnt As Object, ByVal v_vRecordNo As Object, ByVal v_vEffectiveDate As Date, ByRef r_vCampaignID As Object, ByRef r_vDescription As Object, ByRef r_vCampaignDate As Object) As Integer
        ...
        Try
            ...
            'Of course, we ask the question "Why use {party_cnt} when you could just use
            'v_vPartyCnt?"  There must be a very good reason, so we'll leave it...

            ' Construct the sql
            sSQL = "SELECT c.campaign_id, p.caption, c.campaign_date " & _
                    "FROM campaign c, pmcaption p WHERE " & _
                    "c.campaign_id = " & _
                    "(SELECT campaign_id FROM prospect_campaign " & _
                    "WHERE party_cnt = {party_cnt} " & _
                    "AND record_no = {record_no}) " & _
                    "AND p.caption_id = c.caption_id " & _
            ...
            m_lReturn = m_oDatabase.Parameters.Add(sName:="record_no", vValue:=CStr(CInt(v_vRecordNo)), idirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            ' add language
            m_lReturn = m_oDatabase.Parameters.Add(sName:="language_id", vValue:=CStr(m_iLanguageID), idirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMInteger)
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            ' perform the SQL
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=sSQL, sSQLName:="GetCampaignDetails", bStoredProcedure:=False, vResultArray:=vResultArray)
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            ' If its in array then return the data
            If Information.IsArray(vResultArray) Then


                r_vCampaignID = vResultArray(0, 0)


                r_vDescription = vResultArray(1, 0)


                r_vCampaignDate = vResultArray(2, 0)
            Else
                Return gPMConstants.PMEReturnCode.PMNotFound
            End If
    ...
    Private Function AddInputParam() As Integer
        ...
        With m_oDatabase
            ...
            If CampaignID < 1 Then
                ...
                m_lReturn = .Parameters.Add(sName:="campaign_id", vValue:=DBNull.Value, iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)
            Else
                m_lReturn = .Parameters.Add(sName:="campaign_id", vValue:=CStr(CampaignID), idirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)
            End If
    ...
    Private Function AddKeyInputParam() As Integer
        ...
        With m_oDatabase
            ...
            m_lReturn = .Parameters.Add(sName:="record_no", vValue:=CStr(RecordNo), idirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)
    ...
    Private Function AddKeyOutputParam() As Integer
        ...
        With m_oDatabase
            ...
            m_lReturn = .Parameters.Add(sName:="record_no", vValue:=CStr(0), idirection:=gPMConstants.PMEParameterDirection.PMParamOutput, iDataType:=gPMConstants.PMEDataType.PMLong)
    ...
    Private Function GetNewPrimaryKeyID() As Integer
        ...
        With m_oDatabase
            ...
            RecordNo = gPMFunctions.NullToLong(.Parameters.Item("record_no").Value)
