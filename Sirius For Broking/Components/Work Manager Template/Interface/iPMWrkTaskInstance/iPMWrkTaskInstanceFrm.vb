...
Partial Friend Class frmInterface
    ...
    Private m_oBusiness As bPMWrkTaskInstanceTemp.FormClass
    ...
    Public WriteOnly Property UseSuppliedTaskTemplateDetails() As Boolean
        Set(ByVal Value As Boolean)
            m_bUseSuppliedTaskTemplateDetails = Value
        End Set
    End Property

    Public WriteOnly Property DoNotSaveTaskTemplateToDB() As Boolean
        Set(ByVal Value As Boolean)
            m_bDoNotSaveTaskTemplateToDB = Value
        End Set
    End Property
    'PN15774
    ...
    Public Function Initialise() As Integer
        ...
        Try
            ...
            'Milan TODO:Revoke changes  after bObjectmanager latest is build
            'm_lReturn = g_oObjectManager.GetInstance(temp_m_oBusiness, "bPMWrkTaskInstanceTemp.FormClass", vInstancemanager:=gPMConstants.PMGetViaClientManager)
            m_lReturn = g_oObjectManager.GetInstance(temp_m_oBusiness, "bPMWrkTaskInstanceTemp.FormClass", gPMConstants.PMGetViaClientManager)
            ...
            'TODO:MILAN: Moving the call to Load_Renamed to Form_Load
    ...
    Public Function Load_Renamed() As Integer
        ...
        Try
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                ...
                Return result
            End If
            'Added the code from frmInterface_Load
            'start
    ...
    Private Function BusinessToInterface() As Integer
        Dim result As Integer = 0
        Dim lTaskID, lGroupID As Integer
        ...
        Try
            ...
            'DJM 19/02/2003 PN10480 : Copied fix from 1.8.5 issue 4697.
            If Not m_bUseSuppliedTaskTemplateDetails Then
            ...
            ' Need to be a bit more intelligent here.........
            ' Different id's when built on a blank database
            ' Default to Retrieve Client

            m_lReturn = m_oBusiness.GetDefaultTask(r_lTaskID:=lTaskID, r_lGroupID:=lGroupID)


            If m_lPMWrkTaskId = 0 Then
                m_lPMWrkTaskId = lTaskID
            End If

            If m_lPMWrkTaskGroupId <> 0 Then
                cboWrkTaskGroup.ItemId = m_lPMWrkTaskGroupId
            Else
                cboWrkTaskGroup.ItemId = lGroupID
            End If
            ...
            '    m_lReturn = m_oFormFields.FormatControl(txtCustomer, CVar(m_sCustomer))

            'ED 30102002 - Uncommented to display existing description
            m_lReturn = m_oFormFields.FormatControl(txtDescription, m_sDescription)

            'MKW220804 PN12591 START - Display customer field rather than party (shortcode)
            '    If Len(PolicyRef) Then
            '        m_lReturn = m_oFormFields.FormatControl(txtCustomer, CVar(Party) & " - " & CVar(PolicyRef))
            '    Else
            '        m_lReturn = m_oFormFields.FormatControl(txtCustomer, CVar(Party))
            '    End If
            If CStr(m_sCustomer).Trim().Length = 0 Then
                m_sCustomer = m_sParty
            End If
            If Strings.Len(PolicyRef) Then
                m_lReturn = m_oFormFields.FormatControl(txtCustomer, CStr(Customer) & " - " & CStr(PolicyRef))
            Else
                m_lReturn = m_oFormFields.FormatControl(txtCustomer, Customer)
            End If
            'MKW220804 PN12591 END

            'm_lReturn = m_oFormFields.FormatControl(txtDescription, CVar(PolicyRef))

            '    If Len(txtCustomer.Text) Then
            '        txtCustomer.Enabled = False
            '    End If

            '    If Len(txtDescription.Text) Then
            ...
            'TODO:MILAN::
            'If albTaskImages.IconsLarge.Count > 0 Then
            '    If DisplayIcon < 1 Or DisplayIcon > albTaskImages.IconsLarge.Count Then
            '        DisplayIcon = 1
            '    End If
            '    'developer guide no 10. 
            '    'imgIcon1.Image = albTaskImages.IconsLarge.Item(DisplayIcon).Picture.Handle
            '    imgIcon1.Image = albTaskImages.IconsLarge.Item(DisplayIcon).Picture
            'End If
            If Not Information.IsNothing(albTaskImages.Image) Then
                imgIcon1.Image = albTaskImages.Image
            End If
    ...
    Private Function InterfaceToBusiness() As Integer
        ...
        Try
            ...
            'DJM 19/02/2003 PN10480 : Copied fix from 1.8.5 issue 4697.
            If m_bDoNotSaveTaskTemplateToDB Then
                Return result
            End If

            Select Case m_sCallingAppName
                Case "Raised Document", "Raised Task", "Authorise Claim Payment Task"
                    bInstance = True
            ...
            Select Case (Task)
                ...
                Case gPMConstants.PMEComponentAction.PMAdd
                    ...
                    If Information.IsArray(m_vTaskInstKeyArray) Then
                        ...
                        m_lReturn = m_oBusiness.CreateNew(r_lpmwrktaskinstancecnt:=m_lPMWrkTaskInstanceCnt, v_lPMWrkTaskGroupID:=m_lPMWrkTaskGroupId, v_lPMWrkTaskID:=m_lPMWrkTaskId, v_scustomer:=m_sCustomer, v_dttaskduedate:=m_dtTaskDueDate, v_lpmusergroupid:=m_lPmuserGroupID, v_iUserId:=m_iUserID, v_sDescription:=m_sDescription, v_itaskstatus:=m_iTaskStatus, v_iisurgent:=m_iIsUrgent, v_dtdatecreated:=m_dtDateCreated, v_icreatedbyid:=m_iCreatedByID, v_vKeyArray:=m_vTaskInstKeyArray, v_bIsNewInstance:=bInstance)
                    Else
                        'Use Now() for Created Date

                        m_lReturn = m_oBusiness.CreateNew(r_lpmwrktaskinstancecnt:=m_lPMWrkTaskInstanceCnt, v_lPMWrkTaskGroupID:=m_lPMWrkTaskGroupId, v_lPMWrkTaskID:=m_lPMWrkTaskId, v_scustomer:=m_sCustomer, v_dttaskduedate:=m_dtTaskDueDate, v_lpmusergroupid:=m_lPmuserGroupID, v_iUserId:=m_iUserID, v_sDescription:=m_sDescription, v_itaskstatus:=m_iTaskStatus, v_iisurgent:=m_iIsUrgent, v_dtdatecreated:=DateTime.Now, v_icreatedbyid:=m_iCreatedByID, v_bIsNewInstance:=bInstance)

                        If bInstance Then

                            m_lReturn = m_oBusiness.CreateKeys(v_lTaskInstanceID:=m_lPMWrkTaskInstanceCnt, v_lPartyCnt:=PartyCnt, v_sShortName:=Party, v_sPartyType:=PartyType, v_sResolvedName:=ResolvedName, v_lInsuranceFileCnt:=InsuranceFileCnt, v_lInsuranceFolderCnt:=m_lInsuranceFolderCnt)
                        End If
                ...
                Case gPMConstants.PMEComponentAction.PMEdit
                    ...
                    m_lReturn = m_oBusiness.AmendDetails(v_lPMWrkTaskInstanceCnt:=m_lPMWrkTaskInstanceCnt, v_scustomer:=m_sCustomer, v_dttaskduedate:=m_dtTaskDueDate, v_sDescription:=m_sDescription, v_iisurgent:=m_iIsUrgent, v_dtlastmodified:=m_dtLastModified, v_imodifiedbyid:=m_iModifiedByID)
    ...
    Private Function SetFieldValidation() As Integer
        ...
        Try
            ...
            ' {* GENERATED CODE (Begin) *}
            If m_sCallingAppName = "Raised Task" Then
            ...
            Else
                m_lReturn = m_oFormFields.AddNewFormField(ctlControl:=txtDueDate, lFieldType:=gPMConstants.PMEDataType.PMDate, lFormat:=gPMConstants.PMEFormatStyle.PMFormatDateLong, lMandatory:=IIf((Task = gPMConstants.PMEComponentAction.PMEdit), gPMConstants.PMEMandatoryStatus.PMMandatory, gPMConstants.PMEMandatoryStatus.PMNonMandatory))
            ...
            ' MS290601 - If we are adding a template, we don't want these to be mandatory.
            If m_sCallingAppName = "Raised Task" Then
            ...
            Else
                m_lReturn = m_oFormFields.AddNewFormField(ctlControl:=txtCustomer, lFieldType:=gPMConstants.PMEDataType.PMString, lFormat:=gPMConstants.PMEFormatStyle.PMFormatString, lMandatory:=IIf((Task = gPMConstants.PMEComponentAction.PMEdit), gPMConstants.PMEMandatoryStatus.PMMandatory, gPMConstants.PMEMandatoryStatus.PMNonMandatory))
            End If
            ...
            Else
                m_lReturn = m_oFormFields.AddNewFormField(ctlControl:=txtDescription, lFieldType:=gPMConstants.PMEDataType.PMString, lFormat:=gPMConstants.PMEFormatStyle.PMFormatString, lMandatory:=IIf((Task = gPMConstants.PMEComponentAction.PMEdit), gPMConstants.PMEMandatoryStatus.PMMandatory, gPMConstants.PMEMandatoryStatus.PMNonMandatory))
            End If
            ...
            'start ISS853 JAS 21/02/03
            'Added mandatory fields.
            'Also changed names of accompanying labels so that
            'they will be found and emboldened by iPMFormcontrol
            '(lblWrkTask, lblTaskUserGroup)
            m_lReturn = m_oFormFields.AddNewFormField(ctlControl:=cboWrkTask, lFieldType:=gPMConstants.PMEDataType.PMString, lFormat:=gPMConstants.PMEFormatStyle.PMFormatString, lMandatory:=gPMConstants.PMEMandatoryStatus.PMMandatory)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            m_lReturn = m_oFormFields.AddNewFormField(ctlControl:=cboTaskUserGroup, lFieldType:=gPMConstants.PMEDataType.PMString, lFormat:=gPMConstants.PMEFormatStyle.PMFormatString, lMandatory:=gPMConstants.PMEMandatoryStatus.PMMandatory)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If
            'END ISS853 JAS 21/02/03
    ...
    '
    'DJM 12/03/2004 : If we are editting then we are creating a new task after raising a document.
    '                 Don't use the name and date of the person who added the task to the document.
    '                 The user who raised the document should be the created user and now the created time.
    ' ***************************************************************** '
        ...
        Try
            ...
            Else
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    ...
                    'DC171203 -P{N9124 -added new message
                    sMsg = "Task Template Details associated with this Document Template is Missing." & Strings.Chr(13) & Strings.Chr(10) & _
                           "Please contact System Administrator to setup the Task Template."

                    iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:=sMsg, vApp:=ACApp, vClass:=ACClass, vMethod:="BusinessToData", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                    Return result
                End If

                ' MSS280601 If we are creating an instance, we need to find the initial
                ' period that the user set. This is not held in the database so if
                ' we do a comparison of the created date and the due date, we can find
                ' the selected period and alter the date as from todays date.
                If Task = gPMConstants.PMEComponentAction.PMEdit Then
                    m_dtTaskDueDate = DateTime.Now.AddDays(DateAndTime.DateDiff("d", dtDateCreated, m_dtTaskDueDate, FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1))
                End If

                'DJM 12/03/2004 : Double check that the correct values have been passed in.
                If m_dtDateCreated = #12/30/1899# Then
                    m_dtDateCreated = dtDateCreated
                End If
                If m_iCreatedByID = 0 Then
                    m_iCreatedByID = iCreatedByID
                End If
    ...
    Private Function InterfaceToData() As Integer
        Dim result As Integer = 0
        Dim dtDueDate, dtDueTime As Date
        ...
        Try
            ...
            dtDueTime = CDate(m_oFormFields.UnformatControl(txtDueTime))
    ...
    Private Function SetInterfaceDefaults() As Integer
        ...
        Try
            ...
            cboDueDate_NewIndex = cboDueDate.Items.Add(New VB6.ListBoxItem("", 0))
            ...
            For lCount As Integer = 0 To vArray.GetUpperBound(1)
                ...
                cboDueDate_NewIndex = cboDueDate.Items.Add(New VB6.ListBoxItem(CStr(vArray(0, lCount)), CInt(vArray(1, lCount))))
            ...
            Select Case Task
                ...
                Case gPMConstants.PMEComponentAction.PMAdd
                    cboWrkTaskGroup.Enabled = (PMWrkTaskGroupId = 0)
                    cboWrkTask.Enabled = (PMWrkTaskId = 0)
                    cboWrkTask.ShowRequiresKeys = False
                    cboDueDate.Enabled = True
                    txtDueDate.Enabled = True
                    txtDueTime.Enabled = True

                    'PN-69373 code start, changed by chandra shekhar on 23rd March 2010
                    ...
                    Select Case m_sCallingAppName
                        Case "Raised Task", "Document Task", "Authorise Claim Payment Task"
                    ...
                    If m_lStatus = gPMConstants.PMEReturnCode.PMCancel Then
                        ...
                        If m_bDirty Then
                            ...
                            If iMsgResult = System.Windows.Forms.DialogResult.No Then
                                ...
                                result = gPMConstants.PMEReturnCode.PMFalse
                            Else
                                ' Reset the cnt just in case we pass one back
    ...
    Private Sub cboWrkTaskGroup_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cboWrkTaskGroup.Click
        ...
        ' PW220502 - Need to ensure itemid is not 0 if form is loading, otherwise
        ' other dependent combos will not be correctly populated.
        If Not m_bFormLoaded And cboWrkTaskGroup.ItemId = 0 Then
            If cboWrkTaskGroup.ListIndex + 1 <= cboWrkTaskGroup.ListCount Then
                cboWrkTaskGroup.ItemId = cboWrkTaskGroup.ListIndex + 1
            End If
        End If
    ...
    Private Sub frmInterface_Load(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Load
        ...
        With uctPMResizer1
            ...
            .FormMinWidth = 9405
        End With
        'SUMEET: Shifted the following code to Load_Renamed
        'm_lReturn = SetFieldValidation()
        'If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
        '	Status = gPMConstants.PMEReturnCode.PMFalse
        '	iPMFunc.SetMousePointer(gPMConstants.PMEMousePointerStatus.PMMouseNormal)
    ...
    Private Sub cmdOK_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cmdOK.Click
        ...
        Try
            ...
            ' MS280601 - We only need a client and description when we are creating an instance
            If (Task = gPMConstants.PMEComponentAction.PMEdit) Or (m_sCallingAppName = "Raised Task") Then
