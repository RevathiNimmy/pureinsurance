...
<ToolboxData("<{0}:PureInsAddOutputControl runat=""server""> </{0}:PureInsAddOutputControl>")>
Public NotInheritable Class PureInsAddOutputControl 
...
#Region "Fields"
    ...
    Private ctrlOutput(maxInputs) As LabelControlPair
    Private ctrlOutputNames(maxInputs) As String
    Private ctrlOutputValues(maxInputs) As String

    Private Const controlPrefix As String = "ctlPureAddOutput"
    Private Const labelPrefix As String = "lblPureAddOutput"
...
#Region "Properties"
    ...
    Public Overrides Property RuleLine As EngineCommon.IRuleLine
        Get
            ...
            'Rate
            'ruleLineObj.Var3 = GetPostedControlValue(ctrlRate.Control)

            ruleLineObj.Var3 = "" 'reset this as we don't want to keep appending to it
            For i As Integer = 0 To ctrlOutput.GetUpperBound(0)
                ruleLineObj.Var3 = ruleLineObj.Var3 + GetPostedControlValue(ctrlOutput(i).Control)
            ...
            For i As Integer = 0 To ctrlOutputNames.GetUpperBound(0)
                ruleLineObj.Parameters = ruleLineObj.Parameters + ctrlOutputNames(i).ToString
...
#End Region
    ...
    Public Overrides Sub BuildUI()
        ...
        ctrlOutputNames(0) = "Decline_reason"
        ctrlOutputNames(1) = "Refer_reason"
        ctrlOutputNames(2) = "policy_rating_section"
        ctrlOutputNames(3) = "risk_rating_section"
        ctrlOutputNames(4) = "Rate"
        ctrlOutputNames(5) = "rate_type_id"
        ctrlOutputNames(6) = "state_id"
        ctrlOutputNames(7) = "country_id"
        ctrlOutputNames(8) = "Ipt_override"
        ctrlOutputNames(9) = "Sum_insured"
        'ctrlOutputNames(9) = ""
        'ctrlOutputNames(10) = ""
        'ctrlOutputNames(11) = ""
        'ctrlOutputNames(12) = ""
        ...
        If ruleLineObj IsNot Nothing Then
            ...
            tempValArr = Strings.Split(ruleLineObj.Var3, "~", ctrlOutput.GetUpperBound(0) + 1)

            If tempValArr.GetUpperBound(0) < ctrlOutput.GetUpperBound(0) Then
                ReDim ctrlOutput(maxInputs)
            Else
                ctrlOutputValues = Strings.Split(ruleLineObj.Var3, "~", ctrlOutput.GetUpperBound(0) + 1)
            End If
        ...
        For i As Integer = 0 To ctrlOutputNames.GetUpperBound(0)
            ctrlOutput(i) = AddLabelControlPair(ctrlOutputNames(i), ctrlOutputNames(i), ctrlOutputValues(i), Nothing, Nothing)
        Next
    ...
    Public Overrides Sub ClearAndInitialiseInputControlValues()
        ...
        For i As Integer = 0 To ctrlOutput.GetUpperBound(0)
            RuleLineEditorHelper.SetControlValue(ctrlOutput(i).Control, "")
        Next
    End Sub
