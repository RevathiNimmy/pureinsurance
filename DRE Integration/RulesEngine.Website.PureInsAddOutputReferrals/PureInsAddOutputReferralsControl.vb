...
<ToolboxData("<{0}:PureInsAddOutputReferralsControl runat=""server""> </{0}:PureInsAddOutputReferralsControl>")>
Public NotInheritable Class PureInsAddOutputReferralsControl 
...
#Region "Fields"
    ...
    Private ctrlReferrals(maxInputs) As LabelControlPair
    Private ctrlReferralsNames(maxInputs) As String
    Private ctrlReferralsValues(maxInputs) As String

    Private Const controlPrefix As String = "ctlPureAddOutputReferral"
    Private Const labelPrefix As String = "lblPureAddOutputReferral"
...
#Region "Properties"
    ...
    Public Overrides Property RuleLine As EngineCommon.IRuleLine
        Get
            ...
            For i As Integer = 0 To ctrlReferrals.GetUpperBound(0)
                ruleLineObj.Var3 = ruleLineObj.Var3 + GetPostedControlValue(ctrlReferrals(i).Control)
            ...
            For i As Integer = 0 To ctrlReferralsNames.GetUpperBound(0)
                ruleLineObj.Parameters = ruleLineObj.Parameters + ctrlReferralsNames(i).ToString
    ...
    Public Overrides ReadOnly Property TargetControls As System.Collections.Generic.IEnumerable(Of System.Web.UI.Control)
        Get
            ...
            'End If
            'If ctrlReferralsOriginal IsNot Nothing Then
            '    controls.Add(ctrlReferralsOriginal.Control)
            'End If
            If ctrlReferrals.Length > 0 Then
                For i As Integer = 0 To ctrlReferrals.GetUpperBound(0)
                    controls.Add(ctrlReferrals(i).Control)
                Next
            End If
...
#Region "Methods"
    ...
    Public Overrides Sub BuildUI()
        ...
        Try
            ...
            ctrlReferralsNames(0) = "unique_id"
            ctrlReferralsNames(1) = "referral_date"
            ctrlReferralsNames(2) = "reason"
            ctrlReferralsNames(3) = "code"
            ctrlReferralsNames(4) = "orderby"
            ctrlReferralsNames(5) = "can_override"
            ctrlReferralsNames(6) = "retained"
            ctrlReferralsNames(7) = "user_group"
            ctrlReferralsNames(8) = "generated_by"
            ctrlReferralsNames(9) = "task_created"
            ctrlReferralsNames(10) = "back_task_created"
            ...
            If ruleLineObj IsNot Nothing Then
                ...
                tempValArr = Strings.Split(ruleLineObj.Var3, "~", ctrlReferrals.GetUpperBound(0) + 1)

                If tempValArr.GetUpperBound(0) < ctrlReferrals.GetUpperBound(0) Then
                    ReDim ctrlReferrals(maxInputs)
                Else
                    ctrlReferralsValues = Strings.Split(ruleLineObj.Var3, "~", ctrlReferrals.GetUpperBound(0) + 1)
                End If
            ...
            For i As Integer = 0 To ctrlReferralsNames.GetUpperBound(0)
                ctrlReferrals(i) = AddLabelControlPair(ctrlReferralsNames(i), ctrlReferralsNames(i), ctrlReferralsValues(i), Nothing, Nothing)
            Next
    ...
    Public Overrides Sub ClearAndInitialiseInputControlValues()
        ...
        For i As Integer = 0 To ctrlReferrals.GetUpperBound(0)
            RuleLineEditorHelper.SetControlValue(ctrlReferrals(i).Control, "")
        Next
    End Sub
