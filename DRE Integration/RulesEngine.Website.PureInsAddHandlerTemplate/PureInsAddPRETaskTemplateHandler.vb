...
Public NotInheritable Class PureInsAddPRETaskTemplateHandler 
    ...
    Public Overrides Function AssemblyCode(ByVal line As EngineCommon.IRuleLine, ByVal declarations As EngineCommon.RuleEngineDeclarationCollection) As String
        ...
        If (String.Compare(line.Task, MyBase.TaskName, True, LineHandler.Culture) = 0) Then
            Return HandleAddYourObjectNameHereFunction(line, declarations)
        Else
            Return MyBase.NextHandler(line.Task).AssemblyCode(line, declarations)
        End If
    End Function

    Private Function HandleAddYourObjectNameHereFunction(ByVal line As IRuleLine, ByVal declarations As RuleEngineDeclarationCollection) As String
        Dim dataModelCode As String = ""
        'Dim ratingSection As String = ""
        Dim premium As String = ""
        'Dim rate As String = ""
        'Dim sumInsured As String = ""
        Dim yourObjectNameHereObject As String
        Dim yourObjectHereVarNames(maxInputs) As String
        Dim yourObjectHereVarValues(maxInputs) As String
        ...
        'yourObjectHereVarValues = Strings.Split(line.Var3, "~", yourObjectHereVarValues.GetUpperBound(0))
        yourObjectHereVarValues = Strings.Split(line.Var3, "~", maxInputs + 1)
        yourObjectHereVarNames = Strings.Split(line.Parameters, "~", maxInputs + 1)
        ...
        'Get Data Model Code
        yourObjectNameHereObject = String.Format("input.RISK_OBJECTS.{0}_POLICY_BINDER.{0}_OUTPUT_COMMISSION", dataModelCode)

        'Try catch block for variable declarations
        code.AppendLine("try {")

        'Generate the correct c# code for the method
        code.AppendLine(String.Format("AddEntity({0});", yourObjectNameHereObject))
        ...
        'Get Index
        code.AppendLine(String.Format("int outCount = {0}.Count;", yourObjectNameHereObject))

        'Set Properties on new entity
        code.AppendLine(String.Format("{0}[outCount -1].OI = ""OI"" + nextOI.ToString();", yourObjectNameHereObject))
        code.AppendLine(String.Format("{0}[outCount -1].US = 1;", yourObjectNameHereObject))
        ...
        code.AppendLine(String.Format("{0}[outCount -1].PREMIUM = {1};", yourObjectNameHereObject, premium))
        ...
        For i As Integer = 0 To yourObjectHereVarNames.GetUpperBound(0)
            code.AppendLine(String.Format("{0}[outCount -1]." + yourObjectHereVarNames(i).ToUpper + " = NullSafeGet({1});", yourObjectNameHereObject, yourObjectHereVarValues(i)))
        Next
        'end of loop
