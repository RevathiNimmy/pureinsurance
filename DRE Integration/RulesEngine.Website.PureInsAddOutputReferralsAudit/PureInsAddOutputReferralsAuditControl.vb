...
<ToolboxData("<{0}:PureInsAddOutputReferralsAuditControl runat=""server""> </{0}:PureInsAddOutputReferralsAuditControl>")>
Public NotInheritable Class PureInsAddOutputReferralsAuditControl 
...
#Region "Fields"
    ...
    Private Const maxInputs As Integer = 7
    Private ctrlReferralsAudit(maxInputs) As LabelControlPair
    Private ctrlReferralsAuditNames(maxInputs) As String
    Private ctrlReferralsAuditValues(maxInputs) As String

    Private Const controlPrefix As String = "ctlPureAddOutputReferralsAudit"
    Private Const labelPrefix As String = "lblPureAddOutputReferralsAudit"
    ...
    Private Const premiumControlName As String = "ChildIndexVar"
    Private Const dataModelControlName As String = "dataModel"
    'Private Const ReferralOriginalControlName As String = "ReferralOriginal"
...
#Region "Properties"
    ...
    Public Overrides Property RuleLine As EngineCommon.IRuleLine
        Get
            ...
            For i As Integer = 0 To ctrlReferralsAudit.GetUpperBound(0)
                ruleLineObj.Var3 = ruleLineObj.Var3 + GetPostedControlValue(ctrlReferralsAudit(i).Control)
            ...
            For i As Integer = 0 To ctrlReferralsAuditNames.GetUpperBound(0)
                ruleLineObj.Parameters = ruleLineObj.Parameters + ctrlReferralsAuditNames(i).ToString
    ...
    Public Overrides ReadOnly Property TargetControls As System.Collections.Generic.IEnumerable(Of System.Web.UI.Control)
        Get
            ...
            'End If
            'If ctrlReferralsAuditOriginal IsNot Nothing Then
            '    controls.Add(ctrlReferralsAuditOriginal.Control)
            'End If
            If ctrlReferralsAudit.Length > 0 Then
                For i As Integer = 0 To ctrlReferralsAudit.GetUpperBound(0)
                    controls.Add(ctrlReferralsAudit(i).Control)
                Next
            End If
...
#Region "Methods"
    ...
    Public Overrides Sub BuildUI()
        ...
        Try
            'check if we need to add this type of control as if we don't then skip this one as we don't need it yet.
            'If ruleLineObj.

            'End If

            'initialise each control's name
            ctrlReferralsAuditNames(0) = "unique_id"
            ctrlReferralsAuditNames(1) = "type"
            ctrlReferralsAuditNames(2) = "pmuser_id"
            ctrlReferralsAuditNames(3) = "username"
            ctrlReferralsAuditNames(4) = "notes"
            ctrlReferralsAuditNames(5) = "reference"
            ctrlReferralsAuditNames(6) = "approved"
            ctrlReferralsAuditNames(7) = "declined"
            ...
            If ruleLineObj IsNot Nothing Then
                ...
                tempValArr = Strings.Split(ruleLineObj.Var3, "~", ctrlReferralsAudit.GetUpperBound(0) + 1)
                If tempValArr.GetUpperBound(0) < ctrlReferralsAuditValues.GetUpperBound(0) Then
                    ReDim ctrlReferralsAuditValues(maxInputs)
                Else
                    ctrlReferralsAuditValues = Strings.Split(ruleLineObj.Var3, "~", ctrlReferralsAudit.GetUpperBound(0) + 1)
                End If
                'For i As Integer = 0 To ctrlReferralsAudit.GetUpperBound(0)
                '    'need to split the var 3 out and into ctrlReferralsAuditValues
                'Next
            ...
            ctrlPremium = AddLabelControlPair(premiumControlName, "ChildIndexVar", premium, Nothing, Nothing)
            ...
            For i As Integer = 0 To ctrlReferralsAuditNames.GetUpperBound(0)
                ctrlReferralsAudit(i) = AddLabelControlPair(ctrlReferralsAuditNames(i), ctrlReferralsAuditNames(i), ctrlReferralsAuditValues(i), Nothing, Nothing)
            Next
    ...
    Public Overrides Sub ClearAndInitialiseInputControlValues()
        ...
        For i As Integer = 0 To ctrlReferralsAudit.GetUpperBound(0)
            RuleLineEditorHelper.SetControlValue(ctrlReferralsAudit(i).Control, "")
        Next
    End Sub
