...
#End If
            ...
            ''        sClassName:="bDOCSetAccessLevel.Form", _
    ...
    Public Function SetAccessLevel(ByRef iNodeType As Integer, ByRef lNodeNum As Integer, ByRef sNodeName As String, ByRef iUserAccessLevel As Integer) As Integer
        ...
        Try
            ...
            Select Case iNodeType
                Case DOCNode_Folder
                    frmInterface.lblDocFoldName.Text = "Folder Name"
                Case DOCNode_Document
                    frmInterface.lblDocFoldName.Text = "Document Name"
                Case Else
                    frmInterface.lblDocFoldName.Text = "Error Unknown"
            End Select

            frmInterface.txtName.Text = sNodeName

            g_iNodeType = iNodeType
            g_lNodeNum = lNodeNum


            m_lReturn = g_oBusiness.GetNodeAccessLevel(iNodeType:=iNodeType, lNodeNum:=lNodeNum, iAccessLevel:=iAccessLevel)
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            ' Check if user is allowed to change the access level
            If iUserAccessLevel > iAccessLevel Then
                result = gPMConstants.PMEReturnCode.PMFalse
                ' Log Error Message
                iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="User has insufficient privellages to set access level.", vApp:=ACApp, vClass:=ACClass, vMethod:="SetAccessLevel", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return result
            End If

            ' build up access level list
            For iLoop1 As Integer = 9 To iUserAccessLevel Step -1
                frmInterface.cboAccess.Items.Add(CStr(iLoop1))
            Next iLoop1

            ' Set the access level text and new access level to the same
            frmInterface.txtAccess.Text = CStr(iAccessLevel)
            frmInterface.cboAccess.Text = CStr(iAccessLevel)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to set access level.", vApp:=ACApp, vClass:=ACClass, vMethod:="SetAccessLevel", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
