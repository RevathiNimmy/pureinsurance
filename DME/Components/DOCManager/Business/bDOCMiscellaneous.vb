...
<System.Runtime.InteropServices.ProgId("Miscellaneous_NET.Miscellaneous")> _
...
#Else
    Private m_oScanDatabase As Object
#End If
    ...
    Public WriteOnly Property ScanDatabase() As Object
    ...
    Public Function GetDataPath(ByRef lVolumeID As Integer, ByRef sDataPath As String) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GetDataPath1", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
            ...
            Else
                ...
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to get device_id", vApp:=ACApp, vClass:=ACClass, vMethod:="GetDataPath", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
            ...
            'Get device details
            m_sSQL = "SELECT server_unc, share_name FROM DOC_device " & _
                     "WHERE device_id = " & CStr(lDeviceID)

            'hit DB

            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GetDataPath2", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
            ...
            Else
                ...
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to get device details", vApp:=ACApp, vClass:=ACClass, vMethod:="GetDataPath", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return result
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetDataPath", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetAdminLevel(Optional ByRef iAdminLevel As Integer = 0) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETADMINLEVEL", lNumberRecords:=1, bStoredProcedure:=False, vResultArray:=vResultArray)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetAdminLevel", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function Initialise(ByVal v_sUsername As String, ByVal v_sPassword As String, ByVal v_iUserID As Integer, ByVal v_iSourceID As Integer, ByVal v_iLanguageID As Integer, ByVal v_iCurrencyID As Integer, ByVal v_iLogLevel As Integer, ByVal v_sCallingAppName As String, Optional ByRef vDatabase As Object = Nothing, Optional ByRef vScanDatabase As Object = Nothing, Optional ByRef vHistory As Object = Nothing) As gPMConstants.PMEReturnCode
        ...
        Try
            ...
            If Not Information.IsNothing(vScanDatabase) Then
                m_oScanDatabase = vScanDatabase
            End If

            'Store the history object into the private member

            If Not Information.IsNothing(vHistory) Then
                m_oHistory = vHistory
            End If
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Initialise Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="Initialise", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetNextPageName(ByRef sNextPageName As String) As Integer
        ...
        Dim iDataLen, iA, iB, iC, iD, iE As Integer
        Dim vResultArray(,) As Object
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GetNextPageName", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
            ...
            iA = Conversion.Val(Mid(sFilename, 2, 2))
            iB = Conversion.Val(Mid(sFilename, 5, 2))
            iC = Conversion.Val(Mid(sFilename, 8, 2))
            iD = Conversion.Val(Mid(sFilename, 11, 2))
            iE = Conversion.Val(Mid(sFilename, 14, 2))
            ...
            Mid(sFilename, 2, 2) = StringsHelper.Format(iA, "00")
            Mid(sFilename, 5, 2) = StringsHelper.Format(iB, "00")
            Mid(sFilename, 8, 2) = StringsHelper.Format(iC, "00")
            Mid(sFilename, 11, 2) = StringsHelper.Format(iD, "00")
            Mid(sFilename, 14, 2) = StringsHelper.Format(iE, "00")

            'Update next page in system table
            m_sSQL = "UPDATE DOC_system SET next_page = '" & sFilename & "'"


            m_lReturn = m_oDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="UpdatePageName", bStoredProcedure:=False)
    ...
    Public Function ConstructDocRef(ByRef lDocNum As Integer, ByRef sDocRef As String) As Integer
        ...
        Try
            ...
            Mid(sDocRef, 10 - Conversion.Str(lDocNum).Trim().Length, Conversion.Str(lDocNum).Trim().Length) = Conversion.Str(lDocNum).Trim()
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="ConstructDocRef", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Private Function DeleteFolderTree(ByRef lFolderNum As Integer, ByRef sNoAccessName As String) As gPMConstants.PMEReturnCode
        ...
        sSQL = "SELECT access_level, ex_code, folder_level, folder_name " & _
               "FROM DOC_folder WHERE folder_num = " & CStr(lFolderNum)

        'Hit DB

        m_lReturn = m_oDatabase.SQLSelect(sSQL:=sSQL, sSQLName:="GETACCESSEXCODE", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
        ...
        m_lReturn = m_oDatabase.SQLSelect(sSQL:=sSQL, sSQLName:="GETDOCNUMS", bStoredProcedure:=False, vResultArray:=vResultArray)
        ...
        m_lReturn = m_oDatabase.SQLSelect(sSQL:=sSQL, sSQLName:="GETFOLDNUMS", bStoredProcedure:=False, vResultArray:=vResultArray)
        ...
        m_lReturn = m_oDatabase.SQLAction(sSQL:=sSQL, sSQLName:="DELETEFOLDER", bStoredProcedure:=False)
    ...
    Public Function DeleteDocKeyword(ByRef lDocKeywordID As Integer) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="DELDOCKW", bStoredProcedure:=False)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteDocKeyword", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function DeleteFolder(ByRef lFolderNum As Integer, ByRef sNoAccessName As String) As Integer
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteFolder", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            Return result
    ...
    Public Function GetNodeParent(ByRef iNodeType As Integer, ByRef lNodeNum As Integer, ByRef lParentNum As Integer) As Integer
        ...
        Try
            ...
            Select Case iNodeType
                ...
                Case Else
                    ...
                    ' Log Error Message
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Invalid Node Type - " & iNodeType, vApp:=ACApp, vClass:=ACClass, vMethod:="GetNodeParent", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETNODEPARENT", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetNodeParent", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetFolderTree(ByRef lFolderNum As Integer, ByRef vFolderArray(,) As Object) As Integer
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetFolderTree", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetNodeExCode(ByRef iNodeType As Integer, ByRef lNodeNum As Integer, ByRef sExCode As String, Optional ByRef iFolderLevel As Integer = 0) As Integer
        ...
        Try
            ...
            Select Case iNodeType
                ...
                Case Else
                    ...
                    ' Log Error Message
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Invalid Node Type - " & iNodeType, vApp:=ACApp, vClass:=ACClass, vMethod:="GetNodeExCode", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETNODEEXCODE", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
            ...
            Else
                ...
                result = gPMConstants.PMEReturnCode.PMFalse
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Node type " & iNodeType & ", Node number " & _
                           lNodeNum & " does not exist", vApp:=ACApp, vClass:=ACClass, vMethod:="GetNodeExCode", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return result
            End If
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetNodeExCode", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetNodeName(ByRef iNodeType As Integer, ByRef lNodeNum As Integer, ByRef sNodeName As String) As Integer
        ...
        Try
            ...
            Select Case iNodeType
                ...
                Case Else
                    ...
                    ' Log Error Message
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Invalid Node Type - " & iNodeType, vApp:=ACApp, vClass:=ACClass, vMethod:="GetNodeName", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETNODENAME", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetNodeName", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetNodeAccessLevel(ByRef iNodeType As Integer, ByRef lNodeNum As Integer, ByRef iAccessLevel As Integer) As Integer
        ...
        Try
            ...
            Select Case iNodeType
                ...
                Case Else
                    ...
                    ' Log Error Message
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Invalid Node Type - " & iNodeType, vApp:=ACApp, vClass:=ACClass, vMethod:="GetNodeAccessLevel", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETNODEACCESSLEVEL", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
            ...
            Else
                ...
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Node number " & lNodeNum & " not found.", vApp:=ACApp, vClass:=ACClass, vMethod:="GetNodeAccessLevel", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetNodeAccessLevel", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function RenameNode(ByRef iNodeType As Integer, ByRef lNodeNum As Integer, ByRef sNewNodeName As String) As Integer
        ...
        Try
            ...
            Select Case iNodeType
                Case DOCNode_Folder
                    ...
                    m_sSQL = "UPDATE DOC_folder " & _
                             "SET folder_name = " & "'" & sNewNodeName & "' " & _
                             "WHERE folder_num = " & CStr(lNodeNum)

                Case DOCNode_Document
                    'Construct SQL to rename the doc
                    m_sSQL = "UPDATE DOC_document " & _
                             "SET doc_name = " & "'" & sNewNodeName & "' " & _
                ...
                Case Else
                    ...
                    ' Log Error Message
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Invalid Node Type - " & iNodeType, vApp:=ACApp, vClass:=ACClass, vMethod:="RenameNode", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)

            End Select

            ' Execute SQL Statement

            m_lReturn = m_oDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="RENAMENODE", bStoredProcedure:=False)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="RenameNode", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function RenameDoc(ByRef lDocNum As Integer, ByRef sNewName As String) As Integer
        ...
        Try
            ...
            If bExternal Then
                ...
                m_lReturn = m_oHistory.DirectAdd(vTask:=DOCMODDOCUMENT, vCabinetCode:=sCabCode, vCabinetName:="", vDrawerCode:=sDrawCode, vDrawerName:="", vFolderCode:=sFoldCode, vFolderName:="", vDocRef:=sDocRef, vRequestDate:=dDocDate.ToString("yyyyMMdd"), vRequestTime:=dDocDate.ToString("HHMMss"), vDescription:=sNewName)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="RenameDoc", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function AmIExternal(ByRef iNodeType As Integer, ByRef lNodeNum As Integer, ByRef bExternal As Boolean) As Integer
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="AmIExternal", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetDocNames(ByRef vDocNamesArray(,) As Object) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETDOCNAMES", bStoredProcedure:=False, lNumberRecords:=gPMConstants.PMAllRecords, vResultArray:=vDocNamesArray)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetDocNames", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetDocDateOffSets(ByRef iDocDateOffset As Integer, ByRef iExpiryDateOffset As Integer) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETOFFSETS", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
            ...
            If Not Information.IsArray(vResultArray) Then
                ...
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to get dates from system table", vApp:=ACApp, vClass:=ACClass, vMethod:="GetDocDateOffSets", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetDocDateOffSets", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    '
    ' Log Error Message
    'iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Invalid Node Level - " & iNodeLevel, vApp:=ACApp, vClass:=ACClass, vMethod:="UpdateHistory", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
    '
    'Return result
    '
    'End Select
    ...
    '
    ' Log Error Message
    'iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="UpdateHistory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message)
    '
    'Return result
    '
    'End Try
    ...
    Public Function LinkOwner(ByRef lDocNum As Integer, ByRef bLinkOwner As Boolean) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="LINKOWNER", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="LinkOwner", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function DeleteDocInfo(ByRef lDocNum As Integer) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="DELDOCINFO", bStoredProcedure:=False)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteDocInfo", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function DeleteDocKeywords(ByRef lDocNum As Integer) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="DELDOCKEYW", bStoredProcedure:=False)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteDocKeywords", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function DeleteDocAnnotations(ByRef lDocNum As Integer) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="DELDOCANN", bStoredProcedure:=False)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteDocAnnotations", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Private Function DeletePages(ByRef lDocNum As Integer) As Integer
        ...
        If Information.IsArray(vPageArray) Then
            Dim cloudHostingOptionValue As String = String.Empty
            Dim s3DocumentPageName As String = String.Empty
            ...
            If cloudHostingEnabled Then
                sDataPath = String.Empty
                m_lReturn = CType(GetDataPath(lVolumeID:=DOCHD1_ID, sDataPath:=sDataPath), gPMConstants.PMEReturnCode)
            ...
            For I As Integer = vPageArray.GetLowerBound(0) To vPageArray.GetUpperBound(0)
                ...
                If cloudHostingEnabled Then
                    ...
                    s3DocumentPageName = vPageArray(I).ToString().Replace(sDataPath, String.Empty).Replace("\", "/").TrimStart("/")

                    m_lReturn = CType(repository.DeleteFile(s3DocumentPageName).Result, gPMConstants.PMEReturnCode)
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    ...
                    m_sSQL = "UPDATE DOC_page SET doc_num = 0 WHERE page_name = '" & _
                             CStr(vPageArray(I)).Substring(Strings.Len(CStr(vPageArray(I))) - 19, Math.Min(CStr(vPageArray(I)).Length, 15)) & "'"


                    'hit DB

                    m_lReturn = m_oDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="UPDATEPAGE", bStoredProcedure:=False)
                ...
                Else
                    ...
                    m_sSQL = "DELETE FROM DOC_page WHERE page_name = '" & _
                             CStr(vPageArray(I)).Substring(Strings.Len(CStr(vPageArray(I))) - 19, Math.Min(CStr(vPageArray(I)).Length, 15)) & "'"

                    'hit DB

                    m_lReturn = m_oDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="DELPAGE", bStoredProcedure:=False)
    ...
    Public Function GetPageList(ByRef lDocNum As Integer, ByRef vPageArray() As Object) As Integer
        ...
        Try
            ...
            m_sSQL = "SELECT page_name, page_type, volume_id FROM DOC_page " & _
                     "WHERE doc_num = " & CStr(lDocNum)

            'hit DB

            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETPAGELIST", bStoredProcedure:=False, lNumberRecords:=gPMConstants.PMAllRecords, vResultArray:=vResultArray)
            ...
            If Not Information.IsArray(vResultArray) Then
                ' Log Error Message
                result = gPMConstants.PMEReturnCode.PMFalse
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="No pages found for doc_num = " & lDocNum, vApp:=ACApp, vClass:=ACClass, vMethod:="GetPageList", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return result
            End If
            ...
            For I As Integer = vResultArray.GetLowerBound(1) To vResultArray.GetUpperBound(1)
                ...
                vPageArray(I) = sDataPath & _
                                CStr(vResultArray(0, I)).Trim() & _
                                "." & _
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetPageList", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetPageType(ByRef lDocNum As Integer, ByRef sPageType As String) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETPAGETYPE", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
            ...
            If Not Information.IsArray(vResultArray) Then
                ...
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="No Pages Found", vApp:=ACApp, vClass:=ACClass, vMethod:="GetPageType", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return result
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetPageType", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetPageName(ByRef lDocNum As Integer, ByRef sPageName As String) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETPAGENAME", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
            ...
            If Not Information.IsArray(vResultArray) Then
                ...
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="No Pages Found for doc_num = " & lDocNum, vApp:=ACApp, vClass:=ACClass, vMethod:="GetPageName", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return result
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetPageName", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetDocDate(ByRef lDocNum As Integer, ByRef dDocDate As Date) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETDOCDATE", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
            ...
            If Not Information.IsArray(vResultArray) Then
                ...
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to get document date.", vApp:=ACApp, vClass:=ACClass, vMethod:="GetDocDate", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return result
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetDocDate", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetDocLink(ByRef lDocNum As Integer, ByRef lLinkNum As Integer) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETDOCLINK", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
            ...
            If Not Information.IsArray(vResultArray) Then
                ...
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to get document link.", vApp:=ACApp, vClass:=ACClass, vMethod:="GetDocLink", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return result
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetDocLink", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetDocDetails(ByRef lDocNum As Integer, ByRef sDocName As String, ByRef sDocType As String) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETDOCDETAILS", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
            ...
            If Not Information.IsArray(vResultArray) Then
                ...
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to get document details.", vApp:=ACApp, vClass:=ACClass, vMethod:="GetDocDetails", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return result
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetDocDetails", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function DeleteDocuments(ByRef vDocArray As Object, ByRef bExternal As Boolean, ByRef sNoAccessName As String) As Integer
        ...
        Try
            ...
            If Not Information.IsArray(vDocArray) Then
                Return result
            End If
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteDocuments", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function DeleteDoc(ByRef lDocNum As Integer, ByRef bExternal As Boolean, ByRef sNoAccessName As String) As Integer
        ...
        Try
            ...
            m_sSQL = "SELECT access_level, ex_code, doc_name, DOC_doc_info.doc_date, link FROM " & _
                     "DOC_document, DOC_doc_info WHERE DOC_document.doc_num = " & CStr(lDocNum) & _
                     " AND DOC_doc_info.doc_num = " & CStr(lDocNum)

            'Hit DB

            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETDOCEXCODE", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
            ...
            Else
                ...
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteDoc", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
            ...
            If bLinkOwner Then
                ...
                m_lReturn = m_oDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="UPDATEDOC", bStoredProcedure:=False)
            ...
            Else
                ...
                m_lReturn = m_oDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="DELETEDOC", bStoredProcedure:=False)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteDoc", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function AddDocToHistory(ByRef lDocNum As Integer, ByRef lFoldNum As Integer, ByRef sDocName As String, ByRef dDocDate As Date, ByRef sPageName As String, ByRef sDocType As String, ByRef sPageType As String) As Integer
        ...
        Try
            ...
            m_lReturn = m_oHistory.DirectAdd(vTask:=DOCADDDOCUMENT, vCabinetCode:=sCabExCode, vCabinetName:="", vDrawerCode:=sDrawExCode, vDrawerName:="", vFolderCode:=sFoldExCode, vFolderName:="", vDocRef:=sDocRef, vRequestDate:=dDocDate.ToString("yyyyMMdd"), vRequestTime:=dDocDate.ToString("HHMMss"), vEventType:=sDocType, vDescription:=sDocName, vVolume:=DOCHD1_NAME, vPageFile:=sTmpName, vdoctype:=sPageType.ToUpper())
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="AddDocToHistory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function DeleteScannedDoc(ByRef lScanDocNum As Integer) As Integer
        ...
        Try
            ...
            m_lReturn = m_oScanDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="DELETEDOC", bStoredProcedure:=False)
            ...
            m_lReturn = m_oScanDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="DELETEDOCINFO", bStoredProcedure:=False)
            ...
            m_lReturn = m_oScanDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="DELETEKW", bStoredProcedure:=False)
            ...
            m_lReturn = m_oScanDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="DELETEANN", bStoredProcedure:=False)
            ...
            m_lReturn = m_oScanDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="DELETEPAGE", bStoredProcedure:=False)
            ...
            m_lReturn = m_oScanDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="DELETETASK", bStoredProcedure:=False)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteScannedDoc", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function DeleteScannedPageFiles(ByRef lScanDocNum As Integer, ByRef sScanDirectory As String) As Integer
        ...
        Try
            ...
            If FileSystem.Dir(sDirectory & "\*.*", FileAttribute.Normal) = "" Then Directory.Delete(sDirectory)
            ...
            Try
                ...
                m_lReturn = m_oScanDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GETPAGEDETS", bStoredProcedure:=False, lNumberRecords:=gPMConstants.PMAllRecords, vResultArray:=vResultArray)
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    Return gPMConstants.PMEReturnCode.PMFalse
                End If
                ...
                If Not Information.IsArray(vResultArray) Then
                    ...
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogWarning, sMsg:="No pages exist for document " & lScanDocNum, vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteScannedPageFiles", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                ...
                For I As Integer = vResultArray.GetLowerBound(1) To vResultArray.GetUpperBound(1)
                    ...
                    sFilename = sScanDirectory & "Doc" & _
                                lScanDocNum & "\" & _
                                CStr(vResultArray(0, I)).Trim() & "." & _
            ...
            Catch excep As System.Exception
                ...
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteScannedPageFiles", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function DeleteDocTask(ByRef lDocNum As Integer) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLAction(sSQL:=m_sSQL, sSQLName:="DELTASK", bStoredProcedure:=False)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteDocTask", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
...
#End If
        ...
        Try
            ...
            m_lReturn = CType(CheckDatabase(m_sUsername, m_iSourceID, m_iLanguageID, gPMConstants.PMEProductFamily.pmePFSiriusArchitecture, False, oDatabase), gPMConstants.PMEReturnCode)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetSADatabase Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetSADatabase", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function IsSBOInstalled(ByRef bInstalled As Boolean) As Integer
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="IsSBOInstalled Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="IsSBOInstalled", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function MergeFolders(ByRef v_sInsuranceFileCnt As String, ByRef v_sPartyCnt As String, ByRef v_sCompanyID As String) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.Parameters.Add(sName:="insurance_file_cnt", vValue:=v_sInsuranceFileCnt, idirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMString)
            ...
            m_lReturn = m_oDatabase.Parameters.Add(sName:="party_cnt", vValue:=v_sPartyCnt, idirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMString)
            ...
            m_lReturn = m_oDatabase.Parameters.Add(sName:="company_id", vValue:=v_sCompanyID, idirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMString)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="MergeFolders", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            Return result
    ...
    Public Function GetInsuranceFolderCnt(ByRef vClaimCnt As Object, ByRef vInsuranceFolderCnt As Object) As Integer
        ...
        Try
            ...
            'Construct SQL
            m_sSQL = "SELECT I.insurance_folder_cnt FROM Insurance_Folder I" & _
                     " JOIN insurance_file in_f ON" & _
                     " I.insurance_folder_cnt = in_f.insurance_folder_cnt" & _
                     " JOIN claim C ON" & _
                     " C.policy_id = in_f.insurance_file_cnt" & _
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=m_sSQL, sSQLName:="GetInsuranceFolderCnt", bStoredProcedure:=False, lNumberRecords:=1, vResultArray:=vResultArray)
            ...
            Else
                ...
                result = gPMConstants.PMEReturnCode.PMFalse
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetInsuranceFolderCnt", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return result
            End If
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:=gPMConstants.PMErrorText, vApp:=ACApp, vClass:=ACClass, vMethod:="GetInsuranceFolderCnt", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
