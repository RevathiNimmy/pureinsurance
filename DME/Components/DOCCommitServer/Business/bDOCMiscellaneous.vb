...
#End If
    ...
    Public Function RenameDoc(ByRef lDocNum As Integer, ByRef sNewName As String) As Integer
        ...
        Dim sFoldCode As String = String.Empty
        Dim sDrawCode As String = String.Empty
        Dim sCabCode As String = String.Empty
        ...
        Try
            ...
            If bExternal Then
                ...
                ' get cab ex code
                m_lReturn = CType(GetNodeExCode(iNodeType:=DOCNode_Folder, lNodeNum:=lTmp1, sExCode:=ToSafeString(sCabCode)), gPMConstants.PMEReturnCode)
            ...
            For I As Integer = vPageArray.GetLowerBound(0) To vPageArray.GetUpperBound(0)
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    ...
                    m_sSQL = "UPDATE DOC_page SET doc_num = 0 WHERE page_name = '" &
                             CStr(vPageArray(I)).Substring(CStr(vPageArray(I)).Length - 19, Math.Min(CStr(vPageArray(I)).Length, 15)) & "'"
    ...
    Public Function AddDocToHistory(ByRef lDocNum As Integer, ByRef lFoldNum As Integer, ByRef sDocName As String, ByRef dDocDate As Date, ByRef sPageName As String, ByRef sDocType As String, ByRef sPageType As String) As Integer
        ...
        Dim result As Integer = 0
        Dim sDocRef As String = String.Empty
        Dim sTmpName As String = String.Empty
        Dim sCabExCode As String = String.Empty
        Dim sDrawExCode As String = String.Empty
        Dim sFoldExCode As String = String.Empty
        ...
        Try
            ...
            m_lReturn = m_oHistory.DirectAdd(vTask:=DOCADDDOCUMENT, vCabinetCode:=ToSafeString(sCabExCode), vCabinetName:="", vDrawerCode:=ToSafeString(sDrawExCode), vDrawerName:="", vFolderCode:=ToSafeString(sFoldExCode), vFolderName:="", vDocRef:=ToSafeString(sDocRef), vRequestDate:=dDocDate.ToString("yyyyMMdd"), vRequestTime:=dDocDate.ToString("HHMMss"), vEventType:=ToSafeString(sDocType), vDescription:=ToSafeString(sDocName), vVolume:=DOCHD1_NAME, vPageFile:=ToSafeString(sTmpName), vdoctype:=sPageType.ToUpper())
    ...
    Public Function DeleteScannedDoc(ByRef lScanDocNum As Integer) As Integer
        ...
        Try
            ...
            m_lReturn = m_oScanDatabase.SQLAction(sSQL:=ToSafeString(m_sSQL), sSQLName:="DELETEDOC", bStoredProcedure:=False)
            ...
            m_lReturn = m_oScanDatabase.SQLAction(sSQL:=ToSafeString(m_sSQL), sSQLName:="DELETEDOCINFO", bStoredProcedure:=False)
            ...
            m_lReturn = m_oScanDatabase.SQLAction(sSQL:=ToSafeString(m_sSQL), sSQLName:="DELETEKW", bStoredProcedure:=False)
            ...
            m_lReturn = m_oScanDatabase.SQLAction(sSQL:=ToSafeString(m_sSQL), sSQLName:="DELETEANN", bStoredProcedure:=False)
            ...
            m_lReturn = m_oScanDatabase.SQLAction(sSQL:=ToSafeString(m_sSQL), sSQLName:="DELETEPAGE", bStoredProcedure:=False)
            ...
            m_lReturn = m_oScanDatabase.SQLAction(sSQL:=ToSafeString(m_sSQL), sSQLName:="DELETETASK", bStoredProcedure:=False)
    ...
    Public Function DeleteScannedPageFiles(ByRef lScanDocNum As Integer, ByRef sScanDirectory As String) As Integer
        ...
        Try
            ...
            If Directory.GetFiles(sDirectory & "\*.*", FileAttribute.Normal).Equals("") Then Directory.Delete(sDirectory)
            ...
            Try
                ...
                m_lReturn = m_oScanDatabase.SQLSelect(sSQL:=ToSafeString(m_sSQL), sSQLName:="GETPAGEDETS", bStoredProcedure:=False, lNumberRecords:=gPMConstants.PMAllRecords, vResultArray:=CType(vResultArray, Object))
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    Return gPMConstants.PMEReturnCode.PMFalse
                End If
