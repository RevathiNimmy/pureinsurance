    ...
    Private Shared Function BuildKeyString(ByVal v_ePMEProductFamily As PMEProductFamily, ByVal v_ePMERegSettingLevel As PMERegSettingLevel, Optional ByVal v_sSubKey As String = "") As String
        ...
        Try
            ...
            If (v_sSubKey <> "") Then
              ...
              If sKeyString.EndsWith("\") Then ' (Mid(sKeyString, Len(sKeyString), 1) = " \ ") Then
                sKeyString = sKeyString.Substring(0, sKeyString.Length - 1) '  Mid(sKeyString, 1, Len(sKeyString) - 1)
                End If
...
#End Region
    ...
    Friend Shared Sub Decrypt(ByVal v_sPassword As String, ByRef r_sDecryptedPassword As String)
        ...
        For iCntr2 = 1 To iCntr
            ...
            iPos = Informations.inStr(1, sCodeString, Mid(sAString, iCntr2, 1))
            iTemp = iPos - 1
    ...
    Public Shared Sub DeserializeImplementationDataSet(
        ByVal sXMLString As String,
        ByVal oXMLSerializer As Serialization.XmlSerializer,
    ...
    ''' <remarks></remarks>
    Private Shared Sub DeserializeImplementationDataSetWCF(
            ByVal sXMLString As String,
            ByVal oDCSerializer As DataContractSerializer,
    ...
    Friend Shared Sub Encrypt(ByVal sPassword As String, ByRef r_sEncryptedPassword As String)
        ...
        For iCntr2 = 1 To iCntr
            sBString = sBString + Mid(sCodeString, ((Asc(Mid(sAString, iCntr2, 1)) +
    ...
    Friend Shared Function GetPMContactTypeCode(ByVal eContactType As ContactTypeType, Optional ByVal sOtherContactType As String = "") As String
        ...
        Select Case eContactType
            ...
            Case BaseImplementationTypes.ContactTypeType.HOMEPHONE
                sOut = "HOMEPHONE"
            Case BaseImplementationTypes.ContactTypeType.TELEPHONE
                sOut = "TELEPHONE"
            ...
            Case BaseImplementationTypes.ContactTypeType.MAIN
                sOut = "MAIN"
            Case BaseImplementationTypes.ContactTypeType.WORKPHONE
                sOut = "WORKPHONE"
            Case BaseImplementationTypes.ContactTypeType.MOBILE
                sOut = "MOBILE"
            Case BaseImplementationTypes.ContactTypeType.LETTER
                sOut = "LETTER"
    ...
    Friend Shared Sub STSLogMessage(ByVal v_oTraceSetting As TraceSwitch,
                                  ByVal v_oLogLevel As TraceLevel,
    ...
    Public Overloads Shared Function TransformDatasetSAMtoPB(ByVal XMLDataset As String, Optional ByVal SiriusUser As SIRIUSUSER = Nothing, Optional ByRef r_bRemoveAllSW As Boolean = False) As String
        ...
        Using con As SiriusConnection = New SiriusConnectionPMDAO(
                                            SiriusUser.Username, SiriusUser.SourceID,
                                            SiriusUser.LanguageID,
    ...
    Private Shared Sub ProcessDatasetStandardWordingsSAMtoPB(ByVal con As SiriusConnection, ByRef xmlDoc As XmlDocument, ByVal sNamespace As String, ByVal DataModelCode As String, ByVal PolicyBinderID As Integer, Optional ByRef r_bRemoveAllSW As Boolean = False)
        ...
        Dim dt As DataTable = Nothing
        If cacheExistsValue <> StandardWordingPropertiesExist AndAlso
            ...
            If ds IsNot Nothing Then
                If ds.Tables.Count > 0 Then
                    If ds.Tables(0).Rows.Count > 0 Then
                        ...
                        Dim sOriginalDocumentCode As String = String.Empty
                        Dim sIO As String = ""
                                                    ...
                                                    If dt.Rows.Count > 0 Then
                                                        If sCalledVia <> "SKIP" Then
                                                            For Each orRow As DataRow In dt.Rows
                                                                ...
                                                                Using cmd As SiriusCommand = SiriusCommand.FromProcedure("spu_SAM_Get_Original_Document_Code")
                                                                    Dim dtOriginalDocumentCode As DataTable = Nothing
                                                                    cmd.AddInParameter("@Code", SqlDbType.VarChar, 20).Value = RTrim(orRow.Item("DocumentCode").ToString)

                                                                    dtOriginalDocumentCode = con.ExecuteDataTable(cmd)
                                                                    If dtOriginalDocumentCode.Rows.Count > 0 Then
                                                                        sOriginalDocumentCode = RTrim(dtOriginalDocumentCode.Rows(0).Item("Code").ToString)
                                                                    Else
                                                                        sOriginalDocumentCode = RTrim(orRow.Item("DocumentCode").ToString)
                                                                    End If
                                                                ...
                                                                sChildNode = "<" & sName & " CODE=""" & DocumentCode & """" & " DESCRIPTION=""" & sDocumentDescription & """" & " ID=""" & DocumentTemplateID & """" & " ORIGINALDOCUMENTCODE=""" & sOriginalDocumentCode & """" & " TYPE=""" & "SW-5" & """" & " US=""" & "0" & """/>"
                                                                navigator.AppendChild(sChildNode)
                                                                DocumentCode = ""
                                                        ...
                                                        ElseIf sCalledVia = "SKIP" AndAlso dtAddRisk.Rows.Count > 0 AndAlso bRemoveAllSW = False AndAlso (oNode Is Nothing) Then
                                                            For Each oRDARiskRow As DataRow In dtAddRisk.Rows
                                                                ...
                                                                sOriginalDocumentCode = ""
                                                                sChildNode = "<" & sName & " CODE=""" & DocumentCode & """" & " DESCRIPTION=""" & sDocumentDescription & """" & " ID=""" & DocumentTemplateID & """" & " ORIGINALDOCUMENTCODE=""" & sOriginalDocumentCode & """" & " TYPE=""" & "SW-5" & """" & " US=""" & "1" & """/>"
                                                                navigator.AppendChild(sChildNode)
                                                                DocumentCode = ""
                                                                sDocumentDescription = ""
                                                            Next
