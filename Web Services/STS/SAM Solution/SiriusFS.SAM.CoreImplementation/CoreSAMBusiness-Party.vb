...
Partial Public Class CoreSAMBusiness
    ...
    Private Sub AddPartyHistory(ByVal con As SiriusConnection, ByVal v_nPartyKey As Integer, ByVal v_sXMLDataset As String)
        ...
        Try
            ...
            SAMFunc.InitialiseSBOObject(con, oBSirPartyBusiness, _SiriusUser, "bSIRParty.Business")
    ...
    Public Overloads Function AddParty(ByVal con As SiriusConnection, ByVal AddPartyRequest As BaseAddPartyRequestType) As BaseAddPartyResponseType
        ...
        If sPartyType = "PC" Then
            ...
            If oBasePartyPC.OccupationCode <> "" Then
                Try
                    ...
                    Dim strOccupationCode As Object = GetListCode(Of String)(Core.STSListType.UserDefinedTable, "2228226",
            ...
            If oBasePartyPC.EmployersBusinessCode <> "" Then
                Try
                    ...
                    Dim strEmployersBusinessCode As Object = GetListCode(Of String)(Core.STSListType.UserDefinedTable, "2228228",
        ...
        Else
            ...
            If oBasePartyCC.BusinessCode <> "" Then
                Try
                    ...
                    Dim strBusinessCode As Object = GetListCode(Of String)(Core.STSListType.UserDefinedTable, "2228228",
        ...
        If sPartyType = "PC" Then
            If oBasePartyPC.DateOfBirth <> New Date Then
                ...
                If oBasePartyPC.DateOfBirth > Today Then
                    ...
                    Return oResponse
                End If
                If oBasePartyPC.DateOfBirthSpecified = True Then
                ...
                For addCNT As Integer = AddPartyRequest.Party.Addresses.GetLowerBound(0) To _
                    ...
                    If oAddressWithContact.Contacts IsNot Nothing Then
                        ...
                        If vAddContacts Is Nothing Then
                            ...
                            existUBound = 0
                            ReDim Preserve vAddContacts(4, Microsoft.VisualBasic.UBound(vSingleContactsArray) - 1)
                            For CountFor As Integer = 0 To Microsoft.VisualBasic.UBound(vSingleContactsArray, 2)
                            ...
                            'Array.ConstrainedCopy(vSingleContacts, 0, vAddContacts, existUBound, Microsoft.VisualBasic.UBound(vSingleContacts, 2) * 5)
                        Else

                            existUBound = Microsoft.VisualBasic.UBound(vAddContacts)
                            ReDim Preserve vAddContacts(4, Microsoft.VisualBasic.UBound(vSingleContactsArray))
                            For CountFor As Integer = 0 To Microsoft.VisualBasic.UBound(vSingleContactsArray, 2)
                            ...
                            Next
                            'Array.ConstrainedCopy(vSingleContacts, 0, vAddContacts, 0, (Microsoft.VisualBasic.UBound(vSingleContacts, 2) + 1) * 5)
                        End If
                        'Array.ConvertAll(vAddContacts, vSingleContacts)
                        ''Array.Resize(vAddContacts.Rank, existUBound + Microsoft.VisualBasic.UBound(vSingleContacts) + 1)
        ...
        If bOtherParty = True Then
            ...
            ' Call Validate Data
            ValidateAddPartyData(oCoreBusiness, oBasePartyOther, oSAMErrorCollection, nTypeOfPackage)
            ...
            Else
                If oBasePartyOther.Conviction IsNot Nothing Then
                    ProcessConvictions(oBasePartyOther.Conviction,
                                        r_oConvictions)
                End If
            End If

            If oBasePartyOther.Accident IsNot Nothing Then
                ProcessAccidents(oBasePartyOther.Accident,
                                    r_oAccidents)
            End If
            ...
            ProcessOtherPartyInfo(oBasePartyOther,
                                r_oOtherPartyInfo)
        ...
        Try
            ...
            ElseIf sPartyType.StartsWith("OT") Then
                ...
                If oBasePartyOther.Code Is Nothing Or oBasePartyOther.Code.Trim() = "" Then
                    If Not (String.IsNullOrEmpty(oBasePartyOther.BranchCode)) Then
                        Try
                            lBranchId = oCoreBusiness.GetAndValidateListItemFromCode(Core.STSListType.PMLookup, "source", oBasePartyOther.BranchCode, "source_id", oSAMErrorCollection)
                        Catch ex As Exception
                            STSError.AddInvalidField("BranchCode", CStr(STSErrorCodes.InvalidLookupListValue), [String].Format(STSErrorPublisher.MandatoryInputInvalid, "BranchCode"), oBasePartyPC.Source)
                        End Try
                    End If
                    sSurname = oCoreBusiness.GetOtherPartyNumbering(con, lBranchId, oBasePartyOther.Name)
                    If String.IsNullOrEmpty(sSurname) Then
                        Dim STSErrorEX As New STSErrorPublisher(STSErrorCodes.BlankOtherPartyCode, "Other party code is blank or Numbering scheme for Other Party is not set", "")
                        STSErrorEX.SetContext(oResponse.STSError, HttpContext.Current.Request.Url.ToString(), ACMethodName, "Other party code is blank or Numbering scheme for Other Party is not set", True)
                        Return oResponse
                    End If
                Else
                    sSurname = oBasePartyOther.Code
                End If
            ...
            End If
            If oBasePartyPC.EmployersBusinessCode <> "" Then
                sEmployersBusiness = oBasePartyPC.EmployersBusinessCode
            End If
            Dim sDOB As String = ""
            Dim sGenderCode As String
            If bOtherParty Then
                sDOB = oBasePartyOther.DateOfBirth.ToString()
                sGenderCode = CType(IIf(String.IsNullOrEmpty(oBasePartyOther.Gender), "", oBasePartyOther.Gender), String)
            Else
                sDOB = CType(oBasePartyPC.DateOfBirth, String)
                sGenderCode = oBasePartyPC.GenderCode
            End If
            iRet = CType(oGIS.AddParty(
                    sPartyTypeCode:=sPartyType,
                    sBranchCode:=AddPartyRequest.BranchCode,
                    vAddresses:=DirectCast(vAddresses, Object(,)),
                    sTPUserCode:=AddPartyRequest.Party.TPUserCode,
                    sTPIntroducer:=AddPartyRequest.Party.TPIntroducer,
                    vContacts:=DirectCast(vContacts, Object(,)),
                    sSurname:=sSurname,
                    sForename:=oBasePartyPC.Forename,
                    sDateOfBirth:=sDOB,
                    sTitle:=oBasePartyPC.Title,
                    sMaritalStatusCode:=sMaritalStatusCode,
                    sGenderCode:=sGenderCode,
                    sInitials:=oBasePartyPC.Initials,
                    sOccupationCode:=sOccupation,
                    sEmployerBusinessCode:=sEmployersBusiness,
                    sEmploymentStatusCode:=oBasePartyPC.EmploymentStatusCode.ToString,
                    sAlternativeID:=CType(IIf(String.IsNullOrEmpty(sAlternativeIdentifier), "", sAlternativeIdentifier), String),
                    sCompanyName:=oBasePartyCC.CompanyName,
                    sTradingName:="",
                    sBusinessCode:=sBusiness,
                    lAgentCnt:=iAgentCnt,
                    r_lPartyCnt:=oResponse.PartyKey,
                    r_sPartyCode:=oResponse.Shortname,
                    sMainContact:=oBasePartyCC.MainContact,
                    r_oConvictions:=r_oConvictions,
                    r_oAccidents:=r_oAccidents,
                    vAddressContacts:=vAddContacts,
                    r_oSuppBusiness:=r_oSupplierBusiness,
                    r_oOtherPartyInfo:=r_oOtherPartyInfo,
                    r_sTaxNumber:=AddPartyRequest.Party.TaxNumber,
                    r_bDomiciledForTax:=BooleanDataConvert.ToInt16(AddPartyRequest.Party.DomiciledForTax),
                    r_bTaxExempt:=BooleanDataConvert.ToInt16(AddPartyRequest.Party.TaxExempt),
                    r_lPercentage:=CInt(AddPartyRequest.Party.TaxPercentage),
        ...
        If sPartyType = "PC" Then
            ...
            If sEnhancedResolvedName = "1" Then
                oResponse.ResolvedName = Trim(
                    oBasePartyPC.Title & " " &
                    oBasePartyPC.Forename & " " &
                oBasePartyPC.Surname)
            Else
                oResponse.ResolvedName = Trim(
                    oBasePartyPC.Title & " " &
                    oBasePartyPC.Initials & " " &
                oBasePartyPC.Surname)
            End If
        ElseIf sPartyType = "CC" Then
            oResponse.ResolvedName = oBasePartyCC.CompanyName
        Else
            oResponse.ResolvedName = oBasePartyOther.Name
            If oResponse.PartyKey <> 0 Then
                ProcessOtherPartyBranches(con, oBasePartyOther.Branches, oResponse.PartyKey)
            End If
        End If

        '***********************************************************************
        ' process risk data
        If nTypeOfPackage = enumTypeOfPackage.UnknownPackage OrElse
            nTypeOfPackage = enumTypeOfPackage.SAMForInsurancePackage OrElse
        ...
        AnyError = oCoreBusiness.GetTimestamp(con,
                            AddPartyRequest.BranchCode,
                            CoreBusiness.LockName.PartyCnt,
                            oResponse.PartyKey,
                            oResponse.PartyTimestamp,
                            bIsLocked)
        ' Return AnyErrors
        ...
        ElseIf sPartyType.StartsWith("OT") Then
            If Not (String.IsNullOrEmpty(oBasePartyOther.Currency)) Then
                lCurrencyId = oCoreBusiness.GetAndValidateListItemFromCode(Core.STSListType.PMLookup, "currency", oBasePartyOther.Currency, "currency_id", oSAMErrorCollection)
            End If
            If Not (String.IsNullOrEmpty(oBasePartyOther.BranchCode)) Then
                lBranchId = oCoreBusiness.GetAndValidateListItemFromCode(Core.STSListType.PMLookup, "source", oBasePartyOther.BranchCode, "source_id", oSAMErrorCollection)
            End If
            If Not (String.IsNullOrEmpty(oBasePartyOther.SubBranchCode)) Then
                lSubBranchId = oCoreBusiness.GetAndValidateListItemFromCode(Core.STSListType.PMLookup, "sub_branch", oBasePartyOther.SubBranchCode, "sub_branch_id", oSAMErrorCollection)
            End If
            oUpdatePartyAddDetails.Add("currency_id", lCurrencyId)
            oUpdatePartyAddDetails.Add("source_id", lBranchId)
            oUpdatePartyAddDetails.Add("sub_branch_id", lSubBranchId)
            UpdateOtherPartyDetails(con, oBasePartyOther, oResponse.PartyKey, oUpdatePartyAddDetails)
    ...
    Public Overloads Function FindParty(ByVal FindPartyRequest As BaseFindPartyRequestType) As BaseFindPartyResponseType
        Using con As SiriusConnection = New SiriusConnectionPMDAO(
            ...
            ElseIf (FindPartyRequest.GetType Is GetType(SAMForInsuranceV2ImplementationTypes.FindPartyRequestType)) Or
    ...
    Public Function FindPartyVersion2(ByVal con As SiriusConnection, ByVal FindPartyRequest As BaseFindPartyRequestType) As BaseFindPartyResponseType
        ...
        Dim oWithCol() As Object = {0, 2, 3, 4, 5, 9, 10, 14, 15, 16, 17, 18, 21, 23, 31}
        ...
        If FindPartyRequest.PartyType.ToString <> "BDXIgnorePartyType" Then
            ...
            If iPartyType = 0 Then
                oSAMErrorsCollection.AddBusinessRule(
                             SAMConstants.SAMBusinessErrors.PartyTypeNotFound,
        ...
        FindPartyRequest.SourceId = oCoreBusiness.GetAndValidateListItemFromCode(Core.STSListType.PMLookup,
        ...
        If nTypeOfPackage = enumTypeOfPackage.SAMForInsuranceV2Package AndAlso Not String.IsNullOrEmpty(FindPartyRequest.ClaimsRiskIndex) Then
            ...
            Try
                ...
                Else
                    oClaimBusiness.Initialise(sUsername:=_SiriusUser.Username, sPassword:=_SiriusUser.Password,
                                   iUserID:=_SiriusUser.UserID, iSourceID:=_SiriusUser.SourceID,
                                   iLanguageID:=_SiriusUser.LanguageID, iCurrencyID:=_SiriusUser.CurrencyID,
                                   iLogLevel:=SiriusUserDefaults.LogLevel, sCallingAppName:=sACMethodName, vDatabase:=oDatabase)
                ...
                Else
                    ...
                    obSIRFindPartyBusiness.Initialise(sUsername:=_SiriusUser.Username, sPassword:=_SiriusUser.Password,
                                                      iUserID:=_SiriusUser.UserID, iSourceID:=_SiriusUser.SourceID,
                                                      iLanguageID:=_SiriusUser.LanguageID, iCurrencyID:=_SiriusUser.CurrencyID,
                                                      iLogLevel:=SiriusUserDefaults.LogLevel, sCallingAppName:=sACMethodName, vDatabase:=oDatabase)
        ...
        ElseIf nTypeOfPackage = enumTypeOfPackage.SAMForInsuranceV2Package AndAlso (Not String.IsNullOrEmpty(FindPartyRequest.RiskRequestdex) OrElse Not String.IsNullOrEmpty(FindPartyRequest.PartyIndex)) Then
            ...
            With FindPartyRequest
                If Not String.IsNullOrEmpty(FindPartyRequest.PartyIndex) Then
                    If FindPartyRequest.IsAnySelected = True Then
        ...
        Else
            ...
            Try
                ...
                Else
                    ...
                    ElseIf ConfigurationManager.AppSettings("PartyRecordsToFetch") IsNot Nothing Then
                        iRecordCount = CInt(ConfigurationManager.AppSettings("PartyRecordsToFetch"))
                    End If
                    Dim oResultArray1(,) As Object = CType(oResultArray, Object(,))
                    With FindPartyRequest

                        iComReturnValue = obSIRFindPartyBusiness.SearchByQuery(r_vResultArray:=oResultArray1,
                                                                             r_lNumberOfRecords:=iRecordCount,
                                                                             v_vShortName:= .Shortname,
                                                                             v_vName:= .Name,
                                                                             v_vFileCode:= .FileCode,
                                                                             v_vClientType:=sPartyDescription,
                                                                             v_vStatusType:= .Status,
                                                                             v_vAddress1:= .AddressLine1,
                                                                             v_vPostalCode:= .PostCode,
                                                                             v_vAreaCode:= .AreaCode,
                                                                             v_vNumber:= .TelephoneNumber,
                                                                             v_vInsuranceRef:= .PolicyRef,
                                                                             v_vDOB:=oDOB,
                                                                             v_vClaimNumber:= .ClaimNumber,
                                                                             v_vAgentCnt:= .AgentKey,
                                                                             v_vRiskIndex:= .ClaimsRiskIndex,
                                                                             v_vValidSourceArray:=CType(oSources, Object),
                                                                             v_vAgentGroupCnt:= .AgentGroupKey,
                                                                             v_lNumberOfRecords:=FindPartyRequest.MaxRowsToFetch,
                                                                             v_vCaseNumber:=FindPartyRequest.CaseNumber,
                                                                             v_vIsAny:= .IsAnySelected)
    ...
    Public Function FindSpecialParty(ByVal con As SiriusConnection, ByVal oFindPartyRequest As BaseFindPartyRequestType) As BaseFindPartyResponseType
        ...
        ' validate the branch code
        oFindPartyRequest.SourceId = oCoreBusiness.GetAndValidateListItemFromCode(Core.STSListType.PMLookup,
        ...
        If nTypeOfPackage = enumTypeOfPackage.SAMForInsuranceV2Package Then
            ...
            Try
                ...
                If nTypeOfPackage = enumTypeOfPackage.SAMForInsuranceV2Package AndAlso Not String.IsNullOrEmpty(oFindPartyRequest.RiskRequestdex) Then
                    With oFindPartyRequest
                        .PartyTypeSpecified = True
                        dsFindSpecialParty = FindPartyByRiskIndex(con, .RiskRequestdex, .PartyTypeSpecified, .PartyType, .AgentGroupKey, .MaxRowsToFetch).ResultArray
                    End With

                    oResponse.ResultData = dsFindSpecialParty
                Else
                    With oFindPartyRequest
                        comReturnValue = oFindPartyBusiness.SearchSpecialPartyByQuery(
                    r_lNumberOfRecords:=iRecordCount,
                    ...
                    v_vFileCode:= .FileCode,
                    v_vClientType:=sPartyDescription,
                    v_vAgentType:=sAgentDescription,
                    v_vStatusType:= .Status,
                    v_vAddress1:= .AddressLine1,
                    v_vPostalCode:= .PostCode,
                    v_vAreaCode:= .AreaCode,
                    v_vNumber:= .TelephoneNumber,
                    v_vInsuranceRef:= .PolicyRef,
                    v_vBranch:= .PartySourceId,
                    v_vValidSourceArray:=oSources,
                    v_bSuppressSubAgents:=Convert.ToBoolean(IIf(.SupressSubAgentsSpecified, .SupressSubAgents, False)),
                    v_lCommissionLevel:=0, V_BranchCode:= .BranchCode, sAgentGroup:= .AgentGroup,
                    v_sSearchType:= .SearchType,
                    v_vPhnNumber:= .TelephoneNumber,
                    v_vClaimNumber:= .ClaimNumber) 'Till CommissionLevel filter is added on nexus screen, we pass default 0
                    End With

                    dsFindSpecialParty = Utilities.ArrayToDataSet(CType(vResultArray, Object), "BaseFindPartyResponseTypeParties")
                    ...
                    If dsFindSpecialParty IsNot Nothing Then
                        If dsFindSpecialParty.Tables(0).Rows.Count > 0 Then
                            ...
                            dsFindSpecialParty.Tables(0).Columns(35).ColumnName = "PartyTypeId"
    ...
    Public Function CopyArrayWithColumns(ByVal oCopyFrom(,) As Object, ByRef oCopyTo(,) As Object, ByVal oWithCol() As Object) As Object(,)
        ...
        For iCount As Integer = iLBoundColFrom To iUBoundColFrom
            For iCheckIfExists As Integer = 0 To oWithCol.GetUpperBound(0)
                If Cast.ToInt32(iCount).ToString <> Cast.ToInt32(oWithCol(iCheckIfExists)).ToString AndAlso
        ...
        For iCount As Integer = iLBoundColFrom To iUBoundColFrom
            For iCheckIfExists As Integer = 0 To oFilterCol.GetUpperBound(0)
                If Cast.ToInt32(iCount).ToString = Cast.ToInt32(oFilterCol(iCheckIfExists)).ToString Then
                    bFilterThisColumn = True
                    Exit For
                ElseIf Cast.ToInt32(iCount).ToString <> Cast.ToInt32(oFilterCol(iCheckIfExists)).ToString AndAlso
    ...
    Public Overloads Function FindParty(ByVal con As SiriusConnection, ByVal FindPartyRequest As BaseFindPartyRequestType) As BaseFindPartyResponseType
        ...
        If nTypeOfPackage = enumTypeOfPackage.SamForBrokingPackage Then
            If CoreBusiness.PartyIsEmployee(oSAMForBrokingRequest.UserPartyType) Then
                Dim iUpper As Integer = vAdditionalData.GetUpperBound(1) + 1
    ...
    Public Function ForgottenPassword(ByVal ForgottenPasswordRequest As BaseForgottenPasswordRequestType) As BaseForgottenPasswordResponseType
        ...
        Try
            ...
            iRet = oGIS.ForgottenPassword(
                                    v_sDataModelCode:="AOL",
                                    v_sBusinessTypeCode:="",
                                    v_sUsername:=ForgottenPasswordRequest.UserName,
                                    v_sIPAddress:="",
                                    v_sSubject:=sSubject,
                                    v_sMessage:=sMessage,
                                    r_sEmailAddress:=sUserEmailAddress,
                                    r_sNewPassword:=sNewUserPassword,
                                    v_vProfileName:=sProfileName,
        ...
        If (iRet = PMEReturnCode.PMTrue) Then
            Try
                ...
                sMessage = String.Format(sMessage,
                                           ForgottenPasswordRequest.UserName,
    ...
    Public Function GetParty(ByVal GetPartyRequest As BaseGetPartyRequestType) As BaseGetPartyResponseType
            ...
            If Trim(sPartyType) = "PC" Then
                ...
                With dataViewPC.ToTable
                    ...
                    For iCnt As Integer = 0 To .Rows.Count - 1
                        ...
                        Select Case Trim(.Rows(iCnt).Item("ContactTypeCode").ToString)
                            ...
                            Case "WORKPHONE"
                                oPartyPC.Contacts(iCnt).ContactTypeCode = ContactTypeType.WORKPHONE
                                oContact.ItemElementName = ItemChoiceType.Number
                            Case "MOBILE"
                            ...
                            Case "LETTER"
                                ...
                                oContact.ItemElementName = ItemChoiceType.Number
                            Case "MORENAME"
                                oPartyPC.Contacts(iCnt).ContactTypeCode = ContactTypeType.MORENAME
                                oContact.ItemElementName = ItemChoiceType.Number
                            Case "OTHER"
                ...
                If dr.Tables(0).Rows.Count > 0 Then
                    With oPartyPC
                        ...
                        If .DateOfBirth < CDate("01 January 1900") Then
                    ...
                    For iCnt As Integer = 0 To .Rows.Count - 1
                        ...
                        Select Case Trim(.Rows(iCnt).Item("ContactTypeCode").ToString)
                            ...
                            Case "HOMEPHONE"
                                ...
                                oContact.ItemElementName = ItemChoiceType.Number
                            Case "WORKPHONE"
                                oPartyCC.Contacts(iCnt).ContactTypeCode = ContactTypeType.WORKPHONE
                                oContact.ItemElementName = ItemChoiceType.Number
                            Case "MOBILE"
                            ...
                            Case "LETTER"
                                ...
                                oContact.ItemElementName = ItemChoiceType.Number
                            Case "MORENAME"
                                oPartyCC.Contacts(iCnt).ContactTypeCode = ContactTypeType.MORENAME
                                oContact.ItemElementName = ItemChoiceType.Number
                            Case "OTHER"
        ...
        AnyError = oCoreBusiness.GetTimestamp(
                            GetPartyRequest.BranchCode,
                            CoreBusiness.LockName.PartyCnt,
                            GetPartyRequest.PartyKey,
                            oGetPartyResponse.PartyTimestamp,
                            bIsLocked)
        ' Return AnyErrors
        If AnyError Is Nothing = False Then
    ...
    Public Function GetPartySummary(ByVal GetPartySummaryRequest As BaseGetPartySummaryRequestType) As BaseGetPartySummaryResponseType
        ...
        oCoreBusiness.GetSystemOption(GetPartySummaryRequest.BranchCode,
                ...
                If dr.Tables.Count = 1 Then
                    ...
                    For counter = 0 To dr.Tables(0).Rows.Count - 1
                        ...
                        If IsDate(dr.Tables(0).Rows(counter).Item("CoverStartDate")) _
                            ...
                            If Informations.DateDiff(DateInterval.Month, Cast.ToDateTime(dr.Tables(0).Rows(counter).Item("CoverStartDate"), Date.MinValue),
    ...
    Public Overloads Function UpdateParty(ByVal con As SiriusConnection, ByVal UpdatePartyRequest As BaseUpdatePartyRequestType) As BaseUpdatePartyResponseType
        ...
        AnyError = oCoreBusiness.CheckTSAndLock(con,
            BranchCode:=UpdatePartyRequest.BranchCode,
            Lockname:=CoreBusiness.LockName.PartyCnt,
            LockValue:=UpdatePartyRequest.PartyKey,
                ...
                For addCNT As Integer = UpdatePartyRequest.Party.Addresses.GetLowerBound(0) To _
                    ...
                    If oAddressWithContacts IsNot Nothing Then
                        ...
                        If vSingleContactsArray IsNot Nothing Then
                            If vAddContacts Is Nothing Then
                                existUBound = 0
                                ReDim Preserve vAddContacts(4, Microsoft.VisualBasic.UBound(vSingleContactsArray, 2))
                                For CountFor As Integer = 0 To Microsoft.VisualBasic.UBound(vSingleContactsArray, 2)
                                    vAddContacts(0, CountFor) = vSingleContactsArray(0, CountFor)
                                    vAddContacts(1, CountFor) = vSingleContactsArray(1, CountFor)
                                    vAddContacts(2, CountFor) = vSingleContactsArray(2, CountFor)
                            ...
                            Else
                                ...
                                existUBound = Microsoft.VisualBasic.UBound(vAddContacts, 2)
                                ReDim Preserve vAddContacts(4, (existUBound + Microsoft.VisualBasic.UBound(vSingleContactsArray, 2)))
                                For CountFor As Integer = 0 To Microsoft.VisualBasic.UBound(vSingleContactsArray, 2)
                                    vAddContacts(0, existUBound + CountFor) = vSingleContactsArray(0, CountFor)
                                    vAddContacts(1, existUBound + CountFor) = vSingleContactsArray(1, CountFor)
                                    vAddContacts(2, existUBound + CountFor) = vSingleContactsArray(2, CountFor)
        ...
        If bOtherParty = True Then
            ' Call Validate Data
            ValidateUpdatePartyData(oCoreBusiness, oBasePartyOther,
        ...
        If oBasePartyOther.SupplierBusiness IsNot Nothing Then
            ProcessSupplierBusiness(oBasePartyOther.SupplierBusiness,
                                        r_oSuppBusiness,
        ...
        Try
            ...
            iRet = CType(oGIS.UpdateParty(
                    sPartyTypeCode:=sPartyType,
                    sBranchCode:=UpdatePartyRequest.BranchCode,
                    vAddresses:=DirectCast(vAddresses, Object(,)),
                    sTPUserCode:=UpdatePartyRequest.Party.TPUserCode,
                    sTPIntroducer:=UpdatePartyRequest.Party.TPIntroducer,
                    vContacts:=DirectCast(vContacts, Object(,)),
                    sSurname:=oBasePartyPC.Surname,
                    sForename:=oBasePartyPC.Forename,
                    sDateOfBirth:=CType(oBasePartyPC.DateOfBirth, String),
                    sTitle:=oBasePartyPC.Title,
                    sMaritalStatusCode:=sMaritalStatusCode,
                    sGenderCode:=oBasePartyPC.GenderCode,
                    sInitials:=oBasePartyPC.Initials,
                    sOccupationCode:=oBasePartyPC.OccupationCode,
                    sEmployerBusinessCode:=oBasePartyPC.EmployersBusinessCode,
                    sEmploymentStatusCode:=oBasePartyPC.EmploymentStatusCode.ToString,
                    sAlternativeID:=sAlternativeIdentifier,
                    sCompanyName:=oBasePartyCC.CompanyName,
                    sTradingName:="",
                    sBusinessCode:=sBusinessCode,
                    lAgentCnt:=iAgentCnt,
                    r_lPartyCnt:=UpdatePartyRequest.PartyKey,
                    vAddressContacts:=DirectCast(vAddContactsSimple, Object(,)),
                    r_sTaxNumber:=sTaxNumber,
                    r_bDomiciledForTax:=CShort(bDomiciledForTax),
                    r_bTaxExempt:=CShort(bTaxExempt),
                    r_lPercentage:=lPercentage,
                    vSupplierBusiness:=Nothing,
                    v_sFileCode:=UpdatePartyRequest.Party.FileCode,
            ...
            If nTypeOfPackage = enumTypeOfPackage.SAMForInsurancePackage Or
                ...
                If bOtherParty = True Then
                    ...
                    ' process party branch selection
                    ProcessOtherPartyBranches(con, oBasePartyOther.Branches, UpdatePartyRequest.PartyKey)

                    ' process party details
                    UpdateOtherPartyDetails(con, oBasePartyOther, UpdatePartyRequest.PartyKey, oUpdatePartyAddDetails)
                ...
                ElseIf sPartyType = "CC" Then
                    ...
                    For nCount = 0 To UpdatePartyRequest.Party.Addresses.Length
                        If UpdatePartyRequest.Party.Addresses(nCount).AddressTypeCode = ACAddressCorrespondence Then
                            ...
                            UpdatePartyAccountAddressDetails(con, UpdatePartyRequest.Party.Addresses(nCount).AddressLine1,
                                                                    UpdatePartyRequest.Party.Addresses(nCount).AddressLine2,
                                                                    UpdatePartyRequest.Party.Addresses(nCount).AddressLine3,
                                                                    UpdatePartyRequest.Party.Addresses(nCount).AddressLine4,
                                                                    UpdatePartyRequest.Party.Addresses(nCount).PostCode,
                                                                    UpdatePartyRequest.Party.Addresses(nCount).CountryCode,
                                                                    UpdatePartyRequest.PartyKey)
                            Exit For
                        End If
    ...
    '''<param name="sNumber" type="String"></param>
    '''<remarks></remarks> 
    Private Overloads Sub ValidatePartyBank(ByVal conPartyBank As SiriusConnection,
                                        ByRef oSAMErrorCollection As SAMErrorCollection,
                                        ByVal lPartyBankKey As Integer,
                                        Optional ByVal lPartyKey As Integer = 0,
                                        Optional ByVal lAccountKey As Integer = 0,
                                        Optional ByVal bIsBank As Boolean = False,
                                        Optional ByVal sNumber As String = Nothing)
        ...
        If Not r_bIsValid Then
            oSAMErrorCollection.AddInvalidData(SAMInvalidData.ValidationRulesFailed,
                                            "Either the given party bank key is invalid or it does not belong to given PartyKey/AccountKey",
                                            "PartyBankKey",
        ...
        If Not String.IsNullOrEmpty(sNumber) Then
            If Not r_bISNumberExisting Then
                If bIsBank Then
                    oSAMErrorCollection.AddInvalidData(SAMInvalidData.ValidationRulesFailed,
                                                    "Bank account number mismatch for the given party bank item",
                                                    "PartyBankKey",
                                                  lPartyBankKey.ToString())
                    'Else
                    '    oSAMErrorCollection.AddInvalidData(SAMInvalidData.ValidationRulesFailed,
                    '                                "Credit card number mismatch for the given party bank item",
                    '                                "PartyBankKey",
                    '                              lPartyBankKey.ToString())
                End If
    ...
    '''<remarks></remarks> 
    Private Sub ValidatePartyBankLastTransaction(ByVal conPartyBank As SiriusConnection,
                                        ByRef bIsValidTransaction As Boolean,
                                        ByRef bIsValidTransactionParty As Boolean,
                                        ByVal lPartyKey As Integer,
                                        ByVal oTransactionType As LastTransactionTypeWithPartyBank,
                ...
                With oPartyBankItem
                    ...
                    If oPartyBankItem.IsBankItem Then
                        ...
                        With .Bank
                            ...
                            With .BankAddress
                                ...
                                iCountryKey = ToSafeInteger(Cast.ToString(drPartyBank.Item("bank_country"), "0"))
                    ...
                    Else
                        ...
                        With .CreditCard
                            ...
                            .TrackingNumber = Cast.ToString(drPartyBank.Item("cc_tracking_number"), String.Empty)
                            .IsDefaultCreditCard = Cast.ToBoolean(drPartyBank.Item("is_default"), False)
                            'Card Holder Address
    ...
    Public Function UpdatePartyBankItem(ByVal conPartyBank As SiriusConnection, ByVal oCoreBusiness As CoreBusiness, ByVal oPartyBankItem As BasePartyBankType, ByVal lPartyKey As Integer, ByVal bMultiPartyBankTransaction As Boolean) As BaseUpdatePartyBankStatusType
        ...
        With oPartyBankItem
            ...
            Try
                ...
                CreateEvent(conPartyBank, lPartyKey,
                            Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing,
                            1, _SiriusUser.UserID, Now, sDescription,
    ...
    Private Function UpdatePartyBankItem(ByVal conPartyBank As SiriusConnection, ByVal oPartyBankItem As BasePartyBankType, ByVal lPartyKey As Integer) As Integer
        ...
        With oPartyBankItem
            Using cmd As SiriusCommand = SiriusCommand.FromProcedure("spu_PartyBank_Details_Upd")
                ...
                If .IsBankItem Then
                    cmd.AddInParameter("@is_bank", SqlDbType.TinyInt).Value = 1
                    cmd.AddInParameter("@IsDefault", SqlDbType.TinyInt).Value = 0
                Else
                    cmd.AddInParameter("@is_bank", SqlDbType.TinyInt).Value = 0
                End If
                ...
                If .CreditCard IsNot Nothing Then
                    With .CreditCard
                        ...
                        End If
                        If .IsDefaultCreditCard Then
                            cmd.AddInParameter("@IsDefault", SqlDbType.TinyInt).Value = 1
                        Else
                            cmd.AddInParameter("@IsDefault", SqlDbType.TinyInt).Value = 0
                        End If
                        'Credit card address
...
#End Region
        ...
        With oPartyBankItem
            ...
            If .PartyBankKey <> 0 Then
                oSAMErrorCollection.AddInvalidData(
                    SAMConstants.SAMInvalidData.ValidationRulesFailed,
                    "AddPartyBank method does not update existing party bank item.",
    ...
    Private Sub ValidatePartyPCTypeLookup(ByRef oBasePartyPC As BasePartyPCType,
        ...
        If Not String.IsNullOrEmpty(oBasePartyPC.SecOccupationCode) Then
            Try
                ...
                Dim strSecOccupationCode As Object = GetListCode(Of String)(Core.STSListType.GisList, "2228226",
        ...
        If Not String.IsNullOrEmpty(oBasePartyPC.SecEmployersBusinessCode) Then
            Try
                ...
                Dim strSecEmployersBusinessCode As Object = GetListCode(Of String)(Core.STSListType.GisList, "2228228",
            ...
            For Each oLifeStyle As BasePartyPCTypeLifestyle In oBasePartyPC.Lifestyle
                ...
                If Not String.IsNullOrEmpty(oLifeStyle.OccupationCode) Then
                    Try
                        ...
                        Dim strOccupationCode As Object = GetListCode(Of String)(Core.STSListType.GisList, "2228226",
    ...
    Private Sub ValidatePartyCCTypeLookup(ByRef oBasePartyCC As BasePartyCCType,
            ByRef oCoreBusiness As CoreBusiness,
            ByRef STSError As STSErrorPublisher)
        ...
        If Not String.IsNullOrEmpty(oBasePartyCC.TurnoverCode) Then
            Try
                ...
                Dim strTurnoverCode As Object = GetListCode(Of String)(Core.STSListType.PMLookup, "TurnoverBand",
        ...
        If Not String.IsNullOrEmpty(oBasePartyCC.TradeCode) Then
            Try
                ...
                Dim strTradeCode As Object = GetListCode(Of String)(Core.STSListType.GisList, "2228228",
    ...
    Private Sub ValidatePartyOtherTypeLookup(ByRef oBasePartyOther As BasePartyOTHERType,
            ByRef oCoreBusiness As CoreBusiness,
            ByRef STSError As STSErrorPublisher)
    ...
    Private Sub ValidateClientSharedDataLookup(ByRef oClientData As BaseClientSharedDataType,
            ByRef oCoreBusiness As CoreBusiness,
            ByRef STSError As STSErrorPublisher)
        ...
        If oClientData IsNot Nothing Then
            ...
            If Not String.IsNullOrEmpty(oClientData.PaymentCode) Then
                Try
                    ...
                    Dim strPaymentCode As Object = GetListCode(Of String)(Core.STSListType.PMLookup, "Payment_Method",
                ...
                For Each oLoyalty As BaseClientSharedDataTypeLoyaltyScheme In oClientData.LoyaltyScheme
                    If Not String.IsNullOrEmpty(oLoyalty.LoyaltySchemeCode) Then
                        Try
                            ...
                            Dim strLoyaltySchemeCode As Object = GetListCode(Of String)(Core.STSListType.PMLookup, "Loyalty_Scheme",
    ...
    Private Sub ValidateConvictionType(ByVal oConvictions As BaseConvictionType(),
            ByRef oCoreBusiness As CoreBusiness,
            ByRef STSError As STSErrorPublisher)
            ...
            For Each oConviction As BaseConvictionType In oConvictions
                ...
                If Not String.IsNullOrEmpty(oConviction.TypeCode) Then
                    Try
                        ...
                        Dim strTypeCode As Object = GetListCode(Of String)(Core.STSListType.UserDefinedTable, "1114113",
                ...
                If Not String.IsNullOrEmpty(oConviction.StatusCode) Then
                    Try
                        ...
                        Dim strStatusCode As Object = GetListCode(Of String)(Core.STSListType.UserDefinedTable, "1114124",
                ...
                If Not String.IsNullOrEmpty(oConviction.SentenceTypeCode) Then
                    Try
                        ...
                        Dim strSentenceTypeCode As Object = GetListCode(Of String)(Core.STSListType.UserDefinedTable, "1114119",
                ...
                If Not String.IsNullOrEmpty(oConviction.SentenceDurationQualifier) Then
                    Try
                        ...
                        Dim strSentenceDurationQualifier As Object = GetListCode(Of String)(Core.STSListType.UserDefinedTable, "1114122",
                ...
                If Not String.IsNullOrEmpty(oConviction.AlcoholMeasurementMethod) Then
                    Try
                        ...
                        Dim strAlcoholMeasurementMethod As Object = GetListCode(Of String)(Core.STSListType.UserDefinedTable, "1114126",
    ...
    Private Sub ProcessOtherPartySupplierBusiness(ByVal con As SiriusConnection,
    ByVal oSupplierBusinesses As BasePartyOTHERTypeSupplierBusiness(),
    ...
    Private Sub DeleteOtherPartySupplierBusiness(ByVal con As SiriusConnection,
    ByVal PartyKey As Integer)
    ...
    Private Sub AddOtherPartySupplierBusiness(ByVal con As SiriusConnection,
    ByVal oSupplierBusiness As BasePartyOTHERTypeSupplierBusiness,
    ByVal PartyKey As Integer)
    ...
    Private Sub ProcessOtherPartyBranches(ByVal con As SiriusConnection,
                                          ByVal oBranches As BasePartyOTHERTypeBranch(),
                                          ByVal PartyKey As Integer)

        ' delete all existing party supplier business
        DeleteOtherPartyBranches(con, PartyKey)

        If oBranches IsNot Nothing AndAlso oBranches.Length > 0 Then
            ' add each item back in for this party
            For Each oBranch As BasePartyOTHERTypeBranch In oBranches
                AddOtherPartyBranch(con, oBranch, PartyKey)
            Next
        End If

    End Sub

    Private Sub DeleteOtherPartyBranches(ByVal con As SiriusConnection,
                                         ByVal PartyKey As Integer)

        Using cmd As SiriusCommand = SiriusCommand.FromProcedure("spu_Other_Party_PLDSource")
            cmd.AddInParameter("@PartyCnt", SqlDbType.Int).Value = PartyKey
            cmd.AddInParameter("@Branchid", SqlDbType.Int).Value = 0
            con.ExecuteNonQuery(cmd)
        End Using

    End Sub

    Private Sub AddOtherPartyBranch(ByVal con As SiriusConnection,
                                    ByVal oBranch As BasePartyOTHERTypeBranch,
                                    ByVal PartyKey As Integer)

        Using cmd As SiriusCommand = SiriusCommand.FromProcedure("spu_Other_Party_PLSSource")
            cmd.AddInParameter("@PartyCnt", SqlDbType.Int).Value = PartyKey
            cmd.AddInParameter("@Branchid", SqlDbType.Int).Value = oBranch.BranchId
    ...
    Private Sub UpdateOtherPartyDetails(ByVal con As SiriusConnection,
    ByVal oOtherParty As BasePartyOTHERType,
    ByVal PartyKey As Integer, Optional ByVal oUpdatePartyAddDetails As Hashtable = Nothing)
        ...
        Using cmd As SiriusCommand = SiriusCommand.FromProcedure("spe_party_other_upd")
            ...
            cmd.AddInParameter("@date_of_birth", SqlDbType.DateTime).Value = Cast.NullIfDefault(oOtherParty.DateOfBirth)
            cmd.AddInParameter("@gender", SqlDbType.VarChar, 70).Value = Cast.NullIfDefault(oOtherParty.Gender, String.Empty)
            ...
            cmd.AddInParameter("@callfromSAM", SqlDbType.TinyInt).Value = True
            If oOtherParty.IsTPASettleDirectly IsNot Nothing AndAlso oOtherParty.IsTPASettleDirectly.ToUpper() = "TRUE" Then
                cmd.AddInParameter("@is_TPA_Settle_directly", SqlDbType.TinyInt).Value = True
            ElseIf oOtherParty.IsTPASettleDirectly IsNot Nothing AndAlso oOtherParty.IsTPASettleDirectly.ToUpper() = "FALSE" Then
                cmd.AddInParameter("@is_TPA_Settle_directly", SqlDbType.TinyInt).Value = False
            Else
                cmd.AddInParameter("@is_TPA_Settle_directly", SqlDbType.TinyInt).Value = Nothing
            End If
            cmd.AddInParameter("@name", SqlDbType.VarChar, 255).Value = oOtherParty.Name
            ...
            cmd.AddInParameter("@date_passed_test", SqlDbType.DateTime).Value = Date.Today
    ...
    Private Sub ProcessOtherPartyAccidents(
      ByVal con As SiriusConnection,
      ByVal oAccidents As BasePartyOTHERTypeAccident(),
      ByVal PartyKey As Integer)
        ...
        If oAccidents IsNot Nothing Then
            ...
            For Each oAccident As BasePartyOTHERTypeAccident In oAccidents
                ...
                End If
            Next
            If oExistingAccidents IsNot Nothing AndAlso oExistingAccidents.Length > 0 Then
    ...
    Private Sub ProcessOtherPartyConvictions(
    ByVal con As SiriusConnection,
    ByVal oConvictions As BasePartyOTHERTypeConviction(),
    ByVal PartyKey As Integer)
    ...
    ' got from Jaideep
    Private Sub ProcessPartyConvictions(
        ByVal con As SiriusConnection,
        ByVal oBusiness As CoreBusiness,
        ByVal oConvictions As BaseConvictionType(),
        ByVal PartyKey As Integer)
    ...
    Private Sub DeleteAllOtherPartyAccidents(ByVal con As SiriusConnection,
    ByVal PartyKey As Integer)
    ...
    Private Sub AddOtherPartyAccident(
    ByVal con As SiriusConnection,
    ByVal oAccident As BasePartyOTHERTypeAccident,
    ByVal PartyKey As Integer)
    ...
    Private Sub UpdateOtherPartyAccident(
    ByVal con As SiriusConnection,
    ByVal oAccident As BasePartyOTHERTypeAccident,
    ByVal PartyKey As Integer)
    ...
    Private Sub DeleteOtherPartyAccident(
    ByVal con As SiriusConnection,
    ByVal AccidentKey As Integer,
    ByVal PartyKey As Integer)
    ...
    '''<remarks></remarks>
    Private Sub DeleteAllOtherPartyConvictions(ByVal con As SiriusConnection,
    ByVal PartyKey As Integer)
    ...
    Private Sub AddOtherPartyConviction(
    ByVal con As SiriusConnection,
    ByVal oConviction As BasePartyOTHERTypeConviction,
    ByVal PartyKey As Integer)
    ...
    ' Girija: differing with Tech Spec
    Private Sub UpdateOtherPartyConviction(
    ByVal con As SiriusConnection,
    ByVal oConviction As BasePartyOTHERTypeConviction,
    ByVal PartyKey As Integer)
    ...
    Private Sub DeleteOtherPartyConviction(
    ByVal con As SiriusConnection,
    ByVal ConvictionKey As Integer,
    ByVal PartyKey As Integer)
    ...
    '''<remarks></remarks>
    Private Sub UpdatePartyConvictions(
    ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oConviction As BaseConvictionType,
    ByVal PartyKey As Integer)
    ...
    Private Sub UpdatePartyAssociates(
    ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oAssociate As BaseAssociateType,
    ByVal PartyKey As Integer)
    ...
    '''<remarks></remarks>
    Private Sub UpdatePartyLifeStyle(
    ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oLifestyle As BasePartyPCTypeLifestyle,
    ByVal PartyKey As Integer)
        ...
        Using cmd As SiriusCommand = SiriusCommand.FromProcedure("spe_party_lifestyle_upd")
            ...
            If oLifestyle.GenderCodeSpecified Then
                cmd.AddInParameter("@gender_code", SqlDbType.VarChar, 70).Value =
    ...
    Private Sub UpdatePartyLoyaltyScheme(
    ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oLoyaltyScheme As BaseClientSharedDataTypeLoyaltyScheme,
    ByVal PartyKey As Integer)
    ...
    '''<remarks></remarks>
    Private Sub UpdatePartyProspectPolicies(
    ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oProspectPolicy As BaseClientSharedDataTypeProspectPolicies,
    ByVal PartyKey As Integer)
    ...
    '''<param name="oConviction" type="BaseConvictionType"></param>
    '''<remarks></remarks>
    Private Sub AddPartyConviction(
    ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oConviction As BaseConvictionType,
    ByVal PartyKey As Integer)
    ...
    Private Sub AddPartyAssociates(
    ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oAssociate As BaseAssociateType,
    ByVal PartyKey As Integer)
    ...
    Private Sub DeletePartyAssociates(
    ByVal con As SiriusConnection,
    ByVal iAssociateKey As Integer,
    ...
    ' Vivek: differing from Tech Spec
    Private Sub DeletePartyConviction(
    ByVal con As SiriusConnection,
    ByVal iConvictionKey As Integer,
    ByVal iPartyKey As Integer)
    ...
    Private Sub DeletePartyLifeStyle(
    ByVal con As SiriusConnection,
    ByVal iLifestyleKey As Integer,
    ByVal iPartyKey As Integer)
    ...
    Private Sub DeletePartyLoyaltyScheme(
    ByVal con As SiriusConnection,
    ByVal iLoyaltySchemeKey As Integer,
    ByVal iPartyKey As Integer)
    ...
    Private Sub DeletePartyProspectPolicies(
    ByVal con As SiriusConnection,
    ByVal ProspectPolicyKey As Integer,
    ByVal PartyKey As Integer)
    ...
    Private Sub AddPartyLoyaltyScheme(
    ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oLoyaltyScheme As BaseClientSharedDataTypeLoyaltyScheme,
    ByVal PartyKey As Integer)
    ...
    Private Sub AddPartyProspectPolicies(
    ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oProspectPolicies As BaseClientSharedDataTypeProspectPolicies,
    ByVal PartyKey As Integer)
    ...
    Private Sub DeletePartyAddressUsage(
    ByVal con As SiriusConnection,
    ByVal PartyKey As Integer)
    ...
    Private Sub SaveAddressContactUsage(
    ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oAddressWithContacts As BaseAddressWithContactsType,
    ...
    Private Sub SaveContactAddressUsage(ByVal con As SiriusConnection,
    ...
    Private Function SaveAddressDetails(ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oAddressWithContacts As BaseAddressWithContactsType,
    ByVal SourceId As Integer,
    ...
    Private Function SaveContactDetails(ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oContact As BaseContactType,
    ...
    Private Sub ProcessGetPartyData(ByVal oBusiness As CoreBusiness,
    ByVal oRequest As BaseGetPartyRequestType,
    ByVal oResponse As BaseGetPartyResponseType,
    ...
    Private Sub GetPartyData(ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oRequest As BaseGetPartyRequestType,
    ByVal oResponse As BaseGetPartyResponseType,
    ByVal sPartyType As String)
    ...
    Private Sub GetPartyBaseData(ByVal con As SiriusConnection,
    ByVal lPartyCnt As Integer,
    ByVal oParty As BasePartyType,
    ByVal dr As DataRow)
    ...
    Private Function GetPartyAddresses(ByVal con As SiriusConnection,
                        ...
                        If iContactAddressCnt = iAddressCnt Then
                            ...
                            Select Case sContactType
                                ...
                                Case ContactType.WorkPhone
                                    oContact.ContactTypeCode = ContactTypeType.WORKPHONE
                                ...
                                Case ContactType.MoreName
                                    oContact.ContactTypeCode = ContactTypeType.MORENAME
                            End Select
    ...
    Private Function GetPartyContacts(ByVal con As SiriusConnection,
    ByVal oParty As BasePartyType,
    ByVal lPartyCnt As Integer
    ) As BaseContactType()
        ...
        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            ...
            For Each dr As DataRow In dt.Rows
                ...
                If Cast.ToString(dr.Item("Contact_Type_Code"), String.Empty).Trim = "OTHER" Then
                    oContact.ContactTypeDescription = "OTHER"
                Else
                    oContact.ContactTypeDescription = Cast.ToString(dr.Item("ContactTypeDescription"))
                End If
    ...
    Private Function GetPartyAccidents(ByVal con As SiriusConnection,
    ...
    Private Function GetPartySupplierBusiness(ByVal con As SiriusConnection,
    ...
    Private Function GetPartyConvictions(ByVal con As SiriusConnection,
    ...
    Private Function GetPartyConvictions_v2(ByVal con As SiriusConnection,
    ...
    Private Function GetPartyAssociates(ByVal con As SiriusConnection,
    ...
    Private Function GetPartyLifeStyle(ByVal con As SiriusConnection,
    ...
    Private Function GetPartyLoyaltyScheme(ByVal con As SiriusConnection,
    ...
    Private Function GetPartyProspectPolicies(ByVal con As SiriusConnection,
    ...
    Private Sub GetPartyOtherData(ByVal con As SiriusConnection,
    ByVal oRequest As BaseGetPartyRequestType,
    ByVal lPartyCnt As Integer,
    ByVal oParty As BasePartyOTHERType,
    ByVal dr As DataRow)
        ...
        If String.IsNullOrEmpty(dr.Item("date_of_birth").ToString()) Then
            oParty.DateOfBirth = Nothing
        Else
            oParty.DateOfBirth = CDate(dr.Item("date_of_birth"))
        End If
        'oParty.DateOfBirth = CDate(IIf(String.IsNullOrEmpty(dr.Item("date_of_birth").ToString()), Nothing, dr.Item("date_of_birth")))
        ...
        oParty.IsTPASettleDirectly = Convert.ToString(dr.Item("is_TPA_settle_directly"))
        ...
        ' get supplier business
        oParty.Branches = GetOtherPartyBranches(con, lPartyCnt)
    ...
    Private Function GetOtherPartyBranches(ByVal con As SiriusConnection,
                                           ByVal lPartyCnt As Integer) As BasePartyOTHERTypeBranch()

        Dim dt As DataTable = Nothing

        Using cmd As SiriusCommand = SiriusCommand.FromProcedure("spu_Other_party_PLLSource")
            cmd.AddInParameter("PartyCnt", SqlDbType.Int).Value = lPartyCnt
            dt = con.ExecuteDataTable(cmd)
        ...
        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            ...
            Dim oBranches As BasePartyOTHERTypeBranch() = Nothing

            For Each dr As DataRow In dt.Rows
                If Not String.IsNullOrEmpty(dr.Item("Chosen").ToString()) AndAlso Convert.ToInt32(dr.Item("Chosen")) = 1 Then
                    Dim oBranch As New BasePartyOTHERTypeBranch
                    oBranch.BranchId = Convert.ToInt32(dr.Item("source_id"))
                    oBranch.Description = Cast.ToString(dr.Item("description"), String.Empty).Trim
                    Array.Resize(oBranches, iBranchCount + 1)

                    oBranches(iBranchCount) = oBranch
                    iBranchCount += 1
                End If
            Next
            Return oBranches
    ...
    Private Sub ValidateAddPartyData(ByVal oBusiness As CoreBusiness,
                                     ByVal oParty As BasePartyOTHERType,
        ...
        If Not oParty.TypeCode.StartsWith("OT") Then
            ...
            oSamErrorCollection.AddInvalidData(SAMInvalidData.OtherPartyTypeCodeDoesntStartWithOT,
                                                SAMInvalidData.OtherPartyTypeCodeDoesntStartWithOT.ToString,
                                                "TypeCode")
        Else
            ' party type 
    ...
    Private Sub ValidateUpdatePartyData(ByVal oBusiness As CoreBusiness,
        ByVal oParty As BasePartyOTHERType,
        ByVal lPartyCnt As Integer,
        ByVal oSamErrorCollection As SAMErrorCollection,
        ByVal nTypeOfPackage As enumTypeOfPackage)
        ...
        If nTypeOfPackage = enumTypeOfPackage.SAMForInsuranceV2Package Then
            If oParty.Convictions IsNot Nothing Then
                ...
                ValidateConviction(oParty.Convictions,
                                    lPartyCnt,
                                    oSamErrorCollection)
            End If
        Else
            If oParty.Conviction IsNot Nothing Then
                ' Validate Conviction id's
                ValidateConviction(oParty.Conviction,
                                    lPartyCnt,
                                    oSamErrorCollection)
            End If
        End If
        ...
        If oParty.Accident IsNot Nothing Then
            ...
            ValidateAccident(oParty.Accident,
                                lPartyCnt,
                                oSamErrorCollection)
        End If
    ...
    Private Sub ValidateConviction(ByVal oConviction() As SiriusFS.SAM.Structure.BaseImplementationTypes.BasePartyOTHERTypeConviction,
                                        ByVal lPartyCnt As Integer,
                                        ByVal oSamErrorCollection As SAMErrorCollection)
        ...
        For ConvictionCount As Integer = oConviction.GetLowerBound(0) To oConviction.GetUpperBound(0)
            If oConviction(ConvictionCount).ConvictionKey <> 0 Then
                oBusiness.setDefaultValues("Party_Conviction",
                                                "party_cnt",
                                                 lPartyCnt.ToString,
                                                r_dsOptionalValues,
                                                "Party_Conviction_Id",
    ...
    ' Vivek: differing from Tech Spec
    Private Sub ValidateConviction(ByVal oConviction() As BaseImplementationTypes.BaseConvictionType,
                                        ByVal lPartyCnt As Integer,
                                        ByVal oSamErrorCollection As SAMErrorCollection)
    ...
    Private Sub ValidateAccident(ByVal oAccidents() As SiriusFS.SAM.Structure.BaseImplementationTypes.BasePartyOTHERTypeAccident,
                                    ByVal lPartyCnt As Integer,
                                    ByVal oSamErrorCollection As SAMErrorCollection)
        Dim oBusiness As New CoreBusiness
        Dim r_dsOptionalValues As New DataSet
        ...
        For AccidentCount As Integer = oAccidents.GetLowerBound(0) To oAccidents.GetUpperBound(0)
            If oAccidents(AccidentCount).AccidentKey <> 0 Then
                ...
                oBusiness.setDefaultValues("Previous_Accidents",
                                                "party_cnt",
                                                strParty,
                                                r_dsOptionalValues,
    ...
    'GAURAV
    Private Sub ProcessConvictions(ByVal oPartyConvictions() As SiriusFS.SAM.Structure.BaseImplementationTypes.BasePartyOTHERTypeConviction,
                                            ByRef r_oConvictions(,) As Object,
    ...
    ' got from Jaideep
    Private Sub ProcessAssociates(
    ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oAssociates As BaseAssociateType(),
    ByVal PartyKey As Integer)
    ...
    '''<remarks></remarks>
    ' got from Jaideep
    Private Sub ProcessLifeStyle(
    ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oLifeStyles As BasePartyPCTypeLifestyle(),
    ByVal PartyKey As Integer)
    ...
    ' got from Jaideep
    Private Sub ProcessLoyaltyScheme(
    ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oLoyaltySchemes As BaseClientSharedDataTypeLoyaltyScheme(),
    ByVal PartyKey As Integer)
    ...
    ' got from Jaideep
    Private Sub ProcessProspectPolicies(
    ByVal con As SiriusConnection,
    ByVal oBusiness As CoreBusiness,
    ByVal oProspectPolicies As BaseClientSharedDataTypeProspectPolicies(),
    ByVal PartyKey As Integer)
    ...
    Private Sub ProcessAccidents(ByVal oAccidents() As SiriusFS.SAM.Structure.BaseImplementationTypes.BasePartyOTHERTypeAccident,
                                            ByRef r_oAccidents(,) As Object,
                                            Optional ByVal lPartyCnt As Integer = 0)
        Dim oBusiness As New CoreBusiness
    ...
    Private Sub ProcessOtherPartyInfo(ByVal oOtherPartyInfo As SiriusFS.SAM.Structure.BaseImplementationTypes.BasePartyOTHERType,
                                                ByRef r_oOtherPartyInfo() As Object,
                                                Optional ByVal lPartyCnt As Integer = 0,
                                                Optional ByVal SetDefaults As Boolean = False)

        r_oOtherPartyInfo(0) = lPartyCnt

        r_oOtherPartyInfo(1) = IIf(oOtherPartyInfo.LicenseTypeId = 0, Nothing, oOtherPartyInfo.LicenseTypeId) 'Optional
        r_oOtherPartyInfo(2) = oOtherPartyInfo.LicenseNumber 'Optional
        r_oOtherPartyInfo(3) = IIf(oOtherPartyInfo.DateOfBirth = Date.MinValue, Nothing, oOtherPartyInfo.DateOfBirth) 'Optional
        r_oOtherPartyInfo(4) = IIf(oOtherPartyInfo.GenderId = 0, Nothing, oOtherPartyInfo.GenderId) 'Optional
        r_oOtherPartyInfo(5) = IIf(oOtherPartyInfo.DrivingStatusId = 0, Nothing, oOtherPartyInfo.DrivingStatusId) 'Optional
        ...
        r_oOtherPartyInfo(24) = IIf(oOtherPartyInfo.ActiveIndicator = True, 1, 0)
        r_oOtherPartyInfo(25) = IIf(oOtherPartyInfo.AfterHoursIndicator = True, 1, 0)
        ...
        If SetDefaults = True Then
            ...
            Dim oBusiness As New CoreBusiness
            Dim r_dsOptionalValues As DataSet = Nothing
            oBusiness.setDefaultValues("Party_other",
                                            "party_cnt",
                                            lPartyCnt.ToString,
                                            r_dsOptionalValues)
    ...
    Private Sub ProcessSupplierBusiness(ByVal oSuppBusiness() As SiriusFS.SAM.Structure.BaseImplementationTypes.BasePartyOTHERTypeSupplierBusiness,
                                            ByRef r_oSuppBusiness(,) As Object,
                                            Optional ByVal lPartyCnt As Integer = 0)
        ...
        For cnt As Integer = oSuppBusiness.GetLowerBound(0) To oSuppBusiness.GetUpperBound(0)
            ...
            r_oSuppBusiness(cnt, 1) = IIf(oSuppBusiness(cnt).SpecialityId = 0, Nothing, oSuppBusiness(cnt).SpecialityId)
            r_oSuppBusiness(cnt, 2) = IIf(oSuppBusiness(cnt).BusinessId = 0, Nothing, oSuppBusiness(cnt).BusinessId)
        Next
    ...
    ' given by Rahul
    Private Sub ProcessProspect(
            ByVal con As SiriusConnection,
            ByVal oBusiness As CoreBusiness,
            ByVal oProspect As BaseClientSharedDataType,
            ByVal PartyKey As Integer)
    ...
    Private Function GetPartyProspect(ByVal con As SiriusConnection,
    ...
    '''<param name="lPartyCnt" type="Integer"></param>
    '''<remarks></remarks>
    Private Function GetClientAccountDetails(ByVal con As SiriusConnection, ByVal lBranchId As Integer,
    ...
    '''<remarks></remarks>
    Private Sub UpdatePartyProspect(
        ByVal con As SiriusConnection,
        ByVal oBusiness As CoreBusiness,
        ByVal oProspect As BaseClientSharedDataType,
        ByVal PartyKey As Integer)
        ...
        Using cmd As SiriusCommand = SiriusCommand.FromProcedure("spe_party_prospect_upd")
            ...
            cmd.AddInParameter("@party_cnt", SqlDbType.Int).Value = PartyKey
            cmd.AddInParameter("@current_intermediary", SqlDbType.Int).Value =
            ...
            cmd.AddInParameter("@previous_broker_cnt", SqlDbType.Int).Value =
            ...
            cmd.AddInParameter("@previous_insurer_cnt", SqlDbType.Int).Value =
    ...
    '''<remarks></remarks>
    Private Sub UpdatePartyCCDetails(
        ByVal con As SiriusConnection,
        ByVal oBusiness As CoreBusiness,
        ByVal oPartyCCType As BasePartyCCType,
        ByVal PartyKey As Integer, Optional ByVal oUpdatePartyAddDetails As Hashtable = Nothing)
    ...
    '''<remarks></remarks>
    Private Sub UpdatePartyPCDetails(
        ByVal con As SiriusConnection,
        ByVal oBusiness As CoreBusiness,
        ByVal oPartyPCType As BasePartyPCType,
        ByVal PartyKey As Integer)
    ...
    Private Sub UpdatePartyAccountAddressDetails(ByVal con As SiriusConnection, ByVal sAddress1 As String, ByVal sAddress2 As String,
                                                       ByVal sAddress3 As String, ByVal sAddress4 As String, ByVal sPostcode As String,
    ...
    ' given by Gaurav
    Private Sub GetPartyAdditionalData(ByRef oRequest As BaseGetPartyRequestType,
        ByRef oResponse As BaseGetPartyResponseType,
        ByVal sPartyType As String)
    ...
    Public Sub UpdatePartyPCMainDetails(ByVal oParty As BasePartyPCType, ByVal PartyKey As Integer)
        Dim sNationalityCode As String = Nothing
        Dim oCoreBusiness As New CoreBusiness(_SiriusUser)
        Dim oSAMErrorCollection As SAMErrorCollection = New SAMErrorCollection
        Dim STSError As New STSErrorPublisher

        If Not (String.IsNullOrEmpty(oParty.NationalityCode)) Then
            Try
                sNationalityCode = CStr(oCoreBusiness.GetAndValidateListItemFromCode(Core.STSListType.PMLookup, "Nationality", oParty.NationalityCode, "Nationality_id", oSAMErrorCollection))
                If (Convert.ToInt32(sNationalityCode) <= 0) Then
                    oSAMErrorCollection.AddInvalidData(SAMInvalidData.InvalidLookupListValue, [String].Format(STSErrorPublisher.MandatoryInputInvalid, "Nationality_id"), "NationalityCode", oParty.NationalityCode)
                End If
            Catch ex As Exception
                STSError.AddInvalidField("Nationality", CStr(STSErrorCodes.InvalidLookupListValue), [String].Format(STSErrorPublisher.MandatoryInputInvalid, "Nationality_id"), oParty.NationalityCode)
            End Try
            'if there were any errors throw an exception
            oSAMErrorCollection.CheckForErrors()
        End If
        Using con As SiriusConnection = SiriusConnection.FromAny(connectionString:=SAMFunc.ConnectionString)
            Using cmd As SiriusCommand = SiriusCommand.FromProcedure("spu_SAM_updateparty_PC_via_messaging")
                ...
                cmd.AddInParameter("@NationalityCode", SqlDbType.VarChar, 255).Value = oParty.NationalityCode
                con.ExecuteNonQuery(cmd)
            End Using
        End Using
    End Sub
    ...
    Public Overloads Function FindReinsurer(ByVal con As SiriusConnection, ByVal FindReinsurerRequest As BaseFindReinsurerRequestType) As BaseFindReinsurerResponseType
        ...
        For iSourceId = 0 To Microsoft.VisualBasic.UBound(oSources, 2)
            If iSourceId = 0 Then
        ...
        Using cmd As SiriusCommand = SiriusCommand.FromProcedure("Spu_Sir_GetPartyInsurerDetails")
            If String.IsNullOrEmpty(sSourcesString) = False Then
                cmd.AddInParameter("@Branch", SqlDbType.VarChar, 1020).Value = sSourcesString
            End If
...
#Region "CheckRetainedCoInsurerExists"
    Public Function CheckRetainedCoInsurerExists(ByVal oGetRetainedCoInsurerRequest As SAMForInsuranceV2ImplementationTypes.GetRetainedCoInsurerRequestType) As SAMForInsuranceV2ImplementationTypes.GetRetainedCoInsurerResponseType
        Dim oResponse As SAMForInsuranceV2ImplementationTypes.GetRetainedCoInsurerResponseType
        Try
            Using con As SiriusConnection = SiriusConnection.FromAny(connectionString:=SAMFunc.ConnectionString)
                oResponse = CheckRetainedCoInsurerExists(con, oGetRetainedCoInsurerRequest.CoInsurerKeys)
            End Using
        Catch
            Throw
        End Try
    ...
    Private Function CheckRetainedCoInsurerExists(ByVal con As SiriusConnection, ByVal PartyKeys As List(Of Integer)) As SAMForInsuranceV2ImplementationTypes.GetRetainedCoInsurerResponseType

        Dim oResponse As New SAMForInsuranceV2ImplementationTypes.GetRetainedCoInsurerResponseType
        Dim bIsRetainer As Boolean = False
        For Each iCoinCnt As Integer In PartyKeys
            Using cmd As SiriusCommand = SiriusCommand.FromProcedure("spu_SAM_Check_RetainedCoInsurer")
                cmd.AddInParameter("@partCnt", SqlDbType.Int).Value = iCoinCnt
                If Convert.ToInt16(con.ExecuteScalar(cmd)) > 0 Then
                    bIsRetainer = True
                End If
            End Using
            If bIsRetainer = True Then
                Exit For
            End If
        Next
        oResponse.IsRetainedExists = bIsRetainer
        Return oResponse
    End Function
#End Region
End Class