...
<TestFixture()> _
Public Class GetClaimPaymentTaxGroupDetails
    Inherits BaseTest
...
#Region "Call ProxyWS Method"
    ...
    Private Sub Test( _
        ...
        Dim oRequest As New ProxyWS.GetClaimPaymentTaxGroupsRequestType
        Dim oResponse As ProxyWS.GetClaimPaymentTaxGroupsResponseType
        ...
        Try
            ...
            With oRequest
                .BranchCode = "HEADOFF"
                .PartyKey = 0
                .PaymentPartyType = ProxyWS.ClaimPaymentPartyTypeType.CLMPAYABLE
                .AdvancedTax = Nothing
            End With
            ...
            oResponse = oProxy.GetClaimPaymentTaxGroups(oRequest)
...
#Region "Setup NonErrorScenarios Test Cases"
    ...
    Private Sub NoClient(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'oRequest.Claim.Client = Nothing
    End Sub

    Private Sub NoInsurer(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.Insurer = Nothing
    End Sub

    Private Sub NoClaimPeril(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'oRequest.Claim.ClaimPeril = Nothing
    End Sub

    Private Sub NoAddress(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'oRequest.Claim.Client.Address = Nothing
        'oRequest.Claim.Insurer.Address = Nothing
    End Sub

    Private Sub NoContacts(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.Client.Contact = Nothing
        ' oRequest.Claim.Insurer.Contact = Nothing
    End Sub

    Private Sub NoReserves(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'For Each oClaimPeril As ProxyWS.BaseClaimPerilType In oRequest.Claim.ClaimPeril
        '    oClaimPeril.Reserve = Nothing
        'Next
    End Sub

    Private Sub NoRecoveries(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'For Each oClaimPeril As ProxyWS.BaseClaimPerilType In oRequest.Claim.ClaimPeril
        '    oClaimPeril.Recovery = Nothing
        'Next
    End Sub

    Private Sub AllNonMandatoryFieldsMissing(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
    ...
    Private Sub InfoOnlyClaimNoClaimPeril(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
...
#Region "Setup Invalid Lookups Test Cases"
    ...
    Private Sub InvalidLookupBranchCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        oRequest.BranchCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupClientCountryCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'oRequest.Claim.Client.Address.CountryCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupInsurerCountryCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.Insurer.Address.CountryCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupProgressStatusCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.ProgressStatusCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupPrimaryCauseCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'oRequest.Claim.PrimaryCauseCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupHandlerCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'oRequest.Claim.HandlerCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupSecondaryCauseCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'oRequest.Claim.SecondaryCauseCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupCatastropheCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'oRequest.Claim.CatastropheCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupTownCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'oRequest.Claim.TownCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupUserDefFldACode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'oRequest.Claim.UserDefFldACode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupUserDefFldBCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'oRequest.Claim.UserDefFldBCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupUserDefFldCCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.UserDefFldCCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupUserDefFldDCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.UserDefFldDCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupUserDefFldECode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'oRequest.Claim.UserDefFldECode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupPerilTypeCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.ClaimPeril(0).TypeCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupReserveTypeCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.ClaimPeril(0).Reserve(0).TypeCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupRecoveryTypeCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'oRequest.Claim.ClaimPeril(0).Recovery(0).TypeCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupCurrencyCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.CurrencyCode = kInvalidLookupValue
    End Sub
    Private Sub InvalidLookupUnderwritingYearCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
...
#Region "Setup Missing Data Test Cases"
    ...
    Private Sub MissingDataBranchCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        oRequest.BranchCode = String.Empty
    End Sub
    Private Sub MissingDataClientAddressLine1(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        '  oRequest.Claim.Client.Address.AddressLine1 = String.Empty
    End Sub
    Private Sub MissingDataClientCountryCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.Client.Address.CountryCode = String.Empty
    End Sub
    Private Sub MissingDataClientContactDetailItem(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.Client.Contact(0).ContactDetail.Item = String.Empty
    End Sub
    Private Sub MissingDataClientTaxRegistrationNumber(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        '  oRequest.Claim.Client.TaxRegistered = True
        '  oRequest.Claim.Client.TaxRegistrationNumber = String.Empty
    End Sub
    Private Sub MissingDataInsurerAddressLine1(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        '  oRequest.Claim.Insurer.Address.AddressLine1 = String.Empty
    End Sub
    Private Sub MissingDataInsurerCountryCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.Insurer.Address.CountryCode = String.Empty
    End Sub
    Private Sub MissingDataInsurerContactDetailItem(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.Insurer.Contact(0).ContactDetail.Item = Nothing
    End Sub
    Private Sub MissingDataDescription(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        '  oRequest.Claim.Description = String.Empty
    End Sub
    Private Sub MissingDataProgressStatusCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        '  oRequest.Claim.ProgressStatusCode = String.Empty
    End Sub
    Private Sub MissingDataPrimaryCauseCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        '  oRequest.Claim.PrimaryCauseCode = String.Empty
    End Sub
    Private Sub MissingDataLossFromDate(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        '  oRequest.Claim.LossToDateSpecified = False
    End Sub
    Private Sub MissingDataHandlerCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        '  oRequest.Claim.HandlerCode = String.Empty
    End Sub
    Private Sub MissingDataInsuranceFileKey(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.InsuranceFileKey = 0
    End Sub
    Private Sub MissingDataRiskKey(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.RiskKey = 0
    End Sub
    Private Sub MissingDataClaimPerilTypeCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        '  oRequest.Claim.ClaimPeril(0).TypeCode = String.Empty
    End Sub
    Private Sub MissingDataClaimPerilReserveTypeCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.ClaimPeril(0).Reserve(0).TypeCode = String.Empty
    End Sub
    Private Sub MissingDataClaimPerilRecoveryTypeCode(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
...
#Region "Setup Business Warnings Test Cases"
    ...
    Private Sub BusinessWarningInfoOnlyClaimDataTruncated(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.InfoOnly = True
    End Sub
    Private Sub BusinessWarningLossFromDateAfterPolicyEndDate(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ...
        'oTDClaim = Nothing
    End Sub
    Private Sub BusinessWarningLossFromDateBeforePolicyStartDate(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'Dim oTDClaim As OpenClaimXMLStructure.cClaim
        'oTDClaim = m_oTestData.OpenClaim.Claim
        ...
        'oTDClaim = Nothing
    End Sub
    Private Sub BusinessWarningRiskIsDeferred(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'Dim oTDClaim As OpenClaimXMLStructure.cClaim
        'oTDClaim = m_oTestData.OpenClaim.Claim
        ...
        'oTDClaim = Nothing
    End Sub
    Private Sub BusinessWarningPolicyIsVoid(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'Dim oTDClaim As OpenClaimXMLStructure.cClaim
        'oTDClaim = m_oTestData.OpenClaim.Claim
        ...
        'oTDClaim = Nothing
    End Sub
    Private Sub BusinessWarningPolicyIsDifferent(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
...
#Region "Setup Business Errors Test Cases"
    ...
    Private Sub BusinessErrorDuplicateClaimExists(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        'Dim oTDClaim As OpenClaimXMLStructure.cClaim
        'oTDClaim = m_oTestData.OpenClaim.Claim
        ...
        'oTDClaim = Nothing
    End Sub
    Private Sub BusinessErrorLossFromDataInFuture(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        '   oRequest.Claim.LossFromDate = DateAdd(DateInterval.Day, 1, Date.Today)
    End Sub
    Private Sub BusinessErrorLossFromDateAfterReportedDate(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        ' oRequest.Claim.ReportedDate = DateAdd(DateInterval.Day, -1, oRequest.Claim.LossFromDate)
    End Sub
    Private Sub BusinessErrorLossFromDateAfterLossToDate(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        '  oRequest.Claim.LossToDate = DateAdd(DateInterval.Day, -1, oRequest.Claim.LossFromDate)
    End Sub
    Private Sub BusinessErrorReportedDateInFuture(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
        '  oRequest.Claim.ReportedDate = DateAdd(DateInterval.Day, 1, Date.Today)
    End Sub
    Private Sub BusinessErrorInsuranceFileDetailsNotFound(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType)
...
#Region "Test Case Response Handlers"
    ...
    Private Sub ProcessTestCases(ByVal oRequest As ProxyWS.GetClaimPaymentTaxGroupsRequestType, ByVal TestCase As TestCaseScenario)
