...
Partial Class OpenClaim_Peril
    ...
    Protected Sub gvReserves_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles gvReserves.RowUpdating
        ...
        txtEditBox = DirectCast(gvReserves.Rows(e.RowIndex).Controls(4).Controls(0), TextBox)
        oPerilType(gvPerils.SelectedIndex).Reserves(gvReserves.EditIndex).ThisRevision = Convert.ToDecimal(txtEditBox.Text)
        'Praveen
        oPerilType(gvPerils.SelectedIndex).Reserves(gvReserves.EditIndex).CurrentReserve = oPerilType(gvPerils.SelectedIndex).Reserves(gvReserves.EditIndex).CurrentReserve + oPerilType(gvPerils.SelectedIndex).Reserves(gvReserves.EditIndex).ThisRevision
        oPerilType(gvPerils.SelectedIndex).Reserves(gvReserves.EditIndex).Incurred = oPerilType(gvPerils.SelectedIndex).Reserves(gvReserves.EditIndex).Incurred + oPerilType(gvPerils.SelectedIndex).Reserves(gvReserves.EditIndex).ThisRevision
        'Praveen
        'oPerilType(gvPerils.SelectedIndex).Reserves(gvReserves.EditIndex).CurrentReserve = Convert.ToDecimal(txtEditBox.Text)
        'oPerilType(gvPerils.SelectedIndex).Reserves(gvReserves.EditIndex).RevisionAmount = Convert.ToDecimal(txtEditBox.Text)
