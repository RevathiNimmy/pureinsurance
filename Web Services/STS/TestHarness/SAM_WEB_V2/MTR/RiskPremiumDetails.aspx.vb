...
Partial Class MTC_RiskPremiumDetails
    ...
    Private Sub GetHeaderAndRisk()
        ...
        grd_Output.DataBind()
        'gvorinigalRating.DataSource = DirectCast(oGetRatingDetailsResponseType.RatingDetails(1), BaseGetRatingDetailsResponseTypeRow)
        'gvorinigalRating.DataBind()
    ...
    Protected Sub grd_Output_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles grd_Output.RowDeleting
        ...
        For Count As Integer = 0 To oRatings.Length - 1
            If Count <> e.RowIndex Then
                ...
                'added for rating details
                ' oTempRatings(oTempRatings.Length - 1).OriginalFlag = oRatings(Count).OriginalFlag
                'added for rating details
            End If
        ...
        oUpdateRatingDetailsRequestType.TimeStamp = Session("QuoteTimeStamp")
    ...
    'Protected Sub gvorinigalRating_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvorinigalRating.RowDataBound
    '    'If e.Row.RowType = DataControlRowType.DataRow Then
    '    '    Dim d As Decimal

    '    '    d = DirectCast(e.Row.DataItem, BaseGetRatingDetailsResponseTypeRow).ThisPremium > 0
    '    '    e.Row.Visible = True
    '    'Else
    '    '    e.Row.Visible = False
