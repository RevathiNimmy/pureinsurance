        ...
        public ActionResult Index(InitialConfigurationModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    string strSignCert = model.SigningCertificate.Substring(0, model.SigningCertificate.IndexOf(":"));
                    var cert = X509.LocalMachine.My.SubjectDistinguishedName.Find(strSignCert, false).First();
        ...
        private List<string> GetAvailableCertificatesFromStore()
        {
            ...
            try
            {
                foreach (var cert in store.Certificates)
                {
                    ...
                    // George Harris appended the certificates friendly name to the display field
                    string strDisplay = string.Format("{0}", cert.Subject) + ": " + string.Format("{0}", cert.FriendlyName);
                    //list.Add(string.Format("{0}", cert.Subject));
                    list.Add(strDisplay);
                }
            }
