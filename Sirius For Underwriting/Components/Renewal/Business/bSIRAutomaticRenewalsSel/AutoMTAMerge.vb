...
Friend NotInheritable Class AutoMTAMerge
    ...
    Private Const ACRRiskEditStatus As Integer = 6
    ...
    Private Const AcRiskDataRiskEditStatus As Integer = 37
    ...
    ' PreChangeInsFileCnt
    Private m_lPreChangeInsFileCnt As Object
    ' PostChangeInsFileCnt
    Private m_lPostChangeInsFileCnt As Object
    ' CurrentInsFileCnt
    Private m_lCurrentInsFileCnt As Object
    ' RiskCount
    ...
    ' NewRiskCnt
    Private m_lNewRiskCnt As Object
    ' XMLDataSetDef
    ...
    Public WriteOnly Property CurrentRiskIndex() As Integer
        ...
        End Set
    End Property
    Public ReadOnly Property RiskEditStatus() As Integer
        Get
            If Informations.IsArray(m_vListRiskArray) = True Then
                RiskEditStatus = m_vListRiskArray(ACRRiskEditStatus, m_lCurrentRiskIndex)
            Else
                RiskEditStatus = -1
            End If
        End Get
    ...
    Public Function GetListOfRisks() As Integer
        ...
        Try
            ...
            If Not (m_oFindRisk Is Nothing) Then
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetListOfRisksUsingFindRisk Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetListOfRisks")
            ...
            Else
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetListOfRisksUsingRiskData Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetListOfRisks")
                    Return gPMConstants.PMEReturnCode.PMFalse
                End If
            End If
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="BuildCollections Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetListOfRisks")
                Return gPMConstants.PMEReturnCode.PMFalse
            End If
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="ComparePostPreData Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetListOfRisks")
                Return gPMConstants.PMEReturnCode.PMFalse
            End If
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="ProcessCurrentRisks Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetListOfRisks")
                Return gPMConstants.PMEReturnCode.PMFalse
            End If
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetListOfRisks Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetListOfRisks", vErrNo:=Informations.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetListOfRisksUsingFindRisk() As Integer
        ...
        Try
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                If m_lReturn <> gPMConstants.PMEReturnCode.PMNotFound Then
                    bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="m_oFindRisk.SearchAll Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetListOfRisksUsingFindRisk")
                    Return gPMConstants.PMEReturnCode.PMFalse
                End If
            End If
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetListOfRisksUsingFindRisk Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetListOfRisksUsingFindRisk", vErrNo:=Informations.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetListOfRisksUsingRiskData() As Integer
        ...
        Try
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="m_oRiskData.GetRisk Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetListOfRisksUsingRiskData")
                Return gPMConstants.PMEReturnCode.PMFalse
            End If
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetListOfRisksUsingRiskData Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetListOfRisksUsingRiskData", vErrNo:=Informations.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Private Function ProcessCurrentRisks() As Integer
        ...
        For iCnt As Integer = 0 To m_vCurrentRiskArray.GetUpperBound(1)
            ...
            m_vListRiskArray(ACRRiskEditStatus, lUbound) = ToSafeLong(m_vCurrentRiskArray(AcRiskDataRiskEditStatus, iCnt))
    ...
    Public Function MergeExistingMTAChanges() As Integer
        ...
        Try
            ...
            Dim lPreChangeRiskCnt, lPostChangeRiskCnt, lCurrentRiskCnt As Object

            If CStr(m_vListRiskArray(ACRStatus, m_lCurrentRiskIndex)) <> gSIRLibrary.ACRStatusMerge Then
                'We don't need to merge
                'Load the dataset

                m_lReturn = m_oGis.LoadRiskFromDB(r_sXMLDataSetDef:=m_sXMLDataSetDef, r_sXMLDataSet:=m_sXMLDataSet, r_sGisDataModelCode:=m_sDataModelCode, v_lInsuranceFileCnt:=m_lInsuranceFolderCnt, v_lRiskId:=m_lNewRiskCnt)
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    result = gPMConstants.PMEReturnCode.PMFalse
                    bPMFunc.LogMessage("", iType:=gPMConstants.PMEReturnCode.PMError, sMsg:="m_oGis.LoadRiskFromDB Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="MergeExistingMTAChanges")
                    Return result
                End If
                Return result
            End If
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                result = gPMConstants.PMEReturnCode.PMFalse
                bPMFunc.LogMessage("", iType:=gPMConstants.PMEReturnCode.PMError, sMsg:="MTADiffAndMerge Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="MergeExistingMTAChanges")
                Return result
            End If
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                result = gPMConstants.PMEReturnCode.PMFalse
                bPMFunc.LogMessage("", iType:=gPMConstants.PMEReturnCode.PMError, sMsg:="ClearOutputObjects Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="MergeExistingMTAChanges")
                Return result
            End If
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="MergeExistingMTAChanges Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="MergeExistingMTAChanges", vErrNo:=Informations.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    '
    ' ***************************************************************** '
    'UPGRADE_NOTE: (7001) The following declaration (TestStub) seems to be dead code More Informations: http://www.vbtonet.com/ewis/ewi7001.aspx
    ...
    'If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
    'bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="m_oDataSet.LoadFromXML Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="TestStub")
    'Exit Sub
    'End If
    '
    ...
    'If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
    'bPMFunc.LogMessage("", iType:=gPMConstants.PMEReturnCode.PMError, sMsg:="m_oDataset.GetAllOIKey Failed for " & sObjectName, vApp:=ACApp, vClass:=ACClass, vMethod:="TestStub")
    'Exit Sub
    'End If
    '
    ...
    'If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
    'bPMFunc.LogMessage("", iType:=gPMConstants.PMEReturnCode.PMError, sMsg:="m_oDataset.SetPropertyValue Failed for " & sPropertyName, vApp:=ACApp, vClass:=ACClass, vMethod:="ApplyRiskChanges")
    'End If
    'End If
    '
    'lPolicyLinkId = -1
    ...
    'If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
    'bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="m_oDataSet.ReturnAsXML Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="QuoteRisk")
    'Exit Sub
    'End If
    '
    ...
    'If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
    'bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="m_oGis.SaveToDB Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="QuoteRisk")
    'Exit Sub
    'End If
    '
    'Catch excep As System.Exception
    ...
    '
    ' Log Error Message
    'bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="TestStub Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="TestStub", vErrNo:=Informations.Err().Number, vErrDesc:=excep.Message)
    '
    'Exit Sub
    '
    'End Try
    ...
    Private Function ClearOutputObjects() As Integer
        ...
        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
            bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="m_oDataSet.LoadFromXML Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="ClearOutputObjects")
            Return gPMConstants.PMEReturnCode.PMFalse
        End If
        ...
        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
            bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="m_oDataset.GetAllOIKey Failed for " & sObjectName, vApp:=ACApp, vClass:=ACClass, vMethod:="ClearOutputObjects")
            Return gPMConstants.PMEReturnCode.PMFalse
        End If
        ...
        For i As Integer = 0 To vOIKeyArray.GetUpperBound(0)
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="m_oDataset.DelObjectInstance Failed for " & sObjectName, vApp:=ACApp, vClass:=ACClass, vMethod:="ClearOutputObjects")
                Return gPMConstants.PMEReturnCode.PMFalse
            End If
        Next i
        ...
        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
            bPMFunc.LogMessage("", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="m_oDataSet.ReturnAsXML Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="ClearOutputObjects")
            Return gPMConstants.PMEReturnCode.PMFalse
        End If
