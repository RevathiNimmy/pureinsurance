...
Public NotInheritable Class BusinessNET
    ...
    'implement multithreading properties/variable
    Private oQueue As Queue(Of Integer)
    ...
    Public Function ApplyIndexLink(ByVal v_lInsuranceFileCnt As Integer, ByVal v_lRiskID As Integer, ByVal v_dtEffectiveDate As Date) As Integer
        ...
        Dim vSumInsuredArray(,) As Object
        Dim vPercentage As Object 'index linking value
    ...
    '
    ' ***************************************************************** '
    'Private Function CreateBusinessObject(ByRef r_oObject As Object, ByVal v_sClassName As String, oDatabase As dPMDAO.Database) As Integer
    ...
    ''' </summary>
    ''' <param name="v_sGisDataModelCode"></param>
    ''' <param name="r_vInsuranceFileCnt"></param>
    ''' <param name="r_vPolicyLinkID"></param>
    ''' <param name="r_vRiskID"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    ...
    Public Function GIS_LoadFromDB(ByVal v_sGisDataModelCode As String,
        ...
        Try
            ...
            nReturn = oGIS.LoadFromDB(r_sXMLDataSetDef:=sXMLDataSetDef, r_sXMLDataset:=sXMLDataSet,
                                      ...
                                      r_vInsuranceFileCnt:=r_vInsuranceFileCnt, r_vPolicyLinkID:=r_vPolicyLinkID,
                                      r_vRiskID:=r_vRiskID)
            If nReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return nReturn
            End If
    ...
    Public Function CopyRiskStandardWordings(ByVal v_lOldPolicyBinderId As Integer, ByVal v_lNewPolicyBinderId As Integer, ByVal v_sDataModelCode As String, Optional ByVal v_dtEffectiveDate As Date = #12/30/1899#) As Integer
        ...
        Try
            ...
            If Informations.IsArray(vResultArray) Then
                ...
                For iCount As Integer = 0 To vResultArray.GetUpperBound(1)
                    'Removed the code to sync it with BO functionality 
                    'During selection new version of edited version is not created in BO          
    ...
    Public Function CopyPolicyStandardWordings(ByVal v_lOldInsuranceFileCnt As Integer, ByVal v_lNewInsuranceFileCnt As Integer, Optional ByVal v_dtEffectiveDate As Date = #12/30/1899#) As Integer
        ...
        Try
            ...
            sSQL = "SELECT policy_standard_wording_id, document_template_id" & Strings.ChrW(13) & Strings.ChrW(10)
    ...
    Public Function DeleteRenewalsForPolicy() As Integer
        ...
        Try
            ...
            If Informations.IsArray(m_vRenewalsForPolicy) Then
                'loop through and delete each renewal policy from renewal
                m_lReturn = GetPolicyForRenewal(v_lInsuranceFolderCnt:=m_vRenewalsForPolicy(2, 0),
    ...
    Public Function ProcessRenewalSelection(ByVal v_lInsuranceFileCnt As Integer,
        ...
        Catch ex As Exception
            ...
            bPMFunc.LogError(v_sUsername:=m_sUsername, v_sClass:=ACClass, v_sMethod:=kMethodName, r_lFunctionReturn:=result, excep:=ex)
            ' If you want to rollback a transaction or something, do it here
            If sFailedText IsNot Nothing Then
    ...
    Private Function CreateRenewalPolicy(ByRef r_vRenewalList(,) As Object,
        ...
        Dim nMaximumPolicyVersion As Integer
        Try
            ...
            'assign Party Cnt
            oInsuranceFile.InsuranceHolderCnt = r_vRenewalList(PMFieldPosInsuranceHolderCnt, v_lCount)
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                nResult = gPMConstants.PMEReturnCode.PMFalse
                sFailureCriterion = "bPMFunc.GetSystemOption Failed"
            End If
            ...
            m_lReturn = GetMaximumPolicyVersion(v_nInsuranceFileCnt:=oInsuranceFile.InsuranceFileCnt, r_nMaximumPolicyVersion:=nMaximumPolicyVersion)
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If
            oInsuranceFile.PolicyVersion = nMaximumPolicyVersion + 1
            ...
            'Check Retain Policy Numberon Copy 
            Dim m_bIsClonedPolicy As Boolean = False
            m_lReturn = IsClonedPolicyNumber(oInsuranceFile.InsuranceFileCnt, m_bIsClonedPolicy)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            If m_bIsClonedPolicy Then
                ' Remove everything after "_" from policy number. 
                '  sInsuranceRef = oInsuranceFile.InsuranceRef
                Dim iUnderscoreIndex As Integer = -1
                iUnderscoreIndex = sInsuranceRef.Substring(sInsuranceRef.Length - 3).IndexOf("_")
                If iUnderscoreIndex = 0 Then
                    sInsuranceRef = sInsuranceRef.Substring(0, sInsuranceRef.Length - 3)
                ElseIf iUnderscoreIndex = 1 Then
                    sInsuranceRef = sInsuranceRef.Substring(0, sInsuranceRef.Length - 2)
            ...
            If oIsRI2007 = "1" Then
                m_lReturn = gPMComponentServices.CreateBusinessObject(r_oObject:=oReinsurance, v_sClassName:="bSIRReinsuranceRI2007.Form", v_sCallingAppName:=ACApp, v_sUsername:=m_sUsername, v_sPassword:=m_sPassword, v_iUserID:=m_iUserID, v_iSourceID:=m_iSourceID, v_iLanguageID:=m_iLanguageID, v_iCurrencyID:=m_iCurrencyID, v_iLogLevel:=m_iLogLevel, v_oDatabase:=m_oDatabase)
            Else
                m_lReturn = gPMComponentServices.CreateBusinessObject(r_oObject:=oReinsurance, v_sClassName:="bSIRReinsurance.Form", v_sCallingAppName:=ACApp, v_sUsername:=m_sUsername, v_sPassword:=m_sPassword, v_iUserID:=m_iUserID, v_iSourceID:=m_iSourceID, v_iLanguageID:=m_iLanguageID, v_iCurrencyID:=m_iCurrencyID, v_iLogLevel:=m_iLogLevel, v_oDatabase:=m_oDatabase)
            End If
            ...
            '   m_lReturn = CreateBusinessObject(r_oObject:=oChangePolicyStatus,
            '                                v_sClassName:="bSIRChangePolicyStatus.Business")

            'copy risk details to new policy

            m_lReturn = BeginTrans()
            If v_iDoNotCopyRisk = 0 Then
            m_lReturn = CopyRiskData(r_vRenewalList:=r_vRenewalList, v_lCount:=v_lCount,
                                     ...
                                     v_bTMPAnniversary:=v_bTMPAnniversary)
            End If
            ...
            Else
                ...
                If (sOptionValue.ToString = "1") Then
                    oBatchRenewalBusiness = New bSIRRenewalBusiness()
                    oBatchRenewalBusiness.Initialise(sUsername:="", sPassword:="", iUserID:=0, iSourceID:=1, iLanguageID:=0, iCurrencyID:=0, iLogLevel:=0, sCallingAppName:=ACApp, vDatabase:=m_oDatabase)
                    If oBatchRenewalBusiness Is Nothing Then
                        Throw New ApplicationException("Failed to create instance of bSIRRenewalBusiness.Business")
                    End If
                ...
                Else
                    If (ToSafeDouble(oUseNbPaymentTermAtRenSelection(0, 0)) = 0) Then
            ...
            If m_lReturn = gPMConstants.PMEReturnCode.PMTrue Then
                ...
                m_lReturn = IsInstalment(v_lInsuranceFileCnt:=If(CDbl(oUseNbPaymentTermAtRenSelection(0, 0)) = 0, (CInt(r_vRenewalList(PMFieldPosInsuranceFileCnt, v_lCount))), nNewInsuranceFileCnt))
                If m_lReturn = gPMConstants.PMEReturnCode.PMNotFound AndAlso (sPayment_Method = "direct debit" Or sPayment_Method = "credit card" Or sPayment_Method = "instalments") Then
                    m_lReturn = UpdatePaymentMethod(nNewInsuranceFileCnt)
                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
            ...
            If sFailureCriterion <> "" Then
                ...
                m_lReturn = AddTaskToWorkManager(v_sClientName:=CStr(r_vRenewalList(PMFieldPosClientName, v_lCount)),
                                                 ...
                                                 v_dtDueDate:=Informations.DateAdd("w", 2, DateTime.Today),
        ...
        Catch ex As Exception
            ...
            m_lReturn = RollbackTrans()
            ' code for deleting the renewal quotation
            DeletePolicy(nNewInsuranceFileCnt)
            UpdatePolicyStatus(CInt(r_vRenewalList(PMFieldPosInsuranceFileCnt, v_lCount)))
        Finally
        End Try
        Return nResult
    End Function

    Public Function IsClonedPolicyNumber(ByVal v_lInsuranceFileCnt As Integer, ByRef v_bIsClonedPolicyNumber As Boolean) As Integer

        Dim result As Integer = 0

        Const kMethodName As String = "IsClonedPolicyNumber"
        ...
        Try
            ...
            m_lReturn = m_oDatabase.Parameters.Add(sName:="insurance_file_cnt", vValue:=CStr(v_lInsuranceFileCnt), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)
            m_lReturn = m_oDatabase.Parameters.Add(sName:="bSkipNewPolicyNumber", vValue:=CStr(v_bIsClonedPolicyNumber), iDirection:=gPMConstants.PMEParameterDirection.PMParamOutput, iDataType:=gPMConstants.PMEDataType.PMBoolean)
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=ACIsClonedPolicySQL, sSQLName:=ACIsClonedPolicyName, bStoredProcedure:=True)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                result = gPMConstants.PMEReturnCode.PMFalse
            Else
                v_bIsClonedPolicyNumber = gPMFunctions.ToSafeBoolean(m_oDatabase.Parameters.Item("bSkipNewPolicyNumber").Value)
            End If
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function CopyRiskData(ByRef r_vRenewalList(,) As Object,
                                  ByVal v_lCount As Integer,
                                  ByVal v_lNewInsuranceFileCnt As Integer,
                                  ByRef r_lEligibleForRenewal As Integer,
                                  ByRef r_sFailureReason As String,
                                  ByRef r_oReinsurance As Object,
                                  Optional ByVal v_bIsTrueMonthlyPolicy As Boolean = False,
                                  Optional ByVal v_bTMPAnniversary As Boolean = False) As Integer


        Dim nOldInsuranceFileCnt As Integer
        Dim oSIRRiskData As bSIRRiskData.Business
        Dim aoRiskArray(,) As Object = Nothing
        Dim bIsMandatoryRiskPresent As Boolean = False
        Dim nNumberOfRisks As Integer = 0
        ...
        Try
            ...
            nResult = CType(oSIRRiskData.Initialise(sUsername:=ToSafeString(m_sUsername), sPassword:=ToSafeString(m_sPassword), iUserID:=ToSafeInteger(m_iUserID), iSourceID:=ToSafeInteger(m_iSourceID), iLanguageID:=ToSafeInteger(m_iLanguageID), iCurrencyID:=ToSafeInteger(m_iCurrencyID), iLogLevel:=ToSafeInteger(m_iLogLevel), sCallingAppName:=ToSafeString(ACApp), vDatabase:=m_odatabase), gPMConstants.PMEReturnCode)
            'oSIRRiskData = CreateBusinessObject("bSirRiskData.business", m_oDataBase)
            nOldInsuranceFileCnt = r_vRenewalList(PMFieldPosInsuranceFileCnt, v_lCount)
            oSIRRiskData.TransactionType = "REN"

            If oSIRRiskData.GetRenewalRisk(v_lInsuranceFileCnt:=nOldInsuranceFileCnt, r_vResultArray:=aoRiskArray) <> PMEReturnCode.PMTrue Then
            ...
            If Not Informations.IsArray(aoRiskArray) Then
                'RWH(16/11/2000) We do not need to report an error if there are no risks.
                r_sFailureReason = "No risks found"
                Return PMEReturnCode.PMFalse
            End If
            'Check the Number of Risk
            nNumberOfRisks = aoRiskArray.GetUpperBound(1) + 1

            bIsMandatoryRiskPresent = If(aoRiskArray(ACRiskPosIsMandatoryRisk, nNumberOfRisks - 1) = 1, True, False)
            oQueue = New Queue(Of Integer)
            'Loop through and copy each risk details
            For iCount As Integer = 0 To aoRiskArray.GetUpperBound(1)
                If aoRiskArray(ACRiskPosIsMandatoryRisk, iCount) = 1 Then 'Do we have any Mandatory Risk
                    bIsMandatoryRiskPresent = True
                    nNumberOfRisks = nNumberOfRisks - 1
                Else
                    oQueue.Enqueue(aoRiskArray(ACRiskPosCnt, iCount))  'Donot Queue the Mandatory Risk. We will process it seperately
                End If
            Next
            ...
            Dim oRenParameters As RiskData = New RiskData
            With oRenParameters
                .aoRenewalList = r_vRenewalList
                .nNewInsuranceFileCnt = v_lNewInsuranceFileCnt
                .nEligibleForRenewal = r_lEligibleForRenewal
                .sFailureReason = r_sFailureReason
                .bIsTrueMonthlyPolicy = v_bIsTrueMonthlyPolicy
                .bTMPAnniversary = v_bTMPAnniversary
                .oRiskArray = aoRiskArray
            End With

            Dim orenthreads(iNumberOfThreads - 1) As Thread
            For iloop As Integer = 0 To iNumberOfThreads - 1
                orenthreads(iloop) = New Thread(New ParameterizedThreadStart(AddressOf ActionProcessRisk))
                orenthreads(iloop).Start(oRenParameters)
            Next
            For iloop As Integer = 0 To iNumberOfThreads - 1
                orenthreads(iloop).Join()
            Next

            'Process Mandatory Risk if any
            If bIsMandatoryRiskPresent Then
                Dim aoRiskArraySingle(oRenParameters.oRiskArray.GetUpperBound(0), 0) As Object
                For j As Integer = 0 To oRenParameters.oRiskArray.GetUpperBound(0)
                    aoRiskArraySingle(j, 0) = oRenParameters.oRiskArray(j, nNumberOfRisks)
                Next
                Dim sFailureCriterion As String = ""
                If Informations.IsArray(aoRiskArraySingle) Then
                    nResult = ActionProcessRisk(r_vRenewalList:=r_vRenewalList,
                                                oRiskArray:=aoRiskArraySingle,
                                                v_lNewInsuranceFileCnt:=oRenParameters.nNewInsuranceFileCnt,
                                                r_nEligibleForRenewal:=oRenParameters.nEligibleForRenewal,
                                                r_sFailureReason:=sFailureCriterion,
                                                v_bIsTrueMonthlyPolicy:=oRenParameters.bIsTrueMonthlyPolicy,
                                                v_bTMPAnniversary:=oRenParameters.bTMPAnniversary,
                                                v_lCount:=0)
                    If nResult <> gPMConstants.PMEReturnCode.PMTrue Then
                        If (sFailureCriterion = "") Then
                            m_sFailureCriterion = "Failed to copy risk data"
                        Else
                            m_sFailureCriterion = "Failed to copy risk data (" & sFailureCriterion & ")"
                        End If
                    End If
                End If
            End If
        ...
        Catch ex As Exception
            ...
            Throw New ApplicationException("CopyRskData Failed.", ex)
        End Try
        Return nResult
    ...
    Private Function GetProductionOption(ByVal v_vOptionNumber As Integer,
        ByRef r_oValue As Object) As Integer

        Dim nReturn As Integer = 0
        Const kMethodName As String = "GetProductionOption"
        nReturn = gPMConstants.PMEReturnCode.PMTrue
        Try
            nReturn = bPMFunc.getProductOptionValue(v_sUsername:=m_sUsername,
                                              ...
                                              v_vOptionNumber:=v_vOptionNumber,
                                              v_vBranch:=SIRBCHHeadOffice,
                                              r_vUnderwriting:=r_oValue)
            Return nReturn
        Catch ex As Exception
    ...
    Private Function GetPriorTermSchemeInsuranceFile(ByVal lInsuranceFileCnt As Integer,
                                                    ByRef lPriorTermSchemeInsuranceFileCnt As Integer) As Integer
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function ApplyRiskRenewalTaxes(ByVal v_lRenewalRiskCnt As Integer,
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function ApplyRiskFee(ByVal v_lRenewalRiskCnt As Object,
        ...
        Try
            ...
            ' create fee object
            ' m_lReturn = CreateBusinessObject(r_oObject:=oFee,     v_sClassName:="bSIRPartyFee.UBusiness")
            m_lReturn = gPMComponentServices.CreateBusinessObject(r_oObject:=oFee, v_sClassName:="bSIRPartyFee.UBusiness", v_sCallingAppName:=ACApp, v_sUsername:=m_sUsername, v_sPassword:=m_sPassword, v_iUserID:=m_iUserID, v_iSourceID:=m_iSourceID, v_iLanguageID:=m_iLanguageID, v_iCurrencyID:=m_iCurrencyID, v_iLogLevel:=m_iLogLevel, v_oDatabase:=m_oDatabase)
    ...
    ''' Gets the risks renewal override flags
    ''' </summary>
    ''' <param name="v_lRenewalRiskCnt">Risk that tax must be calculated for</param>
    ...
    ''' <param name="v_bRecalcFees">An optional parameter to indicate whether the fees can be copied instead of being recalculated</param>
...
AfterError:
    ...
    Public Function LoadRenewalInsurancefolderParamaters(ByVal v_lBatchId As Long, ByVal v_lInsuranceFolderCnt As Long,
        ...
        Try
            ...
            If oPolicyRenewalRules.GetUpperBound(1) <> ACBatchJobStartColumns.BatchRef Then
                LoadRenewalInsurancefolderParamaters = gPMConstants.PMEReturnCode.PMFalse
                Exit Function
            End If
    ...
    Public Function GetPMRegSettingLocal(
             ByVal v_lPMERegSettingRoot As Long,
             ByVal v_lPMEProductFamily As Long,
             ByVal v_lPMERegSettingLevel As Long,
             ByVal v_sSettingName As String,
             ByRef r_sSettingValue As String) As Long

        Dim nRetrun As Integer

        nRetrun = GetPMRegSetting(v_lPMERegSettingRoot:=v_lPMERegSettingRoot,
                                                        v_lPMEProductFamily:=v_lPMEProductFamily,
                                                        v_lPMERegSettingLevel:=v_lPMERegSettingLevel,
                                                        v_sSettingName:=v_sSettingName,
                                                        r_sSettingValue:=r_sSettingValue)
    ...
    ''' <summary>
    ''' Implement MultiThreading with below Methods 01 Feb 2016
    ''' </summary>
    ''' <param name="oRenParameters">Contain risk collection</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function ActionProcessRisk(ByVal oRenParameters As RiskData) As Integer
        Dim nResult As Integer = PMEReturnCode.PMTrue
        ...
        Try
            ...
            nRiskCnt = GetRiskCntForProcessing()
            While (nRiskCnt > 0)
                'Check for Risk Cnt
                For i As Integer = 0 To oRenParameters.oRiskArray.GetUpperBound(1)
                    If nRiskCnt = oRenParameters.oRiskArray(0, i) Then
                        'Copy 
                        For j As Integer = 0 To (oRenParameters.oRiskArray).GetUpperBound(0)
                            aoRiskArraySingle(j, 0) = oRenParameters.oRiskArray(j, i)
                        Next
                        Exit For
                    End If
                Next

                nResult = ActionProcessRisk(r_vRenewalList:=oRenParameters.aoRenewalList,
                                            oRiskArray:=aoRiskArraySingle,
                                            v_lNewInsuranceFileCnt:=oRenParameters.nNewInsuranceFileCnt,
                                            r_nEligibleForRenewal:=oRenParameters.nEligibleForRenewal,
                                            r_sFailureReason:=sFailureCriterion,
        ...
        Catch ex As Exception
            nResult = PMEReturnCode.PMFalse
            Throw New ApplicationException("ActionProcessRisk Failed.", ex)
        End Try
        Return nResult
    End Function

    Private Function ActionProcessRisk(ByRef r_vRenewalList(,) As Object,
                                       ByVal oRiskArray(,) As Object,
                                      ByVal v_lCount As Integer,
                                      ByVal v_lNewInsuranceFileCnt As Integer,
                                      ByRef r_nEligibleForRenewal As Integer,
                                      ByRef r_sFailureReason As String,
                                      ByVal v_bIsTrueMonthlyPolicy As Boolean,
                                      ByVal v_bTMPAnniversary As Boolean) As Integer
        ...
        Dim sXMLDataSet As String
        Dim lOldPolicyBinderId As Integer
        ...
        Dim oPerilAllocation As Object
        Dim nResult As Integer = PMEReturnCode.PMTrue
        ...
        Dim oIsRI2007 As Object = Nothing
        Dim oReinsurance As Object = Nothing
        Dim m_oDatabase As dPMDAO.Database = Nothing
        Const ACFieldPosRiskID = 0
        Const ACFieldPosGisScreenID = 21
        Const ACRiskRenRule = 38
        Const ACRiskUALRule = 39

        ' set default values
        bExtraRiskDetails = False


        r_nEligibleForRenewal = PMEReturnCode.PMTrue

        ' DBConnect(m_oDatabase)
        ' Create the business object
        Dim oBusiness As bSIRRenSelection.BusinessNET = Nothing
        oBusiness = New bSIRRenSelection.BusinessNET
        nResult = CType(oBusiness.Initialise(sUsername:=m_sUsername, sPassword:=m_sPassword, iUserID:=m_iUserID, iSourceID:=m_iSourceID, iLanguageID:=m_iLanguageID, iCurrencyID:=m_iCurrencyID, iLogLevel:=m_iLogLevel, sCallingAppName:=ACApp, vDatabase:=m_oDatabase), gPMConstants.PMEReturnCode)

        Dim oRiskData As bSIRRiskData.Business
        oRiskData = New bSIRRiskData.Business
        nResult = CType(oRiskData.Initialise(sUsername:=m_sUsername, sPassword:=m_sPassword, iUserID:=m_iUserID, iSourceID:=m_iSourceID, iLanguageID:=m_iLanguageID, iCurrencyID:=m_iCurrencyID, iLogLevel:=m_iLogLevel, sCallingAppName:=ACApp, vDatabase:=m_oDatabase), gPMConstants.PMEReturnCode)

        lOldInsuranceFileCnt = r_vRenewalList(PMFieldPosInsuranceFileCnt, v_lCount)
        oRiskData.TransactionType = "REN"

        Dim lCount As Integer = 0
        Dim nTransactionType As Integer = 10 'For Renewals
        ...
        m_lReturn = CheckRiskRenewalRules(oRiskArray(ACRiskPosFolder, lCount), bRerate, bRecalcTax, bRecalcRI, bRecalcFees)

        If bRerate = True Or v_bTMPAnniversary = True Or v_bIsTrueMonthlyPolicy = False Then
            ...
            m_lReturn = oRiskData.GetGISPolicyLink(v_lInsuranceFolderCnt:=r_vRenewalList(PMFieldPosInsuranceFolderCnt, v_lCount),
                    v_lRiskID:=oRiskArray(ACRiskPosCnt, lCount),
            ...
            'do we have any data
            Dim auxVar_2 As Object = vGisPolicyLinkArray(0, 0)

            If Not (Convert.IsDBNull(auxVar_2) Or Informations.IsNothing(auxVar_2)) Then
                'Make sure GIS object present.
                m_lReturn = oBusiness.GIS_LoadFromDB(Trim(vGisPolicyLinkArray(4, 0)),
                        r_vRenewalList(PMFieldPosInsuranceFolderCnt, v_lCount),
                        vGisPolicyLinkArray(0, 0),
                        oRiskArray(0, lCount))           'copy GIS details to NewInsuranceFileCnt
                ...
                m_lReturn = oBusiness.CopyDataSet(v_sDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)),
                        ...
                        v_vOldInsuranceFileCnt:=r_vRenewalList(PMFieldPosInsuranceFolderCnt, v_lCount),
                        v_vOldRiskID:=oRiskArray(0, lCount),
                        v_vNewInsuranceFileCnt:=r_vRenewalList(PMFieldPosInsuranceFolderCnt, v_lCount),
                ...
                m_lReturn = oBusiness.LoadFromXML(v_sXMLDataSetDef:=sXMLDataSetDef,
                ...
                'RWH(28/02/2001)
                m_lReturn = oBusiness.GIS_SaveToDB(v_sGisDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)))
                If (m_lReturn <> PMEReturnCode.PMTrue) Then
                ...
                m_lReturn = oBusiness.GetPolicyBinderId(v_sDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)),
                        v_lGISPolicyLinkId:=nNewGisPolicyLinkID,
                        r_lPolicyBinderId:=lNewPolicyBinderId)
                If (m_lReturn <> PMEReturnCode.PMTrue) Then
                ...
                m_lReturn = oBusiness.GetPolicyBinderId(v_sDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)),
                        v_lGISPolicyLinkId:=vGisPolicyLinkArray(0, 0),
                        r_lPolicyBinderId:=lOldPolicyBinderId)
                If (m_lReturn <> PMEReturnCode.PMTrue) Then
                    r_sFailureReason = "GetPolicyBinderId"
                    Return PMEReturnCode.PMFalse
                End If
                ...
                m_lReturn = oBusiness.DeleteOutputTable(v_sDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)),
                ...
                Dim nRiskRenrule As Integer = 0
                nRiskRenrule = ToSafeInteger(oRiskArray(ACRiskRenRule, lCount))

                If nRiskRenrule = 1 Then
                    lTransactionType = nTransactionType
                    oBusiness.EncodeTransactionScreenAndType(lQuoteType, lTransactionType, 0, 5)

                    'run renewal script
                    m_lReturn = oBusiness.GIS_NBQuote(v_sGisDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)),
                            v_lQuoteType:=lQuoteType,
                            r_sXMLDataSet:=sXMLDataSet,
                            r_sXMLDataSetDef:=sXMLDataSetDef)

                    If (m_lReturn <> PMEReturnCode.PMTrue) Then
                        r_sFailureReason = AppendString(r_sFailureReason, "Failed Renewal Script")
                        nResult = PMEReturnCode.PMFalse
                    End If

                    m_lReturn = oBusiness.GIS_SaveToDB(v_sGisDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)))
                    If (m_lReturn <> PMEReturnCode.PMTrue) Then
                        r_sFailureReason = AppendString(r_sFailureReason, "SaveToDB")
                        nResult = PMEReturnCode.PMFalse
                    End If
                    ...
                    m_lReturn = oBusiness.CheckOutputTable(
                                    v_sDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)),
                                    v_lPolicyBinderId:=lNewPolicyBinderId,
                                    r_sReasons:=sFailureDetail)
                    If (m_lReturn <> PMEReturnCode.PMTrue) Then
                        r_sFailureReason = AppendString(r_sFailureReason, "CheckOutputTable")
                        nResult = PMEReturnCode.PMFalse
                    End If
                End If


                'm_lReturn = oBusiness.CopyRiskStandardWordings(v_lOldPolicyBinderId:=lOldPolicyBinderId, _
                '        v_lNewPolicyBinderId:=lNewPolicyBinderId, _
                '        v_sDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)))

                m_lReturn = oBusiness.CopyRiskStandardWordings(v_lOldPolicyBinderId:=lOldPolicyBinderId,
                                                               v_lNewPolicyBinderId:=lNewPolicyBinderId,
                                                               v_sDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)),
                                                               v_dtEffectiveDate:=CDate(r_vRenewalList(PMFieldPosCoverEndDate, v_lCount)).AddYears(1))

                If (m_lReturn <> PMEReturnCode.PMTrue) Then
                    r_sFailureReason = AppendString(r_sFailureReason, "CopyRiskStandardWordings")
                    nResult = PMEReturnCode.PMFalse
                End If

                If ((v_bTMPAnniversary = True And v_bIsTrueMonthlyPolicy = True) Or (v_bIsTrueMonthlyPolicy = False)) Then
                    '************ INDEX LINKING GIS STUFF *******************
                    Dim auxVar As Object = oRiskArray(ACFieldPosGisScreenID, lCount)
                    If Not (Convert.IsDBNull(auxVar) Or Informations.IsNothing(auxVar)) Then
                        'index link GIS
                        m_lReturn = oBusiness.GisIndexLink(v_lInsuranceFileCnt:=v_lNewInsuranceFileCnt,
                                v_lRiskID:=oRiskArray(ACFieldPosRiskID, lCount),
                                v_vGisScreenID:=oRiskArray(ACFieldPosGisScreenID, lCount),
                                v_dtEffectiveDate:=Informations.DateAdd("y", 1, r_vRenewalList(PMFieldPosCoverStartDate,
                                v_lCount)),
                                v_sGisDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)))
                        If (m_lReturn <> PMEReturnCode.PMTrue) Then
                            r_sFailureReason = AppendString(r_sFailureReason, r_sFailureReason & "Index Link")
                            nResult = PMEReturnCode.PMFalse
                        End If
                    End If
                End If

                m_lReturn = oBusiness.GetPMRegSettingLocal(
                                            v_lPMERegSettingRoot:=eRegSettingRoot,
                ...
                If bExtraRiskDetails Then
                    m_lReturn =
                            oRiskData.CopyRSASumInsured(v_lOldPolicyLinkID:=vGisPolicyLinkArray(0, 0),
                            v_lNewPolicyLinkID:=nNewGisPolicyLinkID)

                    If (m_lReturn <> PMEReturnCode.PMTrue) Then
                        r_sFailureReason = AppendString(r_sFailureReason, "CopyRSASumInsured")
                        nResult = PMEReturnCode.PMFalse
                    End If
                End If
                ...
                m_lReturn = oBusiness.DeleteOutputTable(v_sDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)),
                         v_lPolicyBinderId:=lNewPolicyBinderId)
                If (m_lReturn <> PMEReturnCode.PMTrue) Then
                    r_sFailureReason = AppendString(r_sFailureReason, "DeleteOutputTable")
                    nResult = PMEReturnCode.PMFalse
                End If
                ...
                If oRiskArray(ACRiskUALRule, lCount) = 1 Then
                    oBusiness.EncodeTransactionScreenAndType(lQuoteType, lTransactionType, 0, 3)
                    ...
                    If (m_lReturn <> PMEReturnCode.PMTrue) Then
                        r_sFailureReason = AppendString(r_sFailureReason, "Check Underwriting Authority Limits")
                        nResult = PMEReturnCode.PMFalse
                    End If
                    ...
                    'Check Output table to see if risk has been referred or declined.
                    m_lReturn = oBusiness.CheckOutputTable(v_sDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)),
                            v_lPolicyBinderId:=lNewPolicyBinderId,
                            r_sReasons:=sFailureDetail)
                    If (m_lReturn <> PMEReturnCode.PMTrue) Then
                        r_sFailureReason = AppendString(r_sFailureReason, "CheckOutputTable")
                ...
                m_lReturn = oBusiness.DeleteOutputTable(
                            v_sDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)),
                            v_lPolicyBinderId:=lNewPolicyBinderId)

                If (m_lReturn <> PMEReturnCode.PMTrue) Then
                    r_sFailureReason = AppendString(r_sFailureReason, "DeleteOutputTable2")
                    nResult = PMEReturnCode.PMFalse
                End If

                oBusiness.EncodeTransactionScreenAndType(lQuoteType, lTransactionType, 0, 1)
                'Quote risk.
                m_lReturn = oBusiness.GIS_NBQuote(
                        v_sGisDataModelCode:=Trim$(vGisPolicyLinkArray(4, 0)),
                        v_lQuoteType:=lQuoteType,
                        r_sXMLDataSet:=sXMLDataSet,
                        r_sXMLDataSetDef:=sXMLDataSetDef)
                ...
                If (m_lReturn <> PMEReturnCode.PMTrue) Then
                    r_sFailureReason = AppendString(r_sFailureReason, "Failed to Quote")
                    nResult = PMEReturnCode.PMFalse
                End If
                ...
                If m_lReturn = PMEReturnCode.PMTrue Then
                    ...
                    oPerilAllocation.InsuranceFolderCnt =
                            r_vRenewalList(PMFieldPosInsuranceFolderCnt, v_lCount)
                    oPerilAllocation.RiskId = lNewRiskCnt
                    ...
                    If (m_lReturn <> PMEReturnCode.PMTrue) Then
                        r_sFailureReason = AppendString(r_sFailureReason, "PopulateRatingSections")
                        nResult = PMEReturnCode.PMFalse
                    End If
                ...
                Const SIROPTEnableRI2007 As Integer = 88
                m_lReturn = GetProductionOption(v_vOptionNumber:=SIROPTEnableRI2007, r_oValue:=oIsRI2007)
                ...
                If m_lReturn <> PMEReturnCode.PMTrue Then
                    Return PMEReturnCode.PMFalse
                End If
                m_lReturn = oReinsurance.SetProcessModes(vTask:=PMEComponentAction.PMAdd, vTransactionType:="REN")


                ' Set RI properties
                oReinsurance.InsuranceFileCnt = v_lNewInsuranceFileCnt 'r_vRenewalList(PMFieldPosInsuranceFileCnt, v_lCount)
                oReinsurance.RiskId = lNewRiskCnt

                If ((v_bIsTrueMonthlyPolicy) And Not v_bTMPAnniversary) And ToSafeInteger(r_vRenewalList(PMFieldPosTMPAutoRenFAC, v_lCount)) = 1 Then
                    oReinsurance.TMPRiskCntUnderRenewal = ToSafeInteger(oRiskArray(ACRiskPosCnt, lCount))
                End If

                ' Calculate RI
                m_lReturn = oReinsurance.CalculateRI
                If m_lReturn <> PMEReturnCode.PMTrue Then
                    r_sFailureReason = AppendString(r_sFailureReason, "Calculating Reinsurance")
                    nResult = PMEReturnCode.PMFalse
                End If
                ...
                If m_lReturn <> PMEReturnCode.PMTrue And m_lReturn <> PMEReturnCode.PMNotFound Then
                    r_sFailureReason = AppendString(r_sFailureReason, "Getting Reinsurance Details")
                    nResult = PMEReturnCode.PMFalse
                End If

                ' Update details, this will ensure minor rounding is handled
                m_lReturn = oReinsurance.Update
                If m_lReturn <> PMEReturnCode.PMTrue Then
                    r_sFailureReason = AppendString(r_sFailureReason, "Updating Reinsurance")
                    nResult = PMEReturnCode.PMFalse
                End If

                ' Validate the reinsurance is all OK
                m_lReturn = oReinsurance.ValidateBands(lReinsPremiumOrSumInsured, lReinsBand)

                ' Must return true AND a zero for lReinsPremiumOrSumInsured
                bIsRIValid = (m_lReturn = PMEReturnCode.PMTrue) And (lReinsPremiumOrSumInsured = 0)
                If Not bIsRIValid Then
                    r_sFailureReason = AppendString(r_sFailureReason, "Validating Reinsurance")
                    nResult = PMEReturnCode.PMFalse
                End If


                m_lReturn = oBusiness.ApplyRiskRenewalTaxes(lNewRiskCnt, v_lNewInsuranceFileCnt, r_sFailureReason, True, lOldInsuranceFileCnt)
                If m_lReturn <> PMEReturnCode.PMTrue Then
                    nResult = PMEReturnCode.PMFalse
                End If

                m_lReturn = oBusiness.ApplyRiskFee(lNewRiskCnt, v_lNewInsuranceFileCnt, r_sFailureReason, True, lOldInsuranceFileCnt)
                If m_lReturn <> PMEReturnCode.PMTrue Then
                    nResult = PMEReturnCode.PMFalse
                End If
                ...
                If sFailureDetail <> "" Then
                    If (sFailureDetail).Trim.Substring(8).ToUpper = "DECLINED" Then
                        m_lReturn = oRiskData.UpdateRiskStatus(v_lRiskCnt:=lNewRiskCnt,
                                                                 v_lRiskStatusID:=2)

                    ElseIf (sFailureDetail).Trim.Substring(8).ToUpper = "REFERRED" Then
                        m_lReturn = oRiskData.UpdateRiskStatus(v_lRiskCnt:=lNewRiskCnt,
                                                                v_lRiskStatusID:=1)
                    End If
                Else
                    Dim iModelType As Integer = oRiskData.GetRIModelTypeByRisk(lNewRiskCnt)

                    If iModelType = 2 Then
                    ...
                    Else
                        ...
                        m_lReturn = oRiskData.UpdateRiskStatus(v_lRiskCnt:=lNewRiskCnt,
                                       v_lRiskStatusID:=If(bIsRIValid = True, 3, 4))
                    End If
                ...
                If m_lReturn <> PMEReturnCode.PMTrue Then
                    r_sFailureReason = AppendString(r_sFailureReason, "Failed to update risk status")
                    nResult = PMEReturnCode.PMFalse
                End If

                If (sFailureDetail <> "") Then
                    r_nEligibleForRenewal = PMEReturnCode.PMFalse
                    sFailureCriterion = PMFailedReRateDesc

                    m_lReturn = oBusiness.AddRenewalReport(v_sReportType:="ManualRenewal",
                            v_vClientName:=r_vRenewalList(PMFieldPosClientName, v_lCount),
                            v_vPolicyNumber:=r_vRenewalList(PMFieldPosInsuranceRef, v_lCount),
                            v_vAgentCode:=r_vRenewalList(PMFieldPosAgentName, v_lCount),
                            v_vCoverStartDate:=r_vRenewalList(PMFieldPosCoverStartDate, v_lCount),
            ...
            Else
                ...
                Throw New ApplicationException(r_sFailureReason)
        ...
        Else 'No only need to point to the risk
            ...
            m_lReturn = oRiskData.AddRiskLink(v_lNewInsuranceFileCnt:=v_lNewInsuranceFileCnt,
                                               ...
                                               v_sStatusFlag:="R", v_lOriginalRiskCnt:=0,
            ...
            Else
                ...
                m_lReturn = oBusiness.ApplyRiskRenewalTaxes(lCurrentRiskCnt, v_lNewInsuranceFileCnt, r_sFailureReason, bRecalcTax, lOldInsuranceFileCnt)
                If m_lReturn <> PMEReturnCode.PMTrue Then
                    nResult = PMEReturnCode.PMFalse
                End If

                'Apply Risk Fees
                m_lReturn = oBusiness.ApplyRiskFee(lCurrentRiskCnt, v_lNewInsuranceFileCnt, r_sFailureReason, bRecalcFees, lOldInsuranceFileCnt)
                If m_lReturn <> PMEReturnCode.PMTrue Then
                    nResult = PMEReturnCode.PMFalse
                End If
        ...
        End If
        If (Not (oReinsurance Is Nothing)) Then
            'm_lReturn = oReinsurance.disponse
            oReinsurance = Nothing
        End If
        Return nResult
    End Function

    Private Function GetRiskCntForProcessing() As Integer
        SyncLock oLock
    ...
    Private Sub DBConnect(ByRef oDataBase As dPMDAO.Database)
        ...
        iReturn = oDataBase.OpenDatabase(m_sUsername, m_iSourceID, m_iLanguageID, ACApp)
        If iReturn <> PMEReturnCode.PMTrue Then
            Throw New Exception("Unable to connect to Sirius database")
        End If
    End Sub

    Private Function AppendString(ByVal sCurrentString As String, ByVal sMessage As String) As String
        Dim sString As String = ""
        If sCurrentString <> "" Then
            sString = sCurrentString & vbCrLf
        Else
            sString = sMessage
        End If
        Return sString
    ...
    Private Function GetMultithreadLimit() As Integer
        Dim nThreadLimit As String = String.Empty
        bPMFunc.GetSystemOption(v_sUsername:=m_sUsername, v_sPassword:=m_sPassword, v_iUserID:=m_iUserID, v_iMainSourceID:=m_iSourceID, v_iLanguageID:=m_iLanguageID, v_iCurrencyID:=m_iCurrencyID, v_iLogLevel:=m_iLogLevel, v_sCallingAppName:=m_sCallingAppName, v_iOptionNumber:=kSystemOptionNumberofThreadsforRiskProcessing, r_sOptionValue:=nThreadLimit)

        If Not String.IsNullOrEmpty(nThreadLimit) AndAlso CInt(nThreadLimit) > 1 Then
            Return CInt(nThreadLimit)
        Else
            Return 1
        End If
    End Function
    ...
    Public Function GetLatestPolicyVersion(ByVal v_nInsuranceFolderCnt As Integer,
        ...
        Catch ex As Exception
            ...
            bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetLatestPolicyVersion Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetLatestPolicyVersion", excep:=ex)
            Return nReturn
        End Try
    ...
    ''' <summary>
    ''' GetMaxiumPolicyVersion
    ''' </summary>
    ''' <param name="v_nInsuranceFileCnt"></param>
    ''' <param name="r_nMaximumPolicyVersion"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function GetMaximumPolicyVersion(ByVal v_nInsuranceFileCnt As Integer,
                                                    ByRef r_nMaximumPolicyVersion As Integer) As Integer
        Dim vResultArray As Object = Nothing
        ...
        Try
            ...
            m_oDatabase.Parameters.Clear()
            m_lReturn = AddInputParameter(v_sName:="InsuranceFileCnt", v_vValue:=v_nInsuranceFileCnt, v_iType:=gPMConstants.PMEDataType.PMLong)

            result = m_oDatabase.SQLSelect(sSQL:=kGetMaxPolicyVersionSQL,
            ...
            Else
                If Informations.IsArray(vResultArray) Then
                    r_nMaximumPolicyVersion = ToSafeLong(vResultArray(0, 0))
                Else
                    result = PMEReturnCode.PMNotFound
                End If
            End If
            Return result
        ...
        Catch ex As Exception
            ...
            bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetMaxiumPolicyVersion Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetMaxiumPolicyVersion", excep:=ex)
            Return result
        End Try
...
Public Class RiskData
    Public aoRenewalList(,) As Object
    Public oRiskArray(,) As Object
    Public nCount As Integer
    Public nNewInsuranceFileCnt As Integer
    Public nEligibleForRenewal As Integer
    Public sFailureReason As String
    Public bIsTrueMonthlyPolicy As Boolean
    Public bTMPAnniversary As Boolean
    Public nRiskcnt As Integer
End Class
