...
Imports SharedFiles
Partial Friend Class frmInterfaceSFB
    Inherits System.Windows.Forms.Form
    ' ***************************************************************** '
    ' Form Name: frmInterfaceSFB
    '
    ' Date: 09/06/1999
    '
    ...
    Private Const ACClass As String = "frmInterfaceSFB"
    ...
    Private m_dtMTAThresholdDate As Date
    ...
    ' 100506 Datasure
    Private m_lCOBRatingID As Integer
    ' Stores the search data from the business object.
    Private m_vCOBRatingID(,) As Object
    ' 110506 Datasure
    Private m_bSectionClick As Boolean
    ...
    Private Function SetFieldValidation() As Integer
        ...
        Try
            ...
            m_lReturn = m_oFormFields.AddNewFormField(ctlControl:=txtMTAThresholdDate, lFieldType:=gPMConstants.PMEDataType.PMDate, lFormat:=gPMConstants.PMEFormatStyle.PMFormatDateLong, lMandatory:=gPMConstants.PMEMandatoryStatus.PMNonMandatory)
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If
    ...
    ' Name: DataToDetail
    '
    ' Description: Populate Tax Band Rate Details
    '              storage.
    '
    ' ***************************************************************** '
    Private Function DataToDetail() As Integer
        ...
        Try
            ...
            ' Code
            m_sCode = CStr(m_vTaxBandRate(ACRCode, lSelectedItem))
            m_lReturn = m_oFormFields.FormatControl(ctlControl:=txtCode, vControlValue:=m_sCode)
            ' Description
            m_sRateDescription = CStr(m_vTaxBandRate(ACRDescription, lSelectedItem))
            m_lReturn = m_oFormFields.FormatControl(ctlControl:=txtDescription, vControlValue:=m_sRateDescription)
            ' Effective Date
            m_dtEffectiveDate = CDate(m_vTaxBandRate(ACREffectiveDate, lSelectedItem))
            m_lReturn = m_oFormFields.FormatControl(ctlControl:=txtEffectiveDate, vControlValue:=m_dtEffectiveDate)

            ' Calculation Basis
            m_lCalcBasis = CInt(m_vTaxBandRate(ACRCalcBasis, lSelectedItem))
            optBasis(m_lCalcBasis).Checked = True
            ' Is value
            chkIsValue.CheckState = CInt(m_vTaxBandRate(ACRIsValue, lSelectedItem))
            ' Rate (value and/or percentage)
            m_lRate = CDbl(m_vTaxBandRate(ACRRate, lSelectedItem))
            ...
            ' Sum insured value
            m_lSumInsuredValue = CInt(m_vTaxBandRate(ACRSumInsuredValue, lSelectedItem))
            m_lReturn = m_oFormFields.FormatControl(ctlControl:=txtSumInsuredValue, vControlValue:=m_lSumInsuredValue)
            ' Rounded?
            chkRounded.CheckState = gPMFunctions.ToSafeInteger(m_vTaxBandRate(ACRSumInsuredRounded, lSelectedItem))
            ' Tax credit?
            chkAllowCredit.CheckState = gPMFunctions.ToSafeInteger(m_vTaxBandRate(ACRAllowTaxCredit, lSelectedItem))
            'Is Passed to Insurer
            chkIsPassedToInsurer.CheckState = gPMFunctions.ToSafeInteger(m_vTaxBandRate(ACRIsPassedToInsurer, lSelectedItem))
            ' MTAThresholdDate
            If CStr(m_vTaxBandRate(ACRMTAThresholdDate, lSelectedItem)) <> "" Then
                m_dtMTAThresholdDate = CDate(m_vTaxBandRate(ACRMTAThresholdDate, lSelectedItem))
                m_lReturn = m_oFormFields.FormatControl(ctlControl:=txtMTAThresholdDate, vControlValue:=m_dtMTAThresholdDate)
            End If
            ' Affected transactions
            chkNB.CheckState = gPMFunctions.ToSafeInteger(m_vTaxBandRate(ACRNB, lSelectedItem))
            chkAMTA.CheckState = gPMFunctions.ToSafeInteger(m_vTaxBandRate(ACRAMTA, lSelectedItem))
            chkRMTA.CheckState = gPMFunctions.ToSafeInteger(m_vTaxBandRate(ACRRMTA, lSelectedItem))
            chkCANC.CheckState = gPMFunctions.ToSafeInteger(m_vTaxBandRate(ACRCANC, lSelectedItem))
            chkREN.CheckState = gPMFunctions.ToSafeInteger(m_vTaxBandRate(ACRREN, lSelectedItem))

            ' Additional transaction types
            chkTTIC.CheckState = gPMFunctions.ToSafeInteger(m_vTaxBandRate(ACRTTIC, lSelectedItem))
            chkTTAC.CheckState = gPMFunctions.ToSafeInteger(m_vTaxBandRate(ACRTTAC, lSelectedItem))
            chkTTF.CheckState = gPMFunctions.ToSafeInteger(m_vTaxBandRate(ACRTTF, lSelectedItem))
            ' 020506 Datasure
            chkTTE.CheckState = gPMFunctions.ToSafeInteger(m_vTaxBandRate(ACRTTE, lSelectedItem))

            ' Filters
            cboCountry.ItemId = gPMFunctions.ToSafeLong(m_vTaxBandRate(ACRCountryID, lSelectedItem))
            cboState.ItemId = gPMFunctions.ToSafeLong(m_vTaxBandRate(ACRStateID, lSelectedItem))
            cboCOB.ItemId = gPMFunctions.ToSafeLong(m_vTaxBandRate(ACRCOBID, lSelectedItem))
            ' 020506 Datasure
            cboRiskGroup.ItemId = gPMFunctions.ToSafeLong(m_vTaxBandRate(ACRRiskGroupId, lSelectedItem))
            cboRiskCode.ItemId = gPMFunctions.ToSafeLong(m_vTaxBandRate(ACRRiskCodeId, lSelectedItem))

            For lLoop As Integer = 0 To cboSection.Items.Count - 1
                If VB6.GetItemData(cboSection, lLoop) = gPMFunctions.ToSafeLong(m_vTaxBandRate(ACRCOBRatingSectionId, lSelectedItem)) Then
                    cboSection.SelectedIndex = lLoop
                End If
            Next lLoop

            ' Currency
            m_iCurrencyID = gPMFunctions.ToSafeLong(m_vTaxBandRate(ACRCurrencyID, lSelectedItem))
            If m_iCurrencyID = 0 Then
                '        'Default to system currency
                '        m_lReturn = g_oObjectManager.GetInstance( _
                ''            oObject:=oPMSystem, _
                ''            sClassName:="bPMSystem.Business", _
                ''            vInstanceManager:=PMGetViaClientManager)
                '        If m_lReturn <> PMTrue Then
                '            DataToDetail = PMFalse
                '            Exit Function
                '        End If
                '
    ...
    ' Name: ClearDetail
    '
    ' Description: Clear Treaty Party Details
    '              storage.
    '
    ' ***************************************************************** '
    Private Function ClearDetail() As Integer
        ...
        chkIsPassedToInsurer.CheckState = CheckState.Unchecked


        ' Affected transactions
        chkNB.CheckState = CheckState.Checked
        chkAMTA.CheckState = CheckState.Checked
        chkRMTA.CheckState = CheckState.Checked
        chkCANC.CheckState = False
        chkREN.CheckState = CheckState.Checked

        ' Additional transaction types
        chkTTIC.CheckState = False
        chkTTAC.CheckState = False
        chkTTF.CheckState = False
        ' 020506 Datasure
        chkTTE.CheckState = False
        ...
        cboCOB.ItemId = 0
        ' 020506 Datasure
        cboRiskGroup.ItemId = 0
        cboRiskCode.ItemId = 0
        'AR20060929 - Multi-Insurer changes
        ...
        ' Log Error.
        iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to update the Detail details from the search data storage", vApp:=ACApp, vClass:=ACClass, vMethod:="ClearDetail", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
    ...
    ' Name: DataRefresh
    '
    ' Description: Populate Treaty Party Refresh storage.
    ' ***************************************************************** '
    Private Function DataRefresh() As Integer
        ...
        Try
            ...
            m_dtMTAThresholdDate = CDate(m_oFormFields.UnformatControl(ctlControl:=txtMTAThresholdDate))
            m_lIsValue = chkIsValue.CheckState

            If m_lIsValue Then
                If Strings.Len(txtRateValue.Text) Then
                ...
                End If
            Else
                If Strings.Len(txtRatePercentage.Text) Then
            ...
            If Strings.Len(txtSumInsuredValue.Text) Then
            ...
            Select Case m_iAction
                Case gPMConstants.PMEComponentAction.PMAdd
                    If Information.IsArray(m_vTaxBandRate) Then
                        lSelectedItem = m_vTaxBandRate.GetUpperBound(1) + 1
                        ReDim Preserve m_vTaxBandRate(m_vTaxBandRate.GetUpperBound(0), lSelectedItem)
                    Else
                        lSelectedItem = 0
                        ReDim m_vTaxBandRate(ACRMaxArray, lSelectedItem)
                    End If
                    sCode = m_sCode

                Case gPMConstants.PMEComponentAction.PMCopy
                    lSelectedItem = m_vTaxBandRate.GetUpperBound(1)
                    sCode = m_sCode

                Case gPMConstants.PMEComponentAction.PMEdit

                    lSelectedItem = Convert.ToString(lvwTaxBandRate.Items.Item(lvwTaxBandRate.FocusedItem.Index).Tag)
                    sCode = m_sCode
            ...
            m_vTaxBandRate(ACRTaxBandId, lSelectedItem) = m_lTaxBandId
            m_vTaxBandRate(ACRTaxBandRateId, lSelectedItem) = m_lTaxBandRateId
            m_vTaxBandRate(ACRCode, lSelectedItem) = sCode
            m_vTaxBandRate(ACRCaptionId, lSelectedItem) = m_lRateCaptionID
            m_vTaxBandRate(ACRDescription, lSelectedItem) = m_sRateDescription
            m_vTaxBandRate(ACREffectiveDate, lSelectedItem) = m_dtEffectiveDate
            m_vTaxBandRate(ACRIsDeleted, lSelectedItem) = m_lIsDeleted
            m_vTaxBandRate(ACRIsValue, lSelectedItem) = m_lIsValue
            m_vTaxBandRate(ACRRate, lSelectedItem) = m_lRate
            m_vTaxBandRate(ACRSumInsuredRounded, lSelectedItem) = chkRounded.CheckState
            m_vTaxBandRate(ACRCalcBasis, lSelectedItem) = m_lCalcBasis
            m_vTaxBandRate(ACRSumInsuredValue, lSelectedItem) = m_lSumInsuredValue
            m_vTaxBandRate(ACRMTAThresholdDate, lSelectedItem) = m_dtMTAThresholdDate
            m_vTaxBandRate(ACRNB, lSelectedItem) = chkNB.CheckState
            m_vTaxBandRate(ACRAMTA, lSelectedItem) = chkAMTA.CheckState
            m_vTaxBandRate(ACRRMTA, lSelectedItem) = chkRMTA.CheckState
            m_vTaxBandRate(ACRCANC, lSelectedItem) = chkCANC.CheckState
            m_vTaxBandRate(ACRREN, lSelectedItem) = chkREN.CheckState
            m_vTaxBandRate(ACRIsPassedToInsurer, lSelectedItem) = chkIsPassedToInsurer.CheckState
            m_vTaxBandRate(ACRCurrencyID, lSelectedItem) = m_iCurrencyID
            m_vTaxBandRate(ACRAllowTaxCredit, lSelectedItem) = chkAllowCredit.CheckState

            m_vTaxBandRate(ACRCountryID, lSelectedItem) = IIf(cboCountry.ItemId = 0, DBNull.Value, cboCountry.ItemId)

            m_vTaxBandRate(ACRStateID, lSelectedItem) = IIf(cboState.ItemId = 0, DBNull.Value, cboState.ItemId)

            m_vTaxBandRate(ACRCOBID, lSelectedItem) = IIf(cboCOB.ItemId = 0, DBNull.Value, cboCOB.ItemId)
            m_vTaxBandRate(ACRCOBDesc, lSelectedItem) = IIf(cboCOB.ItemId = 0, "", cboCOB.ItemCaption)
            m_vTaxBandRate(ACRCountryDesc, lSelectedItem) = IIf(cboCountry.ItemId = 0, "", cboCountry.ItemCaption)
            m_vTaxBandRate(ACRStateDesc, lSelectedItem) = IIf(cboState.ItemId = 0, "", cboState.ItemCaption)
            ' 020506 Datasure

            m_vTaxBandRate(ACRRiskGroupId, lSelectedItem) = IIf(cboRiskGroup.ItemId = 0, DBNull.Value, cboRiskGroup.ItemId)

            m_vTaxBandRate(ACRRiskCodeId, lSelectedItem) = IIf(cboRiskCode.ItemId = 0, DBNull.Value, cboRiskCode.ItemId)
            'AR20060929 - Multi-insurer changes

            m_vTaxBandRate(ACRCOBRatingSectionId, lSelectedItem) = IIf((cboSection.SelectedIndex) = 0, DBNull.Value, VB6.GetItemData(cboSection, cboSection.SelectedIndex))

            m_vTaxBandRate(ACRRiskGroupDesc, lSelectedItem) = IIf(cboRiskGroup.ItemId = 0, "", cboRiskGroup.ItemCaption)
            m_vTaxBandRate(ACRRiskCodeDesc, lSelectedItem) = IIf(cboRiskCode.ItemId = 0, "", cboRiskCode.ItemCaption)
            m_vTaxBandRate(ACRCOBRatingSecDesc, lSelectedItem) = IIf(VB6.GetItemData(cboSection, cboSection.SelectedIndex) = 0, "", cboSection.Text)

            ' Additional transaction types (only save if we are a premium based rate)
            'DC130706 PN29391 save options for original premium also
            If (m_lCalcBasis = 0) Or (m_lCalcBasis = 3) Or (m_lCalcBasis = 4) Then
                m_vTaxBandRate(ACRTTIC, lSelectedItem) = chkTTIC.CheckState
                m_vTaxBandRate(ACRTTAC, lSelectedItem) = chkTTAC.CheckState
                m_vTaxBandRate(ACRTTF, lSelectedItem) = chkTTF.CheckState
                ' 020506 Datasure
                m_vTaxBandRate(ACRTTE, lSelectedItem) = chkTTE.CheckState
            Else
                m_vTaxBandRate(ACRTTIC, lSelectedItem) = False
                m_vTaxBandRate(ACRTTAC, lSelectedItem) = False
                m_vTaxBandRate(ACRTTF, lSelectedItem) = False
                ' 020506 Datasure
                m_vTaxBandRate(ACRTTE, lSelectedItem) = False
            End If
        ...
        Catch excep As System.Exception
            ...
            ' Log Error.
            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to update the data from the search data storage", vApp:=ACApp, vClass:=ACClass, vMethod:="DataRefresh", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
            ...
            If Information.IsArray(m_vTaxBandRate) Then
                For lTemp As Integer = m_vTaxBandRate.GetLowerBound(1) To m_vTaxBandRate.GetUpperBound(1)
                    If CStr(m_vTaxBandRate(ACRCode, lTemp)) <> "" Then
                        ...
                        ' 020506 Datasure
                        ' Risk GroupId
                        ListViewHelper.GetListViewSubItem(oListItem, 3).Text = CStr(m_vTaxBandRate(ACRRiskGroupDesc, lTemp))
                        ' Risk Code Type
                        ListViewHelper.GetListViewSubItem(oListItem, 4).Text = CStr(m_vTaxBandRate(ACRRiskCodeDesc, lTemp))
                        ' Section
                        ListViewHelper.GetListViewSubItem(oListItem, 5).Text = CStr(m_vTaxBandRate(ACRCOBRatingSecDesc, lTemp))
                        ' Country
                        ListViewHelper.GetListViewSubItem(oListItem, 6).Text = CStr(m_vTaxBandRate(ACRCountryDesc, lTemp))
                        ' State
                        ListViewHelper.GetListViewSubItem(oListItem, 7).Text = CStr(m_vTaxBandRate(ACRStateDesc, lTemp))
    ...
    Public Function InterfaceToBusiness() As Integer
        ...
        Try
            ...
            m_lReturn = m_oBusiness.Update(v_vTaxBandRate:=m_vTaxBandRate)
    ...
    Private Function SetInterfaceDefaults() As Integer
        ...
        Try
            ...
            lvwTaxBandRate.Columns.Item(0).Width = CInt(VB6.TwipsToPixelsX(2800))
            ...
            ' 020506 Datasure
            lvwTaxBandRate.Columns.Item(3).Width = CInt(VB6.TwipsToPixelsX(1400))
            lvwTaxBandRate.Columns.Item(4).Width = CInt(VB6.TwipsToPixelsX(1100))
            lvwTaxBandRate.Columns.Item(5).Width = CInt(VB6.TwipsToPixelsX(1400))
            lvwTaxBandRate.Columns.Item(6).Width = CInt(VB6.TwipsToPixelsX(2400))
            lvwTaxBandRate.Columns.Item(7).Width = CInt(VB6.TwipsToPixelsX(2400))
            ...
            m_lReturn = oBusiness.GetBranchBaseCurrency(vSourceId:=g_iSourceID, iCurrencyID:=m_iCurrencyID)
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to get Branch Base Currency", vApp:=ACApp, vClass:=ACClass, vMethod:="SetInterfaceDefaults ", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return result
            End If
            cboCurrency.CurrencyId = m_iCurrencyID
            oBusiness = Nothing
    ...
    Private Function DisplayCaptions() As Integer
        ...
        Try
            ...
            SSTabHelper.SetTabCaption(tabMainTab, 0, "2 - Tax Band Rate")
            ...
            ' 020506 Datasure
            'lvwTaxBandRate.ColumnHeaders(4).Text = "Class of Business"
            lvwTaxBandRate.Columns.Item(3).Text = "Risk Group"
            lvwTaxBandRate.Columns.Item(4).Text = "Risk Type"
            lvwTaxBandRate.Columns.Item(5).Text = "Section"
            lvwTaxBandRate.Columns.Item(6).Text = "Country"
            lvwTaxBandRate.Columns.Item(7).Text = "State"
    ...
    Private Sub SetFormControls()
        ...
        'DC110706 PN29317 add Original Premium as an option to show transaction type options
        chkTTIC.Enabled = (lBasis = 0 Or lBasis = 4 Or lBasis = 3)
        chkTTAC.Enabled = (lBasis = 0 Or lBasis = 4 Or lBasis = 3)
        chkTTF.Enabled = (lBasis = 0 Or lBasis = 4 Or lBasis = 3)
        ' 020506 Datasure
        chkTTE.Enabled = (lBasis = 0 Or lBasis = 4 Or lBasis = 3)

        lblTTPolicy.Enabled = (lBasis = 0 Or lBasis = 4 Or lBasis = 3)
    ...
    End Sub
    ' 110506 Datasure
    Private Sub cboRiskCode_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cboRiskCode.Click

        Try

            m_bSectionClick = False

            If cboRiskCode.ItemId = 0 Then
                m_lCOBRatingID = -1
            Else
                m_lCOBRatingID = cboRiskCode.ItemId
            End If
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                ...
                Exit Sub
            End If
            m_bSectionClick = True

        Catch excep As System.Exception


            ' Log Error.
            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to get the sectionId value", vApp:=ACApp, vClass:=ACClass, vMethod:="cboRiskCode_Click", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    End Sub
    ' 110506 Datasure
    Private Sub cboRiskGroup_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cboRiskGroup.Click
        ' Only show risk codes that match the current risk group
        If cboRiskGroup.ItemId = 0 Then
            cboRiskCode.WhereClause = "risk_group_id > " & cboRiskGroup.ItemId
        Else
            cboRiskCode.WhereClause = "risk_group_id = " & cboRiskGroup.ItemId
        End If
        cboRiskCode.RefreshList()
    End Sub
    ' 110506 Datasure
    Private Sub cboSection_SelectedIndexChanged(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cboSection.SelectedIndexChanged
        Dim lselindex As Integer
        If m_bSectionClick Then
            If VB6.GetItemData(cboSection, cboSection.SelectedIndex) > 0 Then
                lselindex = cboSection.SelectedIndex
                If cboRiskCode.ItemId = 0 Then
                    cboRiskGroup.ListIndex = 0
                End If
                cboSection.SelectedIndex = lselindex
            End If
        End If
        m_bSectionClick = False
    End Sub
    ...
    Private Sub frmInterfaceSFB_Load(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Load
    ...
    Private Sub frmInterfaceSFB_FormClosing(ByVal eventSender As Object, ByVal eventArgs As FormClosingEventArgs) Handles MyBase.FormClosing
    ...
    Private Sub optBasis_CheckedChanged(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles _optBasis_4.CheckedChanged, _optBasis_1.CheckedChanged, _optBasis_0.CheckedChanged, _optBasis_2.CheckedChanged, _optBasis_3.CheckedChanged
    ...
    Private Sub txtMTAThresholdDate_Enter(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles txtMTAThresholdDate.Enter
        m_lReturn = m_oFormFields.GotFocus(ctlControl:=txtMTAThresholdDate)
    End Sub

    Private Sub txtMTAThresholdDate_Leave(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles txtMTAThresholdDate.Leave
        If txtMTAThresholdDate.Text <> "" Then
            m_lReturn = m_oFormFields.LostFocus(ctlControl:=txtMTAThresholdDate)
        End If
    End Sub
    ...
    End Sub
    ' ***************************************************************** '
    ' Name: GetCOBLookupvalue
    '
    ' Description: Retrieves the details from the business object.
    '
    ' ***************************************************************** '
    Public Function GetCOBLookupvalue() As Integer
        ...
        Try
            ...
            m_lReturn = m_oBusiness.GetCOBRatingSectionsForRisk(v_lRiskCodeID:=m_lCOBRatingID, r_vResults:=m_vCOBRatingID)
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                ...
                ' Log Error.
                iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to get details from the business object", vApp:=ACApp, vClass:=ACClass, vMethod:="GetCOBLookupvalue")
                Return result
            End If

            m_lReturn = SetCOBLookupvalue()
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                ...
                ' Log Error.
                iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to Set section value details from the business object", vApp:=ACApp, vClass:=ACClass, vMethod:="GetCOBLookupvalue")
                Return result
            End If
        ...
        Catch excep As System.Exception
            ...
            ' Log Error.
            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to get the details from the business object", vApp:=ACApp, vClass:=ACClass, vMethod:="GetCOBLookupvalue", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    ' 110506 Datasure
    Public Function SetCOBLookupvalue() As Integer
        ...
        Try
            ...
            Dim cboSection_NewIndex As Integer = -1
            cboSection_NewIndex = cboSection.Items.Add("(None)")
            VB6.SetItemData(cboSection, cboSection_NewIndex, 0)

            lRow = m_vCOBRatingID.GetUpperBound(1)
            ' Using the lookup values, populate the control with
            ' the details from the lookup details array.

            For lCntr As Integer = 0 To lRow
                ' Add the details to the control.
                If Strings.Len(CStr(m_vCOBRatingID(ACDetailDesc, lCntr))) > 0 Then
                    cboSection_NewIndex = cboSection.Items.Add(CStr(m_vCOBRatingID(ACDetailDesc, lCntr)))
                    VB6.SetItemData(cboSection, cboSection_NewIndex, CInt(m_vCOBRatingID(ACDetailKey, lCntr)))
                End If
            Next lCntr
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                ...
                ' Log Error.
                iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to set section details from the business object", vApp:=ACApp, vClass:=ACClass, vMethod:="SetCOBLookupvalue")
        ...
        Catch excep As System.Exception
            ...
            ' Log Error.
            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to set the section details from the business object", vApp:=ACApp, vClass:=ACClass, vMethod:="SetCOBLookupvalue", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
