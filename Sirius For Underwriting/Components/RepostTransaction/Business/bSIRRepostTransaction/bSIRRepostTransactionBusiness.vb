...
Finally_Renamed:
    ...
    Public Function CreateReverseStats(ByVal v_nInsuranceFileCnt As Integer, ByRef r_nStatsFolderCnt As Integer, Optional ByRef v_sDocumentRef As String = "") As Integer
        ...
        Try
            ...
            m_lReturn = CreateReverseStatsFolder(v_nInsuranceFileCnt:=v_nInsuranceFileCnt, r_nStatsFolderCnt:=r_nStatsFolderCnt, v_nIsCloneReverse:=IsClonedReverse)
    ...
    Private Function CreateReverseStatsFolder(ByVal v_nInsuranceFileCnt As Integer, ByRef r_nStatsFolderCnt As Integer, Optional v_nIsCloneReverse As Integer = 0) As Integer
        Dim result As Integer = 0
        ...
        Try
            ...
            If IsArray(aoRiskArray) Then
                For lCount = 0 To UBound(aoRiskArray, 2)
                    ...
                    nResult = oReinsurance.SetProcessModes(vTask:=1, vTransactionType:=sTransactionType, vEffectiveDate:=DateTime.Now)
    ...
    Public Function ProcessClaimTransactions(ByVal v_lClaimId As Long, ByVal v_sDocumentRef As String, ByVal v_sRefNumber As String, Optional ByVal v_bRePost As Boolean = False, Optional ByVal v_sTransactionTypeCode As String = "") As Long

        Const kMethodName As String = "ProcessClaimTransactions"
        ProcessClaimTransactions = gPMConstants.PMEReturnCode.PMTrue

        ' Call BeginTrans()
        m_lReturn = m_oDatabase.SQLBeginTrans

        m_lReturn = ReverseClaimTransactionsbyDocumentRef(v_lClaimId, v_sDocumentRef, v_sTransactionTypeCode, v_sRefNumber)
        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
        ...
        If v_bRePost Then
            If Left(v_sDocumentRef, 3) = "CLD" Or Left(v_sDocumentRef, 3) = "CLC" Then
                m_lReturn = RePostClonedClaimTransaction(v_lClaimId:=v_lClaimId, v_sDocumentRef:=v_sDocumentRef, v_sRefNumber:=v_sRefNumber, v_sTransactionType:=v_sTransactionTypeCode)
            Else
                'm_lReturn = RePostTransactions(v_lInsuranceFileCnt, v_sDocumentRef, v_sRefNumber, v_sTransactionType)
            End If
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                ProcessClaimTransactions = gPMConstants.PMEReturnCode.PMFalse
                ' Call RollbackTrans()
    ...
    Public Function ReverseClaimTransactions(ByVal v_lClaimId As Long, ByVal v_lStatsFolderCnt As Long, ByVal m_sTransactionCode As String, ByVal sDocumentRef As String) As Long
        ...
        Try
            ...
            If Left(sDocumentRef, 3) = "CLC" Or Left(sDocumentRef, 3) = "CLD" Then
                IsClonedReversal = 1
                lcloned = 1
 ...
 Catch excep As Exception
    ...
    Public Function RePostClonedClaimTransaction(ByVal v_lClaimId As Long, _
        ...
        Try
            ...
            m_lReturn = oObject.RaiseClaimTransaction(v_lClaimId, v_sTransactionType)
            If (m_lReturn <> gPMConstants.PMEReturnCode.PMTrue) Then
                RePostClonedClaimTransaction = gPMConstants.PMEReturnCode.PMFalse
                Exit Function
            End If
    ...
    Public Function AddDataFixUtilityLog _
        ...
        Optional ByVal v_ClaimId As Long = 0) As Long
        ...
        Try
            ...
            With m_oDatabase
                ...
                m_lReturn = .Parameters.Add( _
                      sName:="old_document_ref", _
                      vValue:=v_sOldDocumentRef, _
                      iDirection:=PMEParameterDirection.PMParamInput, _
                      iDataType:=gPMConstants.PMEDataType.PMString)
                ...
                m_lReturn = .Parameters.Add( _
                       sName:="new_document_id", _
                       vValue:=v_sNewDocumentid, _
                       iDirection:=PMEParameterDirection.PMParamInput, _
                       iDataType:=gPMConstants.PMEDataType.PMLong)
            ...
            m_lReturn = m_oDatabase.SQLAction( _
                sSQL:="spu_DataFixUtility_log_add", _
                sSQLName:="spu_DataFixUtility_log_add", _
                bStoredProcedure:=True, _
                lRecordsAffected:=lRecordsAffected)
