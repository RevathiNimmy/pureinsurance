...
Public NotInheritable Class Interface_Renamed
    ...
    Private m_bPolicyLevel As Boolean
    ...
    '
    'Histroy:       ??/??/????  ??? Created
    '               23/03/2003  APS Changed to use new variable name
    ' ***************************************************************** '
    ...
    Public Function SetKeys(ByRef vKeyArray(,) As Object) As Integer
        ...
        Try
            ...
            For lRow As Integer = vKeyArray.GetLowerBound(1) To vKeyArray.GetUpperBound(1)
                ...
                Select Case CStr(vKeyArray(gPMConstants.PMENavLetGetKeyColPosition.PMKeyName, lRow)).Trim().ToUpper()
                    Case "INSURANCE_FILE_CNT"

                        m_lInsuranceFileCnt = CInt(vKeyArray(gPMConstants.PMENavLetGetKeyColPosition.PMKeyValue, lRow))
                    Case "RISK_CNT"

                        m_lRiskCnt = CInt(CStr(vKeyArray(gPMConstants.PMENavLetGetKeyColPosition.PMKeyValue, lRow)))
                        g_bRiskFlag = True
                    Case "POLICY_LEVEL"

                        If StringsHelper.ToDoubleSafe(CStr(vKeyArray(gPMConstants.PMENavLetGetKeyColPosition.PMKeyValue, lRow))) = 1 Then
                            m_bPolicyLevel = True
                            g_bRiskFlag = False
                        Else
                            m_bPolicyLevel = False
                        End If
                End Select
            ...
            'TR - PN5292 - Fix for Zimnat. Needs more work, but this will do the job
            'From the NB roadmap, PolicyLevel taxes are always entered with
            'processmode = Add (because when written ProcessMode can only be
            'set at Roadmap header level. So a new key's been added so that if
            'we're in the NB roadmap, at PolicyLevel, then m_bPolicyLevel is
            'set to True, and this is used to change the mode from Add to Edit
            'To use this add the key "POLICY_LEVEL" = 1 to your roadmap
            If m_bPolicyLevel And m_iTask = gPMConstants.PMEComponentAction.PMAdd Then
                m_iTask = gPMConstants.PMEComponentAction.PMEdit
            End If
    ...
    Private Function LoadInterface() As Integer
        ...
        'TR - Unload the form if there are no taxes
        If frmInterface.ErrorNumber = gPMConstants.PMEReturnCode.PMOK Then
            frmInterface.Close()
            result = gPMConstants.PMEReturnCode.PMOK
            ' Check if we have had an error so far.
        ElseIf frmInterface.ErrorNumber = gPMConstants.PMEReturnCode.PMFalse Then
    ...
    Public Sub New()
        ...
        Try
            ...
            'AK 9/1/01 - to intialise the class for Insurance file type
            g_bRiskFlag = False
