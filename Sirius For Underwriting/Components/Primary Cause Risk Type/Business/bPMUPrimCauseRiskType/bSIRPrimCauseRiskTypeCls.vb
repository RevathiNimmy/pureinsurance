...
Public NotInheritable Class Business
    ...
    ' JMK 19/10/2001 return "A" for Agency and "U" for Underwriting
    ...
    Public Function GetPrimCauseRiskTypeGrp(ByRef r_vPrimCauseRiskTypeGrp(,) As Object) As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.Parameters.Add(sName:="primary_cause_id", vValue:=CStr(m_lPrimaryCauseId), idirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            m_lReturn = m_oDatabase.SQLSelect(sSQL:=ACSelectPrimaryCauseRiskTypeGroupSQL, sSQLName:=ACSelectPrimaryCauseRiskTypeGroupName, bStoredProcedure:=ACSelectPrimaryCauseRiskTypeGroupStored, vResultArray:=r_vPrimCauseRiskTypeGrp)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                ' Log Error Message
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="m_oDatabase.SQLSelect failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetPrimCauseRiskTypeGrp")
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetPrimCauseRiskTypeGrp Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetPrimCauseRiskTypeGrp", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    '
    ' ***************************************************************** '
    Public Function Update(ByVal v_vPrimCauseRiskTypeGrp(,) As Object, Optional v_sUniqueId As String = "", Optional v_sScreenHierarchy As String = "") As Integer
        ...
        Try
            ...
            m_lReturn = m_oDatabase.SQLAction(sSQL:=ACDeletePrimaryCauseRiskTypeGroupSQL, sSQLName:=ACDeletePrimaryCauseRiskTypeGroupName, bStoredProcedure:=ACDeletePrimaryCauseRiskTypeGroupStored)
            ...
            If Not Information.IsArray(v_vPrimCauseRiskTypeGrp) Then
                m_lReturn = CType(CommitTrans(), gPMConstants.PMEReturnCode)
                Return result
            End If

            'Readd the treaty parties
            For lTemp As Integer = v_vPrimCauseRiskTypeGrp.GetLowerBound(1) To v_vPrimCauseRiskTypeGrp.GetUpperBound(1)


                If CDbl(v_vPrimCauseRiskTypeGrp(ACTPPrimaryCauseId, lTemp)) <> 0 Then
                    ...
                    m_lReturn = m_oDatabase.Parameters.Add(sName:="risk_type_group_id", vValue:=CStr(v_vPrimCauseRiskTypeGrp(ACTPRiskTypeGroupId, lTemp)), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)
                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                        m_lReturn = CType(RollbackTrans(), gPMConstants.PMEReturnCode)
                        Return gPMConstants.PMEReturnCode.PMFalse
                    End If
                    ...
                    m_lReturn = m_oDatabase.SQLAction(sSQL:=ACInsertPrimaryCauseRiskTypeGroupSQL, sSQLName:=ACInsertPrimaryCauseRiskTypeGroupName, bStoredProcedure:=ACInsertPrimaryCauseRiskTypeGroupStored)
                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                        m_lReturn = CType(RollbackTrans(), gPMConstants.PMEReturnCode)
