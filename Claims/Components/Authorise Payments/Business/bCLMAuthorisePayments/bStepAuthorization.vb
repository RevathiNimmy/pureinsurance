...
Public NotInheritable Class StepAuthorization
    ...
    Private m_bIsRecommenderOn As Boolean
    ...
    Private Const ACPaymentCurrency As Integer = 4
    Private Const ACClaimPaymentCurrencyID As Integer = 5
    Private Const kIsRecommender As Integer = 6
    Private Const kRecommendLimit As Integer = 7
    ...
    Public Property LastStep() As Boolean
        ...
        End Set
    End Property
    Public Property PaymentCurrencyID() As Decimal
        Get
    ...
    Public Property IsRecommenderOn() As Boolean
        Get
            Return m_bIsRecommenderOn
        End Get
        Set(ByVal value As Boolean)
            m_bIsRecommenderOn = value
        End Set
    End Property
    ...
    Private Function ProcessStep(ByVal v_lApproved As Integer) As Integer
        ...
        If bGroupsExists Or IsRecommenderOn Then
            ...
            If bUserIsUnique = True Or IsRecommenderOn Then
                ...
                End If
            Else
                m_sProcessErrorMessage = "You are either the person who entered this payment or you have authorized a previous step." & Strings.ChrW(13) & Strings.ChrW(10) & "You cannot authorize 2 steps on the same payment."
            End If
        Else
    ...
    Public Function GetStepGroupCode(ByRef r_sGroupCode As String, Optional ByRef r_sErrorMessage As String = "") As Integer
        ...
        Try
            ...
            If bGroupsExists Then
                ...
                If Informations.IsArray(vStepDetails) Then
                    ...
                    r_sErrorMessage = m_sProcessErrorMessage
                Else
                    r_sGroupCode = ""
                    'GetStepGroupCode = PMError
                    m_sProcessErrorMessage = "The step numbers for the Debtor User Groups are not setup correctly. " &
                                             "The system cannot find step number " & CStr(lApprovalStep) & "." &
                                             Strings.ChrW(13) & Strings.ChrW(10) & " Please contact your system administrator."
                End If
            Else
                bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Debtor User Group for Claim Payments Group Type Does Not Exist", vApp:=ACApp, vClass:=ACClass, vMethod:="GetStepGroupCode", vErrNo:=Informations.Err().Number, vErrDesc:=Informations.Err().Description)
                r_sErrorMessage = "Debtor User Group for Claim Payments Group Type Does Not Exist"
                r_sGroupCode = ""
                result = gPMConstants.PMEReturnCode.PMError
            End If

            If r_sErrorMessage = "" Then
                Return gPMFunctions.NullToInteger(vStepDetails(0, 0))
            End If
            Return result
    ...
    End Function
    Public Function GetStepDetails(ByVal v_lApprovalStep As Integer, ByRef r_vStepDetails(,) As Object) As Integer
        ...
        Try
            ...
            With m_oDatabase
                ...
                m_lReturn = .Parameters.Add(sName:="SourceID", vValue:=CStr(m_iSourceID), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    result = m_lReturn
                    bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetStepDetails Failed to add Parameter", vApp:=ACApp, vClass:=ACClass, vMethod:="GetStepDetails", vErrNo:=Informations.Err().Number, vErrDesc:=Informations.Err().Description)
                    Return result
    ...
    Private Function IsUserUnique(ByRef r_bUserIsUnique As Boolean, ByVal v_lApprovalStep As Integer) As Integer
        ...
        ' START CHANGES - Changed By: AAB  - Changed On: 29-Apr-2004 13:36
        ' Changed so the Payment creator cannot approve the payment at any Stage
        ...
        End If
        'End If
        ' END CHANGES - Changed By: AAB  - Changed On: 29-Apr-2004 13:36
    ...
    Private Function CheckUserAuthorityLimit(ByRef r_bUserWithinLimits As Boolean) As Integer
        ...
        With m_oDatabase
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                ...
                Return result
            End If
            'AAB-28-Jan-2004 11:51 - Added KeepNulls = True, so NullToBoolean & NullToCurrency can work below
        ...
        If Informations.IsArray(vAuthorityLimit) Then
            ...
            Dim oCurCovt As Object
            Dim lPaymentCurrencyID As Long = 0
            Dim dPaymentLimit As Decimal = 0
            Dim dConvertPaymentAmt As Object 

            oCurCovt = New bACTCurrencyConvert.Form()
            m_lReturn = oCurCovt.Initialise(sUserName:=ToSafeString(m_sUsername), sPassword:=ToSafeString(m_sPassword), iUserID:=ToSafeInteger(m_iUserID), iSourceID:=ToSafeInteger(m_iSourceID), iLanguageID:=ToSafeInteger(m_iLanguageID),
                                            iCurrencyID:=ToSafeInteger(m_iCurrencyID), iLogLevel:=ToSafeInteger(m_iLogLevel), sCallingAppName:=ToSafeString(ACApp), vDatabase:=CType(m_oDatabase, dPMDAO.Database))

            If m_lReturn = gPMConstants.PMEReturnCode.PMFalse Then
                ' Log Error Message
                bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to get instance of bACTCurrencyConvert.Form ", vApp:=ACApp, vClass:=ACClass, vMethod:="Initialise", vErrNo:=Informations.Err().Number, vErrDesc:=Informations.Err().Description)
                Return result
            End If
            ...
            Select Case m_lPaymentType
                ...
                Case ACClaimPaymentsType
                    If gPMFunctions.NullToBoolean(vAuthorityLimit(ACHasClaimPaymentAuthority, 0)) Then
                        ...
                        lPaymentCurrencyID = gPMFunctions.NullToInteger(vAuthorityLimit(ACClaimPaymentCurrencyID, 0))
                        dPaymentLimit = gPMFunctions.NullToCurrency(vAuthorityLimit(ACClaimPaymentLimit, 0))

                        If m_lPaymentCurrencyID <> lPaymentCurrencyID Then
                            m_lReturn = oCurCovt.CurrencyToCurrencyConversion(v_lCurrencyIdFrom:=ToSafeInteger(m_lPaymentCurrencyID), v_crCurrencyAmountFrom:=ToSafeDecimal(m_cPaymentAmount),
                                                                  v_lCurrencyIdTo:=ToSafeLong(lPaymentCurrencyID), r_crCurrencyAmountTo:=dConvertPaymentAmt, v_lCompanyId:=ToSafeInteger(m_iSourceID))
                            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                                result = m_lReturn
                                bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="CheckUserAuthorityLimit Failed to run the stored proc", vApp:=ACApp, vClass:=ACClass, vMethod:="CheckUserAuthorityLimit", vErrNo:=Informations.Err().Number, vErrDesc:=Informations.Err().Description)
                                Return result
                            End If
                        End If
                        r_bUserWithinLimits = dPaymentLimit >= dConvertPaymentAmt
                    ElseIf NullToBoolean(vAuthorityLimit(kIsRecommender, 0)) = True Then
                        If NullToCurrency(vAuthorityLimit(kRecommendLimit, 0)) >=
                           m_cPaymentAmount Then
                            r_bUserWithinLimits = True
                        Else
                ...
                Case ACPaymentsType
                    ...
                    ElseIf gPMFunctions.NullToBoolean(vAuthorityLimit(kIsRecommender, 0)) Then
                        If NullToCurrency(vAuthorityLimit(kRecommendLimit, 0)) >= m_cPaymentAmount Then
                            r_bUserWithinLimits = True
                        Else
                            r_bUserWithinLimits = False
                        End If
