...
Friend NotInheritable Class Reinsurance
    ...
    Private m_cRecoveredToDate As Decimal
    ...
    Public Property RecoveredToDate() As Decimal
        Get
            Return m_cRecoveredToDate
        End Get
        Set(ByVal Value As Decimal)
            m_cRecoveredToDate = Value
        End Set
    End Property
    'End-(Arul Stephen)-(Bug Fixing-PN58889)
        ...
        Get
            ...
            If Informations.IsArray(RILines) Then
                For lCount As Integer = RILines.GetLowerBound(1) To RILines.GetUpperBound(1)
                    cPaymentValue += CDbl(RILines(MainModule.RI2007ArrangementLineEnum.DBCRIL2007ThisPayment, lCount))
                Next
            End If
    ...
    Public ReadOnly Property TotalThisReserve() As Decimal
        Get
            ...
            If Informations.IsArray(RILines) Then
                For lCount As Integer = RILines.GetLowerBound(1) To RILines.GetUpperBound(1)
                    cReserveValue += CDbl(RILines(MainModule.RI2007ArrangementLineEnum.DBCRIL2007ThisReserve, lCount))
                Next
            End If
    ...
    Public Function Round() As Integer
        ...
        Try
            ...
            For lCount As Integer = RILines.GetLowerBound(1) To RILines.GetUpperBound(1)
                Select Case RILines(MainModule.RI2007ArrangementLineEnum.DBCRIL2007Type, lCount)
                    Case "R"
                    ...
                    Case "T"
                        ...
                        lRounding = Math.Min(lRounding, lCount)
                End Select
            Next lCount
            ...
            If Math.Abs(cDiff) < Tolerance Then
                RILines(MainModule.RI2007ArrangementLineEnum.DBCRIL2007ThisReserve, lRounding) = CDbl(RILines(MainModule.RI2007ArrangementLineEnum.DBCRIL2007ThisReserve, lRounding)) + cDiff
            End If

            ' Check payment rounding
            cDiff = ThisPayment - TotalThisPayment
            If Math.Abs(cDiff) < Tolerance Then
                RILines(MainModule.RI2007ArrangementLineEnum.DBCRIL2007ThisPayment, lRounding) = CDbl(RILines(MainModule.RI2007ArrangementLineEnum.DBCRIL2007ThisPayment, lRounding)) + cDiff
            End If
