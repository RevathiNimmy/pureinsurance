...
Option Explicit On
Imports SSP.Shared
'Modified by Sudhanshu Behera on 6/24/2010 11:14:54 AM refer developer guide no. 129
...
Public NotInheritable Class Business
    ...
    '
    ' Description: Gets the Lookup values for a CLMLossSchedule
    '
    '
    ' ***************************************************************** '
    ...
    Public Function GetLossScheduleTypes(ByRef r_vResultArray(,) As Object) As Integer
        ' ***************************************************************** '
        ' Name: GetLossScheduleTypes
        '
        ' Description:  Get the undeleted peril types (inc loss schedule type id)
        ' ***************************************************************** '
        ...
        m_lReturn = m_oDatabase.SQLSelect(sSQL:=ACGetLossScheduleTypesSQL, sSQLName:=ACGetLossScheduleTypesName, bStoredProcedure:=True, vResultArray:=r_vResultArray)
        ...
        result = gPMConstants.PMEReturnCode.PMFalse
        bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetLossScheduleTypes Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetLossScheduleTypes", vErrNo:=Informations.Err().Number, vErrDesc:=Informations.Err().Description)

        Return result
    End Function

    Public Function GetItemDetails(ByVal v_lItemDetailId As Integer, ByRef r_vResults(,) As Object) As Integer
        ' ***************************************************************** '
        ' Name: GetItemDetails
        '
        ' Description:  Get Data from item detail table
        '
        ' Created By:  'PB/CMG 19092002 For Loss Schedule
        '
        ' ***************************************************************** '
        ...
        result = gPMConstants.PMEReturnCode.PMTrue
        Dim bParamError As Boolean = gPMConstants.PMEReturnCode.PMFalse
        ...
        Try
            ...
            'Add the category parameter (INPUT)
            m_lReturn = m_oDatabase.Parameters.Add(sName:="item_detail_id", vValue:=CStr(v_lItemDetailId), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                bParamError = gPMConstants.PMEReturnCode.PMTrue
            End If

            If bParamError <> gPMConstants.PMEReturnCode.PMFalse Then
                ' Log Error Message
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="oParameters.Add failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetItemDetails")

                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            'Execute SQL Statement
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=ACGetItemDetailsSQL, sSQLName:=ACGetItemDetailsName, bStoredProcedure:=True, vResultArray:=r_vResults)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                ' Log Error Message
                bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="m_oDatabase.SQLSelect failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetItemDetails")
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetItemDetails Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetItemDetails", vErrNo:=Informations.Err().Number, vErrDesc:=excep.Message, excep:=excep)
