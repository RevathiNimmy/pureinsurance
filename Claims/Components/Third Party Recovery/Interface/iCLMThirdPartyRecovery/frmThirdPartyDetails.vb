...
Imports SharedFiles
Friend Partial Class frmThirdPartyDetails
	Inherits System.Windows.Forms.Form
	
	Private Const ACClass As String = "frmThirdPartyDetails"
	...
	Private m_nFindClaimMode As gPMConstants.PMEComponentAction
	...
	Private Sub cmdOK_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cmdOK.Click
		...
		If g_lButton = ACEditButton Then
			...
			If lRecoveryTypeId <> m_lRecoveryTypeID Then
				...
				'Validation to Check Recovery Type Id is In frminterface
				m_lReturn = CheckRecoveryTypeID(lRecoveryTypeId)
				If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
					DisplayMessage(ACInvalidRecoveryType, Mid(lblRecoveryType.Name, 4))
					Exit Sub
		...
		Select Case ClaimMode
			Case gPMConstants.PMEComponentAction.PMEdit
				...
				ElseIf cboRecoveryType.Enabled Then 
					frmInterface.DataChanged = True
				End If
				'Add existing RevisedReserve to new entered value.

				m_cRevisedReserve = CDbl(gPMFunctions.UnFormatField(iFormatTypeIn:=gPMConstants.PMEFormatStyle.PMFormatCurrency, iDataTypeOut:=gPMConstants.PMEDataType.PMCurrency, vFieldValue:=txtRevisedReserve.Text)) + m_cRevisedReserve
	...
	Private Sub frmThirdPartyDetails_Load(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Load
	...
	Private Function SetInterfaceDefaults() As Integer
		...
		Try 
			...
			If Information.IsArray(g_vThirdPartyRecoveryTypes) Then
				
				'Load Details of Recovery Type in the combo


				LoadDataInCombo(cboRecoveryType, g_vThirdPartyRecoveryTypes, g_vThirdPartyRecoveryTypes.GetLowerBound(1), g_vThirdPartyRecoveryTypes.GetUpperBound(1) + 1)
	...
	Private Function DisplayCaptions() As Integer
		...
		Try 
			...
			Me.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACThirdPartyDetails, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
			...
			SSTabHelper.SetTabCaption(tabMainTab, 0, iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACTabTitleGeneral, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
			

			lblRecoveryType.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACRecoveryType, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
			
			Select Case ClaimMode
				Case gPMConstants.PMEComponentAction.PMAdd, gPMConstants.PMEComponentAction.PMEdit
					

					lblInitialReserve.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACThirdPartyInitialReserve_U, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
					

					lblRevisedReserve.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACThirdPartyRevisedReserve, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
					
					

					lblCurrentReserve.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACCurrentReserve, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
				...
				Case gPMConstants.PMEComponentAction.PMView
					...
					lblRevisedReserve.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACThirdPartyThirdPartyAmount, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
					
					
					

					lblCurrentReserve.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACNewReserve_U, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
			...
			Select Case ClaimMode
				Case gPMConstants.PMEComponentAction.PMAdd
					...
					txtRevisedReserve.BackColor = SystemColors.Control
			...
			If ClaimMode = gPMConstants.PMEComponentAction.PMEdit Then
				...
				lblRevisedReserveLoss.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACRevisedReserveLoss, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
			Else

				lblRevisedReserveLoss.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACThirdPartyRevisedReserveLoss, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
			End If
			

			lblLossCurrency.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACLossCurrency, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
			

			lblCurrency.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACReceiptCurrency, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
			

			lblCurrencyRate.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACReceiptToLossRate, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
			

			lblTaxType.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACTaxType, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
			

			lblTaxBand.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACTaxBand, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
			

			lblTaxAmount.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACTaxAmount, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
			

			lblNetPayment.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACNetAmount, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
		...
		Try 
			...
			Select Case ClaimMode
				...
				Case gPMConstants.PMEComponentAction.PMEdit
					...
					'RWH(25/06/01) Don't show current revised value here.
					'        txtRevisedReserve = FormatField( _
					''                            iFormatType:=PMFormatCurrency, _
					''                            vFieldValue:=m_cRevisedReserve)
					
					txtCurrentReserve.Text = gPMFunctions.FormatField(iFormatType:=gPMConstants.PMEFormatStyle.PMFormatCurrency, vFieldValue:=CStr(NewReserve))
	...
	Private Sub frmThirdPartyDetails_Resize(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Resize
		If isInitializingComponent Then
			Exit Sub
		End If
	...
	Private Sub frmThirdPartyDetails_Closed(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Closed
	...
	Private Sub txtRevisedReserve_Leave(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles txtRevisedReserve.Leave
		...
		Select Case ClaimMode
			Case gPMConstants.PMEComponentAction.PMEdit
				'RWH(25/06/01) Need to ADD Revised Reserve to CurrentReserve.

				cTempCurrentReserve = CDbl(gPMFunctions.UnFormatField(iFormatTypeIn:=gPMConstants.PMEFormatStyle.PMFormatCurrency, iDataTypeOut:=gPMConstants.PMEDataType.PMCurrency, vFieldValue:=txtCurrentReserve.Text)) + CDec(txtRevisedReserve.Text.Trim()) ' - m_cReceivedTodate
			...
			Case gPMConstants.PMEComponentAction.PMView
				...
				'no point in validating if we haven't change any thing
				...
				If CDbl(CDbl(gPMFunctions.UnFormatField(iFormatTypeIn:=gPMConstants.PMEFormatStyle.PMFormatCurrency, iDataTypeOut:=gPMConstants.PMEDataType.PMCurrency, vFieldValue:=txtCurrentReserve.Text)) - CDec(txtRevisedReserve.Text.Trim())) < 0 Then
					...
					sInfoMessage = CStr(iPMFunc.GetResData(g_iLanguageID, ACInvalidThirdPartyAmount, gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
					
					iReply = MessageBox.Show(sInfoMessage, "Recovery Amount", MessageBoxButtons.YesNo)
