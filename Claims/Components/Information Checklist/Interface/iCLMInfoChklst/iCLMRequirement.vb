...
Friend Partial Class frmRequirement
	Inherits System.Windows.Forms.Form
	' Constant for the functions to identify
	' which class this is.
	Private Const ACClass As String = "frmRequirement"
    ...
    Private Function DisplayCaptions() As Integer
        ...
        Try
            ...
            If Task = gPMConstants.PMEComponentAction.PMAdd Then
                ...
                Me.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACAddRequirementTitle, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
            Else

                Me.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACEditRequirementTitle, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
            End If
            ...
            lblRequirement.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACRequirement, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))


            lblReference.Text = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACReference, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
    ...
    Private Sub cmdOK_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cmdOK.Click
        ...
        If g_dtDateRecv <> "" And (g_dtTimeRecv = "" Or g_dtTimeRecv = " ") Then
        ...
        If Information.IsDate(g_dtDateReqt) And Information.IsDate(g_dtDateCrit) And IsTime(g_dtTimeReqt) And IsTime(g_dtTimeCrit) Then
            'If (CDate(g_dtDateReqt) = CDate(g_dtDateCrit)) And _
            '(CDate(g_dtTimeReqt) > CDate(g_dtTimeCrit)) Then
        ...
        Me.PrtyClmId = 0 'as nothing is being passed for
        'requirement screen
    ...
    Private Sub frmRequirement_Load(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Load
    ...
    Private Function CheckMandatory() As Boolean
        ...
        Try
            ...
            If txtRequirement.Text.Trim() = "" Then ' Requirement text box
                DisplayMessage(ACMandatoryFieldMsg, Mid(txtRequirement.Name, 4))
                Return False
            Else
            ...
            If txtDateRequested.Text.Trim() = "" Then ' Requirement text box
                DisplayMessage(ACMandatoryFieldMsg, Mid(txtDateRequested.Name, 4))
                Return False
            Else
                '   If all the Mandatory fields are having values SET the CheckMandatory = True
                result = True
            ...
            '        Call DisplayMessage(ACMandatoryFieldMsg, Mid(txtContact.Name, 4))
            ...
            '        Call DisplayMessage(ACMandatoryFieldMsg, Mid(txtDescription.Name, 4))
            '        CheckMandatory = False
            '        Exit Function
            '    Else
            '    '   If all the Mandatory fields are having values SET the CheckMandatory = True
    ...
    Private Function SetInterfaceDefaults() As Integer
        ...
        Try
            ...
            '    g_dtDateReq = Format(Date, "short date")
            ...
            Else
                ...
                'DC200201 do not set date
                'g_dtDateRecv = Format(frmRequirement.DateRecv, ACDateConversion)
                'AJM (25/07/2001) - Do not default a date but if one is entered, show it!
                If Me.DateRecv = "" Then
                    g_dtDateRecv = ""
                Else
                    g_dtDateRecv = StringsHelper.Format(Me.DateRecv, ACDateConversion)
                ...
                'DC200201 do not set time
                'g_dtTimeRecv = frmRequirement.TimeRecv
                'AJM (25/07/2001) - Do not default a time but if one has been enetered, show it!
                If Me.TimeRecv = "" Then
                    g_dtTimeRecv = ""
                Else
    ...
    Private Sub txtTimeReceived_Leave(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles txtTimeReceived.Leave
        ...
        If txtTimeReceived.Text.Trim() <> "" Then
            If IsTime(txtTimeReceived.Text) Then
                ...
                'eck 260803 PN6291
                If g_dtDateRecv = "" Then
                    g_dtDateRecv = StringsHelper.Format(DateTime.Now, ACDateConversion)
                End If
            Else
                DisplayMessage(ACInvaildTimeMsg, "Time Received")
                txtTimeReceived.Text = ""
                txtTimeReceived.Focus()
            End If
        End If

        'If Trim(txtTimeReceived.Text) <> "" Then
        '    If IsTime(txtTimeReceived.Text) Then
        '        'g_dtTimeRecv = Format(txtTimeReceived.Text, ACTimeConversion)
        '
        '        If IsDate(txtDateReceived.Text) And _
        ''           IsDate(txtDateRequested.Text) And _
        ''           IsTime(txtTimeRequested.Text) Then
        '
        '            If (CDate(txtDateReceived.Text) = _
        ''                CDate(txtDateRequested.Text)) And _
        ''               (CDate(txtTimeRequested.Text) > _
        ''                CDate(txtTimeReceived.Text)) Then
        '
        '                txtTimeReceived.SetFocus
        '
        '            Else
        '
        '                g_dtTimeRecv = Format(txtTimeReceived.Text, ACTimeConversion)
        ...
        '
        '    Else
        '         Call DisplayMessage(ACInvaildTimeMsg, "Received_Time")
        '         txtTimeReceived.Text = ""
        '         txtTimeReceived.SetFocus
        '    End If
        '
    ...
    Private Sub txtTimeRequested_Leave(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles txtTimeRequested.Leave
        If txtTimeRequested.Text.Trim() <> "" Then
            ...
            Else
                DisplayMessage(ACInvaildTimeMsg, "Time Requested")
                txtTimeRequested.Text = ""
                txtTimeRequested.Focus()
            End If
        End If

        'If Trim(txtTimeRequested.Text) <> "" Then
        '    If IsTime(txtTimeRequested.Text) Then
        '        'g_dtTimeReqt = Format(txtTimeRequested.Text, ACTimeConversion)
        '
        '        If IsDate(txtDateReceived.Text) And _
        ''           IsDate(txtDateRequested.Text) And _
        ''           IsTime(txtTimeReceived.Text) Then
        '
        '            If (CDate(txtDateReceived.Text) = _
        ''                CDate(txtDateRequested.Text)) And _
        ''               (CDate(txtTimeRequested.Text) > _
        ...
        '
        '    Else
        '         Call DisplayMessage(ACInvaildTimeMsg, "Requested Time")
        '         txtTimeRequested.Text = ""
        '         txtTimeRequested.SetFocus
        '    End If
        '
        '    If Trim(txtDateRequested.Text) = "" Then
        '        txtTimeRequested.Text = ""
        '        g_dtTimeReqt = ""
        '    End If
        'Else
