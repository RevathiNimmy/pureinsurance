...
Module bPMDocFunctions
    ...
    'Developer Guide No. TODO - Commented by arun This function are not there in previous version.
    ...
    '		Dim result As Integer = 0
    '		Dim SiriusDocumentUtility, oConvert As Object
    ...
    Public Function GetWordVersion(ByRef r_sVersion As String) As Integer
        ...
        Try
            ...
            If lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                ...
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to start Word", vApp:=ACApp, vClass:=ACClass, vMethod:="GetWordVersion", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
            ...
            If lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to close Word", vApp:=ACApp, vClass:=ACClass, vMethod:="GetWordVersion", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetWordVersion Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetWordVersion", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function StartWord(ByRef r_oWord As Word.Application, Optional ByRef r_lWordHandle As Integer = 0, Optional ByRef r_sWordVersion As String = "") As Integer
        ...
        Try
            ...
            sGUID = iPMFunc.GetGUID()
            r_oWord.Caption = sGUID
            r_lWordHandle = FindWindow(Nothing, sGUID)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error.
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to start Word", vApp:=ACApp, vClass:=ACClass, vMethod:="StartWord", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function CloseWord(ByRef r_oWord As Word.Application, ByVal lHandle As Integer, Optional ByRef bSaveChanges As Boolean = True) As Integer
        Dim result As Integer = 0
        Try
            Dim dLoopTime As Date
        ...
        Catch excep As System.Exception
            ...
            ' Log Error.
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to close Word", vApp:=ACApp, vClass:=ACClass, vMethod:="CloseWord", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function ClearDirectory(ByVal sDirectory As String) As Integer
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="ClearDirectory Failed. Directory Name [" & sDirectory & "]", vApp:=ACApp, vClass:=ACClass, vMethod:="ClearDirectory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function MoveFolderContents(ByVal sSourceDir As String, ByVal sDestDir As String) As Integer
        ...
        Try
            ...
            For Each iSubFolder As DirectoryInfo In iFolder.GetDirectories
                ...
                'If detination is relative path then first copy and then delete source
                'If Left$(sDestDir, 2) = "\\" Then
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="MoveFolderContents Failed." & Strings.Chr(13) & Strings.Chr(10) & "Source Directory [" & sSourceDir & "]" & Strings.Chr(13) & Strings.Chr(10) & "Target Directory [" & sDestDir & "]", vApp:=ACApp, vClass:=ACClass, vMethod:="MoveFolderContents", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function CreateFolderTree(ByVal sFolderName As String, Optional ByVal ClearDirectoryIfExists As Boolean = False) As Integer
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="CreateFolderTree Failed attempting to create " & sFolderName, vApp:=ACApp, vClass:=ACClass, vMethod:="CreateFolderTree", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetDocumentDirectory(ByRef r_sDocDirectory As String) As Integer
        ...
        Try
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to get Document directory from Registry.", vApp:=ACApp, vClass:=ACClass, vMethod:="GetDocumentDirectory", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetDocumentDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetDocumentDirectory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetClientDirectory(ByRef r_sClientDir As String, Optional ByVal bUnique As Boolean = False) As Integer
        ...
        Try
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to get Client directory from Registry.", vApp:=ACApp, vClass:=ACClass, vMethod:="GetClientDirectory", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
            ...
            If bUnique Then
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to get a temp directory.", vApp:=ACApp, vClass:=ACClass, vMethod:="GetClient", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
            ...
            If Not Directory.Exists(sDir) Then
                ...
                ' did we succeed...?
                If Not Directory.Exists(sDir) Then
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetClientDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetClientDirectory", vErrNo:=0, vErrDesc:="Unable to create the directory (" & sDir & _
                                       ")")
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetClientDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetClientDirectory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetUniqueName(ByRef r_sName As String) As Integer
        Dim result As Integer = 0
        Try
            Dim oFSO As Scripting.FileSystemObject 'As Object
            Dim sTemp As String = ""

            result = gPMConstants.PMEReturnCode.PMTrue

            oFSO = New Scripting.FileSystemObject 'New Object()
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetUniqueName Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetUniqueName", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetExportDirectory(ByRef r_sExportDirectory As String, Optional ByVal bUnique As Boolean = False) As Integer
        ...
        Try
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Or sDir = "" Then
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetExportDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetExportDirectory", vErrNo:=Information.Err().Number, vErrDesc:="Unable to find the registry entry for the PrintFile directory location")
            ...
            If bUnique Then
                m_lReturn = CType(GetUniqueName(r_sName:=sTemp), gPMConstants.PMEReturnCode)
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to get a temp directory.", vApp:=ACApp, vClass:=ACClass, vMethod:="GetExportDirectory", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
            ...
            If Not Directory.Exists(sDir) Then
                ...
                ' did we succeed...?
                If Not Directory.Exists(sDir) Then
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetExportDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetExportDirectory", vErrNo:=0, vErrDesc:="Unable to create the directory (" & sDir & _
                                       ")")
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetExportDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetExportDirectory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function GetZipDirectory(ByRef r_sZipDirectory As String) As Integer
        ...
        Try
            ...
            'Get the DocZipTemp dir
            m_lReturn = CType(gPMFunctions.GetPMRegSetting(v_lPMERegSettingRoot:=gPMConstants.PMERegSettingRoot.pmeRSRLocalMachine, v_lPMEProductFamily:=gPMConstants.PMEProductFamily.pmePFSiriusSolutions, v_lPMERegSettingLevel:=gPMConstants.PMERegSettingLevel.pmeRSLClient, v_sSettingName:="DocZipPMDir", r_sSettingValue:=sTemp), gPMConstants.PMEReturnCode)

            'Make sure we have an install path
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Or sTemp = "" Then
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetZipDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetZipDirectory", vErrNo:=Information.Err().Number, vErrDesc:="Unable to find the registry entry for the doczip directory location")
            ...
            If Not Directory.Exists(sZipDir) Then
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetZipDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetZipDirectory", vErrNo:=0, vErrDesc:="Unable to create the directory (" & sZipDir & _
                                   ")")
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetZipDirectory Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="GetZipDirectory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function UnZip(ByVal sZipFile As String, ByVal sOutputDirectory As String, Optional ByVal v_bDeleteZipFile As Boolean = False) As Integer
        ...
        Try
            ...
            If Not File.Exists(sZipFile) Then
                result = gPMConstants.PMEReturnCode.PMError
                ' Log Error.
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="File (" & sZipFile & ") does not exist", vApp:=ACApp, vClass:=ACClass, vMethod:="UnZip", vErrNo:=Information.Err().Number, vErrDesc:="Unzip failed")
            ...
            If Not Directory.Exists(sOutputDirectory) Then
                result = gPMConstants.PMEReturnCode.PMError
                ' Log Error.
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Directory (" & sOutputDirectory & ") does not exist", vApp:=ACApp, vClass:=ACClass, vMethod:="UnZip", vErrNo:=Information.Err().Number, vErrDesc:="Unzip failed")
            ...
            If sOutputDirectory.ToLower() <> sZipFileFolderName Then
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    result = m_lReturn
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to copy file." & Strings.Chr(13) & Strings.Chr(10) & _
            ...
            If oZipper Is Nothing Then
                result = gPMConstants.PMEReturnCode.PMError
                ' Log Error.
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to load zipper object", vApp:=ACApp, vClass:=ACClass, vMethod:="UnZip", vErrNo:=Information.Err().Number, vErrDesc:="Unzip failed")
            ...
            If bIsHTML Then
                ...
                If Not Directory.Exists(sDependencyDir) Then
                    ...
                    If Not Directory.Exists(sDependencyDir) Then
                        result = gPMConstants.PMEReturnCode.PMError
                        ' Log Error.
                        iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to create directory (" & sDependencyDir & ")", vApp:=ACApp, vClass:=ACClass, vMethod:="UnZip", vErrNo:=Information.Err().Number, vErrDesc:="Unzip failed")
    ...
    Public Function GetFileNameAndType(ByVal v_sFolder As String, ByRef r_sFileName As String, ByRef r_bHTMLFormat As Boolean) As Integer
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetFileNameAndType Failed. File Name : " & v_sFolder, vApp:=ACApp, vClass:=ACClass, vMethod:="GetFileNameAndType", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function DelDirectory(ByRef r_sDocDirectory As String) As Integer
        ...
        Try
            ...
            If Directory.Exists(r_sDocDirectory) Then
                ...
                Directory.Delete(r_sDocDirectory)
                '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="DelDirectory Failed. Directory Name : " & r_sDocDirectory, vApp:=ACApp, vClass:=ACClass, vMethod:="DelDirectory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function IsFolderExists(ByVal v_sFolderName As String) As Integer
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to Check FolderExists for '" & v_sFolderName & "'", vApp:=ACApp, vClass:=ACClass, vMethod:="IsFolderExists", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function IsFileExists(ByVal v_sFileName As String) As Integer
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to Check FileExists for '" & v_sFileName & "'", vApp:=ACApp, vClass:=ACClass, vMethod:="IsFileExists", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function CreateFolder(ByVal v_sFolderName As String) As Integer
        ...
        Try
            ...
            If Directory.Exists(v_sFolderName) Then
                ...
                result = gPMConstants.PMEReturnCode.PMTrue
            Else
                'Developer Guide No.172
                If CBool(Directory.CreateDirectory(v_sFolderName).Exists) Then
                    ' did we succeed...?
                    If Directory.Exists(v_sFolderName) Then
                        result = gPMConstants.PMEReturnCode.PMTrue
                    Else
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="CreateFolder Unable to create the directory (" & v_sFolderName & ")", vApp:=ACApp, vClass:=ACClass, vMethod:="CreateFolder", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function DeleteFile(ByVal v_sFileName As String) As Integer
        ...
        iRetries = gPMConstants.PMFileOperationRetryAttempts
        Do Until iRetries = 0
            ...
            lStoreErrNum = Information.Err().Number
            sStoreErrDesc = Information.Err().Description
            ...
            If File.Exists(v_sFileName) Then
                If iRetries = 0 Then
                    ' Delete file fails
                    ' Log Error Message
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Unable to Delete file '" & v_sFileName & "'", vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteFile", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Else
                    gPMFunctions.LogMessageToFile(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to delete " & v_sFileName & ". Retries remaining: " & CStr(iRetries), vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteFile", excep:= New Exception(sStoreErrDesc))
                    dStoreDateTime = DateTime.Now
                    Do Until Math.Abs(DateAndTime.DateDiff("s", dStoreDateTime, DateTime.Now, FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1)) >= gPMConstants.PMFileOperationRetryWait
                        Application.DoEvents()
                    Loop
...
Err_DeleteFile:
        ...
        ' Log Error Message
        iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to Delete file '" & v_sFileName & "'", vApp:=ACApp, vClass:=ACClass, vMethod:="DeleteFile", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
    ...
    Public Function CopyFile(ByVal v_sSourceFile As String, ByVal v_sDestinationFile As String, Optional ByVal v_bOverWriteFiles As Boolean = True, Optional ByVal v_bDeleteSoureFile As Boolean = False, Optional ByRef r_vErrorMessage As String = "") As Integer
        ...
        Dim bCanCopy, bFileCopiped As Boolean
        ...
        Try
            ...
            v_sSourceFile = v_sSourceFile.Trim()
            ...
            If v_sSourceFile.Length = 0 Then
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Source file name is null", vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFile", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return gPMConstants.PMEReturnCode.PMError
            End If
            ...
            If File.Exists(v_sSourceFile) Then
                ...
                If Not Directory.Exists(sDestinationFolderName) Then
                    ...
                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                        ...
                        ' Log Error Message
                        iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Unable to create the Destination Folder (" & sDestinationFolderName & ")", vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFile", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                        Return result
                    End If
                End If
                ...
                If CBool(oFolder.Attributes) And 1 Then
                    sErrorMessage = sErrorMessage & "Read-Only" & Strings.Chr(13) & Strings.Chr(10)
                    bCanCopy = False
                End If

                If CBool(oFolder.Attributes) And 4 Then
                ...
                If CBool(oFolder.Attributes) And 64 Then
                    sErrorMessage = sErrorMessage & "Link or shortcut" & Strings.Chr(13) & Strings.Chr(10)
                    bCanCopy = False
                End If

                'Below lines are commented as Advised by Danny's mail(19/11/2009)
                ...
                Else
                    ...
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Unable to Copy the File (" & v_sSourceFile & ") because" & Strings.Chr(13) & Strings.Chr(10) & _
            ...
            Else
                ' Source file Missing
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Source File Missing (" & v_sSourceFile & ")", vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFile", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return gPMConstants.PMEReturnCode.PMError
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to copy " & v_sSourceFile & " to " & v_sDestinationFile, vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFile", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function CopyFolder(ByVal v_sSourceFolder As String, ByVal v_sDestinationFolder As String, Optional ByVal v_bOverWriteFiles As Boolean = True, Optional ByRef r_vErrorMessage As String = "") As Integer
        ...
        Try
            ...
            If v_sSourceFolder.Length = 0 Then
                ...
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Source Folder name is null", vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFolder", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return gPMConstants.PMEReturnCode.PMError
            End If
            ...
            If v_sDestinationFolder.Length = 0 Then
                ...
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Destination Folder name is null", vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFolder", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return gPMConstants.PMEReturnCode.PMError
            End If
            ...
            Else
                ...
                ' Log Error Message
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Missing Source Folder [" & v_sSourceFolder & "]", vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFolder", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                Return gPMConstants.PMEReturnCode.PMError
            End If
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to Copy Directory" & Strings.Chr(13) & Strings.Chr(10) & "From : [" & v_sSourceFolder & "]" & Strings.Chr(13) & Strings.Chr(10) & "To   : [" & v_sDestinationFolder & "]", vApp:=ACApp, vClass:=ACClass, vMethod:="CopyFolder", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function NoOfFilesInDirectory(ByVal v_sDirectoryName As String, Optional ByRef r_vFirstFileName As String = "", Optional ByVal v_sFileExtension As String = "") As Integer
        ...
        Try
            ...
            Else
                ...
                v_sFileExtension = v_sFileExtension.Trim()
                If v_sFileExtension.Length = 0 Then
                    result = -1
                    ' Log Error Message
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="File Extension is Null.", vApp:=ACApp, vClass:=ACClass, vMethod:="NoOfFilesInDirectory", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                    Return result
                End If
            End If
            ...
            If Directory.Exists(v_sDirectoryName) Then
                ...
                If Information.IsNothing(v_sFileExtension) Then
                    lCount = oFilesCollection.Count ' Return the number of files
                End If
            End If
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogError, sMsg:="Failed to Get No Of Files In Directory (" & v_sDirectoryName & ")", vApp:=ACApp, vClass:=ACClass, vMethod:="NoOfFilesInDirectory", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Public Function Zip(ByVal sPath As String, ByVal sInputDocumentFileExtension As String) As Integer
        ...
        Try
            ...
            If oZipper Is Nothing Then
                result = gPMConstants.PMEReturnCode.PMError
                ' Log Error.
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to load zipper object", vApp:=ACApp, vClass:=ACClass, vMethod:="Zip", vErrNo:=Information.Err().Number, vErrDesc:="Zip failed")
            ...
            If Directory.Exists(sDependencyDir) Then
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    ...
                    result = gPMConstants.PMEReturnCode.PMError
                    ' Log Error.
                    iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to delete the dependency folder. (" & sDependencyDir & "')", vApp:=ACApp, vClass:=ACClass, vMethod:="Zip", vErrNo:=Information.Err().Number, vErrDesc:="Zip failed")
                End If
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                ...
                result = gPMConstants.PMEReturnCode.PMError
                ' Log Error.
                iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to delete the file. (" & sFileIn & "')", vApp:=ACApp, vClass:=ACClass, vMethod:="Zip", vErrNo:=Information.Err().Number, vErrDesc:="Zip failed")
            End If
        ...
        Catch excep As System.Exception
            ...
            ' Log Error.
            iPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to zip the document. (" & sFileIn & ")", vApp:=ACApp, vClass:=ACClass, vMethod:="Zip", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
