...
Friend NotInheritable Class General
    ...
    'm_frmInterface should be an object of frmInterface
    ...
    Public Function GetInterfaceDetails() As Integer
        ...
        Try
            ...
            End If
            Dim vTempRelations As Object
            If m_frmInterface.Task = gPMConstants.PMEComponentAction.PMAdd Then
                'Even if we are adding a new agent we still need the relations
                m_lReturn = m_oBusiness.GetRelationshipTypeLookups(vRelationships:=vTempRelations)
                m_frmInterface.Relationships = vTempRelations
                ' Check for errors
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    ' Failed to assign the details.
                    Return gPMConstants.PMEReturnCode.PMFalse
            ...
            Select Case (m_frmInterface.Task)
                Case gPMConstants.PMEComponentAction.PMAdd
                    ...
                    Else
                        ...
                        End If
                        'SD 14/08/2002 Document List can only be stored after we
                        ' have a party count
                        m_lReturn = m_frmInterface.StoreSuppressedDocumentList(m_oBusiness.PartyCnt)
                        ' Check for errors.
                        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
    ...
    Private Function DisableForm(ByRef lDisabled As Integer) As Integer
        ...
        result = gPMConstants.PMEReturnCode.PMTrue
        If lDisabled = -1 Then
            If m_frmInterface.IsViewableOnly Then
                m_frmInterface.EnableForm(frmInterface.EnabledLevel.elDisabledTotal)
            Else
                m_frmInterface.EnableForm(frmInterface.EnabledLevel.elDisabledStandard)
            End If
        Else
            m_frmInterface.EnableForm(frmInterface.EnabledLevel.elEnabled)
        End If
        Return result
    End Function
    'PRIVATE Methods (End)
