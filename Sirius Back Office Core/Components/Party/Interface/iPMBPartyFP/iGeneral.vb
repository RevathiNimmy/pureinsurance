...
Friend NotInheritable Class General
    ...
    ' RAW 15/04/2003 : PS181 : disable controls in view mode
    ' ***************************************************************** '
            ...
            Select Case (m_frmInterface.Task)
                ...
                Case gPMConstants.PMEComponentAction.PMEdit
                    ...
                    If m_frmInterface.Status = gPMConstants.PMEReturnCode.PMCancel Then
                        ...
                        'Developer Guide No 243
                        sMessage = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACCancelDetailsTitle, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))
    ...
    Private Function DisableForm(ByRef lDisabled As Integer) As Integer
        ...
        For Each ctlFormControl As Control In ContainerHelper.Controls(m_frmInterface)
            ...
            ElseIf (TypeOf ctlFormControl Is RadioButton) Then
                ControlHelper.SetEnabled(ctlFormControl, Not lDisabled)
            ElseIf (TypeOf ctlFormControl Is ListView) Then
                ControlHelper.SetEnabled(ctlFormControl, Not lDisabled)
                'TODO: Control cannot be of ToolStripMenuItem
                'ElseIf (TypeOf ctlFormControl Is ToolStripMenuItem) Then 
                '	ControlHelper.SetEnabled(ctlFormControl, Not lDisabled)
            ElseIf (TypeOf ctlFormControl Is ToolStrip) Then
                DisableToolBar(v_oToolBar:=ctlFormControl, v_lDisabled:=lDisabled)
                ' RAW 15/04/2003 : PS181 : end
            End If

            ' RAW 15/04/2003 : PS181 : added
            Select Case (ctlFormControl.Name)
                Case "cmdAddAd", "cmdDeleteAd", "cmdEditAd", "cmdAddCon", "cmdDeleteCon", "cmdEditCon"
                    ControlHelper.SetEnabled(ctlFormControl, Not lDisabled)
                Case "uctPartyTax1"

                    'TODO Changes done as per VB functionality
                    'ctlFormControl.ReadOnly = lDisabled
                    DirectCast(ctlFormControl, uctPartyTaxControl.uctPartyTax).ReadOnly_Renamed = lDisabled
    ...
    '
    ' ***************************************************************** '
    Private Function DisableToolBar(ByVal v_oToolBar As ToolStrip, ByVal v_lDisabled As Integer) As Integer
        ...
        ' Set all of the toolbar controls to the disable state.
        For Each ctlButton As ToolStripButton In v_oToolBar.Items
            ctlButton.Enabled = Not v_lDisabled
        Next ctlButton
