...
Public NotInheritable Class Business
    ...
    '
    ' Description: Select a single/multiple records from the database.
    '
    '
    ' ***************************************************************** '
    Public Function GetDetails(ByRef r_vResultArray(,) As Object, Optional ByVal v_sAgentCode As String = "", Optional ByVal v_lTransactionTypeID As Integer = 0) As Integer
        ...
        Try
            ...
            ' Add the agentcode INPUT parameter

            'developer guide no. 85(Latest Guide)
            m_lReturn = m_oDatabase.Parameters.Add(sName:="agentcode", vValue:=IIf(v_sAgentCode <> "", v_sAgentCode, DBNull.Value), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMString)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            ' Add the transtypeid INPUT parameter

            'developer guide no. 85(Latest Guide)
            m_lReturn = m_oDatabase.Parameters.Add(sName:="transtypeid", vValue:=IIf(v_lTransactionTypeID <> 0, CStr(v_lTransactionTypeID), DBNull.Value), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)
            ...
            m_lReturn = m_oDatabase.SQLSelect(sSQL:=ACGetTranExportDetailsSQL, sSQLName:=ACGetTranExportDetailsName, bStoredProcedure:=True, vResultArray:=r_vResultArray)
    ...
    ' Name: Update (Public)
    '
    ' Description: updates Transaction_Export_Folder.accounts_export_status with "m" or "p"
    '
    '
    ' ***************************************************************** '
    Public Function Update(ByVal v_vTransArray As Object) As Integer
        Dim result As Integer = 0
        ...
        Try
            ...
            lRowCount = v_vTransArray.GetUpperBound(1)
            ...
            For lRow As Integer = 0 To lRowCount
                ...
                m_lReturn = m_oDatabase.Parameters.Add(sName:="exportfoldercnt", vValue:=CStr(CInt(v_vTransArray(ACCnt, lRow))), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)
                ...
                m_lReturn = m_oDatabase.Parameters.Add(sName:="statuscode", vValue:=CStr(v_vTransArray(ACExportStatus, lRow)), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMString)
                ...
                ' Execute SQL Statement
                m_lReturn = m_oDatabase.SQLAction(sSQL:=ACUpdateAccountsExportStatusSQL, sSQLName:=ACUpdateAccountsExportStatusName, bStoredProcedure:=True)
