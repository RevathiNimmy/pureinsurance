    ...
    Public Function ProcessCommand() As Integer
        ...
        Try
            ...
            Else
                ...
                If ReflectionHelper.GetMember(m_frmInterface, "DeleteMode") Then

                    ' CJB200802 Start - show confirmation when deleting
                    ' (Neighbourhood Bug Ref 438)


                    sTitle = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACRemoveClientTitle, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))


                    sMessage = CStr(iPMFunc.GetResData(iLangID:=g_iLanguageID, lId:=ACRemoveClient, iDataType:=gPMConstants.PMEResourseFileDataType.PMResString, bResFile:=My.Resources.ResourceManager))

                    iMsgResult = MessageBox.Show(sMessage, sTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2)

                    ' Check message result.
                    If iMsgResult = System.Windows.Forms.DialogResult.No Then
                        ' Don't continue with the delete
                        Return gPMConstants.PMEReturnCode.PMFalse
                    End If
                    ' CJB200802 End
                    ...
                    m_lReturn = ReflectionHelper.GetMember(m_frmInterface, "DeleteParty")
                ...
                ' do a check here...
                ' TF081298 - removed NavigatorV3
                '        If Len(m_frmInterface.LongName) >= 0 Then ' Or Len(m_frmInterface.InsReference) > 0 Then
