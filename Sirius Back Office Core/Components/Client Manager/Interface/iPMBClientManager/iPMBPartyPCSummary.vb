...
Partial Friend Class frmPartyPCView
    ...
    ' which class this is.
    Private Const ACClass As String = "frmPartyPCView"
    ...
    Public Function LoadInterface() As gPMConstants.PMEReturnCode
        ...
        Try
            ...
            If m_bFilterOutOtherBranches And g_iSourceID <> uctPartyPCControl1.PartySourceID Then
            ...
            If uctPartyPCControl1.IDReference.Trim() = String.Empty Then
                m_sShortName = objCM.m_sShortName
            Else
                m_sShortName = uctPartyPCControl1.IDReference.Trim()
            End If
    ...
    Private Sub cmdEdit_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cmdEdit.Click
        ...
        m_lReturn = uctPartyPCControl1.EditClick()

        ' Check the return value.
        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
            Exit Sub
        End If
        objCM.m_frmParentMdiForm = Me.m_parentMdiForm
    ...
    Private Sub cmdHelp_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles cmdHelp.Click
        ...
        Try
            ...
            'JSB 28/06/01
            '    m_lReturn& = uctPartySummControl1.ShowHelpScreen
    ...
    Private Sub frmPartyPCView_Activated(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Activated
    ...
    Private Sub frmPartyPCView_Load(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            ' PW220502 - Hide the quote menu option if this is
            '            an underwriting system


            mnuGII.Available = False
    ...
    Private Sub frmPartyPCView_Paint(ByVal eventSender As Object, ByVal eventArgs As PaintEventArgs) Handles MyBase.Paint
    ...
    Private Sub frmPartyPCView_FormClosing(ByVal eventSender As Object, ByVal eventArgs As FormClosingEventArgs) Handles MyBase.FormClosing
        ...
        Try
            ...
            If Not m_bEditing Then
                ...
                'start'
                If m_parentMdiForm.StatusBar1.Items.Count > 0 Then  'end'
    ...
    Private Sub frmPartyPCView_Resize(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Resize
        ...
        If Me.WindowState <> FormWindowState.Minimized Then
            'eck 2000 Roadmap
            Me.Height = 530 'VB6.TwipsToPixelsY(7830)
            Me.Width = 741 ' VB6.TwipsToPixelsX(10995)
        End If

    End Sub

    Private Sub frmPartyPCView_Closed(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Closed
    ...
    Public Sub mnuGIICV_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles mnuGIICV.Click
        'JSB 18/06/01
        'Set flag so we know to add a GII policy if the user select a GII process type
        objCM.g_bCallGII = True
        ' Call Toolbar Control function

        If objCM.ShowPolicyDetail(v_lPartyCnt:=m_lPartyCnt, v_sPartyType:=m_sPartyType, v_lInsuranceFolderCnt:=0, v_lInsFileCnt:=0, v_lInsuranceFileStructureId:=0, v_sShortName:=m_sShortName, v_sInsReference:="", v_bFromEvent:=False, v_lPolicyTypeId:=PMBConst.PMBPolicyTypeGIICommercialVehicle, v_vGeminiPolicyStatus:=DBNull.Value) <> gPMConstants.PMEReturnCode.PMTrue Then
            'inform user that failed to add policy
            MessageBox.Show("Failed to added Gemini II Household policy", "Client Manager", MessageBoxButtons.OK, MessageBoxIcon.Information)
            'Continue as not serious
            Exit Sub
        End If
    ...
    Public Sub mnuGIIHousehold_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles mnuGIIHousehold.Click
        'JSB 18/06/01
        'Set flag so we know to add a GII policy if the user select a GII process type
        objCM.g_bCallGII = True
        ' Call Toolbar Control function

        If objCM.ShowPolicyDetail(v_lPartyCnt:=m_lPartyCnt, v_sPartyType:=m_sPartyType, v_lInsuranceFolderCnt:=0, v_lInsFileCnt:=0, v_lInsuranceFileStructureId:=0, v_sShortName:=m_sShortName, v_sInsReference:="", v_bFromEvent:=False, v_lPolicyTypeId:=PMBConst.PMBPolicyTypeGIIHousehold, v_vGeminiPolicyStatus:=DBNull.Value) <> gPMConstants.PMEReturnCode.PMTrue Then
            'inform user that failed to add policy
            MessageBox.Show("Failed to added Gemini II Household policy", "Client Manager", MessageBoxButtons.OK, MessageBoxIcon.Information)
            'Continue as not serious
            Exit Sub
    ...
    Public Sub mnuGIIMotor_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles mnuGIIMotor.Click
        'JSB 18/06/01
        'Set flag so we know to add a GII policy if the user select a GII process type
        objCM.g_bCallGII = True
        ' Call Toolbar Control function

        If objCM.ShowPolicyDetail(v_lPartyCnt:=m_lPartyCnt, v_sPartyType:=m_sPartyType, v_lInsuranceFolderCnt:=0, v_lInsFileCnt:=0, v_lInsuranceFileStructureId:=0, v_sShortName:=m_sShortName, v_sInsReference:="", v_bFromEvent:=False, v_lPolicyTypeId:=PMBConst.PMBPolicyTypeGIIMotor, v_vGeminiPolicyStatus:=DBNull.Value) <> gPMConstants.PMEReturnCode.PMTrue Then
            'inform user that failed to add policy
            MessageBox.Show("Failed to added Gemini II motor policy", "Client Manager", MessageBoxButtons.OK, MessageBoxIcon.Information)
            'Continue as not serious
            Exit Sub
        End If
    ...
    Public Sub mnuGotoBankGuarantee_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles mnuGotoBankGuarantee.Click
        m_lReturn = objCM.ShowBankGuarantee(lPartyCnt:=m_lPartyCnt)
        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
            gPMFunctions.RaiseError("mnuGotoBankGuarantee", "ShowBankGuarantee Failed", gPMConstants.PMELogLevel.PMLogError)
        End If
    End Sub
    ...
    ' Start - (Sankar) - (Tech Spec - QBENZCR001 - Client Manager view Account Transactions.doc) - (5.1.9)
    Public Sub mnuGotoInsuredAccounts_Click(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles mnuGotoInsuredAccounts.Click
        ...
        End If
    End Sub
    ' End - (Sankar) - (Tech Spec - QBENZCR001 - Client Manager view Account Transactions.doc) - (5.1.9)
    ...
    Private Sub mnuWindow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuWindow.Click

    End Sub

    Private Sub frmPartyPCView_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
