...
Module NumberToWord
    ...
    Public Function NumToWord(ByVal v_cValue As Decimal, Optional ByVal v_bConvertDecimal As Boolean = True, Optional ByVal v_sIntegerPartCode As String = "Pounds", Optional ByVal v_sDecimalPartcode As String = "Pence") As String
        ...
        Try
            ...
            sNText = (round_currency(v_cValue, 2, True)).ToString().Trim 'round up decimal
            lDecPlace = (sNText.Trim().IndexOf("."c) + 1) 'Position of decimal

            sIntegerPart = sNText.Substring(0, If(lDecPlace = 0, sNText.Length, lDecPlace - 1)).Trim()
            lIntegerPartLen = sIntegerPart.Length
            sDecimalPart = sNText.Substring(sNText.Length - (If(lDecPlace = 0, 0, CInt(System.Math.Abs(lDecPlace - sNText.Length))))).Trim()

            If String.IsNullOrEmpty(sDecimalPart.Trim()) Then
                sDecimalPart = "0"
            ElseIf sDecimalPart.Length = 1 Then
            ...
            Do While lCurrentIntegerPartLen > 0
                NumParts(lCount) = If(lCurrentIntegerPartLen > 3, Right(sIntegerPart, 3), sIntegerPart.Trim())
                sIntegerPart = If(lCurrentIntegerPartLen > 3, informations.Left(sIntegerPart, (If(lCurrentIntegerPartLen < 3, 3, lCurrentIntegerPartLen)) - 3), "")
                lCurrentIntegerPartLen = sIntegerPart.Length
                lCount += 1
            Loop
            ...
            If lIntegerPartLen > 0 Then
                'sNTW = New StringBuilder(sNTW.ToString().Trim() & " " & v_sIntegerPartCode.Trim() & (If(v_bConvertDecimal, " and ", "")))
                sNTW = New StringBuilder(sNTW.ToString().Trim() & " " & CStr(v_sIntegerPartCode).Trim() & (If(v_bConvertDecimal, " and ", "")).ToString())
            Else
                'sNTW = New StringBuilder(sNTW.ToString().Trim() & " Zero " & v_sIntegerPartCode.Trim() & (If(v_bConvertDecimal, " and ", "")))
                sNTW = New StringBuilder(sNTW.ToString().Trim() & " Zero " & v_sIntegerPartCode.Trim() & (If(v_bConvertDecimal, " and ", "")).ToString())
            End If
    ...
    Private Function GetTens(ByVal v_stenstext As String) As String
        ...
        If Val(v_stenstext.Substring(0, 1)) = 1 Then ' If value between 10-19
            Select Case Val(v_stenstext)
                Case 10 : sGT = "Ten" '
                Case 11 : sGT = "Eleven"
                Case 12 : sGT = "Twelve"
        ...
        Else
            ...
            Select Case Val(v_stenstext.Substring(0, 1))
                Case 2 : sGT = "Twenty "
                Case 3 : sGT = "Thirty "
                Case 4 : sGT = "Forty "
                Case 5 : sGT = "Fifty "
    ...
    Private Function round_currency(ByVal v_cValue As Decimal, ByVal v_lNumberOfDigit As Integer, Optional ByVal v_bBankerRounding As Boolean = True) As Decimal
        ...
        Dim lSign As Integer
            ...
            'get the sign -1 or 1
            lSign = System.Math.Sign(v_cValue)

            'make sure its positive
            v_cValue = System.Math.Abs(v_cValue)
            ...
            If v_bBankerRounding Then
                If System.Math.Floor(v_cValue) = v_cValue Then
            ...
            Return lSign * System.Math.Floor(v_cValue) / nPower
