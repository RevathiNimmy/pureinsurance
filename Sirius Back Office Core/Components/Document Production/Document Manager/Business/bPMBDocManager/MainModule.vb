...
Module MainModule
    ...
    Public Const ACApp As String = "bPMBDocManager"
    ...
    <ThreadStatic()> Public g_sDocPreBodyFragment As String
    <ThreadStatic()> Public g_sDocEndBodyFragment As String

    Sub Main_Renamed()
    End Sub
    Public Function BreakStringIntoArray(ByVal v_sStartTag As String,
                                        ByVal v_sEndTag As String,
                                        ByRef r_sString As String, ByRef r_vArray() As Object,
        ...
        sSTR = r_sString.Split(New String() {v_sStartTag}, StringSplitOptions.None)

        For lCnt = 0 To sSTR.GetUpperBound(0)
            ...
            If lCnt = 0 Then
                ...
                End If
            Else
                lPos = sSTR(lCnt).IndexOf(v_sEndTag, 1) + 1

                If lPos > 0 Then
                    sTmpLine = Informations.Left$(sSTR(lCnt), lPos - 1 + Len(v_sEndTag))
    ...
    Public Function SetArrayIndexWithPreserve(ByRef r_vArray As Object) As Integer
        ...
        If informations.IsArray(r_vArray) Then
            ReDim Preserve r_vArray(r_vArray.GetUpperBound(0) + 1)
        Else
             If r_vArray Is Nothing Then
                r_vArray = New Object() {}
            End If
            ReDim r_vArray(0)
        End If
    ...
    Public Function SetValueToArray(ByRef r_vArray As Object, ByVal v_vValue As Object, _
        ...
        If v_bForamtString Then
            ...
            r_vArray(r_vArray.GetUpperBound(0)) = v_vValue
        Else
            r_vArray(r_vArray.GetUpperBound(0)) = v_vValue
        End If
    ...
    Public Function RemoveInvalidCharacters(ByRef r_sXML As String) As Integer
        ...
        If Len(r_sXML) > 0 Then
            'r_sXML = Replace(r_sXML, "£", ChrW(194) & "£", , , vbTextCompare)
            'Saj240224
            r_sXML = r_sXML.Replace(Chr(145), Chr(39)) ', , , vbTextCompare)
            r_sXML = r_sXML.Replace(Chr(146), Chr(39)) ', , , vbTextCompare)
            r_sXML = r_sXML.Replace(Chr(147), Chr(34)) ', , , vbTextCompare)
            r_sXML = r_sXML.Replace(Chr(148), Chr(34)) ', , , vbTextCompare)
            r_sXML = r_sXML.Replace(Chr(150), Chr(45)) ', , , vbTextCompare)
            r_sXML = r_sXML.Replace(Chr(151), Chr(45)) ', , , vbTextCompare)
            r_sXML = r_sXML.Replace(Chr(160), " ") ', , , vbTextCompare)
            r_sXML = r_sXML.Replace("&", "&amp;") ', , , vbTextCompare)
            r_sXML = r_sXML.Replace("&amp;lt;", "&lt;") ', , , vbTextCompare)
            r_sXML = r_sXML.Replace("&amp;gt;", "&gt;") ', , , vbTextCompare)
            r_sXML = r_sXML.Replace("&amp;amp;", "&amp;") ', , , vbTextCompare)
            r_sXML = r_sXML.Replace("ÂÂ£", "Â£") ', , , vbTextCompare)
            'r_sXML = Replace(r_sXML, ChrW(-4056), "(", , , vbTextCompare)
            'r_sXML = Replace(r_sXML, ChrW(-3913), "(", , , vbTextCompare)
            r_sXML = r_sXML.Replace(Chr(133), "...") ', , , vbTextCompare)
            r_sXML = r_sXML.Replace(Chr(183), "") ', , , vbTextCompare)
            r_sXML = r_sXML.Replace(Chr(148), Chr(34))
            r_sXML = r_sXML.Replace(Chr(147), Chr(34))
            r_sXML = r_sXML.Replace("#amp#", "&amp;") ',, , , vbTextCompare)
        End If
