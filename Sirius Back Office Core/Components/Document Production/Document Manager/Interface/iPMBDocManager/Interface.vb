...
Public NotInheritable Class Interface_Renamed
    ...
    Private m_iSplitDocFileNo As Integer
    ...
    Private m_oObjectManager As bObjectManager.ObjectManager 'jmf 29/8/2003 bObjectManager on server and client
    ...
    Dim m_bSectionBreakExist As Boolean = False
    Dim m_iSectionBreakIfCnt As Integer = 0
    ...
    Public Property RiskArray() As Object
        Get
            Return VB6.CopyArray(m_vRiskArray)
        End Get
        Set(ByVal Value As Object)
            m_vRiskArray = Value
        End Set
    ...
    Public Property MailshotArray() As Object
        Get
            Return VB6.CopyArray(m_vMailshotArray)
        End Get
        Set(ByVal Value As Object)
    ...
    Public Property PolicySharesArray() As Object
        Get
            Return VB6.CopyArray(m_vPolicySharesArray)
        End Get
        Set(ByVal Value As Object)
    ...
    Public Function Initialise() As Integer
        ...
        Try
            ...
            m_sUsername = m_oObjectManager.UserName.Trim()
    ...
    Public Function InitialiseBusiness(ByRef sUsername As String, ByRef sPassword As String, ByRef iUserID As Integer, ByRef iSourceID As Integer, ByRef iLanguageID As Integer, ByRef iCurrencyID As Integer, ByRef iLogLevel As Integer, ByRef sCallingAppName As String, Optional ByRef vDatabase As Object = Nothing) As Integer
        ...
        Try
            ...
            Dim vCurrencyDetailsArray(,) As Object
            ...
            m_oDocTemplate.Initialise(sUsername:=sUsername, sPassword:=sPassword, iUserID:=iUserID, iSourceID:=iSourceID, iLanguageID:=iLanguageID, iCurrencyID:=iCurrencyID, iLogLevel:=iLogLevel, sCallingAppName:=sCallingAppName, vDatabase:=vDatabase)
            ...
            If m_sCCMDocProduction = "1" AndAlso (isKCMApplicableForSelectedDocument = "0" OrElse (Not String.IsNullOrEmpty(m_sCCMDocumentName))) Then
                If Not String.IsNullOrEmpty(m_sCCMDocumentName) Then
                    ...
                    If String.IsNullOrEmpty(m_sDocTemplatePath) Then ''pick GIS path if not entered in system options
                        ...
                        If nResult <> gPMConstants.PMEReturnCode.PMTrue Then
                            iPMFunc.LogMessage(iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to get Client from Registry.", vApp:=ACApp, vClass:=ACClass,
        ...
        Try
            ...
            If lCount = 1 Then
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    ...
                    bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to copy template from Server to Client." & Strings.Chr(13) & Strings.Chr(10) &
                                       "sServer       : " & sServer & Strings.Chr(13) & Strings.Chr(10) &
                                       "sClient       : " & sClient & Strings.Chr(13) & Strings.Chr(10) &
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    result = m_lReturn
                    bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to Unzip the Template." & Strings.Chr(13) & Strings.Chr(10) &
                                       "Template        : " & sClient & Strings.Chr(13) & Strings.Chr(10) &
                ...
                If m_sDocFileExtension.ToUpper() <> "XML" Then
                    ...
                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                        result = gPMConstants.PMEReturnCode.PMError
                        ' Log Error.
                        bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to UpdateTemplateNumberAndDependencies." & Strings.Chr(13) & Strings.Chr(10) &
                                           "m_sClient : " & m_sClient & Strings.Chr(13) & Strings.Chr(10) &
                                           "lOldID : " & CStr(m_lDocumentTemplateId) & Strings.Chr(13) & Strings.Chr(10) &
                ...
                Else
                    ...
                    File.Move(m_sClient & "\Doc " & CStr(m_lMailshotDocumentTemplateId) & "." & m_sDocFileExtension, m_sClient & "\temp\Doc 0." & m_sDocFileExtension)
                End If
                ...
                If m_sDocFileExtension.ToUpper() <> "XML" Then
                    ...
                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                        result = gPMConstants.PMEReturnCode.PMError
                        ' Log Error.
                        bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to CopyDocAndDependencies" & Strings.Chr(13) & Strings.Chr(10) &
                                           "From : " & m_sClient & Strings.Chr(13) & Strings.Chr(10) &
            ...
            If lCount > 1 Then
                If m_sDocFileExtension.ToUpper() <> "XML" Then
                    ...
                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                        ...
                        ' Log Error.
                        bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to CopyDocAndDependencies" & Strings.Chr(13) & Strings.Chr(10) &
                                           "From : " & m_sClient & "\Temp" & Strings.Chr(13) & Strings.Chr(10) &
            ...
            If m_sDocFileExtension.ToUpper() <> "XML" Then
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    ...
                    bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to UpdateTemplateNumberAndDependencies." & Strings.Chr(13) & Strings.Chr(10) &
                                       "m_sClient : " & m_sClient & Strings.Chr(13) & Strings.Chr(10) &
                                       "lOldID : 0" & Strings.Chr(13) & Strings.Chr(10) &
                ...
                End If
            Else
                File.Copy(m_sClient & "\temp\Doc 0." & m_sDocFileExtension, m_sClient & "\Doc " & CStr(lCount) & "." & m_sDocFileExtension)
            End If
            Return result
        Catch excep As System.Exception
            result = gPMConstants.PMEReturnCode.PMError
        ...
        Try
            ...
            If m_sDocFileExtension.ToUpper() <> "XML" Then
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    ...
                    bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to UpdateTemplateNumberAndDependencies." & Strings.Chr(13) & Strings.Chr(10) &
                                       "m_sClient : " & m_sClient & Strings.Chr(13) & Strings.Chr(10) &
                                       "lOldID : " & ToSafeString(lTempFileNo) & Strings.Chr(13) & Strings.Chr(10) &
            ...
            Else
                'Rename File
                File.Move(m_sClient & "\Doc " & ToSafeString(lTempFileNo) & "." & m_sDocFileExtension, m_sClient & "\Doc " & ToSafeString(lCount) & "." & m_sDocFileExtension)
            End If
    ...
    Private Function CopyPolicySharesServerToClient(ByRef lCount As Integer) As Integer
        ...
        Try
            ...
            If lCount = 1 Then
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    CopyPolicySharesServerToClient(m_lReturn)
                    bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to copy file from Server to Client." & Strings.Chr(13) & Strings.Chr(10) &
                                       "sServer       : " & sServer & Strings.Chr(13) & Strings.Chr(10) &
                                       "sClient       : " & sClient & Strings.Chr(13) & Strings.Chr(10) &
                ...
                If m_sDocFileExtension.ToUpper() <> "XML" Then
                    ...
                    If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                        ...
                        bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to UpdateTemplateNumberAndDependencies." & Strings.Chr(13) & Strings.Chr(10) &
                                           "m_sClient : " & m_sClient & Strings.Chr(13) & Strings.Chr(10) &
                                           "lOldID : " & ToSafeString(m_lDocumentTemplateId) & Strings.Chr(13) & Strings.Chr(10) &
                ...
                Else
                    Directory.CreateDirectory(m_sClient & "\temp\")
                    File.Move(m_sClient & "\Doc " & ToSafeString(m_lDocumentTemplateId) & "." & m_sDocFileExtension, m_sClient & "\temp\Doc 0." & m_sDocFileExtension)
                End If
            ...
            If m_sDocFileExtension.ToUpper() <> "XML" Then
                ...
                End If
            Else
                File.Copy(m_sClient & "\temp\Doc 0." & m_sDocFileExtension, m_sClient & "\Doc " & ToSafeString(lCount) & "." & m_sDocFileExtension)
            End If
    ...
    Private Function CopyServerToClient(ByVal lTypeId As Integer, ByVal lTemplateId As Integer) As Integer
        ...
        If FileSystem.Dir(sServer, FileAttribute.Normal) = "" And lTypeId = lLETTER_TYPE_ID Then
            'If letter not found then search in subdoc folder
            If FileSystem.Dir(m_sServer & "\Type " & CStr(lSUBDOC_TYPE_ID) & "\Doc " & CStr(lTemplateId) & ".zip", FileAttribute.Normal) <> "" Then
        ...
        ' RAM20040301 : 1. Use the bPMDocFunctions UnZip Function.
                ...
                If iCurrentNestingLevel > 0 Then
                    ...
                    If Not cInputFile.EOF Then
                        ...
                        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                            ...
                            Exit Do
                        End If
                        If UCase(m_sDocFileExtension) = "XML" Then
    ...
    Private Function ResolveDocumentXML(ByRef sFileName As String,
        ...
        Dim sParentDocumentWorkDirectoryRiskLoop As String
        ...
        Dim sGroup, sLoop1, sLoop2, sLoop3, sSubGroup As String
                    ...
                    Select Case sFieldType
                        ...
                        Case DbTag
                            ...
                            Else
                                ...
                                If sValue.Contains(Strings.ChrW(13).ToString()) Then
                                    sValue = Replace(sValue, Chr(13), "<br />")
                                End If

                                If (sFieldName <> "SWCODE" And sFieldName <> "SWDESC" And Not (sFieldName.Contains("STANDARDWORDING"))) Then
                                    If Not (sValue.Contains("&lt;") OrElse sValue.Contains("&gt;")) Then
                                ...
                                If InStr(1, sFieldName, "THISPAYMENTWORD") > 0 _
                                    ...
                                    If Double.TryParse(sValue, NumberStyles.Number, CultureInfo.CurrentCulture.NumberFormat, dbNumericTemp) Then
                                        sValue = NumToWord(v_cValue:=Math.Abs(CDec(sValue)), v_sIntegerPartCode:=sCurrencyMajorPart, v_sDecimalPartcode:=sCurrencyMinorPart)
                                    Else
                                ...
                                ElseIf UCase(Right(sFieldName, Len("CASHAMOUNTWORDSMAJOR"))) = "CASHAMOUNTWORDSMAJOR" Then
                                    ...
                                    If sValue <> "" Then
                                        sValue = NumToWord(v_cValue:=Math.Abs(IIf(sValue > 0, Math.Floor(CDbl(sValue)), Math.Ceiling(CDbl(sValue)))), v_bConvertDecimal:=False, v_sIntegerPartCode:="", v_sDecimalPartcode:="")
                                    End If
                                ElseIf UCase(Right(sFieldName, Len("CASHAMOUNTWORDSMINOR"))) = "CASHAMOUNTWORDSMINOR" Then
                                    'Convert decimal part to just words
                                    If sValue <> "" Then
                                        sValue = NumToWord(v_cValue:=Math.Abs(CDec(sValue) - (IIf(sValue > 0, Math.Floor(CDbl(sValue)), Math.Ceiling(CDbl(sValue))))), v_bConvertDecimal:=True, v_sIntegerPartCode:="", v_sDecimalPartcode:="")
                        ...
                        Case LoopTag
                            ...
                            If (IsArray(vKeyArray)) Then
                                ...
                                ' RAM20040301 : Use FSO to delete the file, rather than Kill
                                m_lReturn = bPMDocFunctions.DeleteFile(m_sClient & "\" & sLoopFileTemp)
                                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                                    ResolveDocumentXML = gPMConstants.PMEReturnCode.PMFalse
                                    ' Log Error.
                                    bPMFunc.LogMessage(
                                                        ...
                                                        If (iRiskCount <> 0) Then
                                                            ...
                                                            If Not (Convert.IsDBNull(auxVar) Or IsNothing(auxVar)) Then
                                                                If (vRiskArray(iTRAILER_IDX, iRiskCount - 1) <> 0) Then
                                                                    ...
                                                                    Else
                                                                        If (m_bRunningOnServer = False) Then
                                                                            MsgBox("Could not locate Risk Trailer clause, continuing....", vbExclamation, "Merge Document")
                                                                        Else
                                                                            Call bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError,
                            ...
                            If sEquation <> "" Then
                                ...
                                If Not bCondition Then
                                    ...
                                    If Not bEndIfFoundInSameLine Then
                                        ...
                                        Dim bSuppressIFCondLines As Boolean = True
                                        Dim bTempSectionBreakExist As Boolean = False
                                        Dim sTempContent As String = ""
                                        Dim bSubSectionFound As Boolean = False
                                        ...
                                        Do While Not cInputFile.EOF
                                            ...
                                            Else
                                                ...
                                                Else
                                                    ...
                                                    Dim bCheckHeaderExist As Boolean = True
                                                    ...
                                                    End If
                                                    If InStr(1, sNextLine, "<w:sectPr") > 0 Then
                                                        m_bSectionBreakExist = True
                                                        bTempSectionBreakExist = True
                                                        m_iSectionBreakIfCnt = m_iSectionBreakIfCnt + 1
                                                    End If
                                                    If sNextLine.Contains("<w:hdr ") Then ' header found
                                                        Dim iHdrPos As Integer = InStr(1, sNextLine, "<w:hdr ")
                                                        If Not sNextLine.Contains("</w:ftr") Then
                                                            Dim sFirstPart As String = Left(sNextLine, iHdrPos - 1)

                                                            m_lReturn = RemoveContentsOfTag("<w:t>", "</w:t>", sFirstPart)
                                                            sNextLine = sFirstPart & Mid(sNextLine, iHdrPos)
                                                        End If
                                                        bSuppressIFCondLines = False
                                                        bCheckHeaderExist = False
                                                    End If
                                                    If bCheckHeaderExist Then
                                                        If sNextLine.Contains("<w:ftr ") Then
                                                            bSuppressIFCondLines = False
                                                        End If
                                                    End If

                                                    If bSuppressIFCondLines Then
                                                        If Left(sNextLine, 4).ToString.Trim = "<w:p" Then
                                                            sNextLine = ""
                                                        Else
                                                            If bTempSectionBreakExist Then
                                                                m_lReturn = RemoveContentsOfTag("<w:t>", "</w:t>", sNextLine)
                                                                If sFirstTemp = "" Then
                                                                    sFirstTemp = sTemp
                                                                End If
                                                                sTempContent = sTempContent + sNextLine
                                                            Else
                                                                sNextLine = ""
                                                            End If
                                                        ...
                                                        End If
                                                    Else
                                                        Dim sPropersubSectTag As String = GetTagProperName("<wx:sub-section", sNextLine)
                                                        If sPropersubSectTag <> "" Then
                                                            bSubSectionFound = True
                                                        End If
                                                    End If
                                                    If sNextLine.Contains("</w:hdr") AndAlso Not sNextLine.Contains("ftr") AndAlso Not sNextLine.Contains("<w:hdr ") Then
                                                        bSuppressIFCondLines = True
                                                    End If
                                                    If sNextLine.Contains("</w:ftr") Then
                                                        Dim iFtrPos As Integer = InStr(1, sNextLine, "</w:ftr>")

                                                        If iFtrPos > 0 Then
                                                            If InStr(iFtrPos, sNextLine, "<w:hdr ") = 0 AndAlso InStr(iFtrPos, sNextLine, "<w:ftr ") = 0 Then

                                                                'Dim iEndLinePos As Integer = InStr(iFtrPos, sNextLine, "</w:p>")

                                                                'sNextLine = Left(sNextLine, iEndLinePos + 5)
                                        ...
                                        Loop
                                        sLastTemp = Mid(sTemp, sFirstTemp.Length() + 1)
                                        bTempSectionBreakExist = False
                                        Dim sProperTrTag As String = GetTagProperName("<w:tr", sTempContent)
                                        If sProperTrTag <> "" AndAlso sTempContent <> "" Then
                                            Dim sTempContentOrignal As String = sTempContent
                                            RemoveContentsOfTag(sProperTrTag, "</w:tr>", sTempContent, False)
                                            sTempContent = Replace(sTempContent, sProperTrTag + "</w:tr>", "")
                                            Dim sPropersubSectTag As String = GetTagProperName("<wx:sub-section", sTempContent)
                                            If sPropersubSectTag <> "" OrElse bSubSectionFound Then
                                                sTempContent = ""
                                            End If
                                            Dim lTempPos As Integer = 1
                                            Dim sTempRemoveCnontent As String
                                            Do While InStr(lTempPos, sTempContent, "</") > 0
                                                sTempRemoveCnontent = ""
                                                lTempPos = InStr(lTempPos, sTempContent, ">")
                                                Dim sTempTag As String = Left(sTempContent, lTempPos)
                                                If sFirstTemp.LastIndexOf(Replace(sTempTag, "/", "")) > 0 Then
                                                    sTempRemoveCnontent = (Mid(sFirstTemp, sFirstTemp.LastIndexOf(Replace(sTempTag, "/", "")) + 1))
                                                Else
                                                    Exit Do
                                                End If
                                                sFirstTemp = Left(sFirstTemp, sFirstTemp.Length - sTempRemoveCnontent.Length)
                                                sTempContent = sTempRemoveCnontent + sTempContent

                                                sTempContentOrignal = sTempRemoveCnontent + sTempContentOrignal
                                                RemoveContentsOfTag(Replace(sTempTag, "/", ""), sTempTag, sTempContent, False)
                                                sTempContent = Replace(sTempContent, Replace(sTempTag, "/", "") + sTempTag, "")
                                                lTempPos = 1
                                            Loop


                                            sTemp = Replace(sTemp, sTempContentOrignal, sTempContent)
                                            sPropersubSectTag = GetTagProperName("<wx:sub-section", sTemp)
                                            If sPropersubSectTag <> "" Then
                                                sTemp = Replace(sTemp, "<wx:sub-section>", "")
                                            End If
                                            bSubSectionFound = False
                        ...
                        Case SubDocumentTag, DocumentSplitTag
                            ...
                            If Not m_bSectionBreakExist Then
                                m_lReturn = GetFullLines(sStartingPartOfSplitLine, sEndingPartOfSplitLine)
        ...
        Finally
            ...
            If bFullFileNameIsOpen = True Then
                ' RVH 08/11/2004: (Performance) Remove the file class
                cInputFile = Nothing
                m_lReturn = bPMDocFunctions.DeleteFile(sFullFileName)
            End If
            If bMergeFileIsOpen = True Then
                'Close #iFileNumOut
                oTextStreamOut.Close()
                m_lReturn = bPMDocFunctions.DeleteFile(sMergeFile)
            End If
    ...
    Public Sub BreakStringIntoList(ByVal v_sStartTag As String,
        ...
        sStr = Split(v_sString, v_sStartTag)
    ...
    Private Function bQuestionAlreadyAsked(ByRef v_sQuestion As String) As Boolean
        ...
        If Not (m_colQuestions Is Nothing) Then
            If m_colQuestions.Count > 0 Then
                For iCount As Integer = 1 To m_colQuestions.Count
    ...
    Private Function ExtractTotalInfo(ByRef sCurrentLine As String, ByRef lFieldStart As Integer, ByRef lFieldEnd As Integer, ByRef iTotalNumber As Integer, ByRef sEquation As String, ByRef cInputFile As FileClass) As Integer
        ...
        Do While Not bComplete
            For lTemp As Integer = 1 To sTemp.Length - m_sFieldStartMarker.Length + 1
                If sTemp.Substring(lTemp - 1, Math.Min(sTemp.Length, m_sFieldStartMarker.Length)) = m_sFieldStartMarker Then
                    iCount += 1
                End If

                If sTemp.Substring(lTemp - 1, Math.Min(sTemp.Length, m_sFieldEndMarker.Length)) = m_sFieldEndMarker Then
                    iCount -= 1
                    If iCount = 0 Then
                        lFieldEnd = lTemp
                        bComplete = True
        ...
        If lFieldEnd <> 0 Then
            sTemp = sTemp.Substring(m_sFieldStartMarker.Length, Math.Min(sTemp.Length, lFieldEnd - m_sFieldEndMarker.Length - 1))
        End If
    ...
    Private Function ResolveEquation(ByVal iTotalNumber As Integer,
        ...
        While bSpan
            ...
            If nFieldStart = 0 Then
                bSpan = False
            Else
                sResolved = sResolved.Substring(0, nFieldStart - 1) & Strings.Chr(148).ToString() & sResolved.Substring(nFieldStart + 5)
            End If
        End While

        If sResolved.IndexOf(Strings.Chr(226).ToString() & Strings.Chr(128).ToString() & Strings.Chr(157).ToString(), StringComparison.CurrentCulture) >= 0 Then
            sResolved = sResolved.Replace(Strings.Chr(226).ToString() & Strings.Chr(128).ToString() & Strings.Chr(157).ToString(), """")
        End If

        If sResolved.IndexOf(Strings.Chr(226).ToString() & Strings.Chr(128).ToString() & Strings.Chr(156).ToString(), StringComparison.CurrentCulture) >= 0 Then
            sResolved = sResolved.Replace(Strings.Chr(226).ToString() & Strings.Chr(128).ToString() & Strings.Chr(156).ToString(), """")
        End If
        ...
        For iTemp As Integer = 1 To sResolved.Length
            ...
            If Strings.Asc(sChar(0)) > 128 Then
                Select Case Strings.Asc(sChar(0))
                    Case 145, 146, 147, 148
                        Mid(sResolved, iTemp, 1) = """"
                    Case Else
                        Mid(sResolved, iTemp, 1) = Strings.Chr(Strings.Asc(sChar(0)) - 128).ToString()
                End Select
            End If
        Next iTemp
        If sResolved.IndexOf("%"c) >= 0 Then
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=PMELogLevel.PMLogInfo, sMsg:="ResolveEquation - Engine Failed. Error Running Rule." & Strings.Chr(13) & Strings.Chr(10) & "Error No     : " & CStr(Err_No) & Strings.Chr(13) & Strings.Chr(10) & "Error Desc   : " & Err_Description & Strings.Chr(13) & Strings.Chr(10) & "Error Line   : " & CStr(Err_Line) & Strings.Chr(13) & Strings.Chr(10) & "Error Column : " & CStr(Err_Col) & Strings.Chr(13) & Strings.Chr(10) & "Error Text   : " & Err_Text & Strings.Chr(13) & Strings.Chr(10), vApp:=ACApp, vClass:=ACClass, vMethod:="ResolveEquation", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Private Function ProcessMailshot() As Integer
        ...
        Dim oDocument, oSelection As Object
        ...
        Try
            ...
            If m_sDocFileExtension.ToUpper() <> "XML" Then
                ...
                oDocument = m_oWord.Documents.Open(sClient)
            ...
            For lTemp As Integer = m_vMailshotArray.GetLowerBound(1) To m_vMailshotArray.GetUpperBound(1)
                ...
                If lTemp <> m_vMailshotArray.GetLowerBound(1) Then
                    ...
                    If m_sDocFileExtension.ToUpper() <> "XML" Then
                        ...
                        oSelection.InsertFile(FileName:=sClient, Range:="", ConfirmConversions:=False, link:=False, Attachment:=False)
            ...
            If m_sDocFileExtension.ToUpper() <> "XML" Then
                ...
                If CInt(sVersion) >= 9 Then
                    ...
                    oDocument.SaveAs(FileName:=sClient, FileFormat:=8)
                Else

                    oDocument.SaveAs(FileName:=sClient)
                End If
            ...
            Else
                ...
                File.Move(sMainFileName, sClient)
    ...
    Public Function InsertBreakUsingSiriusDocumentUtility(ByRef v_sDocument As String) As Integer
        ...
        Try
            ...
            oConvert.AppendBreakToDocument(v_sDocument)
    ...
    Public Function AppendDocumentUsingSiriusDocumentUtility(ByVal v_sSourceDocument As String, ByVal v_sDestDocument As String, ByVal v_bAppendBreak As Boolean) As Integer
        ...
        Try
            ...
            oConvert.AppendDocument(v_sDestDocument, v_sSourceDocument, v_bAppendBreak)
    ...
    Private Function ProcessPolicyShares() As Integer
        ...
        Try
            ...
            Else
                ...
                File.Copy(sMainFileName, sClient, True)
    ...
    Private Function ProcessRisksXML() As Integer
        ...
        Dim sClient, sFileName, sTempClientFolder As String
        ...
        For lTemp As Integer = m_vRiskArray.GetLowerBound(1) To m_vRiskArray.GetUpperBound(1)
            ...
            If lTemp = 0 Then
                ...
                File.Copy(sFileName, sMainFileName, True)
        ...
        If m_bAddTOC Then
            ...
            oRange = oDocument.Range(Start:=iTemp, End_Renamed:=iTemp)
            ...
            For lTemp As Integer = 1 To oDocument.Sections.Count
                ...
                With oDocument.Sections(lTemp).Headers(1).PageNumbers
                ...
                If lTemp > 1 Then
                    ...
                    oDocument.Sections(lTemp).Footers(1).PageNumbers.Add(PageNumberAlignment:=1, FirstPage:=True)
                End If
            Next lTemp
            ...
            If CInt(sVersion) >= 9 Then
                ...
                Else
                    ...
                    oDocument.SaveAs(FileName:=sClient, FileFormat:=11)
                End If
            Else
    ...
    Private Function GetSubDocXmlContents(ByRef cInputFile As FileClass, ByRef sFile As String, ByVal v_bMegeStylesAlso As Boolean) As String
        ...
        Dim lPosEnd As Long
        Dim vParagraphArrayStyle As Object
    ...
    Private Function UpdateTemplateNumberAndDependencies(ByRef sPath As String, ByRef lOldId As Integer, ByRef lNewId As Integer) As Integer
        ...
        If IsFolderExists(sDependencyDir) = gPMConstants.PMEReturnCode.PMTrue Then
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                result = gPMConstants.PMEReturnCode.PMFalse
                ' Log Error Message
                bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Unable to Copy Folder." & Strings.Chr(13) & Strings.Chr(10) &
                                   "From [" & sDependencyDir & "] " & Strings.Chr(13) & Strings.Chr(10) &
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                ...
                bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to Copy File." &
                                   "Soure File [" & sTempFile & Strings.Chr(13) & Strings.Chr(10) &
                                   "Destination File [" & sNewClient & Strings.Chr(13) & Strings.Chr(10) &
            ...
            If IsFileExists(sXML_ListFile) = gPMConstants.PMEReturnCode.PMTrue Then
                ...
                If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                    ...
                    bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to Copy File." &
                                       "Soure File [" & sTempFile & Strings.Chr(13) & Strings.Chr(10) &
                                       "Destination File [" & sXML_ListFile & Strings.Chr(13) & Strings.Chr(10) &
                                           "Error Message : " & m_sFileCopyMsg, vApp:=ACApp, vClass:=ACClass, vMethod:="UpdateTemplateNumberAndDependencies", vErrNo:=Information.Err().Number, vErrDesc:=Information.Err().Description)
                    Return result
                End If
    ...
    Private Function CopyDocAndDependencies(ByRef sSource As String, ByRef sDestination As String) As Integer
        Dim result As Integer = 0
        Dim sTemp, sParentFile, sDependencyDir, sTempDest, sDestinationFolder, sErrorMessage As String
        ...
        If IsFolderExists(sDependencyDir) = gPMConstants.PMEReturnCode.PMTrue Then
            ...
            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                result = m_lReturn
                bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to Copy Directory Contents" & Strings.Chr(13) & Strings.Chr(10) &
                                   "Source Directory [" & sDependencyDir & "]" & Strings.Chr(13) & Strings.Chr(10) &
                                   "Destination Directory [" & sDestinationFolder & "]" & Strings.Chr(13) & Strings.Chr(10) &
    ...
    Private Function DeleteClient(ByRef sPath As String, ByRef sFileName As String) As Integer
        ...
        m_lReturn = bPMDocFunctions.DeleteFile(v_sFileName:=sParentFile)
        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
            result = gPMConstants.PMEReturnCode.PMError
            ' Log Error.
    ...
    Private Function ProcessSubDocXml(ByRef sSourceTemplate As String,
        ...
        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
            result = m_lReturn
            bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Failed to copy file." & Strings.Chr(13) & Strings.Chr(10) &
                               "Source File   : " & m_sClient & "\" & sSourceTemplate & Strings.Chr(13) & Strings.Chr(10) &
                               "Target File   : " & sTmpFile & Strings.Chr(13) & Strings.Chr(10) &
        ...
        ' RAM20040301 : Use FSO to delete the file, rather than Kill
        m_lReturn = bPMDocFunctions.DeleteFile(sTmpFile)
        If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
            result = gPMConstants.PMEReturnCode.PMFalse
            ' Log Error.
    ...
    Private Function RemoveFormatTagsFromOurTags(ByRef r_sLine As String) As Integer
        ...
        Do While lFieldStart > 0
            ...
            lFieldEnd = Strings.InStr(lFieldStart, sLine, m_sFieldEndMarker)
            ...
            sFieldCode = sLine.Substring(lFieldStart - 1, Math.Min(sLine.Length, lFieldLen))
            ...
            lFieldStart = Strings.InStr(lFieldStart + 1, sLine, m_sFieldStartMarker)
    ...
    Private Function RemoveFormatTags(ByRef r_sFieldCode As String) As Integer
        ...
        Do While sFieldCode.IndexOf("<"c) >= 0
            ...
            sFormatString = sFieldCode.Substring(lFormatStart - 1, Math.Min(sFieldCode.Length, lFormatLength))
...
Err_ConvertDocument:
    ...
    Private Function EvaluateVariableExpression(ByVal v_vStrInput As String, ByRef cInputFile As FileClass) As String
            ...
            If iVariableStart > 0 Then
                ...
                Do While Not bComplete
                    For iTemp As Integer = 1 To strTemp.Length - m_sFieldStartMarker.Length + 1
                        If strTemp.Substring(iTemp - 1, Math.Min(strTemp.Length, m_sFieldStartMarker.Length)) = m_sFieldStartMarker Then
                            iCount += 1
                        End If

                        If strTemp.Substring(iTemp - 1, Math.Min(strTemp.Length, m_sFieldEndMarker.Length)) = m_sFieldEndMarker Then
                            iCount -= 1
                            If iCount = 0 Then
                                iFieldEnd = iTemp
                                bComplete = True
                ...
                'What's between the tokens?
                If iFieldEnd <> 0 Then
                    strTemp = strTemp.Substring(m_sFieldStartMarker.Length, Math.Min(strTemp.Length, iFieldEnd - m_sFieldEndMarker.Length - 1))
                End If
        ...
        Else
            ...
            Select Case Trim(strTemp.Substring(0, 2))
                ...
                If v_bRemoveFirstAndLastLineContentsOnly Then
                    If lCnt = 0 Or lCnt = UBound(vParagarphArray) Then
                        If Left$(sTemp, Len(sProperTagName)) = sProperTagName Then
                            If Not IsEmptyString(sTemp) Or InStr(1, sTemp, "<w:br w:type=") > 0 Or InStr(1, sTemp, "<w:binData") > 0 Or InStr(1, sTemp, "<w:hdr") > 0 Or InStr(1, sTemp, "<w:ftr") > 0 Or InStr(1, sTemp, "<w:sectPr") > 0 Or InStr(1, sTemp, "wx:val=" & ChrW(34) & "Wingdings") > 0 Then
    ...
    Private Function ExtractParameterDetails(ByVal v_iCurrentNestingLevel As Integer, ByVal v_sName As String, ByRef r_sValue As String, ByRef r_iType As Integer) As Integer
        ...
        Try
            ...
            If m_sParameterXML.Length <= 0 Then
                ...
                ' empty string ��
                sValue = Strings.Chr(147).ToString() & Strings.Chr(148).ToString()
            Else
                ...
                strXPathSearhExpression = XPathSearhExpressionStart & Strings.Chr(34).ToString() & v_sName & Strings.Chr(34).ToString() & XPathSearhExpressionEnd
                ...
                If v_iCurrentNestingLevel > 1 Then
                    Select Case iType
                        Case gPMConstants.PMEFormatStyle.PMFormatString
                            sValue = Strings.Chr(147).ToString() & sValue & Strings.Chr(148).ToString()
                        Case gPMConstants.PMEFormatStyle.PMFormatDateLong, gPMConstants.PMEFormatStyle.PMFormatDateMedium, gPMConstants.PMEFormatStyle.PMFormatDateShort, gPMConstants.PMEFormatStyle.PMFormatDayOnlyLong, gPMConstants.PMEFormatStyle.PMFormatDayOnlyMedium, gPMConstants.PMEFormatStyle.PMFormatDayOnlyShort, gPMConstants.PMEFormatStyle.PMFormatMonthOnlyLong, gPMConstants.PMEFormatStyle.PMFormatMonthOnlyMedium, gPMConstants.PMEFormatStyle.PMFormatMonthOnlyShort, gPMConstants.PMEFormatStyle.PMFormatDateYearOnly
                            If Information.IsDate(sValue) Then
                                sValue = Strings.Chr(147).ToString() & CDate(sValue).ToString("dd-MMM-yyyy") & Strings.Chr(148).ToString()
                            Else
                                sValue = Strings.Chr(147).ToString() & (#12/29/1899#).ToString("dd-MMM-yyyy") & Strings.Chr(148).ToString()
                            End If
                        Case gPMConstants.PMEFormatStyle.PMFormatDateTimeLong, gPMConstants.PMEFormatStyle.PMFormatDateTimeMedium, gPMConstants.PMEFormatStyle.PMFormatDateTimeShort
                            If Information.IsDate(sValue) Then
                                sValue = Strings.Chr(147).ToString() & CDate(sValue).ToString("dd-MMM-yyyy HH:MM:ss") & Strings.Chr(148).ToString()
                            Else
                                sValue = Strings.Chr(147).ToString() & (#12/29/1899#).ToString("dd-MMM-yyyy HH:MM:ss") & Strings.Chr(148).ToString()
                            End If
                        Case gPMConstants.PMEFormatStyle.PMFormatBoolean
                            If sValue = "1" Or sValue = "True" Or gPMFunctions.ToSafeBoolean(sValue) Then
                                sValue = Strings.Chr(147).ToString() & "Yes" & Strings.Chr(148).ToString()
                            Else
                                sValue = Strings.Chr(147).ToString() & "No" & Strings.Chr(148).ToString()
                            End If
                        Case Else
                            If sValue = "" Then
                                sValue = "0"
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            bPMFunc.LogMessage(sUsername:=m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="ExtractParameterDetails Failed. v_sName = " & v_sName & Strings.Chr(13) & Strings.Chr(10) & strErrorMessage, vApp:=ACApp, vClass:=ACClass, vMethod:="ExtractParameterDetails", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message, excep:=excep)
    ...
    Private Function FixBrokenMarkers(ByRef cInputFile As FileClass, ByRef r_sCurrentLine As String) As Integer
        ...
        For lMarkerLoop As Integer = 1 To 2
            ...
            Do While Strings.InStr(lStartOfMarker + 1, r_sCurrentLine, sFirstHalfOfMarker) > 0

                'Find the first half of the marker.
                lStartOfMarker = Strings.InStr(lStartOfMarker + 1, r_sCurrentLine, sFirstHalfOfMarker)
                ...
                Do While Mid(r_sCurrentLine, lNextCharacter, 1) = "<"
                    ...
                    lNextCharacter = Strings.InStr(lNextCharacter, r_sCurrentLine, ">") + 1
    ...
    Private Function FixIfStatement(ByRef r_sCurrentLine As String) As Integer
        ...
        'Fix IF statements.
        lStartOfMarker = 0
        Do While Strings.InStr(lStartOfMarker + 1, r_sCurrentLine, "&lt;@IF_") > 0
            lStartOfMarker = Strings.InStr(lStartOfMarker + 1, r_sCurrentLine, "&lt;@IF_")
    ...
    Private Function FixSplitMergeCodes(ByRef r_sCurrentLine As String) As Integer
        ...
        lStartOfMergeCode = 1
        Do While Strings.InStr(lStartOfMergeCode, r_sCurrentLine, m_sFieldStartMarker) > 0

            lStartOfMergeCode = Strings.InStr(lStartOfMergeCode, r_sCurrentLine, m_sFieldStartMarker)

            lCurrentNestingLevel = 1
            lStartPos = Strings.InStr(lStartOfMergeCode + 1, r_sCurrentLine, m_sFieldStartMarker)
            lEndPos = Strings.InStr(lStartOfMergeCode, r_sCurrentLine, m_sFieldEndMarker)
            Do While lCurrentNestingLevel > 0
                If lStartPos <> 0 And lStartPos < lEndPos Then
                    lCurrentNestingLevel += 1
                    lStartPos = Strings.InStr(lStartPos + 1, r_sCurrentLine, m_sFieldStartMarker)
                Else
                    lCurrentNestingLevel -= 1
                    lEndOfMergeCode = lEndPos + m_sFieldEndMarker.Length
                    lEndPos = Strings.InStr(lEndPos + 1, r_sCurrentLine, m_sFieldEndMarker)
                End If
            Loop
            ...
            Do While Strings.InStr(lStartPos, sTempLine, "<") > 0 And Strings.InStr(lStartPos, sTempLine, "<") < lEndOfMergeCode
                lEndPos = Strings.InStr(lStartPos, sTempLine, "<")
                ...
                lStartPos = lEndPos
                lEndPos = Strings.InStr(lStartPos + 1, sTempLine, ">")
    ...
    Private Sub CreateXML(ByRef sOutPut As String)
        ...
        Dim docType As XmlElement
        Dim oXEMainGroup As XmlElement
        Dim oXEsubGroup As XmlElement
        Dim oXELoop1 As XmlElement
        Dim oXELoop2 As XmlElement
        Dim oXELoop3 As XmlElement
    ...
    Private Overloads Function GenerateDocumentPure(ByVal oSWArray As ArrayList) As Integer
        ...
        m_lReturn = RemoveEmptySectionBreakXML(sDocFile)
        ...
        File.WriteAllText(sClient, sDocFile)
    ...
    Private Function GenerateDocumentCCM(ByVal sCCMDocumentName As String) As Integer
                    ...
                    Else
                        ...
                        For Each drSubGrp As DataRow In dtSubGrpDetails.Rows
                            ...
                            ElseIf Not String.IsNullOrEmpty(sLoop1) AndAlso String.IsNullOrEmpty(sLoop2) AndAlso String.IsNullOrEmpty(sLoop3) Then
                                ...
                                oSWArray.Add("<@DB_" & sLoop1 & "_" & sLoop1.Substring(0, nDataModelLength) & drRow("ColumnName") & "@> ")
        ...
        File.WriteAllBytes(sClientFile, oDocTemplatePackList)
        ...
        '            nResult = bPMDocFunctions.DeleteFile(sFilePath) ''delete temp files created for ccm clauses
        '            If nResult <> gPMConstants.PMEReturnCode.PMTrue Then
        '                Return gPMConstants.PMEReturnCode.PMFalse
        '            End If
        '        End If
    ...
    Private Sub CreateInputXMLForCCM(ByRef sOutPut As String)
        ...
        Dim oXMLRootElementDS_DataModels As XmlElement

        xmlDoc.Load(New StringReader(kXMLHeader))

        m_dtDocument.DefaultView.Sort = "RISKCNT ASC, DataModel ASC, SubGroup ASC, LOOP1 ASC, LOOP2 ASC , LOOP3 ASC, DataStructureName ASC"

        m_dtDocument = m_dtDocument.DefaultView.ToTable
        Dim bRiskHeaderAdded As Boolean = False

        For Each row As DataRow In m_dtDocument.Select("", "RISKCNT ASC, DataModel ASC, SubGroup ASC, LOOP1 ASC, LOOP2 ASC , LOOP3 ASC, instance4 ASC, DataStructureName ASC")
            i = i + 1
    ...
    Private Sub CopySubDocToCCMFolder(ByVal sTmpFile As String, ByVal sFileName As String)
        ...
        dataBuffer = File.ReadAllBytes(System.IO.Path.GetDirectoryName(sTmpFile) & "\" & sFileName)
    ...
    Public Function GetVariableValue(ByVal sName As String) As String
        ...
        Catch excep As System.Exception
            ...
            iPMFunc.LogMessage(sUsername:="", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="GetVariableValue Failed for : " & sName, vApp:=ACApp, vClass:=ACClass, vMethod:="GetVariableValue", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message)
    ...
    Public Function SetVariableValue(ByVal sName As String, ByVal vValue As String) As Integer
        ...
        Catch excep As System.Exception
            ...
            iPMFunc.LogMessage(sUsername:="", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="SetVariableValue Failed for : " & sName, vApp:=ACApp, vClass:=ACClass, vMethod:="SetVariableValue", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message)
    ...
    Public Function ParseFunctionNameAndParams(ByVal v_vInputString As String, ByRef r_sFunctionName As String, ByRef r_vParamArray As Object) As Integer
        ...
        Try
            ...
            strFunctionName = v_vInputString.Substring(iStart - 1, Math.Min(v_vInputString.Length, iEnd))
            ...
            If iEnd > 1 Then
                strArguments = v_vInputString.Substring(iStart - 1, Math.Min(v_vInputString.Length, iEnd - iStart))
            End If
            ...
            If strArguments.Length > 0 Then
                ...
                strArguments = strArguments.Replace(Strings.Chr(20).ToString(), " ")
                strArguments = strArguments.Replace(Strings.Chr(160).ToString(), " ")
        ...
        Catch excep As System.Exception
            ...
            iPMFunc.LogMessage(sUsername:="", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="ParseFunctionNameAndParams Failed for : " & v_vInputString, vApp:=ACApp, vClass:=ACClass, vMethod:="ParseFunctionNameAndParams", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message)
    ...
    Public Function CallBuiltinFunction(ByVal v_sFunctionName As String, ByVal v_vParamArray() As Object, ByRef r_bResultIsNumeric As Boolean) As String
        ...
        Try
            ...
            Select Case strTemp
                ...
                Case "CHR"
                    ...
                    If IsValidArguments(v_vParamArray, iNoOfArgumentNeeded) Then
                        vReturnValue = Strings.Chr(ToSafeInteger(v_vParamArray(0))).ToString()
                    End If
                Case "INSTR"
                    '    InStr(String1, String2)            Returns the position of String2 within String1
                    iNoOfArgumentNeeded = 2
                ...
                Case "LEN"
                    ...
                    iNoOfArgumentNeeded = 1
                    If IsValidArguments(v_vParamArray, iNoOfArgumentNeeded) Then
                        vReturnValue = Strings.Len(ToSafeString(v_vParamArray(0)))
                    End If
                Case "MID"
                    '    Mid(String, Start, Length)         Returns a portion of a string
                ...
                Case "DATEPART"
                    ...
                    iNoOfArgumentNeeded = 2
                    If IsValidArguments(v_vParamArray, iNoOfArgumentNeeded) Then
                        vReturnValue = DateAndTime.DatePart(ToSafeString(v_vParamArray(0)), ToSafeDate(v_vParamArray(1)), FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1)
                    End If
                Case "FORMATDATETIME"
                    '    FormatDateTime(Date,Format)        Formats a date to a given format
                    iNoOfArgumentNeeded = 2
                    If IsValidArguments(v_vParamArray, iNoOfArgumentNeeded) Then
                        vReturnValue = Strings.FormatDateTime(ToSafeDate(v_vParamArray(0)), Conversion.Val(ToSafeString(v_vParamArray(1))))
                    End If
                Case "WEEKDAYNAME"
                    '    WeekdayName (Day)                  Returns the name of a weekday number
                    iNoOfArgumentNeeded = 1
        ...
        Catch excep As System.Exception
            result = ""
            iPMFunc.LogMessage(sUsername:="", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="CallBuiltInFunction Failed for : " & v_sFunctionName, vApp:=ACApp, vClass:=ACClass, vMethod:="CallBuiltInFunction", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message)
            Return result
        End Try
    End Function
    ...
    Public Function ProperSplit(ByVal v_sInputString As String, Optional ByVal v_sDelimter As String = ",") As Object
        ...
        Try
            ...
            If iStringLength > 0 Then
                ...
                Do While iDelimPosition <> 0
                    ...
                    iDQPos1 = (v_sInputString.IndexOf(Strings.Chr(34).ToString()) + 1)
                    sDatafield = ""
                    '
                    If iDQPos1 <> 0 And iDQPos1 < iDelimPosition Then
                        ...
                        iDQPos2 = Strings.InStr(iDQPos1 + 1, v_sInputString, Strings.Chr(34).ToString())
                    ...
                    If sDatafield.Length <> 0 AndAlso sDatafield.Substring(0, 1) = Strings.Chr(147).ToString() Then
                        sDatafield = v_sInputString.Substring(1)
                    End If

                    If sDatafield.Length <> 0 AndAlso sDatafield.Substring(sDatafield.Length - 1) = Strings.Chr(148).ToString() Then
                        sDatafield = sDatafield.Substring(0, sDatafield.Length - 1)
                    End If
                ...
                If v_sInputString.Trim <> "" Then
                    If v_sInputString.Substring(0, 1) = Strings.Chr(34).ToString() Then
                        v_sInputString = v_sInputString.Substring(1)
                    End If

                    If v_sInputString.Substring(v_sInputString.Length - 1) = Strings.Chr(34).ToString() Then
                        v_sInputString = v_sInputString.Substring(0, v_sInputString.Length - 1)
                    End If
                End If
        ...
        Catch excep As System.Exception
            ...
            iPMFunc.LogMessage(sUsername:="", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="ProperSplit Failed for : " & v_sInputString, vApp:=ACApp, vClass:=ACClass, vMethod:="ProperSplit", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message)
    ...
    Function NoOfOccurences(ByVal v_sSource As String, ByVal v_sStringToMatch As String) As Integer
        ...
        Try
            ...
            While nPos
                nCount += 1
                nPos = Strings.InStr(nPos + iOffset, v_sSource, v_sStringToMatch)
            End While
        ...
        Catch excep As System.Exception
            ...
            iPMFunc.LogMessage(sUsername:="", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="NoOfOccurences Failed for : " & v_sSource & " : " & v_sStringToMatch, vApp:=ACApp, vClass:=ACClass, vMethod:="NoOfOccurences", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message)
    ...
    Private Function RemoveEmptySectionBreakXML(ByRef r_sLine As String,
                                         Optional ByVal v_bCheckAllLines As Boolean = False,
                                         Optional ByVal v_bRemoveFirstAndLastLineContentsOnly As Boolean = False) As Integer
        Dim lCnt As Long
        Dim sTemp As String
        ...
        Dim vParagraphArrayWxSect As Object
        vParagraphArrayWxSect = Nothing

        Try
            If Len(Trim$(r_sLine)) > 0 Then
                m_lReturn = BreakStringIntoArray("<wx:sect", "</wx:sect>", r_sLine, vParagraphArrayWxSect)
            End If

            If Not IsArray(vParagraphArrayWxSect) Then
                Return gPMConstants.PMEReturnCode.PMTrue
            End If

            For lIndex As Integer = UBound(vParagraphArrayWxSect) To 0 Step -1
                vParagarphArray = Nothing
                bHasContent = False

                ' Check if this section has any actual text content (excluding headers/footers)
                Dim sSectionContent As String = vParagraphArrayWxSect(lIndex)
                Dim sTempContent As String = sSectionContent

                ' Remove header content for content check
                Dim iHdrStart As Integer = InStr(1, sTempContent, "<w:hdr")
                If iHdrStart > 0 Then
                    Dim iHdrEnd As Integer = InStr(iHdrStart, sTempContent, "</w:hdr>")
                    If iHdrEnd > 0 Then
                        sTempContent = Left(sTempContent, iHdrStart - 1) & Mid(sTempContent, iHdrEnd + 8)
                    End If
                End If

                ' Remove footer content for content check
                Dim iFtrStart As Integer = InStr(1, sTempContent, "<w:ftr")
                If iFtrStart > 0 Then
                    Dim iFtrEnd As Integer = InStr(iFtrStart, sTempContent, "</w:ftr>")
                    If iFtrEnd > 0 Then
                        sTempContent = Left(sTempContent, iFtrStart - 1) & Mid(sTempContent, iFtrEnd + 8)
                    End If
                End If

                ' Check for actual content after removing headers/footers
                If InStr(1, sTempContent, "<w:t>") > 0 Then
                    bHasContent = True
                End If

                ' If no content found, still process but mark for continuous section break
                ' Don't skip - we want to preserve headers/footers

                m_lReturn = BreakStringIntoArray("<w:sectPr", "</w:sectPr>", vParagraphArrayWxSect(lIndex), vParagarphArray)

                If Not IsArray(vParagarphArray) Then
                    sReturnLine = vParagraphArrayWxSect(lIndex) & sReturnLine
                    Continue For
                End If

                For lCnt = UBound(vParagarphArray) To 0 Step -1
                    sTemp = ToSafeString(vParagarphArray(lCnt))
                    sProperTagName = GetTagProperName("<w:sectPr", sTemp)

                    If sProperTagName.ToString.Trim <> "" Then
                        Dim bMergeSection As Boolean = False

                        ' Create a copy for content checking without modifying original headers/footers
                        Dim sSectionForContentCheck As String = sTemp

                        ' Remove headers and footers from content check copy only
                        Dim sProperHeaderFooterTag As String = GetTagProperName("<w:hdr", sSectionForContentCheck)
                        While sProperHeaderFooterTag.ToString.Trim <> ""
                            Dim iHdrEndPos As Integer = InStr(1, sSectionForContentCheck, "</w:hdr>")
                            If iHdrEndPos > 0 Then
                                Dim iHdrStartPos As Integer = InStr(1, sSectionForContentCheck, sProperHeaderFooterTag)
                                sSectionForContentCheck = Left(sSectionForContentCheck, iHdrStartPos - 1) & Mid(sSectionForContentCheck, iHdrEndPos + 8)
                            End If
                            sProperHeaderFooterTag = GetTagProperName("<w:hdr", sSectionForContentCheck)
                        End While

                        sProperHeaderFooterTag = GetTagProperName("<w:ftr", sSectionForContentCheck)
                        While sProperHeaderFooterTag.ToString.Trim <> ""
                            Dim iFtrEndPos As Integer = InStr(1, sSectionForContentCheck, "</w:ftr>")
                            If iFtrEndPos > 0 Then
                                Dim iFtrStartPos As Integer = InStr(1, sSectionForContentCheck, sProperHeaderFooterTag)
                                sSectionForContentCheck = Left(sSectionForContentCheck, iFtrStartPos - 1) & Mid(sSectionForContentCheck, iFtrEndPos + 8)
                            End If
                            sProperHeaderFooterTag = GetTagProperName("<w:ftr", sSectionForContentCheck)
                        End While

                        ' Check if section has actual content after removing headers/footers from check
                        Dim sContents As String = GetParagarphText(sSectionForContentCheck)
                        If sContents.ToString.Trim <> "" Or InStr(1, sSectionForContentCheck, "<w:t>") > 0 Then
                            bMergeSection = True
                        End If

                        ' Only modify section break type if no content, but preserve original headers/footers
                        If Not bMergeSection Then
                            ' Make it continuous section break while keeping headers/footers intact
                            Dim iTagEndPos As Integer = InStr(1, sTemp, ">")
                            If iTagEndPos > 0 Then
                                sTemp = Left(sTemp, iTagEndPos) & "<w:type w:val=" & Chr(34) & "continuous" & Chr(34) & "/>" & Mid(sTemp, iTagEndPos + 1)
                            End If
                        End If
                    End If

                    sReturnLine = sTemp & sReturnLine
                Next
            Next
    ...
    ' RAM20030423   : Created
    ' ***************************************************************** '
    Public Function NthInstanceOf(ByVal sString As String, ByVal sSearched As String, Optional ByVal Instance As Integer = 1, Optional ByVal nStartFrom As Integer = 1, Optional ByVal eCompareMethod As CompareMethod = CompareMethod.Text, Optional ByVal bFromLastMatch As Boolean = False) As Integer
        Dim result As Integer = 0
        ...
        Try
            ...
            Select Case Math.Sign(Instance)
                Case 0
                    result = IIf(sString = "" And sSearched = "", 0, (sString.LastIndexOf(sSearched) + 1))
                Case 1
                    nStartFrom = Strings.InStr(nStartFrom, sString, sSearched, eCompareMethod)
                    ...
                    Do While nStartFrom > 0
                        ...
                        nStartFrom = Strings.InStr(nStartFrom + 1, sString, sSearched, eCompareMethod)
                    Loop
        ...
        Catch excep As System.Exception
            ...
            iPMFunc.LogMessage(sUsername:="", iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="NthInstanceOf Failed for : " & sString & " : " & sSearched, vApp:=ACApp, vClass:=ACClass, vMethod:="NthInstanceOf", vErrNo:=Information.Err().Number, vErrDesc:=excep.Message)
