    ...
    public partial class SSTabHelper : Component, ISupportInitialize, IExtenderProvider
    {
        ...
        private static void TabControl_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            if ((cTabControl.Alignment == TabAlignment.Top) || (cTabControl.Alignment == TabAlignment.Bottom))
            {
                ...
                g.FillRectangle(new SolidBrush(bgColor), tmpRec);
        ...
        private static Font GetModifiedFont(Font tabFont, ActiveTabFontStyleEnum activeTabEnum)
        {
            ...
            switch (activeTabEnum)
            {
                case ActiveTabFontStyleEnum.Bold:
                    res = Microsoft.VisualBasic.Compatibility.VB6.Support.FontChangeBold(res, true);
                    break;
                case ActiveTabFontStyleEnum.Bold_Italic:
                    res = Microsoft.VisualBasic.Compatibility.VB6.Support.FontChangeBold(res, true);
                    res = Microsoft.VisualBasic.Compatibility.VB6.Support.FontChangeItalic(res, true);
                    break;
                case ActiveTabFontStyleEnum.Italic:
                    res = Microsoft.VisualBasic.Compatibility.VB6.Support.FontChangeItalic(res, true);
                    break;
                case ActiveTabFontStyleEnum.Strikeout:
                    res = Microsoft.VisualBasic.Compatibility.VB6.Support.FontChangeStrikeout(res, true);
                    break;
                case ActiveTabFontStyleEnum.Underline:
                    res = Microsoft.VisualBasic.Compatibility.VB6.Support.FontChangeUnderline(res, true);
                    break;
            }
        ...
        /// <summary>
        /// Sets the number of tabpages into a tabcontrol.
        /// </summary>
        /// <param name="TabCtrl">The Tab Control to set.</param>
        /// <param name="value">The Tabs value being set.</param>
        public static void SetTabs(TabControl TabCtrl, int value)
        {
            try
            {
                ...
                if (value > 0)
                {
                    int tabsCount = GetTabCount(TabCtrl);
                    if (value < tabsCount)
                    {
                        for (int index = tabsCount-1; index >= value; index--)
                        {
                            if (TabCtrl.TabPages[index].Controls.Count == 0)
                            {
                            ...
                            {
                                raiseError |= true;
                                errorMessage = "At least one TabPage contains controls.";
                            }
                        }
                    }
                    else if (value > tabsCount)
                    {
                        raiseError = false;
                        for (int index = tabsCount; index < value; index++)
                        {
                            TabCtrl.TabPages.Add("NewTab" + index.ToString());
                        }
                    }
                    if (TabsVisible.ContainsKey(TabCtrl.GetHashCode()))
                    {
                        TabsVisible.Remove(TabCtrl.GetHashCode());
                        pagesCache.Remove(TabCtrl.GetHashCode());
                    }
                }
                else
                {
                    errorMessage = "Invalid property array index";
                    raiseError = true;
                }
