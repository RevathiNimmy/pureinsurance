...
namespace Artinsoft.VB6.Gui
{
    ...
    public class ContainerHelper
    {
        ...
        /// </summary>
        public class MenuItemsCollection : System.Windows.Forms.Control.ControlCollection, IEnumerator<Control>, IEnumerable<Control>
        {
            ...
            public MenuItemsCollection(MenuStrip owner)
                : base(owner)
            {
                this.controlEnumerator = owner.Items.GetEnumerator();
            }
                ...
                : base(new MenuItemControl(owner))
            {
                if (owner is ToolStripMenuItem)
                    this.controlEnumerator = ((ToolStripMenuItem)owner).DropDownItems.GetEnumerator();
            }
            ...
            Control IEnumerator<Control>.Current
            {
                get
                {
                    if (controlEnumerator != null)
                    {
                        if (controlEnumerator.Current != null)
                            return new MenuItemControl((ToolStripItem)controlEnumerator.Current);
                    }
                    return null;
                }
            }
        ...
        private static Form GetOwnerForm(Object ControlObject)
        {
            ...
            {
                Menu = (ToolStripItem)ControlObject;
        ...
        private static int GetControlIndex(Object ControlObject, out string ArrayName)
        {
            ...
            foreach (FieldInfo fInfo in FormContainer.GetType().GetFields())
            {
                if (fInfo.FieldType.IsArray)
                {
                    Array FormArray = (Array)fInfo.GetValue(FormContainer);
                    ...
                    {
                        ObjectToCompare = FormArray.GetValue(i);
        ...
        /// <summary>
        /// Loads Dynamically a control into a form, the control belongs to a control array in VB6.
        /// </summary>
        /// <param name="FormContainer">The form where the control array is declared.</param>
        ...
        public static void LoadControl(Form FormContainer, String ControlName, int Index)
        {
            ...
            FieldInfo fInfo = FormContainer.GetType().GetField(ControlName);
            if ((fInfo != null) && (fInfo.FieldType.IsArray))
            {
                Array ControlArray = (Array)fInfo.GetValue(FormContainer);

                //A base control to use as template
                BaseObject = FindBaseObject(ControlArray);
                if (BaseObject != null)
                {
                    ...
                    ControlArray.SetValue(CloneObject(BaseObject), Index);
                    fInfo.SetValue(FormContainer, ControlArray);
                }
            }
        }
        ...
        public static void UnloadControl(Object ObjectToUnload)
        {
            ...
            Index = GetControlIndex(ObjectToUnload, out ArrayName);
        ...
        {
            Object ObjectToUnload = null;
            Object BaseObject = null;
            Control BaseControl = null;
            ToolStripItem BaseToolStrip = null;
            ...
            if ((fInfo != null) && (fInfo.FieldType.IsArray))
            {
                ...
                if (!ObjectToUnload.GetType().GetProperty("Name").GetValue(ObjectToUnload, null).ToString().Equals("DinamicallyLoadedControl", StringComparison.CurrentCultureIgnoreCase))
                ...
                if (BaseObject != null)
                {
                    ...
                    if (BaseObject is Control)
                    {
                        ...
                        BaseControl.Parent.Controls.Remove((Control)ObjectToUnload);
                        ControlArray.SetValue(null, Index);
                        fInfo.SetValue(FormContainer, ControlArray);
                    }
                    else if (BaseObject is ToolStripItem)
                    {
                        BaseToolStrip = (ToolStripItem)BaseObject;

                        BaseToolStrip.Owner.Items.Remove((ToolStripItem)ObjectToUnload);
                        ControlArray.SetValue(null, Index);
                        fInfo.SetValue(FormContainer, ControlArray);
                    }
                }
        ...
        private static void CleanEventHandlers(object ControlToUnload, object baseObject)
        {
            ...
            foreach (EventInfo eInfo in ControlToUnload.GetType().GetEvents())
            {
                EventDelegates = GetEventSubscribers(baseObject, eInfo.Name);
                ...
                if (EventDelegates != null)
                {
                    foreach (Delegate del in EventDelegates)
                        eInfo.RemoveEventHandler(ControlToUnload, del);
                }
            }
        }
        ...
        private static Object FindBaseObject(Array ControlArray)
        {
            ...
            {
                if ((obj != null) && !(obj is ToolStripSeparator)) return obj;
            }
            return null;
        }
        ...
        private static Object CloneObject(Object BaseControl)
        {
            ...
            if (BaseControl != null)
            {
                ...
                if (BaseControl is ISupportInitialize)
                    ((System.ComponentModel.ISupportInitialize)Result).BeginInit();

                //Adds the control to the container
                if (BaseControl is Control)
                    ((Control)BaseControl).Parent.Controls.Add((Control)Result);
                else if (BaseControl is ToolStripItem)
                    ((ToolStripItem)BaseControl).Owner.Items.Add((ToolStripItem)Result);

                if (BaseControl is ISupportInitialize)
                    ((System.ComponentModel.ISupportInitialize)Result).EndInit();

                //Copy the properties as set in the base control to the new control
                foreach (PropertyInfo pInfo in BaseControl.GetType().GetProperties())
                {
                    if (pInfo.Name.Equals("Name", StringComparison.CurrentCultureIgnoreCase))
                        pInfo.SetValue(Result, "DinamicallyLoadedControl", null);
                    else
                        if (ListOfProperties.ContainsKey(pInfo.Name))
                        {
                            try
                            {
                                pInfo.SetValue(Result, ListOfProperties[pInfo.Name], null);
                            }
                            catch { }
                        }
                }
                ...
                foreach (EventInfo eInfo in BaseControl.GetType().GetEvents())
                {
                    EventDelegates = GetEventSubscribers(BaseControl, eInfo.Name);
                    ...
                    if (EventDelegates != null)
                    {
                        foreach (Delegate del in EventDelegates)
                        {
                            try
                            {
                                eInfo.AddEventHandler(Result, del);
                            }
                            catch { }
                        }
                    }
        ...
        private static Dictionary<string, object> GetPropertiesToCopy(object BaseControl)
        {
            ...
            foreach (PropertyInfo pInfo in BaseControl.GetType().GetProperties())
            {
                if ((pInfo.PropertyType.IsSerializable) && (pInfo.PropertyType.IsPublic)
                    && (pInfo.CanRead) && (pInfo.CanWrite)
                    && !(pInfo.Name.Equals("Visible", StringComparison.CurrentCultureIgnoreCase)))
                {
                    if (!Result.ContainsKey(pInfo.Name))
                        Result.Add(pInfo.Name, pInfo.GetValue(BaseControl, null));
                }
            }
        ...
        /// </summary>
        private static Dictionary<object, Dictionary<string, Delegate[]>> EventSubscribersCache = new Dictionary<object, Dictionary<string, Delegate[]>>();
    ...
    public class NestedControlEnumerator : IEnumerator<Control>, IEnumerable<Control>
    {
        ...
        public bool MoveNext()
        {
            ...
            if (currentNestedEnumerator != null)
            {
                ...
                if (currentIsValid && controlEnumerator.Current is GroupBox)
                {
                    currentNestedEnumerator =
                      new NestedControlEnumerator(((Control)controlEnumerator.Current));
                    hasNext = MoveNext();
                }
                //note: A TabPage is also a Panel which works for us
                else if (currentIsValid && controlEnumerator.Current is Panel)
                {
                    currentNestedEnumerator = new NestedControlEnumerator((Control)controlEnumerator.Current);
                    hasNext = MoveNext();
                }
                //It must check into the TabControls too
                else if (currentIsValid && controlEnumerator.Current is TabControl)
                {
                    currentNestedEnumerator = new NestedControlEnumerator((Control)controlEnumerator.Current);
                    hasNext = MoveNext();
                }
                else if (currentIsValid && controlEnumerator.Current is MenuStrip)
                {
                    currentNestedEnumerator = new NestedControlEnumerator((MenuStrip)controlEnumerator.Current);
                    hasNext = MoveNext();
                }
