...
namespace Artinsoft.VB6.Utils
{
    ...
    public class ArraysHelper
    {
        ...
        public static A RedimPreserve<A>(A arraySource, int[] lengths, int[] lowerBounds) where A : class
        {
            ...
            if (array != null)
            {
                res = Array.CreateInstance(arrayType.GetElementType(), lengths, lowerBounds);
                InitialValueProvider valueProvider = new InitialValueProvider(arrayType.GetElementType(), null);
                //Multiple dimensions
                if (array.Rank > 1)
        ...
        public static A CastArray<A>(Array srcArray) where A : class
        {
            ...
            try
            {
                ...
                while (pos >= 0)
                {
                    res.SetValue(Convert.ChangeType(srcArray.GetValue(indexes), itemType), indexes);
                    pos = CalculateIndexes(ref indexes, pos, lowerBounds, upperBounds);
                }
            }
            catch (Exception)
        ...
        private class InitialValueProvider
        {
            ...
            /// </summary>
            private enum InitialValueMethod { String, Constructor, ValueType, CreateInstanceValueType };
            /// <summary>
