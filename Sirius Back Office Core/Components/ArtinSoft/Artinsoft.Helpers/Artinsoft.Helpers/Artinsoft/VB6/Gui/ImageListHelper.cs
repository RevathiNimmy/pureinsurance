...
namespace Artinsoft.VB6.Gui
{
    ...
    public partial class ImageListHelper : UserControl
    {
        ...
        /// </summary>
        private bool internalImageListInitialized = false;
        ...
        public ImageList NETImageList
        {
            get
            {
                if ((!internalImageListInitialized) && (!string.IsNullOrEmpty(_VB6ImageList)))
                {
                    internalImageListInitialized = true;
        ...
        private bool LoadVB6ImageListFromControlName(string VB6ImageListName)
        {
            ...
            if (parentForm != null)
            {
                ctrl = Artinsoft.VB6.Gui.ContainerHelper.Controls(ParentForm)[VB6ImageListName];
                if (ctrl != null)
        ...
        private void LoadImages(object VB6ImageList)
        {
            object Img = null;
            Icon vb6Icon = null;
            System.Drawing.Image NETImg = null;
            string key = string.Empty;
            object Images = Artinsoft.VB6.Utils.ReflectionHelper.GetMember(VB6ImageList, "ListImages");

            if (Images == null)
                throw new InvalidCastException("Couldn't retrieve internal VB6 image list");

            for (int i = 1; i <= Convert.ToInt32(Artinsoft.VB6.Utils.ReflectionHelper.GetMember(Images, "Count")); i++)
            {
                vb6Icon = null;
                Img = Artinsoft.VB6.Utils.ReflectionHelper.Invoke(Images, "get_item", new object[] { i });
                if (Img == null)
                    throw new InvalidCastException("Couldn't retrieve internal VB6 image item");

                key = Convert.ToString(Artinsoft.VB6.Utils.ReflectionHelper.GetMember(Img, "Key"));
                try
                {
                    NETImg = Microsoft.VisualBasic.Compatibility.VB6.Support.IPictureToImage(Artinsoft.VB6.Utils.ReflectionHelper.GetMember(Img, "Picture"));
                }
                catch
                {
                    try
                    {
                        ...
                        object iconHandle = Artinsoft.VB6.Utils.ReflectionHelper.GetMember(Artinsoft.VB6.Utils.ReflectionHelper.Invoke(Img, "ExtractIcon", new object[] { }), "Handle");
        ...
        private void LoadBasicProperties(object VB6ImageList)
        {
            ...
            imgSize.Height = Convert.ToInt32(Artinsoft.VB6.Utils.ReflectionHelper.GetMember(VB6ImageList, "ImageHeight"));
            imgSize.Width = Convert.ToInt32(Artinsoft.VB6.Utils.ReflectionHelper.GetMember(VB6ImageList, "ImageWidth"));
            internalImageList.ImageSize = imgSize;

            useMaskColor = (bool)Artinsoft.VB6.Utils.ReflectionHelper.GetMember(VB6ImageList, "UseMaskColor");
            if (useMaskColor)
                internalImageList.TransparentColor = (Color)Artinsoft.VB6.Utils.ReflectionHelper.GetMember(VB6ImageList, "MaskColor");

            internalImageList.Tag = Artinsoft.VB6.Utils.ReflectionHelper.GetMember(VB6ImageList, "Tag");
        }
        ...
        internal static bool IsValid(object ctrl)
        {
