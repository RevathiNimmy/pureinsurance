    ...
    Public Function GetInterfaceDetails() As Integer
        ...
        Try
            ...
            If m_frmInterface.Task = gPMConstants.PMEComponentAction.PMEdit Or m_frmInterface.Task = gPMConstants.PMEComponentAction.PMView Then
                ...
                ' to the interface.(PSL There is no intermediate getBusiness call now)
            ...
            If m_frmInterface.Task = gPMConstants.PMEComponentAction.PMView Then
                ...
                End If
            ElseIf (m_frmInterface.Task = gPMConstants.PMEComponentAction.PMEdit) Then
                'DD120901
                ' Disable the items when just editing
                ' and additional ones for imported data
                'CHECK_PSL find out which controls are read only
                '        bIsInHouse = (m_frmInterface.chkIsInHouse = 1)
                '        For Each ctl In m_frmInterface.Controls
                '            If InStr(ctl.Tag, "E;") > 0 Or _
                ''                (InStr(ctl.Tag, "IH;") > 0 And bIsInHouse) Then
                '                ctl.Enabled = False
                '            End If
                '        Next ctl
                'eck310102 - Premium Finance
                '        If Not bIsInHouse Then
                '            m_frmInterface.cmdEdit.Caption = "View"
                '        End If
            End If
            ...
            Select Case (m_frmInterface.Task)
                ...
                Case gPMConstants.PMEComponentAction.PMEdit
                    ...
                    If m_frmInterface.Status = gPMConstants.PMEReturnCode.PMCancel Then
                        ...
                        If m_frmInterface.Changed Then
                            ' Get string messages
