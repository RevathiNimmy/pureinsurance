...
<System.Runtime.InteropServices.ProgId("SIRInsuranceFolder_NET.SIRInsuranceFolder")>
Public NotInheritable Class SIRInsuranceFolder
    ...
    Public Property InsuranceFolderID() As Integer
        Get
    ...
    Public Function SetPropertiesFromDB(ByRef oFields As DataRow) As Integer
        ...
        Try
            ...
            With oFields
                ...
                InsuranceFolderID = oFields("insurance_folder_id")
                SourceID = oFields("source_id")
                InsuranceHolderCnt = oFields("insurance_holder_cnt")
                Code = oFields("code")
                ...
                If Convert.IsDBNull(oFields("inception_date")) Or Informations.IsNothing(oFields("inception_date")) Then

                    InceptionDate = Nothing
                Else

                    InceptionDate = oFields("inception_date")
                End If

                If Convert.IsDBNull(oFields("arc_archive_folder_id")) Or Informations.IsNothing(oFields("arc_archive_folder_id")) Then

                    ArcArchiveFolderID = Nothing
                Else

                    ArcArchiveFolderID = oFields("arc_archive_folder_id")
                End If

                If Convert.IsDBNull(oFields("quote_insurance_ref")) Or Informations.IsNothing(oFields("quote_insurance_ref")) Then

                    QuoteInsuranceRef = Nothing
                Else

                    QuoteInsuranceRef = oFields("quote_insurance_ref")
                End If

                If Convert.IsDBNull(oFields("next_insurance_ref")) Or Informations.IsNothing(oFields("next_insurance_ref")) Then

                    NextInsuranceRef = Nothing
                Else

                    NextInsuranceRef = oFields("next_insurance_ref")
                End If

                If Convert.IsDBNull(oFields("last_insurance_ref")) Or Informations.IsNothing(oFields("last_insurance_ref")) Then

                    LastInsuranceRef = Nothing
                Else

                    LastInsuranceRef = oFields("last_insurance_ref")
                End If

                RenewalCount = oFields("renewal_count")
    ...
    Private Function AddInputParam() As Integer
        ...
        With m_oDatabase
            ...
            m_lReturn = .Parameters.Add(sName:="insurance_folder_id", vValue:=CStr(InsuranceFolderID), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)
            ...
            m_lReturn = .Parameters.Add(sName:="insurance_holder_cnt", vValue:=CStr(InsuranceHolderCnt), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)
            ...
            m_lReturn = .Parameters.Add(sName:="inception_date", vValue:=InceptionDate, iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMDate)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            m_lReturn = .Parameters.Add(sName:="arc_archive_folder_id", vValue:=ArcArchiveFolderID, iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            m_lReturn = .Parameters.Add(sName:="quote_insurance_ref", vValue:=QuoteInsuranceRef, iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMString)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            m_lReturn = .Parameters.Add(sName:="next_insurance_ref", vValue:=NextInsuranceRef, iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMString)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            m_lReturn = .Parameters.Add(sName:="last_insurance_ref", vValue:=LastInsuranceRef, iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMString)

            If m_lReturn <> gPMConstants.PMEReturnCode.PMTrue Then
                Return gPMConstants.PMEReturnCode.PMFalse
            End If

            m_lReturn = .Parameters.Add(sName:="renewal_count", vValue:=CStr(RenewalCount), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)
    ...
    Private Function AddKeyInputParam() As Integer
        ...
        With m_oDatabase
            ...
            m_lReturn = .Parameters.Add(sName:="insurance_folder_cnt", vValue:=CStr(InsuranceFolderCnt), iDirection:=gPMConstants.PMEParameterDirection.PMParamInput, iDataType:=gPMConstants.PMEDataType.PMLong)
    ...
    Private Function AddKeyOutputParam() As Integer
        ...
        With m_oDatabase
            ...
            m_lReturn = .Parameters.Add(sName:="insurance_folder_cnt", vValue:=CStr(0), iDirection:=gPMConstants.PMEParameterDirection.PMParamOutput, iDataType:=gPMConstants.PMEDataType.PMLong)
    ...
    Private Function GetNewPrimaryKeyID() As Integer
        ...
        With m_oDatabase
            ...
            InsuranceFolderCnt = .Parameters.Item("insurance_folder_cnt").Value
    ...
    Public Function DuplicatePolicyFix() As Integer
        ...
        Try
            ...
            With m_oDatabase
                ...
                ' Execute SQL Statement
                m_lReturn = .SQLAction(sSQL:=ACDuplicatePolicyFixSQL, sSQLName:=ACDuplicatePolicyFixName, bStoredProcedure:=ACDuplicatePolicyFixStored)
        ...
        Catch excep As System.Exception
            ...
            ' Log Error Message
            bPMFunc.LogMessage(m_sUsername, iType:=gPMConstants.PMELogLevel.PMLogOnError, sMsg:="Duplicate Policy Fix Failed", vApp:=ACApp, vClass:=ACClass, vMethod:="DuplicatePolicyFix", vErrNo:=Informations.Err().Number, vErrDesc:=excep.Message, excep:=excep)
